...
function workEvent(c,time,date){
  ...
  return entry(time,text[0],text[1],{placeId:p?.id,mood:"집중",stress:Math.min(100,25+(hash(`${c.id}:${dayKey(date)}:work`)%35))});
}
...
function socialEvent(c,time,date){
  ...
  if(!p)return null;
  const food=catalogChoice(c,p,"foods",`${c.id}:${dayKey(date)}:food`);
  if(pick){
    const action=food?`${pick.other.name}와 함께 ${food.name} 먹는 중`:`${pick.other.name}와 데이트 중`;
    return entry(time,action,`${pick.r.type}인 ${pick.other.name}와 ${p.name}에서 시간을 보내고 있어요.`,{placeId:p.id,itemId:food?.id,withId:pick.other.id,mood:"즐거움",stress:Math.max(0,10)});
  }
  return entry(time,`${p.name} 방문`,food?`오늘은 ${food.name}을 골라 천천히 즐기고 있어요.`:"가벼운 외출을 즐기고 있어요.",{placeId:p.id,itemId:food?.id,mood:"평온"});
}
...
function build(c,date=new Date()){
  ...
  if(lunchPlace){
    const food=catalogChoice(c,lunchPlace,"foods",`${c.id}:${dayKey(date)}:lunch-food`);
    list.push(entry(750,`${lunchPlace.name}에서 점심`,food?`${food.name}을 골라 식사하고 있어요.`:"점심을 먹으며 잠깐 쉬고 있어요.",{placeId:lunchPlace.id,itemId:food?.id,mood:"보통"}));
  }
...
export function timeline(c,date=new Date()){
  ...
  if(!old||old.signature!==sig){
    const current=nowMin(date), past=(old?.entries||[]).filter(x=>x.minute<=current);
    const future=build(c,date).filter(x=>x.minute>current);
    c.days[key]={signature:sig,entries:[...past,...future].sort((a,b)=>a.minute-b.minute)};
    save();
  }
