import {state,save,characterViewFor,explicitCharacterViewFor} from "./state.js?v=20260819catalogcomponent1";
import {characterPlanSpeech} from "./speech-styles.js?v=20260819catalogcomponent1";
...
function build(c,date=new Date()){
  ...
  const scheduled=(state.routines?.[c.id]||[]).filter(item=>Number(item.day)===date.getDay()).slice().sort((a,b)=>mins(a.start)-mins(b.start));
...
const ENGINE_VERSION="20260819-scene-pairing1";
