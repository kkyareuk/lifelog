import {currentDuties} from './salary.js';
...
import {meetingScene,planMeetingJourney,entranceRoom} from './meeting-journey.js?v=20260909dev305';
import {dailyInteractionLine,oneSidedJoke} from './scene-context.js?v=20260909dev305';
import {careRoutineFor} from "./creative-options.js?v=20260909dev305";
import {drinkExperience} from "./drink-log.js?v=20260909dev305";
import {characterMood,environmentConversation} from "./character-mood.js?v=20260909dev305";
import {localizeLifeLog} from "./life-log-localization.js?v=20260909dev305";
import {state,save,setDirectiveSceneResolver,characterViewFor as readCharacterViewFor,explicitCharacterViewFor,recordAutomaticRelationshipMoment,directCharacterActivity,contactAllowed} from "./state.js?v=20260909dev305";
import {characterPlanSpeech} from "./speech-styles.js?v=20260909dev305";
import {canTravelBetween,transportBetween,transportSceneCopy} from "./town-profile.js?v=20260909dev305";
...
function relationIndex(){
  ...
  for(const relation of list){
    for(const a of relationshipMembers(relation))for(const b of relationshipMembers(relation)){
      if(a===b)continue;
...
const related=c=>{
  ...
  relationList().filter(r=>r.temporalStatus!=="past"&&r.a!==r.b&&(r.a===c.id||r.b===c.id)).forEach(relation=>{
...
function personalityFlavor(c,desc,seed="",date=new Date()){
  ...
  types.forEach(type=>{
    const bank={
      ...
      "다정하고 세심함":socialScene?["상대의 표정과 속도를 살피며 부담되지 않는 방식으로 맞춰 주었어요.","자기 방식보다 상대가 편안한지를 먼저 확인하며 행동했어요.","작은 반응도 놓치지 않고 필요해 보이는 부분만 조용히 챙겼어요.",({ko:"상대가 대답을 망설이자 더 캐묻지 않고 다른 이야기로 넘어갔어요.",en:"When the other person hesitated, they stopped pressing and moved to another topic.",ja:"相手が答えをためらうと、それ以上聞かず別の話に移りました。"}[state.uiLanguage]||"상대가 대답을 망설이자 더 캐묻지 않고 다른 이야기로 넘어갔어요."),"도움을 과하게 드러내지 않고 상대가 스스로 해낸 것처럼 자리를 내주었어요."]:["자기 몸과 주변 상태를 세심하게 살피며 무리하지 않는 방식으로 이어 갔어요.","사소한 변화도 지나치지 않고 정성스럽게 손을 보았어요.","나중에 사용할 사람까지 생각해 보기 편한 상태로 남겨 두었어요.","작은 손상이 더 커지지 않도록 조심스러운 손길로 다루었어요.","서두르기보다 오래 편안하게 쓸 수 있는 쪽을 골랐어요."],
  ...
  types.forEach(type=>{
    const extraBank={
      ...
      "완고하고 통제적":[({ko:"다른 의견이 나오자 자기 방식대로 하자며 결론을 서둘렀어요.",en:"When another opinion came up, they pushed for a quick decision in favor of their own method.",ja:"別の意見が出ると、自分のやり方にしようと結論を急ぎました。"}[state.uiLanguage]||"다른 의견이 나오자 자기 방식대로 하자며 결론을 서둘렀어요."),"상대가 충분히 설명해도 기존 방식이 더 안전하다며 쉽게 뜻을 바꾸지 않았어요.","예외를 한 번 허용하면 질서가 무너진다고 생각해 같은 규칙을 끝까지 요구했어요.","예정 밖의 변화가 생기자 다른 사람보다 먼저 세부 순서를 다시 정해 알려 주었어요."],
...
function entry(time,title,desc,extra={}){
  const item={time:clock(time),minute:time,title:resolveEntityParticles(title),desc:resolveEntityParticles(compactLogDescription(desc)),...extra};
  const characterId=extra.characterId||extra.giftActorId||extra.participantOrder?.[0]||"";
  return localizeLifeLog(item,state.uiLanguage,state,characterId);
}
...
function withResidenceLocation(c,item,date=new Date()){
  ...
  const oldLabel=rooms[item.room]?.name||({living:'거실',bedroom:'침실',study:'서재',kitchen:'주방',dining:'식당',bath:'욕실',entry:'현관'})[item.room],newLabel=rooms[resolved.room]?.name;
  if(oldLabel&&newLabel&&oldLabel!==newLabel){for(const key of ['title','desc','baseTitle','baseDesc'])if(typeof resolved[key]==='string')resolved[key]=resolved[key].replaceAll(oldLabel+'에서',newLabel+'에서').replaceAll(oldLabel+'의 ',newLabel+'의 ');}
...
function workEvent(c,time,date){
  ...
  const variants={
    ...
    "종교인":[["기도와 의식을 준비하는 중","공동체의 기도와 의식에 필요한 공간과 물품을 차분히 준비하고 있어요."],["공동체의 이야기를 듣는 중","찾아온 사람의 이야기를 듣고 위로와 도움이 필요한 부분을 함께 살피고 있어요."]],
...
const RELATION_SCENE_SEEDS=[
  ...
  ["갑작스러운 비에 우산을 나누는 중","누가 더 젖었는지 장난스럽게 확인하면서도 상대의 보폭에 맞춰 처마가 있는 길로 천천히 걷고 있어요.","entry"],
  ["함께 찍은 사진을 고르는 중","잘 나온 얼굴만 찾기보다 둘 다 간직하고 싶은 순간이 무엇인지 물어보고 공개해도 되는 사진을 따로 골랐어요.","living"],
  ["서로의 음악을 바꿔 듣는 중","좋아하는 곡을 한 곡씩 추천하고 취향이 다르더라도 끝까지 들어 본 뒤 마음에 남은 부분을 이야기하고 있어요.","living"],
  ["냉장고 재료로 메뉴를 짜는 중","남은 재료와 유통기한을 함께 확인하고 요리와 설거지 가운데 각자 편한 몫을 나눴어요.","kitchen"],
  ["따뜻한 음료를 건네는 중","상대가 원하는 온도와 단맛을 먼저 묻고 대화를 원하지 않는 기색이면 잔만 가까운 곳에 조용히 두었어요.","kitchen"],
  ["망가진 물건을 함께 살피는 중","누구 탓인지 따지기 전에 고칠 수 있는지 확인하고 직접 수리할지 전문가에게 맡길지 의견을 맞추고 있어요.","study"],
  ["장보는 목록을 맞추는 중","각자 필요한 물건과 예산을 적고 이미 있는 것은 빼면서 충동구매를 강요하지 않는 목록을 만들었어요.","kitchen"],
  ["서로의 일정을 응원하는 중","중요한 일정의 시간과 필요한 준비를 기억해 두었다가 부담이 되지 않는 짧은 응원을 건넸어요.","study"],
  ["말없이 간식을 나누는 중","상대 몫을 당연히 정하지 않고 먹어도 되는지 먼저 묻고 손이 닿기 편한 곳에 접시를 옮겨 두었어요.","living"],
  ["새로운 취미를 소개하는 중","잘해야 한다는 압박 없이 가장 쉬운 방법부터 보여 주고 흥미가 없다는 반응도 자연스럽게 받아들였어요.","study"],
  ["외출 뒤 소지품을 확인하는 중","열쇠와 지갑처럼 잃어버리기 쉬운 물건을 함께 확인하되 상대의 가방을 허락 없이 뒤지지 않았어요.","entry"],
  ["함께 쓸 공간을 정돈하는 중","각자의 물건을 마음대로 버리지 않고 자주 쓰는 자리와 비워 둘 자리를 합의해 정리하고 있어요.","living"],
  ["피곤한 기색을 먼저 알아채는 중","평소와 다른 말수와 움직임을 보고 이유를 캐묻기보다 쉬거나 혼자 있을 선택지를 건넸어요.","bedroom"],
  ["고마웠던 일을 뒤늦게 전하는 중","지나간 호의를 당연하게 넘기지 않고 어떤 도움이 되었는지 구체적으로 떠올려 감사의 말을 전했어요.","living"],
  ["사소한 오해를 바로잡는 중","전해 들은 말로 결론을 내리지 않고 직접 뜻을 물어본 뒤 자기가 잘못 이해한 부분을 솔직하게 고쳤어요.","study"],
  ["서로의 물건을 빌리는 중","언제까지 필요한지와 사용해도 되는 범위를 확인하고 돌려줄 때의 상태까지 약속했어요.","study"],
  ["기념할 방법을 의논하는 중","거창한 계획을 밀어붙이지 않고 둘에게 의미 있는 방식과 감당할 수 있는 비용을 함께 고르고 있어요.","living"],
  ["낯선 길을 함께 확인하는 중","한 사람이 방향을 정해 끌고 가기보다 지도와 표지판을 같이 보며 불안한 구간에서는 속도를 맞췄어요.","entry"],
  ["잠들기 전 조명을 조절하는 중","각자 편한 밝기와 잠드는 시간을 확인하고 먼저 쉬는 사람을 위해 소리와 화면 빛을 낮췄어요.","bedroom"],
  ["다음에 먹고 싶은 것을 기억하는 중","스쳐 지나가듯 말한 취향을 메모해 두었다가 다음 약속을 정할 때 선택지로 조심스럽게 꺼냈어요.","kitchen"]
];
...
function relationshipHomeEntry(c,pick,time,date){
  ...
  const {overall="",awareness="",mutualAwareness="",trust="",fear="",closeness="",comfort="",annoyance="",attention="",jealousy="",conflictIntensity="",expectation="",aggression=""}=directedView;
  ...
  const thought=[overall,mutualAwareness,trust,fear,comfort,annoyance,attention,conflictIntensity,expectation,aggression].filter(Boolean).join(" ");
  ...
  const fearLevel={"정하지 않음":0,"가소로움":-1,"가소롭게 여김":-1,"전혀 두렵지 않음":0,"거의 두렵지 않음":1,"조금 두려움":2,"경계하며 두려워함":3,"많이 두려움":4,"공포를 느낌":5,"극도로 두려워함":6}[fear]??0;
  const dismissiveFear=fearLevel<0;
  const afraid=fearLevel>=2;
  const stronglyAfraid=fearLevel>=4;
  ...
  const reactsAngrily=(/바로 따짐|끝까지 결론/.test(c.conflictStyle||"")||/공격적인 반응이 잦음|매우 공격적으로/.test(c.aggressionLevel||""))&&/쉽게 욱함|거의 참지 않음/.test(c.impulseControl||"");
  ...
  else if(dismissiveFear)scripts=[
    [`${other.name}을 전혀 위협적으로 여기지 않는 중`,`${other.name}을 가소롭게 여겨 경계할 가치조차 없다고 생각했어요. 얕보는 판단과 실제 능력은 다를 수 있어, 무례하게 자극하기보다 상황을 한 번 더 확인했어요.`,"living"],
    [`${other.name}의 위협을 대수롭지 않게 넘기는 중`,`${other.name}의 말과 행동을 두려워하지 않았고 오히려 우습게 보았어요. 방심해서 중요한 신호를 놓치지 않도록 표정은 풀어도 주변 상황은 차분히 살폈어요.`,"study"]
  ];
  else if(afraid&&!stronglyAfraid)scripts=[
    [`${other.name}을 조금 두려워하며 반응을 살피는 중`,`${fear}이라는 시선 때문에 평소보다 말과 거리를 조심스럽게 골랐어요. 막연한 불안과 실제 행동을 구분하며, 확인되지 않은 위협을 사실로 단정하지 않았어요.`,"living"],
    [`${other.name} 앞에서 긴장을 알아차린 중`,`${other.name}을 마주하자 몸이 먼저 굳었어요. 두려움의 정도를 스스로 확인한 뒤 필요 이상으로 다가가지 않고, 언제든 자리를 옮길 수 있는 거리를 지켰어요.`,"entry"]
  ];
  else if(stronglyAfraid)scripts=[
    [`${other.name}을 크게 두려워하며 안전한 거리를 확보하는 중`,`${fear}이라는 시선이 몸의 긴장으로 선명하게 나타났어요. 혼자 견디거나 상대를 자극하지 않고, 빠져나갈 길과 도움을 청할 사람을 확인한 뒤 필요한 말만 짧게 전했어요.`,"entry"],
    [`${other.name}과 마주치기 전에 안전을 먼저 확인하는 중`,`${other.name}을 향한 강한 두려움을 억지로 무시하지 않았어요. 실제 위험 신호와 불안에서 나온 예상을 구분하면서도, 위험할 가능성이 있으면 곧바로 거리를 벌릴 수 있게 준비했어요.`,"study"]
  ];
  else if(dislikedLooks.length&&loving)scripts=[
    [`${other.name}에게서 비선호하는 특징을 보고도 마음을 구분하는 중`,`${dislikedLooks.join("·")}을 평소 선호하지 않지만, 그 특징 하나가 ${other.name}을 사랑하는 마음 전체를 지우지는 않았어요. 바꾸라고 요구하지 않고 자기 취향과 상대의 선택을 따로 존중했어요.`,"living"]
  ];
  else if(dislikedLooks.length&&!loving)scripts=[
    [`${other.name}에게서 비선호하는 특징을 알아차린 중`,`${dislikedLooks.join("·")}은 이 캐릭터가 강하게 선호하지 않는 특징이에요. 그렇다고 무례하게 평가하거나 바꾸라고 요구하지 않고 시선을 거둔 뒤 원래 대화를 이어 갔어요.`,"living"]
  ];
...
const homeActivityPoolFor=(c,date=new Date(),minute=nowMin(date))=>{
  ...
  const importantPeople=state.order.map(id=>state.characters[id]).filter(other=>other&&other.id!==c.id).map(other=>{
    const view=characterViewFor(c.id,other.id),rank=Number(String(view.importance||"").match(/^\d+/)?.[0])||999;
    const emotion=/없어서는|깊이 사랑/.test(view.overall||"")?0:/소중|좋아|호감/.test(view.overall||"")?1:2;
    return {other,view,rank,emotion};
  }).filter(item=>item.rank<=3||item.emotion<=1).sort((a,b)=>a.rank-b.rank||a.emotion-b.emotion);
  const important=importantPeople[0];
  if(important&&hash(`${c.id}:${dayKey(date)}:important-thought`)%4===0)pool.push([
    `${important.other.name}을 문득 떠올리는 중`,
    `하던 일을 잠깐 멈추고 자기 삶에서 중요한 ${important.other.name}의 표정과 최근 나눈 말을 떠올렸어요. 연락을 재촉하지 않고 전하고 싶은 말을 마음속으로 한 번 정리했어요.`,
    "living",`important-thought:${important.other.id}`,{thoughtOfId:important.other.id}
  ]);
  return pool;
};
...
function medievalize(c,item,date){return adaptTownActivity(state,c,item)}
...
function scheduledChoiceEntries(c,date){
  ...
  const localized=(choice,key)=>{
    const copy=choice.copy?.[state.uiLanguage]||choice.copy?.ko||{};
  ...
  (state.scheduledChoices||[]).filter(choice=>choice&&(choice.characterId===c.id||choice.targetId===c.id)).forEach(choice=>{
    ...
    if(choice.startAt<dayStart||choice.startAt>=dayEnd||choice.characterId!==c.id)return;
    const minute=atMinute(choice.startAt),place=choice.placeType?placeFor([choice.placeType],`${choice.id}:place`,c,date):null;
    const title=localized(choice,"title")||"함께 정한 일을 하는 중",desc=localized(choice,"desc")||"마을 주인과 미리 정한 일을 일정에 맞춰 시작했어요.";
    if(place)entries.push(entry(minute,title,desc,{...shared,townId:townFor(c,date).id,placeId:place.id}));
    else entries.push(homeEntry(c,minute,title,desc,choice.kind==="work"?"study":"living",shared));
  });
  return entries;
}
function canDrive(c){
...
function buildScene(c,date){
  ...
  const list=c.autonomousActivityBlocks?.includes("sleep")?[]:[entry(wake,"기상",wakeScene(c,date),{home:true,room:usableSleepRoom(c.sleepRoomId)||"bedroom",mood:"평온",stress:5})];
...
function liveGapEvent(c,last,n,date){
  ...
  if(last?.placeId){
    ...
    if(shortStay){
      const priorText=`${last?.title||""} ${last?.desc||""}`;
      const catalogItem=Object.values(state.catalog||{}).flat().find(item=>item?.id===last?.itemId);
      const foodName=catalogItem?.name||([[/피자/,"피자"],[/치킨|닭튀김|후라이드/,"치킨"],[/빵|베이글|크루아상|식빵/,"빵"],[/국|수프|스프|찌개|탕/,"국"],[/파스타|스파게티/,"파스타"],[/케이크/,"케이크"],[/초밥|스시/,"초밥"]].find(([pattern])=>pattern.test(priorText))?.[1]||"");
      ...
      if(recentAtPlace.length<2){
        const stayScenes={
          ...
          음식점:foodName?[`${foodName}을 천천히 먹는 중`,`주문한 ${foodName}을 서두르지 않고 맛보며 남은 양과 속도를 자기에게 맞추고 있어요.`]:["식사를 천천히 마무리하는 중","고른 음식을 서두르지 않고 먹은 뒤 식탁을 정돈하며 잠시 쉬고 있어요."],
        ...
        return entry(minute,stay[0],personalityFlavor(c,stay[1],"short-stay-continuation",date),{townId:currentTown.id,placeId:place.id,itemId:last?.itemId,mood:last.mood||"보통"});
      }
  ...
  const previousCatalogItem=Object.values(state.catalog||{}).flat().find(item=>item?.id===last?.itemId);
  const preparedFood=previousCatalogItem?.name||([[/피자/,"피자"],[/치킨|닭튀김|후라이드/,"치킨"],[/빵|베이글|크루아상|식빵/,"빵"],[/국|수프|스프|찌개|탕/,"국"],[/파스타|스파게티/,"파스타"],[/케이크/,"케이크"],[/초밥|스시/,"초밥"]].find(([pattern])=>pattern.test(previousContext))?.[1]||"만든 음식");
  const followups=[
    ...
    [/요리하는|아침 준비|빵을 굽|빵을 만들|식사를 준비|간식을 챙기|끓이|굽는 중|볶는 중/,[`${preparedFood}을 천천히 먹는 중`,`조금 전 준비한 ${preparedFood}을 식탁에 놓고 서두르지 않게 먹으며 맛과 온도를 천천히 확인하고 있어요.`,"kitchen"]],
...
function placeObjectScene(place,first,second,relation,date){
  ...
  if(!pool.length)return null;
  const object=pool[hash(`${first.id}:${second.id}:${place.id}:${dayKey(date)}:${Math.floor(nowMin(date)/45)}:object`)%pool.length];
  const viewA=characterViewFor(first.id,second.id)||{},viewB=characterViewFor(second.id,first.id)||{};
  const tense=automaticConflictAllowed(first,second,viewA,viewB,date,state);
...
function interactionPair(group,date=new Date(),placeKey=""){
  ...
  for(let i=0;i<people.length;i++){
    for(let j=i+1;j<people.length;j++){
      ...
      const relations=pairRelations(first.id,second.id);
      const relation=relations.find(r=>r.temporalStatus!=="past")||relations[0]||null;
      ...
      const candidate={first,second,relation,priority,score:relationImportance(first,second,relation)+familiarity*35+perception*18,key,perceptionSide:relativeSide};
      if(scores)scores.set(scoreKey,candidate);
      candidates.push(candidate);
    }
  }
  candidates.sort((a,b)=>b.priority-a.priority||b.score-a.score||a.key.localeCompare(b.key));
...
function concreteInteraction(place,first,second,relation,date=new Date()){
  ...
  // them; tension and one-sided feelings use each character's own reaction.
  const mutualComfortScene=safeView(a)&&safeView(b)&&viewSignals(a).romantic===viewSignals(b).romantic&&hash(`${first.id}:${second.id}:${dayKey(date)}:${Math.floor(nowMin(date)/45)}:familiar`)%3===0;
  if(hasContext&&!mutualComfortScene&&!automaticConflictAllowed(first,second,a,b,date,state)){
    const seed=`${dayKey(date)}:${Math.floor(nowMin(date)/45)}:${place?.id}`;
    const react=(actor,other,view)=>{
      const prior=(actor.days?.[dayKey(date)]?.entries||[]).filter(e=>e.minute<nowMin(date)).at(-1);
      return relationshipReaction(actor,other,view,relation,{seed,language:state.uiLanguage,previousKeys:[prior?.relationshipCue].filter(Boolean),moodScore:prior?characterMood(actor,prior,state,'ko').score:0});
    };
    const ar=react(first,second,a),br=react(second,first,b);
    const title=(other,reaction)=>state.uiLanguage==='en'?`${reaction.title} · ${other.name}`:state.uiLanguage==='ja'?`${other.name}と${reaction.title}`:`${togetherWith(other.name)} ${reaction.title}`;
    return {title:title(second,ar),firstTitle:title(second,ar),secondTitle:title(first,br),first:ar.text,second:br.text,relationshipContext:true,relationshipCues:{[first.id]:ar.key,[second.id]:br.key}};
  }
...
function sharedPlaceScene(c,current,date,sharedContext=null){
  ...
  if(!dating){
    ...
    if(objectScene&&!scene.relationshipContext&&!/때리|싸우|충돌|밀어|몸싸움/.test(`${scene.title} ${scene.first} ${scene.second}`))scene={...scene,...objectScene};
...
export function resolveHomeEncounter(c,current,otherScene,date){
  ...
  if(pending&&pending.room===otherScene.room){
    ...
    return {...current,room:pending.room,title:pending.title,desc:pending.desc,homeEncounter:{...pending,arrived:true}};
  }
  if(sameLiveLocation(current,otherScene))return current;
  return {...current,minute:nowMin(date),title:line(`${other.name}에게 가는 중`,`Going to ${other.name}`,`${other.name}のところへ移動中`),desc:line('상대가 있는 방으로 이동하고 있어요. 도착한 뒤 이야기를 나눠요.','They are heading to the other room and will talk after arriving.','相手がいる部屋へ移動しています。到着してから話します。'),homeEncounter:{room:otherScene.room,arrivesAt:date.getTime()+60000,title:pending?.title||current.title,desc:pending?.desc||current.desc},holdMinutes:2};
}
