...
export function careerSchedule(world,c,now=Date.now()){
 const weekly=(world.routines?.[c.id]||[]).filter(work),monthly=(world.monthlyRoutines?.[c.id]||[]).filter(work),current=new Date(now),shifts=[];
