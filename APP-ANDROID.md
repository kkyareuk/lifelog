...
최신 준비 빌드: **1.0.204 / versionCode 219** — [걸음걸이·발소리·태블릿 캐릭터 동기화 및 Play 업로드 안내](docs/android-release-219.md). 서명된 AAB와 바로 설치할 수 있는 debug APK 생성 완료이며 Play Console 업로드/출시는 아직 하지 않았습니다.
...
현재 준비판은 `targetSdk 36`, `versionCode 219`, `versionName 1.0.204`로 설정되어 있습니다. 웹 화면을 고친 뒤에는 반드시 `npm.cmd run app:sync`를 먼저 실행해야 최신 HTML·CSS·JavaScript와 서비스 워커 캐시 버전이 앱에 들어갑니다.
