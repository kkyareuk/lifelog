...
const build=String(window.DRAWER_VILLAGE_NATIVE_BUILD||"20260811aa");
...
function bind(){
  ...
  $("[data-download-character-stats]")?.addEventListener("click",event=>{
    event.stopPropagation();
    downloadCharacterStatisticsReport(event.currentTarget.closest("[data-character-stats-dialog]"));
  });
...
const APP_TABS=["observe","home","character","catalog","relationship","routine","town","shop","settings"];
...
if(!maintenanceEnabled()){
  import("./auth.js?v=20260811aa").catch(error=>{
...
if("serviceWorker" in navigator){
  ...
  if(nativeRuntime){
    ...
    navigator.serviceWorker.register("./sw.js?v=20260811aa",{updateViaCache:"none"}).then(registration=>registration.update()).catch(error=>console.warn("오프라인 업데이트 준비 실패",error));
  }
}
