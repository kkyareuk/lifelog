(() => {
  ...
  const JOBS=["무직","회사원","공무원","정치인","교사","교수","의사","간호사","약사","변호사","경찰","소방관","기자","작가","편집자","연예인","아이돌","배우","웹툰 작가","일러스트레이터","프로그래머","연구원","요리사","카페 직원","미용사","대학생","고등학생","자영업·직접 입력"];
  const defaultState=()=>({schema:1,activeTab:"observe",activeId:null,characters:{},order:[],relationships:{},worlds:{w1:{id:"w1",name:"평행마을",district:"중심 구역",area:"중심 거리",bg:"world-assets/cozy-town.png",places:[
    ...
    {id:"p5",name:"별꼬리 공원",type:"공원",emoji:"🌳",x:62,y:73,color:"#68c889"},
    {id:"p6",name:"평행 의회",type:"공공시설",emoji:"🏛️",x:87,y:68,color:"#c49bd8"}]}},worldOrder:["w1"],homes:{},routines:{},settings:{dark:false,pace:30},lastSaved:null});
  let state=load(), pendingFile=null, accountStatus="Google 로그인";
  function load(){try{const x=JSON.parse(localStorage.getItem(KEY));return x?.schema===1?x:defaultState()}catch{return defaultState()}}
  let saveTimer; function save(){clearTimeout(saveTimer);saveTimer=setTimeout(()=>{state.lastSaved=Date.now();localStorage.setItem(KEY,JSON.stringify(state));renderSave();window.dispatchEvent(new CustomEvent("parallel-city-saved"))},180)}
  ...
  function avatar(c,cls=""){return c?.icon?`<img class="sprite ${cls}" src="${c.icon}" alt="">`:c?.photo?`<img class="avatar ${cls}" src="${c.photo}" alt="">`:`<span class="avatar ${cls}">${initials(c?.name)}</span>`}
  ...
  function characterEvent(c,now=new Date()){
    ...
    if(mins>=540&&mins<1080&&c.job!=="무직"&&!["대학생","고등학생"].includes(c.job)){
      const political=c.job==="정치인", work=places().find(p=>p.id===(political?"p6":c.workPlaceId))||places().find(p=>p.type==="회사");
      const scripts={의사:"환자를 진료하는 중",간호사:"환자 상태를 살피는 중",교사:"수업을 진행하는 중",교수:"강의와 연구를 하는 중",정치인:"의회에서 의정 활동 중",기자:"취재 내용을 정리하는 중",요리사:"주문받은 요리를 만드는 중",프로그래머:"새 기능을 개발하는 중",연구원:"실험 결과를 분석하는 중"};
      return {title:scripts[c.job]||`${c.job}으로 일하는 중`,placeId:work?.id,home:false,time:"09:00",desc:`${c.job} 직업 일정에 따라 근무하고 있어요.`};
    }
  ...
  function observe(){const c=active(),e=characterEvent(c),p=currentPlace(c),w=Object.values(state.worlds)[0];return `${roster()}<div class="observe-grid"><div class="world-viewport panel"><section class="world"><img class="world-bg" src="${w.bg}" alt=""><div class="world-tools"><div class="glass"><small>현재 시각</small><br><b>${new Date().toLocaleString("ko-KR",{hour:"2-digit",minute:"2-digit",month:"long",day:"numeric",weekday:"short"})}</b></div><div class="glass"><small>관찰 중</small><br><b>${esc(c.name)} · ${esc(e.title)}</b></div></div><div class="places">${w.places.map(placeHTML).join("")}</div><div class="people">${state.order.map(cid=>personHTML(state.characters[cid])).join("")}</div></section><div class="zoom"><button data-zoom="-">−</button><button data-zoom="reset">⌂</button><button data-zoom="+">＋</button></div></div><aside><div class="detail panel"><div class="hero">${c.photo?`<img src="${c.photo}" alt="">`:`<span class="fallback">${initials(c.name)}</span>`}</div><span class="mood">${esc(c.mood)}</span><h2>${esc(c.name)}</h2><div class="sub">${esc(c.job)}</div><div class="scene"><small>CURRENT SCENE</small><h3>${esc(e.title)}</h3><p class="sub">${esc(e.desc)}</p>${p?`<b>📍 ${esc(p.name)} · ${esc(w.name)}</b>`:"<b>🏠 집 안</b>"}</div></div><div class="log panel">${logHTML(c)}</div></aside></div>`}
  ...
  function bind(){
    $("#account-btn")?.addEventListener("click",()=>window.ParallelCityAuth?.toggle());
    ...
    $("#file-picker").onchange=e=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=()=>{if(pendingFile==="icon")removeFlatBackground(r.result,result=>{active().icon=result;save();render();toast("투명 아이콘을 저장했어요")});else{active().photo=r.result;save();render();toast("사진을 저장했어요")}};r.readAsDataURL(f);e.target.value=""};
    ...
    let scale=1;document.querySelectorAll("[data-zoom]").forEach(b=>b.onclick=()=>{scale=b.dataset.zoom==="reset"?1:Math.max(.65,Math.min(1.6,scale+(b.dataset.zoom==="+" ? .15 : -.15)));const world=document.querySelector(".world-viewport .world");if(world)world.style.zoom=scale});
  }
  ...
  function removeFlatBackground(src,done){const im=new Image();im.onload=()=>{const max=700,ratio=Math.min(1,max/Math.max(im.width,im.height)),cv=document.createElement("canvas");cv.width=Math.round(im.width*ratio);cv.height=Math.round(im.height*ratio);const x=cv.getContext("2d",{willReadFrequently:true});x.drawImage(im,0,0,cv.width,cv.height);const d=x.getImageData(0,0,cv.width,cv.height),p=d.data,corners=[[0,0],[cv.width-1,0],[0,cv.height-1],[cv.width-1,cv.height-1]],bg=corners.map(([cx,cy])=>{const i=(cy*cv.width+cx)*4;return[p[i],p[i+1],p[i+2]]}).reduce((a,v)=>a.map((n,i)=>n+v[i]/4),[0,0,0]);for(let i=0;i<p.length;i+=4){const distance=Math.hypot(p[i]-bg[0],p[i+1]-bg[1],p[i+2]-bg[2]);if(distance<34)p[i+3]=0;else if(distance<70)p[i+3]=Math.round(p[i+3]*(distance-34)/36)}x.putImageData(d,0,0);done(cv.toDataURL("image/png"))};im.src=src}
  ...
  window.ParallelCity={
    ...
    replaceState(next){if(!next||next.schema!==1)return;state=clone(next);localStorage.setItem(KEY,JSON.stringify(state));render()},
    setAccountStatus(text){accountStatus=text;const b=$("#account-btn");if(b)b.textContent=text}
  };
