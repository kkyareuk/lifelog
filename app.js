import {receiveCharacterTransfers,personalState,characterEditorActive,runIsolatedWorld} from './state.js?v=20260909dev289';
import {bindSharedCharacters,syncSharedCharacterEditor,leaveSharedCharacterEditor,saveSharedCharacter} from './shared-characters.js?v=20260909dev289';
import {runBackgroundAction} from './background-actions.js?v=20260909dev289';
import {TOUCH_REACTIONS,touchReactions} from "./touch-reactions.js?v=20260909dev289";
import {nextRoutinePhaseAt} from './routine-scenes.js?v=20260909dev289';
import {wardrobeCount} from "./state.js?v=20260909dev289";
import {captureMeetingPositions} from './meeting-journey.js?v=20260909dev289';
import {bindMailbox,composeAnnouncement,letterWatermark,mailboxNeedsRefresh} from './mailbox-center.js?v=20260909dev289';
import {bedPerspective} from './bed-perspective.js?v=20260909dev289';
import './user-profile.js?v=20260909dev289';
import {withSharedWorld,buildSharedWorld} from './shared-world.js?v=20260909dev289';
import {bindSharedUi,activeShared,canEditShared} from './shared-ui.js?v=20260909dev289';
import "./group-push.js?v=20260909dev289";
import {EXTRA_FAMILY,hospitalPurposes} from "./creative-options.js?v=20260909dev289";
import {installSettingsTransfer} from "./settings-transfer.js?v=20260909dev289";
import {state, active, save, replaceState, createCharacter, deleteCharacter, setActive, setActiveHome, updateCharacter, setCharacterBodyChoices, updateCharacterView, toggleChip, addRelationship, updateRelationship, deleteRelationship, setHomeImage, setRoomFloorImage, setHomeBackground, setHomeExteriorImage, setPlaceInteriorImage, setCharacterImage, setWorldBackground, addPlace, deletePlace, movePlace, moveHomeOnTown, updatePlace, reorderPlace, addTownDecoration, updateTownDecoration, moveTownDecoration, reorderTownDecoration, deleteTownDecoration, resetAll, cloneState, setHomeEditMode, updateHome, createHome, createTownHome, deleteHome, addCharacterResidence, removeCharacterResidence, updateCharacterResidence, updateRoom, addRoom, setHomeFloorCount, setActiveHomeFloor, setRoomType, deleteRoom, addPet, updatePet, deletePet, setPetImage, addCar, updateCar, deleteCar, addFurniturePlacement, moveFurniturePlacement, updateFurniturePlacement, deleteFurniturePlacement, addFurnitureProp, deleteFurnitureProp, assignFurnitureBed, advanceHomeLifeSimulation, setHomeResidents, moveCharacter, addCatalogItem, updateCatalogItem, deleteCatalogItem, toggleFavorite, toggleOwned, togglePlaceStock, setCharacterPane, addTown, switchTown, deleteTown, recordCharacterInteraction, setDailyQuestion, updateRoutineDays, deleteRoutine as deleteStateRoutine, deleteMonthlyRoutine as deleteStateMonthlyRoutine, scheduleCharacterChoice, settleScheduledChoices, directCharacterActivity} from "./state.js?v=20260909dev289";
import {roomPermissionMarkup,bindRoomPermissionEditor,readRoomPermissionEditor} from "./room-permissions.js?v=20260909dev289";
import {bindHomeEditorUI,homeEditorCopy,fitFurnitureSelection,filteredFurniture} from "./home-editor-ui.js?v=20260909dev289";
import {toggleDislike} from "./state.js?v=20260909dev289";
import {eventFor,forceCharactersHome,nextSceneRefreshDelay,timeline,withSimulationBatch} from "./simulation.js?v=20260909dev289";
import {setCharacterSceneImage} from "./state.js?v=20260909dev289";
import {SCENE_IMAGE_VARIANTS,normalizeSceneImageVariants} from "./character-scene-image.js?v=20260909dev289";
import {mountDictionary,refreshDictionaryImage} from "./dictionary.js?v=20260909dev289";
import {nativeLogContents,homeLogMarkup,buildingDetailDialogs,entranceTransitionEnds} from "./views.js?v=20260909dev289";
import {mailEnvelope,createContactMailbox} from "./notification-mail.js?v=20260909dev289";
import {renderApp, relationshipMapMarkup, catalogCardMarkup, catalogSubgenreOptions, setAccountLabel, setAccountEntitlements, setMobileTownMode, setMobileTownPanel, setMobileTownPlacement, setSettingsPane, setNativeShopSection, translateDynamicInterface, appearancePreviewColor, hairCurlPreviewPath} from "./views.js?v=20260909dev289";
import {initializeLocalMediaState,persistLocalImage,informationOnlyState,localMediaUsage,isPendingLocalImage} from "./local-media.js?v=20260909dev289";
import {SPEECH_STYLE_OPTIONS,characterQuestionPrompt,characterContactSpeech,characterContactTitle} from "./speech-styles.js?v=20260909dev289";
import {CONTACT_VOICE_VERSION,characterMomentSpeech} from "./contact-voice.js?v=20260909dev289";
import {characterNotificationsAvailable,characterNotificationPermission,requestCharacterNotificationPermission,initializeCharacterNotifications,replaceCharacterNotifications,scheduleCharacterNotification,cancelCharacterNotifications,characterNotificationLargeIcon} from "./character-notifications.js?v=20260909dev289";
import {mergeImportedBackupState} from "./sync-merge.js?v=20260909dev289";
import {normalizeRoomLayout,snapRoomLayout} from "./room-layout.js?v=20260909dev289";
import {installDirectSteps} from './direct-steps.js?v=20260909dev289';
import {FURNITURE_PROPS,furnitureCapacity,furnitureCatalogForRoom,furnitureFootprint,furnitureGridForRoom,furnitureIcon,furnitureLabel,furniturePropIcon,furniturePropLabel,isBedFurniture,normalizeFurniturePlacement,snapFurniturePosition,supportsFurnitureProps} from "./furniture-layout.js?v=20260909dev289";
import {HOME_SURFACE_KEYS,HOME_WALL_KEYS,homeSurfaceImage,homeSurfaceLabel,wallSurfaceImage,normalizeHomeSurface,normalizeWallSurface} from "./home-surfaces.js?v=20260909dev289";
import {homeLifeNextDelay} from "./home-simulation.js?v=20260909dev289";
import {syncBackgroundMusic} from './background-music.js?v=20260909dev289';
import {previewFootstep,stopMovementAudio,syncMovementAudio} from "./audio.js?v=20260909dev289";
import {TOWN_TYPE_SUBTYPES} from "./town-profile.js?v=20260909dev289";
import {accountStorage as localStorage} from "./account-storage.js?v=20260909dev289";
import {ACHIEVEMENTS,evaluateAchievements,googlePlayAchievementStatus,openGooglePlayAchievements,signInGooglePlayAchievements,syncGooglePlayAchievements,localizedAchievement} from "./achievements.js?v=20260909dev289";
...
import {switchAccountState} from "./state.js?v=20260909dev289";
import {translateText} from "./views.js?v=20260909dev289";
import {scheduleTownLighting} from "./town-lighting.js?v=20260909dev289";
import {PLACEMENTS,characterPlacement} from "./character-placement.js?v=20260909dev289";
import {characterMood} from "./character-mood.js?v=20260909dev289";
import {recoverableBuildings,restoreBuildings} from "./state.js?v=20260909dev289";
import {groupErrorMessage,showMultiplayerList,showMultiplayerDetail} from "./groups.js?v=20260909dev289";
...
installSettingsTransfer({translate:translateText,toast:showToast,render:()=>render(),limit:characterLimit});
function openProfileExportDialog(){
  ...
  dialog.innerHTML=`<form method="dialog"><div class="title"><div><h2>프로필 내보내기</h2><small>한글이 깨지지 않는 기본 글꼴로 관공서 제출 서류처럼 만들고, 아래에 서랍마을 기록 도장을 찍어요.</small></div><button value="cancel">×</button></div><p>${translateText("캐릭터 설정 파일에는 사진·생활 로그·관계를 포함하지 않아요. 전체 백업에는 관계와 생활 기록도 포함해요.")}</p><div class="profile-export-options"><button type="button" data-settings-transfer="character-code-export">${({ko:"사진 포함 공유 코드 만들기",en:"Create a sharing code with photos",ja:"写真付き共有コードを作成"}[state.uiLanguage]||"사진 포함 공유 코드 만들기")}</button><button type="button" data-settings-transfer="character-code-import">${({ko:"캐릭터 코드로 불러오기",en:"Import by character code",ja:"キャラクターコードで読み込む"}[state.uiLanguage]||"캐릭터 코드로 불러오기")}</button><button type="button" data-settings-transfer="character-export">${translateText("캐릭터 설정 다운로드")}</button><button type="button" data-settings-transfer="character-import">${translateText("캐릭터 설정 불러오기")}</button><button type="button" data-settings-transfer="all-export">${translateText("전체 설정 백업 다운로드")}</button><button type="button" data-export-format="png"><b>PNG 증명서</b><small>이미지 파일로 바로 저장</small></button><button type="button" data-export-format="pdf"><b>PDF 증명서</b><small>같은 문서를 PDF로 저장·인쇄</small></button></div></form>`;
...
function bindDirectActivityCommand(root,characterId,close){
  ...
  command.querySelector("[data-direct-social-submit]")?.addEventListener("click",async event=>{
    ...
    let accepted=false;event.currentTarget.disabled=true;try{accepted=await execute(kind,{targetId,subjectId,topic})}catch(error){showToast(error.message);return}finally{command.querySelector("[data-direct-social-submit]").disabled=false}
...
function openHomeOccupantSheet(button){
  ...
  const commandMarkup=character&&(!state.sharedContext||character.ownerUid===window.ParallelCityAuth?.getInfo?.()?.user?.uid)?directActivityCommandMarkup(character,sleeping):"";
  ...
  if(character)bindDirectActivityCommand(dialog,character.id,close);
...
function bind(){
  ...
  $$('[data-group-manage]').forEach(button=>button.onclick=async()=>{const {selectManagementPane}=await import('./groups.js?v=20260909dev289');selectManagementPane(button.dataset.groupManage);render()});
  ...
  $('[data-group-catalog-publish]')?.addEventListener('click',event=>runGroupAction(event.currentTarget,async()=>{const {chooseCatalog}=await import('./settings-transfer.js?v=20260909dev289');const chosen=await chooseCatalog(state.catalog);if(chosen)await groupApi.publishCatalog(chosen)}));
  $('[data-group-catalog-import]')?.addEventListener('click',event=>runGroupAction(event.currentTarget,async()=>{const transfer=await import('./settings-transfer.js?v=20260909dev289');transfer.mergeCatalogFile({format:'drawer-village-catalog',version:1,catalog:Object.fromEntries((groupApi.getSnapshot().catalog||[]).map(c=>[c.id,c.items||[]]))});render()}));
  ...
  $$('[data-character-command]').forEach(button=>button.addEventListener('click',event=>{
    ...
    if(!character)return;
    const scene=eventFor(character),home=scene?.home?state.homes?.[scene.visitHomeId||character.homeId]:null;
    button.dataset.characterId=character.id;
    button.dataset.homeOccupant="character";
    button.dataset.occupantName=character.name||"이름 없음";
    button.dataset.occupantTitle=scene?.title||"집에서 시간을 보내는 중";
    button.dataset.occupantDesc=scene?.desc||"";
    button.dataset.occupantRoom=home?.rooms?.[scene?.room]?.name||home?.name||state.world.name||"마을";
...
import("./auth.js?v=20260909dev289").catch(error=>{
...
if("serviceWorker" in navigator){
  ...
  if(nativeRuntime){
    ...
    navigator.serviceWorker.register("./sw.js?v=20260909dev289",{updateViaCache:"none"}).then(registration=>registration.update()).catch(error=>console.warn("오프라인 업데이트 준비 실패",error));
  }
}
const lockPortrait=()=>{
