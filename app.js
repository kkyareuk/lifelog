...
import {FURNITURE_PROPS,furnitureCapacity,furnitureCatalogForRoom,furnitureFootprint,furnitureGridForRoom,furnitureIcon,furnitureLabel,furniturePropIcon,furniturePropLabel,isBedFurniture,normalizeFurniturePlacement,snapFurniturePosition,supportsFurnitureProps} from "./furniture-layout.js?v=20260909dev305";
...
function openHomeOccupantSheet(button){
  ...
  if(character){
    ...
    dialog.append(occupantPanel({world:state,character,actor:!groupId||actor?.ownerUid===uid?actor:null,canSelect:owned,close,
      openActivities:id=>document.dispatchEvent(new CustomEvent('drawer-open-activities',{detail:{id,anchor:button}})),
