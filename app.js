...
import {eventFor} from "./simulation.js?v=20260805d";
import {renderApp, setAccountLabel, setAccountEntitlements} from "./views.js?v=20260805d";
...
function openRelationDialog(id=""){
  ...
  dialog.innerHTML=`<form method="dialog"><div class="title"><div><h2>${old?"관계 편집":"관계 추가"}</h2></div><button value="cancel">×</button></div>
    ...
    <label class="protection-role">돌봄·안전 책임의 방향<select name="protectionRole"><option value="none">돌봄 책임을 따로 정하지 않음</option><option value="a-protects-b">왼쪽 인물이 오른쪽 인물의 안전과 생활을 주로 챙김</option><option value="b-protects-a">오른쪽 인물이 왼쪽 인물의 안전과 생활을 주로 챙김</option><option value="mutual">서로의 안전과 생활을 함께 챙김</option></select><small>관계 이름과 별개인 항목이에요. 친구·상관·가족이어도 선택할 수 있고, 위험하거나 도움이 필요할 때 누가 먼저 살피고 책임지는지에 반영됩니다.</small></label>
  ...
  const relationTypeHelp={
    ...
    "유사가족":"법적·혈연 가족은 아니지만 가족처럼 생활하고 의지하는 관계예요. 서로를 꼭 가족이라고 명명할 필요는 없습니다.",
    "보호·피보호":"가족 호칭이 아니라 안전과 생활을 누가 우선 책임지는지를 중심으로 한 관계예요. 상관·동료·친구 사이에도 설정할 수 있습니다."
  };
  const updateType=()=>{
    refreshStages();
    const crush=f.type.value==="짝사랑",parent=f.type.value==="부모·자녀";
    f.querySelector(".crush-direction").hidden=!crush;
    f.querySelector(".parent-direction").hidden=!parent;
    f.querySelector(".relation-member-picker").hidden=crush||parent;
