...
import("./auth.js?v=20260804c").catch(error=>{
  console.warn("로그인 기능을 불러오지 못했지만 게임은 계속 실행됩니다.",error);
  setAccountLabel("Google 로그인");
});
if("serviceWorker" in navigator){
  navigator.serviceWorker.register("./sw.js?v=20260804c").catch(error=>console.warn("오프라인 업데이트 준비 실패",error));
}
