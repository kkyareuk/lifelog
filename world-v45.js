...
(()=>{
 ...
 const names={
  ...
  gym:['파동 체육관','새벽 피트니스'],cinema:['유성 영화관'],mall:['평행백화점','오로라 몰','라비앙 명품관','프레스티지 갤러리아'],station:['중앙역','별마루역'],pet:['꼬리별 동물병원','포근 펫숍'],
 ...
 function neighborhood(name,theme,seed,count=12){return{id:uid(),name,theme,background:'',buildings:makeBuildings(seed,count,theme)}}
 function district(name,theme,seed,hoodNames){return{id:uid(),name,theme,background:'',neighborhoods:hoodNames.map((n,i)=>neighborhood(n,theme,seed+i,theme==='city'?15:12))}}
 function freshWorld(){
  const village={id:'village',name:'평행도시',theme:'town',districts:[
  ...
  return{activeWorldId:'village',activeDistrictId:village.districts[0].id,activeNeighborhoodId:village.districts[0].neighborhoods[0].id,items:[village]};
 }
 function ensureWorld(){
  ...
  if(Number(state.worldSemanticVersion||0)<45){
   const village=state.worlds.items.find(item=>item.id==='village'),city=state.worlds.items.find(item=>item.id==='city');
   if(village&&city){village.name='평행도시';village.districts.push(...city.districts);state.worlds.items=state.worlds.items.filter(item=>item.id!=='city');state.worlds.activeWorldId='village'}
   const used=new Set(state.characters.map(c=>c.workBuildingId).filter(Boolean));
   state.worlds.items.forEach(item=>item.districts.forEach(d=>d.neighborhoods.forEach(n=>{
    const kept=[],types=new Set();
    (n.buildings||[]).forEach(b=>{const generated=b.description===`${b.name}에서 시간을 보낼 수 있어요.`;if(!generated||used.has(b.id)||b.icon||b.photo||(!types.has(b.type)&&kept.filter(x=>x.description===`${x.name}에서 시간을 보낼 수 있어요.`).length<12)){kept.push(b);types.add(b.type)}});
    n.buildings=kept;
   })));
   const first=state.worlds.items[0]?.districts[0]?.neighborhoods[0];
   if(first&&!first.buildings.some(b=>/명품/.test(b.name)))first.buildings.push({id:uid(),type:'mall',name:'라비앙 명품관',x:76,y:26,color:'#d5b35b',open:'10:30',close:'20:30',description:'명품 패션과 주얼리 매장을 둘러볼 수 있는 고급 쇼핑관'});
   state.worldSemanticVersion=45;queueMicrotask(()=>save?.());
  }
 ...
 function choose(c,type,seed,index=0){
  ...
  let list=pool;
  if(type==='mall'){
   const luxury=pool.filter(b=>/명품|프레스티지|갤러리아/.test(b.name)),ordinary=pool.filter(b=>!/명품|프레스티지|갤러리아/.test(b.name));
   if(c.income==='rich'&&luxury.length)list=luxury;else if(c.income==='low'&&ordinary.length)list=ordinary;
  }
 ...
 function renderMap(){
  if(state.observationMode==='real'&&window.ParallelCityGoogleBridge){window.ParallelCityGoogleBridge.show();return}
  ...
  if(picker)[...picker.children].forEach(card=>card.style.display='');
 ...
 function navMarkup(){
  ...
  return`<div class="world-switch">${state.worlds.items.map(x=>`<button class="${x.id===w.id?'active':''}" data-world="${x.id}">🏙️ ${esc(x.name)}</button>`).join('')}<button data-add-world>＋ 도시</button></div>
 ...
 function bindEditor(){
  ...
  $('[data-add-world]')?.addEventListener('click',()=>{const name=prompt('새 도시 이름을 적어 주세요.','새 도시');if(!name)return;const d=district('중앙 구역','city',uid(),['중앙 상가']);const item={id:uid(),name,theme:'city',districts:[d]};state.worlds.items.push(item);state.worlds.activeWorldId=item.id;state.worlds.activeDistrictId=d.id;state.worlds.activeNeighborhoodId=d.neighborhoods[0].id;save();renderEditor()});
 ...
 const tab=$('.tab[data-view="places"]');if(tab)tab.textContent='마을';
  ...
  const homeField=$('#charHome')?.parentElement;if(homeField){homeField.hidden=false;homeField.removeAttribute('hidden');const label=homeField.querySelector('label');if(label)label.textContent='현실 지도 생활권'}
