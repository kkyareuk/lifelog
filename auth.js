...
async function sharedTownRequest(action,body={}){
  ...
  const result=await response.json();if(response.ok&&action==='advance'&&result.lives&&groupState.activeGroupId===gid&&Number(groupState.group?.lifeUpdatedAt||0)<=Number(result.lifeUpdatedAt||0)){const updates=new Map(result.lives.map(l=>[l.id,l]));groupState.residents=groupState.residents.map(r=>updates.has(r.id)?{...r,...updates.get(r.id)}:r);groupState.group={...groupState.group,lifeUpdatedAt:result.lifeUpdatedAt,lifeNextAt:result.lifeNextAt};emitGroupState()}if(!response.ok)throw giftError(Object.assign(new Error(result.message||'Shared town failed'),{code:result.message||'groups/server-error'}));if(['deleteMail','sendMail','respondMailGift','respond','propose','requestResidence','removeMember'].includes(action))await refreshMailbox(true).catch(error=>console.warn('Mailbox refresh',error.code));return result;
}
