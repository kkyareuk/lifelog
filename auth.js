...
import {requestDeadline} from './request-deadline.js?v=20260909dev305';
...
import {mergeCloudRestoreState,mergeDeviceAndCloudState} from "./sync-merge.js?v=20260909dev305";
...
async function readCloudGameState(rootData,options={}){const value=await readRawCloudGameState(rootData,options);return value?applyCharacterTransfers(value,accountMailbox.uid===(options.uid||user?.uid)?accountMailbox.data.characterTransfers||[]:[]):value}
async function readRawCloudGameState(rootData,{fresh=false,uid=user?.uid}={}){
...
async function prepareState(local,manifest,previousState,session,{uploadPhotos=true}={}){
  if(local.characters&&accountMailbox.uid===session.uid)applyCharacterTransfers(local,accountMailbox.data.characterTransfers||[]);
  ...
  await mapConcurrent(jobs,3,async(job,i)=>{
    ...
    try{assertSession(session);if(!photoJobs.has(job.value))photoJobs.set(job.value,uploadDataUrl(job.value,manifest,session,knownItems));jobs[i].node[jobs[i].key]=await photoJobs.get(job.value)}
      ...
      photoFailures+=1;photoErrors.push(error.code||error.message||"storage/unknown");
...
let notificationRosterCache={uid:'',at:0,rows:[]};
async function readNotificationRoster(){const uid=user?.uid;if(!uid)return [];if(notificationRosterCache.uid===uid&&Date.now()-notificationRosterCache.at<120000)return notificationRosterCache.rows;const rows=[];for(const g of groupState.groups||[]){try{const members=await getDocs(collection(db,'groups',g.id,'residents'));rows.push(...members.docs.map(d=>({...d.data(),id:d.id,notificationGroupId:g.id,notificationGroupName:g.name})));}catch{}}if(user?.uid!==uid)return [];notificationRosterCache={uid,at:Date.now(),rows};return rows;}
const groupSnapshot=()=>{const s={...groupState,notificationResidents:notificationRosterCache.uid===user?.uid?notificationRosterCache.rows.filter(r=>groupState.groups?.some(g=>g.id===r.notificationGroupId)):[],...(accountMailbox.uid===user?.uid?accountMailbox.data:{})};const visible=window.DrawerVillageSafety?.filterSnapshot(s)||s;return {...visible,membershipMembers:groupState.members}};
...
function watchActiveGroup(groupId,{force=false}={}){
  const subscriptionKey=user&&groupId?`${user.uid}:${groupId}`:"";
  if(!force&&subscriptionKey&&subscriptionKey===groupSubscriptionKey&&groupUnsubscribers.length&&!groupState.error){emitGroupState();return}
  ...
  const listen=(reference,key,mapSnapshot)=>onSnapshot(reference,{includeMetadataChanges:true},snapshot=>{
    ...
    groupState={...groupState,subscriptionErrors:{...groupState.subscriptionErrors,[key]:error?.code||"groups/load-failed"},error:error?.code||"groups/load-failed",loading:false};emitGroupState();
  });
...
async function updateGroupRules(patch={}){
  ...
  const rules={
    ...
    allowMemberCareerAdd:patch.allowMemberCareerAdd===true,
...
async function prepareWorldPackage(pack){
 ...
 const prepared=await prepareState({package:pack},structuredClone(normalizeManifest(previous.data()?.mediaManifest,null)),null,session);
...
async function migratePersonalTown(input){
 const cloud=await sharedCloudState(),{makeWorldPackage}=await import('./world-transfer.js?v=20260909dev305');
...
async function refreshSharedResidents(){
  ...
  if(!gid)return;
  const mine=groupState.residents.filter(r=>r.ownerUid===account.uid&&!r.independentCharacter);
  ...
  for(const h of groupState.homes.filter(h=>h.ownerUid===account.uid&&!h.layoutJson)){const home=cloud.homes?.[h.sourceHomeId];if(home)batch.update(doc(db,'groups',gid,'homes',h.id),{layoutJson:JSON.stringify(sharedProfile(home)),exteriorImage:publicImage(home.exteriorImage||home.image),updatedAt:serverTimestamp()});}
...
async function sharedTownRequest(action,body={}){
  const requestUser=requireGroupUser(),gid=body.groupId||groupState.activeGroupId;
  const token=await requestUser.getIdToken();if(user?.uid!==requestUser.uid)throw Error('Account changed');
  ...
  const result=await response.json();if(user?.uid!==requestUser.uid)throw Error('Account changed');if(response.ok&&groupState.activeGroupId===gid){const confirmed=applyConfirmedTown(groupState,action,result);if(confirmed!==groupState){groupState=confirmed;emitGroupState()}}if(response.ok&&action==='advance'&&result.lives&&groupState.activeGroupId===gid&&Number(groupState.group?.lifeUpdatedAt||0)<=Number(result.lifeUpdatedAt||0)){const updates=new Map(result.lives.map(l=>[l.id,l]));groupState.residents=groupState.residents.map(r=>updates.has(r.id)?{...r,...updates.get(r.id)}:r);groupState.group={...groupState.group,lifeUpdatedAt:result.lifeUpdatedAt,lifeNextAt:result.lifeNextAt};emitGroupState()}if(!response.ok)throw giftError(Object.assign(new Error(result.message||'Shared town failed'),{code:result.message||'groups/server-error'}));if(['deleteMail','sendMail','respondMailGift','respond','propose','requestResidence','removeMember'].includes(action))await refreshMailbox(true).catch(error=>console.warn('Mailbox refresh',error.code));return result;
}
...
async function requestGroupAdmission(characterId,townId="",options={}){
  const account=requireGroupUser(),gid=options.groupId||groupState.activeGroupId,targetTown=townId||groupState.selectedTownId||groupState.group?.towns?.[0]?.id,cloud=await sharedCloudState(),c=cloud.characters?.[characterId];
  if(!c||user?.uid!==account.uid)throw Error('Character or group changed');
  const h=cloud.homes?.[c.homeId],resident={sourceCharacterId:c.id,name:c.name,job:c.jobTitle||c.job||'',townId:targetTown,sourceHomeId:c.homeId||'',profileJson:JSON.stringify(sharedProfile(withWardrobe(c,cloud.catalog))),scheduleJson:JSON.stringify({routines:cloud.routines?.[c.id]||[],monthlyRoutines:cloud.monthlyRoutines?.[c.id]||[]}),photo:publicImage(c.photo),icon:publicImage(c.icon)};
  return sharedTownRequest('requestResidence',{groupId:gid,requestId:options.requestId||crypto.randomUUID(),kind:'admission',resident,...(h?{home:{sourceHomeId:c.homeId,name:h.name,layoutJson:JSON.stringify(sharedProfile(h))}}:{})});
}
...
window.DrawerVillageGroups={
  court:(action,input={})=>{if(!["saveCourtRank","requestCourtRank","respondCourtRank","readCourt","saveCourtTheme","saveCourtProfile","saveCourtDistance"].includes(action))throw Error("invalid-action");return sharedTownRequest(action,input)},
  ...
  deleteResident:input=>sharedTownRequest('removeMember',{...input,deletePermanently:true,confirm:true}),createResident:createSharedResident,saveResident:saveSharedResident,refreshSlotUsage,readMailTargets:input=>sharedTownRequest('readMailTargets',input),saveMemberGroups:async input=>{const result=await sharedTownRequest('saveMemberGroups',input);window.dispatchEvent(new Event('drawer-village-mail-targets-changed'));return result},publishWorldCode:async pack=>sharedTownRequest('publishWorldCode',{package:await prepareWorldPackage(pack)}),readWorldCode:code=>sharedTownRequest('readWorldCode',{code}),revokeWorldCode:code=>sharedTownRequest('revokeWorldCode',{code}),migrateTown:migratePersonalTown,publishCharacterCode,readCharacterCode:code=>sharedTownRequest('readCharacterCode',{code}),revokeCharacterCode:code=>sharedTownRequest('revokeCharacterCode',{code}),respondMailGift:input=>sharedTownRequest('respondMailGift',input),sendMail:async input=>sharedTownRequest('sendMail',{...input,giftWorkflow:2,gift:input.gift?await prepareWorldPackage(input.gift):null,...(input.gift?{positions:window.ParallelCity.getMeetingPositions?.([input.sourceId,input.targetId])}:{})}),readMoveCandidates:()=>sharedTownRequest('readMoveCandidates'),requestAdmission:(characterId,options={})=>{if(characterId.startsWith('group:')){const [,sourceGroupId,sourceResidentId]=characterId.split(':');return sharedTownRequest('requestResidence',{groupId:options.groupId||groupState.activeGroupId,requestId:options.requestId||crypto.randomUUID(),kind:'admission',sourceGroupId,sourceResidentId,townId:options.townId||groupState.selectedTownId||groupState.group.towns[0].id})}return requestGroupAdmission(characterId,options.townId,options)},requestCohabitation:input=>sharedTownRequest('requestResidence',{requestId:crypto.randomUUID(),kind:'cohabitation',...input}),deleteRelationship:input=>sharedTownRequest("deleteRelationship",input),propose:async input=>{let mode='propose';if(['owner','manager','operator'].includes(activeGroupRole())){mode=await chooseProposalMode(input.kind);if(!mode)throw Object.assign(Error('취소했어요.'),{code:'groups/cancelled'})}return sharedTownRequest('propose',{requestId:crypto.randomUUID(),...input,applyAsManager:mode==='apply'})},respond:input=>sharedTownRequest('respond',input),saveView:input=>sharedTownRequest('saveView',input),registerDevice:input=>sharedTownRequest('registerDevice',input),unregisterDevice:input=>sharedTownRequest('unregisterDevice',input),refreshResidents:refreshSharedResidents,advanceLife:advanceSharedLife,command:command=>sharedTownRequest('advance',{command:{...command,positions:window.ParallelCity.getMeetingPositions?.([command.characterId,command.targetId])}}),saveTownEdit:input=>sharedTownRequest('saveTownEdit',input),careerWorld:input=>sharedTownRequest('careerWorld',input),walletSharing:input=>sharedTownRequest('walletSharing',input),characterMoney:input=>sharedTownRequest('characterMoney',input),saveBuilding:input=>sharedTownRequest('saveBuilding',input),createTown:async({name})=>{if(!await upload({silent:true,reason:"멀티 마을 생성"}))throw Error("Cloud upload failed");assertMultiplayerTownSlot();const result=await sharedTownRequest('saveTown',{create:true,townId:crypto.randomUUID(),patch:{name},revision:Number(groupState.group?.buildingRevision)||0});await refreshSlotUsage();window.DrawerVillageGroups.selectTown(result.town.id);return result;},saveTown:input=>sharedTownRequest('saveTown',input),saveHomeMember:input=>sharedTownRequest('saveHomeMember',input),saveHomeLayout:input=>sharedTownRequest('saveHomeLayout',input),saveHomePlacement:input=>sharedTownRequest('saveHomePlacement',input),saveDecoration:input=>sharedTownRequest('saveDecoration',input),
  setDetailActive:setGroupDetailActive,
  select:(groupId,options)=>watchActiveGroup(String(groupId||""),options),
...
async function deleteOwnAccount(){
 ...
 if(!user)throw Error(text('먼저 로그인해 주세요.','Sign in first.','先にログインしてください。'));
 if(deletingAccount||busy||switchingAccount)throw Error(text('동기화가 끝난 뒤 다시 시도해 주세요.','Try again after synchronization finishes.','同期完了後に再試行してください。'));
