...
(()=>{
 ...
 const JOBS=['무직','초·중·고등학생','대학생','회사원','공무원','교사','교수','의사','간호사','약사','연구원','개발자','디자이너','출판 편집자','작가','기자','변호사','회계사','요리사','제과제빵사','바리스타','미용사','메이크업 아티스트','예술가','음악가','배우','아이돌','운동선수','경찰','소방관','군인','배달 기사','운전기사','자영업·직접 입력'];
 const value=id=>document.querySelector(`#${id}`)?.value;
 function installJobPicker(){
  const original=$('#charJob');if(!original)return;if($('#charJobPreset')){window.syncParallelJobPicker?.();return}
  original.type='hidden';
  const wrap=document.createElement('div');wrap.className='job-picker-v45';wrap.innerHTML=`<label>직업 선택</label><select id="charJobPreset">${JOBS.map(job=>`<option>${esc(job)}</option>`).join('')}</select><input id="charJobCustom" placeholder="가게 사장, 조향사처럼 직접 입력">`;
  original.parentElement.append(wrap);
  const sync=()=>{const job=active?.().job||'',preset=JOBS.includes(job)&&job!=='자영업·직접 입력'?job:'자영업·직접 입력';$('#charJobPreset').value=preset;$('#charJobCustom').value=preset==='자영업·직접 입력'?job:'';$('#charJobCustom').style.display=preset==='자영업·직접 입력'?'block':'none';original.value=job};
  $('#charJobPreset').onchange=()=>{const custom=$('#charJobPreset').value==='자영업·직접 입력';$('#charJobCustom').style.display=custom?'block':'none';active().job=custom?$('#charJobCustom').value:$('#charJobPreset').value;original.value=active().job;save?.()};
  $('#charJobCustom').oninput=()=>{active().job=$('#charJobCustom').value.trim();original.value=active().job;save?.()};sync();
  window.syncParallelJobPicker=sync;
 }
 function installRealAreaFields(){
  const home=$('#charHome');if(!home)return;
  const homeLabel=home.parentElement?.querySelector('label');if(homeLabel)homeLabel.textContent='현실 지도 집 생활권';
  if($('#charRealWorkArea')){$('#charRealWorkArea').value=active()?.realWorkArea||'';return}
  const wrap=document.createElement('div');wrap.innerHTML='<label>현실 지도 직장·학교 생활권</label><input id="charRealWorkArea" placeholder="예: 광교중앙역, 홍대입구역">';
  home.parentElement.after(wrap);$('#charRealWorkArea').value=active()?.realWorkArea||'';$('#charRealWorkArea').oninput=()=>{active().realWorkArea=$('#charRealWorkArea').value.trim();save?.()};
 }
 function safeCollectProfile(){
  const c=active();if(!c)return c;
  c.name=(value('charName')||'').trim()||c.name||'이름 없음';
  c.job=$('#charJobPreset')?.value==='자영업·직접 입력'?($('#charJobCustom')?.value||'').trim():($('#charJobPreset')?.value||value('charJob')||c.job);
  c.home=(value('charHome')||c.home||'').trim();c.mood=value('charMood')||c.mood;c.income=value('charIncome')||c.income;c.spending=value('charSpending')||c.spending;c.rhythm=value('charRhythm')||c.rhythm;c.transport=value('charTransport')||c.transport;c.pet=value('charPet')||c.pet;
  c.realWorkArea=(value('charRealWorkArea')||c.realWorkArea||'').trim();
  c.photoPosition={x:Number(value('photoPositionX')??c.photoPosition?.x??50),y:Number(value('photoPositionY')??c.photoPosition?.y??50)};
  c.workBuildingId=value('charVillageWork')||c.workBuildingId||'';const building=window.ParallelCityVillage?.locate?.(c.workBuildingId);if(building)c.work=building.name;
  if(value('charHomeType'))c.homeType=value('charHomeType');if(value('charBedCount'))c.bedCount=Number(value('charBedCount'));if(value('charSleepArrangement'))c.sleepArrangement=value('charSleepArrangement');if(value('charDogBreed'))c.dogBreed=value('charDogBreed');if(value('charCatBreed'))c.catBreed=value('charCatBreed');if(value('charPetColor'))c.petColor=value('charPetColor');
  return c;
 }
 ...
 function bindProfileUpload(){
  ...
  input.addEventListener('change',async event=>{
   ...
   try{safeCollectProfile();const photo=await compress(file),character=state.characters.find(c=>c.id===id);if(!character)return;character.photo=photo;await persist('프로필 사진을 저장했어요.')}
 ...
 function bindModeSwitch(){
  ...
  $$('[data-observation-mode]').forEach(button=>{if(button.dataset.modeBound)return;button.dataset.modeBound='1';button.onclick=()=>{
   ...
   if(mode==='real'){
    ...
    Promise.all(state.characters.map(character=>{const event=typeof currentEvent==='function'?currentEvent(character,minute):null;if(event)event.anchorArea=/출근|근무|수업|회사|학교/.test(`${event.title||''} ${event.detail||''}`)?(character.realWorkArea||character.home):character.home;return event&&!event.home?Promise.resolve(window.resolveRealEventPlace?.(character,event)):null})).then(()=>{save?.();window.ParallelCityGoogleBridge?.show()}).catch(console.warn);
 ...
 function install(){
  ...
  installJobPicker();installRealAreaFields();bindProfileUpload();bindObservationDrag();addBuildingPhotoControl();updatePlaceCard();bindModeSwitch();applyActiveCharacterTheme?.();
