...
async function playRefund(db,item){
 ...
 for(const record of matches.docs){if(!['diamonds_100','ad_free'].includes(record.data().productId))continue;const event=db.collection('playDiamondRefunds').doc(crypto.createHash('sha256').update(record.id+':'+item.voidedTimeMillis+':'+(item.voidedQuantity||'full')).digest('hex'));
  await db.runTransaction(async tx=>{const [prior,receipt]=await Promise.all([tx.get(event),tx.get(record.ref)]);if(prior.exists)return;const saved=receipt.data(),root=db.collection('users').doc(saved.uid),[u,deleted]=await Promise.all([tx.get(root),tx.get(db.collection('deletedAccounts').doc(saved.uid))]),already=Number(saved.refundedQuantity)||0,total=Number(saved.quantity)||1,quantity=Math.max(0,Math.min(total-already,Number(item.voidedQuantity)||total-already));
   const ent=u.data()?.entitlements||{};if(!deleted.exists)tx.set(root,{entitlements:saved.productId==='ad_free'?{...ent,adFree:false}:{...ent,diamondPaid:(Number(ent.diamondPaid)||0)-quantity*100}},{merge:true});tx.update(record.ref,{refundedQuantity:already+quantity,revoked:already+quantity>=total});tx.create(event,{receiptId:record.id,quantity,createdAt:Date.now()});
  });
 }
}
