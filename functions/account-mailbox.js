module.exports=({db,clock=Date.now})=>async uid=>{
 ...
 }));const notices=await db.collection('villageAnnouncements').where('createdAt','>',cutoff).orderBy('createdAt','desc').limit(30).get();const incoming=entries.find(([key])=>key==='incomingMail');incoming[1].push(...notices.docs.map(d=>({id:'notice-'+d.id,...d.data(),recipientUid:uid})));const transfers=await db.collection('users').doc(uid).collection('characterTransfers').get();return {...Object.fromEntries(entries),characterTransfers:transfers.docs.map(d=>d.data())};
};
