...
module.exports=({db,clock=Date.now})=>async(uid,input)=>db.runTransaction(async tx=>{
 ...
 if(existing.exists||group.data().ownerUid===uid){const role=group.data().ownerUid===uid?'owner':existing.data().role||'member',joinedAt=existing.data()?.joinedAt||clock();if(group.data().ownerUid===uid)tx.set(member,{uid,role,joinedAt,displayName:existing.data()?.displayName||String(input.displayName||'').slice(0,60)},{merge:true});tx.set(index,{groupId,role,joinedAt});return {groupId,alreadyJoined:true};}
