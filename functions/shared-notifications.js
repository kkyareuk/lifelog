...
module.exports=({db})=>{
  const deliver=onDocumentCreated({document:'notificationOutbox/{eventId}',region:'asia-northeast3',retry:true},async event=>{
    ...
    const source=await db.collection('groups').doc(item.groupId).collection(['mail-received','member-returned'].includes(item.kind)?'mail':'proposals').doc(item.proposalId).get(),info=source.data(),other=info?.senderUid===item.uid?info?.recipientUid:info?.senderUid;if(other&&await require('./user-safety').blocked(db,{get:ref=>ref.get()},item.uid,other)){await event.data.ref.update({deliveredAt:Date.now(),skipped:'account-blocked'});return}
