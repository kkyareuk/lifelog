...
module.exports=({db,signedInUser})=>{
 ...
 app.post('/discovery/testReward',async(req,res)=>{try{const identity=await signedInUser(req);res.json(await require('./discovery-test-rewards').createDiscoveryTestRewards({db}).grant(identity,req.body||{}))}catch(e){res.status(e.status||503).json({code:e.status?e.message:'ads-unavailable'})}});
