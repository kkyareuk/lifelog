...
module.exports=({db,membership,notify,clock,id,engine})=>async(uid,input)=>db.runTransaction(async tx=>{
 ...
 if(['member','announcement'].includes(input.audience)){
  ...
  let recipients=members.docs.filter(d=>d.id!==uid&&(input.audience==='member'?d.id===input.targetUid:scope==='all'||scope.startsWith('role:')&&(d.id===group.ownerUid?'owner':d.data().role==='operator'?'manager':d.data().role)===(scope.slice(5)==='operator'?'manager':scope.slice(5))||subgroup?.memberIds.includes(d.id)));if(!recipients.length)fail('recipient-missing',404);if(recipients.length>200)fail('recipient-limit');const checks=await Promise.all(recipients.map(r=>require('./user-safety').blocked(db,tx,uid,r.id)));recipients=recipients.filter((r,i)=>!checks[i]);if(!recipients.length)fail('contact-unavailable',403);
