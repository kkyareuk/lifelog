...
module.exports=({db,membership,notify,clock,id,engine})=>async(uid,input)=>db.runTransaction(async tx=>{
 ...
 if(['member','announcement'].includes(input.audience)){
  ...
  const recipients=members.docs.filter(d=>d.id!==uid&&(input.audience==='member'?d.id===input.targetUid:scope==='all'||scope.startsWith('role:')&&d.data().role===scope.slice(5)||subgroup?.memberIds.includes(d.id)));if(!recipients.length)fail('recipient-missing',404);if(recipients.length>200)fail('recipient-limit');
