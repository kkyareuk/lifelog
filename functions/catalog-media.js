...
function mergeCatalogItems(existing,incoming,kind){
 const merged=new Map(existing.map(item=>[item.id,item]));
 for(const item of incoming){
  validateCatalogMedia(item);
  const old=merged.get(item.id);
  if(old){if((!old.image||local(old.image))&&typeof item.image==='string'&&/^https:\/\//.test(item.image))merged.set(item.id,{...old,image:item.image});continue}
