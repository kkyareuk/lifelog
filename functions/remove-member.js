...
async function inTransaction({db,clock=Date.now},tx,uid,input){
 ...
 if(selected.some(r=>{const life=JSON.parse(r.lifeJson||'{}'),profile=JSON.parse(r.profileJson||'{}');return (life.wallet||profile.wallet)?.poolId}))fail('money-unshare-first',409);
