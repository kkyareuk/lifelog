...
await page.locator('[data-discovery-feedback]').click();assert.equal(await page.evaluate(()=>qaReport),null);await page.locator('[data-fit-reason=too-kind]').click();await page.locator('[data-fit-answer="0"]').click();await page.screenshot({path:resolve(output,lang+'-reasons.png')});await page.locator('[data-fit-send]').click();assert.equal(await page.evaluate(()=>qaReport.category),'discovery-answer-fit');assert.equal(await page.evaluate(()=>JSON.parse(qaReport.message).questionId),'qa');assert.deepEqual(await page.evaluate(()=>JSON.parse(qaReport.message).reasonIds),['too-kind']);assert.deepEqual(await page.evaluate(()=>JSON.parse(qaReport.message).closestAnswerIndices),[0]);await page.screenshot({path:resolve(output,lang+'.png')});await page.evaluate(()=>document.querySelector('dialog[open]').close());await page.waitForTimeout(100);}
