...
await service.saveHomeLayout('member',{groupId:'g',id:'own',revision:0,layout:{image:'https://example.invalid/home.png',rooms:{living:{name:'Living',accessCharacterIds:['a'],wallMaterial:'cream-plain',furniturePlacements:[{id:'photo-bed',item:'커플 침대',image:'https://example.invalid/shared-bed.webp',imageWidth:240,imageHeight:400,assignedCharacterIds:['a']}] }},deletedRoomKeys:['bed']}});
assert.equal(JSON.parse(data.get('groups/g/homes/own').layoutJson).rooms.living.furniturePlacements[0].image,'https://example.invalid/shared-bed.webp');
assert.deepEqual(JSON.parse(data.get('groups/g/homes/own').layoutJson).deletedRoomKeys,['bed']);assert.equal(JSON.parse(data.get('groups/g/homes/own').layoutJson).image,'https://example.invalid/home.png');assert.deepEqual(JSON.parse(data.get('groups/g/homes/own').layoutJson).rooms.living.accessCharacterIds,['a']);
