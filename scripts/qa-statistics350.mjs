...
for(const score of [49.9,50,50.1]){
 await page.evaluate(value=>{g.state.uiLanguage='ko';for(const c of Object.values(g.state.characters)){c.perceptionStyle='균형형';c.discovery={scores:{perceptionStyle:value}}}window.DrawerVillageNavigation.go('statistics');document.querySelectorAll('dialog[open]').forEach(d=>d.close())},score);
 const row=page.locator('.discovery-statistics tr').filter({hasText:'감각과 직관'});const text=await row.textContent();assert.equal(text.includes('균형형'),score===50,text);
}
console.log('PASS village averages, exact-50 balance, below/above direction, KO/EN/JA');
