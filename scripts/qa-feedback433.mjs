...
try{
 ...
 const conversation=await page.evaluate(async()=>{
  const {bindSceneDepth,scheduleSceneDepth}=await import('/scene-depth.js');
  const root=document.createElement('div');document.body.append(root);
  const results=[];
  for(const width of [95,140,240]){
   root.innerHTML=`<div class="room" style="position:relative;width:${width}px;height:360px;min-width:0;container-type:size"><div class="room-people has-home-life" style="position:absolute;inset:0;width:100%;height:100%">${['피콜로','메다'].map((name,i)=>`<div class="home-person home-life-person home-life-using is-conversing conversation-slot-${i+1}" data-character-id="${i}" style="--life-x:${33+i*34}%;--life-y:50%"><span class="home-person-visual" style="width:48px;height:48px;animation:none!important"><span class="avatar" style="display:block;width:48px;height:48px;background:green"></span></span><span class="home-person-status" data-interaction-id="talk" data-person-name="${name}"><b>${name}</b><small>추억에 대해 대화하는 중</small></span></div>`).join('')}</div></div>`;
   bindSceneDepth(root);await new Promise(r=>setTimeout(r,100));
   const measure=()=>[...root.querySelectorAll('.home-person-visual')].map(e=>{const r=e.getBoundingClientRect();return {left:r.left,right:r.right,top:r.top}});
   const first=measure();scheduleSceneDepth();await new Promise(r=>setTimeout(r,60));const second=measure();
   const visible=[...root.querySelectorAll('.home-person-status')].filter(e=>!e.hidden);
   results.push({width,gap:second[1].left-second[0].right,sameY:Math.abs(second[0].top-second[1].top),stable:Math.abs(first[0].left-second[0].left),labels:visible.map(e=>e.textContent)});
  }
  root.remove();return results;
 });
 for(const r of conversation){assert(r.gap>=5,JSON.stringify(r));assert(r.sameY<2,JSON.stringify(r));assert(r.stable<1,JSON.stringify(r));assert.deepEqual(r.labels,['피콜로 * 메다추억에 대해 대화하는 중']);}
 const homeSound=await page.evaluate(async()=>{
  const audio=await import('/web-audio.js?v=20260909dev305');audio.setAudioSetting(g.state,'soundMuted',false);audio.setAudioSetting(g.state,'backgroundMusicMuted',true);g.state.activeTab='observe';document.querySelectorAll('dialog[open]').forEach(d=>d.close());document.documentElement.classList.add('native-app');
  const hud=document.createElement('div');hud.className='game-observe-hud';hud.innerHTML='<div class="game-hud-roster-options"><button data-multiplayer-resident="a" class="on">A</button><button data-multiplayer-resident="b">B</button></div><div class="swipe-target">Swipe here</div>';
  document.querySelectorAll('.game-observe-hud').forEach(e=>e.remove());document.body.append(hud);let selected='';window.DrawerVillageGroups.selectResident=id=>{selected=id};qaBindCharacterSwipe();
  await new Promise(r=>setTimeout(r,80));const before=qaSounds.length,target=hud.querySelector('.swipe-target');
