...
try{
 ...
 await page.screenshot({path:out+'/house445.png'});
 ...
 await page.evaluate(()=>{window.DrawerVillageNavigation.go('shop');qaRender()});await page.waitForTimeout(100);assert.equal(await page.locator('.web-account-status').count(),0,'shop omits login status');assert.equal(await page.locator('.drawer-shop-content [data-account-identity]').count(),0,'shop omits linked login');
 ...
 await page.evaluate(async()=>{
  document.body.innerHTML='<div id="bed" style="position:relative;width:180px;height:300px"><span class="room-furniture-art" style="display:block;width:180px;height:300px"><img class="couple-bed-base" src="/assets/furniture/couple-bed/couple-bed-base.png" style="width:180px;height:300px;object-fit:contain"></span></div>';
 ...
 await page.screenshot({path:out+'/groups445.png'});
  console.log('PASS445 house canvas dimensions, preserved layout, restored scroll, tap cycling and independent attached-object drag');
