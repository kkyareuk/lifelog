...
let c=fresh(),patch=discoveryAnswer(c,q('early'),5);assert.equal(patch.discovery.scores.planningStyle,52);assert.equal(patch.discovery.scores.interference,52);assert.equal(patch.discovery.scores.impulseControl,52);
assert.equal(discoveryAnswer(c,q('early'),6).discovery.scores.planningStyle,48);
patch=discoveryAnswer(c,q('rain'),5);assert.equal(patch.discovery.scores.planningStyle,52);assert.equal(patch.discovery.scores.impulseControl,49);assert.equal(discoveryAnswer(c,q('rain'),6).discovery.scores.aggressionLevel,2);
