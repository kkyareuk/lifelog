...
(async()=>{
 ...
 const staff=await relations.propose('op',{groupId:'g',requestId:'staff-schedule',kind:'schedule',patch:{...schedule,memberIds:['a','c']}});assert.equal(staff.status,'accepted');assert.ok(data.has('groups/g/schedules/accepted-staff-schedule'));assert.ok(!data.has('groups/g/proposals/staff-schedule-member'));
 const staffRelation=await relations.propose('host',{groupId:'g',requestId:'staff-relation',patch:{...draft,groupMembers:['a','b']}});assert.equal(staffRelation.status,'accepted');assert.ok(data.has('groups/g/relationships/accepted-staff-relation'));
