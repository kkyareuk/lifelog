...
try{
 ...
 await page.evaluate(async()=>{const storage=await import('/account-storage.js?v=20260907hotfix255');storage.accountStorage.switchScope('checkout-test');storage.accountStorage.setItem('drawer-village-cart',JSON.stringify({town_slot_1:2}));localStorage.setItem('drawer-village-cart',JSON.stringify({green_tea:1}));});
 await page.goto(origin+'/payment.html?cart=1');await page.waitForSelector('.order-line');assert.equal(await page.locator('#itemCount').textContent(),'2개');const mode=await page.evaluate(()=>window.PARALLEL_CITY_CONFIG.paymentEnvironment);assert.equal(mode,'live');console.log('PASS live checkout shows the account-scoped cart, no payment attempted');
