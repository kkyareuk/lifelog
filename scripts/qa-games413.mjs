...
try{
 ...
 for(const size of [{width:360,height:648},{width:384,height:832}]){await page.setViewportSize(size);raw.notebook=true;raw.drama=true;
  for(const lang of ['ko','en','ja']){await page.evaluate(lang=>s.state.uiLanguage=lang,lang);raw.players.forEach(p=>p.icon='./assets/home-ui/profile-placeholder.png');raw.phase='move';await page.evaluate(g=>paint(g)(),engine.view(raw,'qa'));await page.waitForTimeout(150);
   assert(await page.locator('.mp-missions').isVisible());assert.equal(await page.locator('.mp-card,.mp-hand,[data-notebook]').count(),0);assert.equal(await page.locator('.mp-top>img').getAttribute('src'),'./assets/home-ui/profile-placeholder.png');
