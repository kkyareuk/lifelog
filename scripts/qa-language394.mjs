...
const {chromium,webkit}=require(playwrightPath),output=resolve(root,"tmp/qa-language394");
...
try{
 ...
 for(const language of ['ko','en','ja']){
  ...
  if(language==='ko'){
   await box.locator('[data-field="selfKo"]').fill('짐');await box.locator('[data-field="referenceKo"]').fill('그');assert.match(await box.locator('[data-language-preview="ko"]').textContent(),/짐이/);
  }else if(language==='en'){
  ...
  await page.screenshot({path:resolve(output,'language394-'+language+'.png')});await dialog.locator('[data-save-mobile-character-editor]').first().click();await dialog.waitFor({state:'detached'});
 ...
 assert.equal(errors.length,0,errors.join('\n'));console.log('PASS394 UI: KO/EN/JA mobile profile, custom inputs, live preview, save/reload, language preservation, import-code profile roundtrip, no overflow');
