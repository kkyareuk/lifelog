...
try{
 ...
 await page.evaluate(()=>{DrawerVillageNavigation.go('settings')});await page.locator('[data-settings-transfer=world-transfer]:visible').first().click();
 ...
 await dialog.getByRole('button',{name:'닫기',exact:true}).click();await page.locator('[data-settings-transfer=world-transfer]:visible').first().click();
 await dialog.locator(':scope > select').first().selectOption('home');await dialog.getByRole('button',{name:'공유 코드 만들기',exact:true}).click();await page.waitForFunction(()=>qaPublish.length===1);await page.waitForTimeout(50);assert.equal(await dialog.locator('section input').inputValue(),'ABCDEF-123456-ABCDEF');assert.equal(await page.evaluate(()=>qaPublish[0].kind),'home');
 await dialog.getByRole('button',{name:'닫기',exact:true}).click();await page.setViewportSize({width:1280,height:800});await page.locator('[data-settings-transfer=world-transfer]:visible').first().click();assert.ok(await dialog.evaluate(e=>e.getBoundingClientRect().width<=560));
