...
assert.match(gradle,/versionCode\s+229/);assert.match(gradle,/versionName\s+"1\.0\.212"/);
assert.doesNotMatch(manifest,/android:screenOrientation=|PROPERTY_COMPAT_ALLOW_RESTRICTED_RESIZABILITY/);
assert.match(index,/app\.css\?v=20260906dev229/);assert.match(prepareApp,/DRAWER_VILLAGE_NATIVE_BUILD="20260906dev229"/);
assert.doesNotMatch(`${views}\n${app}\n${auth}\n${prepareApp}\n${index}`,/20260906dev22[78]/);
...
assert.match(auth,/getInfo:\(\)=>\(\{ready:authSettled,user,startupSyncing:switchingAccount/);
assert.match(views,/if\(configured&&!window\.DrawerVillageAuthStartupFailed&&\(!authInfo\|\|!authInfo\.ready\|\|authInfo\.startupSyncing\)\)return accountLoading\(\)/);
assert.match(app,/document\.querySelector\("\.village-account-loading"\)\)render\(\)/);
...
console.log("PASS 229: startup blocks on account download; Android backup export, first-login adoption, bed, portrait, tablet, and header fixes remain intact");
