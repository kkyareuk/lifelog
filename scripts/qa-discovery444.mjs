...
try{
 ...
 for(const language of ['ko','en','ja']){
  ...
  assert(await page.locator('.discovery-rail-button').evaluate(el=>el.classList.contains('is-cooling-down')));
 ...
 await page.evaluate(async()=>{const original=window.ParallelCityAuth?.getInfo;window.ParallelCityAuth={...window.ParallelCityAuth,getInfo:()=>({...original?.(),startupSyncing:true})};const {considerDiscovery}=await import('/character-discovery.js?v=20260909dev305');considerDiscovery(g.state.characters[g.state.activeId],{});});
 assert(await page.locator('.discovery-rail-button').isVisible(),'sync keeps discovery visible');
  console.log('PASS444 discovery opens, skip closes, second character shares remaining account cooldown in KO/EN/JA');
