...
try{
 ...
 const ids=await page.evaluate(async()=>{const g=await import('/state.js?v=20260907hotfix264');g.resetAll();const a=g.createCharacter(10),b=g.createCharacter(10),town=g.addTown(10),c=g.createCharacter(10);g.switchTown(g.state.characters[a].townId);g.state.activeId=a;g.state.activeTab='observe';g.save(true);for(const tab of ['observe','character'])await window.ParallelCityAuth.markGuideSeen(tab);return{a,b,c,town}});
 await page.reload();await page.waitForFunction(()=>window.ParallelCity);
 await page.evaluate(async(id)=>{const g=await import('/state.js?v=20260907hotfix264'),sim=await import('/simulation.js?v=20260907hotfix264'),c=g.state.characters[id];sim.eventFor(c);const now=new Date(),key=`${now.getFullYear()}-${now.getMonth()+1}-${now.getDate()}`;Object.defineProperty(c.days[key],'signature',{configurable:true,get(){throw new Error('injected scene failure')}});window.ParallelCity.mediaChanged()},ids.a);
 await page.locator('[data-open-game-hud-roster]').click();await page.locator(`[data-home-character="${ids.b}"]`).click();
 assert.equal(await page.evaluate(async()=>(await import('/state.js?v=20260907hotfix264')).state.activeId),ids.b);
 await page.locator('[data-open-game-hud-roster]').click();await page.locator(`[data-observe-town="${ids.town}"]`).click();assert.equal(await page.evaluate(async()=>(await import('/state.js?v=20260907hotfix264')).state.activeTownId),ids.town);
