...
try{
 ...
 await page.locator('[data-direct-category=social]').click();await page.locator('[data-direct-target]').first().click();assert.equal(await page.locator('[data-social-section]').count(),3);await page.locator('[data-social-section=conflict]').click();await page.screenshot({path:resolve(output,'social-conflict-phone.png')});await page.locator('[data-direct-social-action=taunt]').click();assert.equal(await page.locator('[data-direct-social-submit]').isEnabled(),true);
 await page.locator('[data-social-section=friendly]').click();await page.locator('[data-direct-social-action=dine]').click();await page.locator('[data-direct-payment]').selectOption('request');await page.screenshot({path:resolve(output,'social-payment-phone.png')});await page.locator('[data-direct-social-submit]').click();await page.waitForFunction(()=>!document.querySelector('.direct-command-dialog'));
 ...
 await page.evaluate(()=>{game.state.characters[qaIds[0]].jobTitle='의사';game.state.dailyQuestion={day:new Date().toISOString().slice(0,10),minute:0,characterId:qaIds[0],kind:'everyday',shown:true,answered:false};location.hash='tab=mailbox'});await page.waitForTimeout(300);await page.evaluate(()=>document.querySelectorAll('dialog.page-guide[open]').forEach(d=>d.close()));await page.locator('[data-open-daily-question]').click();await page.locator('[data-character-question-option]').first().click();await page.waitForFunction(()=>!document.querySelector('dialog.mail-letter'));assert.ok((await page.locator('.mail-read-label').allTextContents()).includes('답변 완료'));
 await page.evaluate(()=>location.hash='tab=observe');await page.waitForTimeout(400);await page.locator('[data-character-command]').first().click();await page.locator('[data-direct-category=work]').click();await page.locator('[data-work-task=consult]').click();await page.waitForFunction(()=>!document.querySelector('.direct-command-dialog'));assert.ok(await page.evaluate(()=>game.state.characterDirectives[qaIds[0]].copy.ko.title.includes('진료')));
 assert.deepEqual(errors,[]);console.log('PASS command-only popup, three social categories, action selection, payer perspective, settings render');
