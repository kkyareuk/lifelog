...
const {chromium,webkit}=require(playwrightPath),output=resolve(root,"tmp/qa-language396");
...
try{
 ...
 for(const language of ['ko','en','ja']){
  ...
  assert.equal(await dialog.locator('.character-language-settings').isVisible(),false);await dialog.locator('[data-open-language-settings]').click();const popup=dialog.locator('.character-language-dialog[open]');await popup.waitFor();
  const box=popup.locator('.character-language-settings');assert.equal(await box.count(),1);
  ...
  await page.screenshot({path:resolve(output,'language396-'+language+'.png')});await popup.locator('[data-close-language-settings]').click();assert.equal(await popup.count(),0);await dialog.locator('[data-save-mobile-character-editor]').first().click();await dialog.waitFor({state:'detached'});
 ...
 assert.equal(errors.length,0,errors.join('\n'));console.log('PASS396 UI: KO/EN/JA mobile profile, custom inputs, live preview, save/reload, language preservation, import-code profile roundtrip, no overflow');
