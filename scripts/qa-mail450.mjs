...
try{
 ...
 await page.evaluate(()=>document.querySelector('.mail-letter[open]').close());
 await page.evaluate(()=>{g.state.dailyQuestion={day:new Date().toLocaleDateString('en-CA'),characterId:'',kind:'everyday',answered:false};qaRender()});
 assert(await page.evaluate(()=>!!g.state.characters[g.state.dailyQuestion.characterId]),'orphan question regenerates with real sender');
 const daily=await page.locator('[data-open-daily-question]').count();
 assert.equal(daily,1,'valid daily letter is present');
 if(daily){await page.locator('[data-open-daily-question]').tap();await page.waitForTimeout(150);assert.equal(await page.locator('.mail-letter[open]').count(),1,'daily letter must open, never silently fail');await page.evaluate(()=>document.querySelector('.mail-letter[open]').close())}
 await page.evaluate(()=>{g.state.dailyQuestion={day:'2000-01-01',characterId:id,kind:'weekend',answered:false};document.querySelector('[data-mail-folder=\"inbox\"]').click();g.state.dailyQuestion.day='2000-01-01';g.state.dailyQuestion.kind='weekend'});
 await page.locator('[data-open-daily-question]').tap();
 assert(await page.locator('.mail-letter[open] h2').textContent().then(t=>t.includes('주말')),'opening preserves the selected question across midnight');
 await page.evaluate(()=>document.querySelector('.mail-letter[open]').close());
