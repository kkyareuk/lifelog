...
assert.match(app,/spouseLegalValues=\["법적으로 관계가 등록됨","법적으로 관계가 등록되지 않음"\]/);
assert.match(app,/legalStatus:f\.legalStatus\.value/);
assert.doesNotMatch(state,/relation\.type==="부부"\?"법적으로 관계가 등록됨"/);
assert.match(state,/SPOUSE_LEGAL_STATUSES\.has\(value\)\?value/);
assert.match(gradle,/versionCode\s+216/);
assert.match(gradle,/versionName\s+"1\.0\.201"/);
console.log("PASS 216: catalog names, account-scoped mail deletion, and registered/unregistered spouses");
