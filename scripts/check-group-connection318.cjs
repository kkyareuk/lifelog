...
const c={console,Date,Map,Set,callbacks:[],user:{uid:'u'},groupSubscriptionKey:'',groupUnsubscribers:[],groupState:{groups:[]},readGroupContext:()=>({}),writeGroupContext(){},emitGroupState(){},groupRefs:()=>({}),collection:()=>({}),db:{},onSnapshot:(ref,...args)=>{const [success,error]=args.filter(x=>typeof x==='function');c.callbacks.push({success,error});return ()=>{}},stopGroupSubscriptions:()=>{c.groupUnsubscribers=[];c.groupSubscriptionKey=''},migratedSharedProfiles:new Set()};
