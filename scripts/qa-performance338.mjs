...
const baseline=process.argv.includes('--before'),count=process.argv.includes('--nine')?9:80;const root=resolve('.'),out=resolve('qa-performance338');await mkdir(out,{recursive:true});
const server=createServer(async(req,res)=>{try{const pathname=decodeURIComponent(new URL(req.url,'http://localhost').pathname),file=resolve(root,'.'+(pathname==='/'?'/index.html':pathname));if(!file.startsWith(root+sep))throw Error();const body=baseline&&pathname==='/app.js'?execFileSync('git',['show','964dc88:app.js']):await readFile(pathname==='/auth.js'?resolve('scripts/ios-preview-auth.mjs'):file);res.writeHead(200,{'Content-Type':({'.html':'text/html','.js':'text/javascript','.mjs':'text/javascript','.json':'application/json','.css':'text/css','.png':'image/png','.jpg':'image/jpeg','.webp':'image/webp','.svg':'image/svg+xml','.woff2':'font/woff2'})[extname(file)]||'application/octet-stream'}).end(body)}catch{res.writeHead(404).end()}});await new Promise(r=>server.listen(0,'127.0.0.1',r));
...
try{
 ...
 await p.evaluate(async count=>{const g=await import('/state.js?v=20260909dev305');g.createCharacter();const base=structuredClone(g.active());for(let i=1;i<count;i++){const c=structuredClone(base);c.id='perf-'+i;c.name='테스트 '+i;g.state.characters[c.id]=c;g.state.order.push(c.id);}document.querySelectorAll('dialog[open]').forEach(d=>d.close());for(const tab of ['observe','settings','town','character'])localStorage.setItem('drawer-village-guide-'+tab,'1');},count);
 ...
 const results=await p.evaluate(async()=>{const result=[];for(let i=0;i<2;i++)for(const tab of ['character','settings','town','observe','home']){const start=performance.now();window.DrawerVillageNavigation.go(tab);const sync=performance.now()-start;await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));result.push({tab,syncMs:sync,paintMs:performance.now()-start});}return result;});
 ...
 const report={characters:count,cpuSlowdown:4,results,hot,scope:'Local synthetic profiles, no images, no real multiplayer network'};await writeFile(`docs/performance338-${count}-${baseline?'before':'after'}.json`,JSON.stringify(report,null,2));console.log(JSON.stringify(report));await p.close();
