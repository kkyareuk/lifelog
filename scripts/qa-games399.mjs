...
const {chromium,webkit}=require(playwrightPath),output=resolve(root,"tmp/qa-games399");
...
try{
 ...
 for(const lang of ['ko','en','ja'])for(const phase of ['plan','debate','vote']){
  await page.evaluate(({lang,phase})=>{g.state.uiLanguage=lang;fixture.phase=phase;document.querySelectorAll('dialog[open]').forEach(d=>d.close());gameUI.openPlazaGames('g','one')},{lang,phase});await page.locator('[data-game-action]').waitFor({state:'visible'});
  ...
  await page.locator('[data-game-action] button').click();await page.waitForTimeout(50);assert(await page.evaluate(()=>gameCalls.some(c=>c.action==='submitGame'&&c.input.characterId==='p0'&&c.input.phaseIndex===0)));await page.locator('[data-social-dialog] header button').click();
 }
 console.log('PASS399 packaged mafia plan/discussion/vote UI in KO EN JA, narrow screen bounds and submission handlers');
