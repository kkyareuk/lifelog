...
const checks=[
  ...
  [css.includes('background-image:radial-gradient(circle at 18% 35%')&&!css.match(/\.relationship-stage\{[^}]*relationship-mosaic/s),"체크무늬 없이 관계색을 살린 관계 메인 배경"],
  ...
  [views.includes('relationshipMotionFor(heroLeft.id,heroRight.id,official)')&&css.includes('.relationship-motion-stranger .relationship-hero-pair::before{content:"?')&&css.includes('relationship-motion-crush-forward')&&css.includes('@keyframes relationship-cuddle-left')&&css.includes('@keyframes relationship-flash'),"배치 선호와 이방인·짝사랑 방향·쌍방 사랑·갈등을 함께 반영한 관계 연출"],
  ...
  [css.includes('.relationship-stage{')&&css.includes('background-color:#fff8eb!important')&&!css.match(/\.relationship-stage\{[^}]*relationship-mosaic/s),"관계 메인 무대를 체크무늬 없는 전용 종이 배경으로 분리"],
