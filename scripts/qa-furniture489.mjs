...
try{
 ...
 await p.evaluate(()=>{localStorage.setItem('drawer-village-feature-tour-v2-home','done');window.DrawerVillageNavigation.go('home')});
 await p.locator('[data-home-edit]').first().click();await p.screenshot({path:'tmp/photo489-before.png'});await p.locator('.home-tools-header').getByRole('button',{name:'가구 추가',exact:true}).click();await p.locator('[data-furniture-photo-library]').click();
 await p.locator('.furniture-photo-dialog select').selectOption('커플 침대');
 const png=await p.evaluate(()=>{const c=document.createElement('canvas');c.width=120;c.height=200;const x=c.getContext('2d');x.fillStyle='#e978a4';x.fillRect(10,5,100,190);return c.toDataURL('image/png').split(',')[1]});
 await p.locator('.furniture-photo-dialog input[type=file]').setInputFiles({name:'bed.png',mimeType:'image/png',buffer:Buffer.from(png,'base64')});
 await p.locator('.furniture-photo-dialog').waitFor({state:'detached'});
 const custom=p.locator('[data-furniture-kind="custom"]');await custom.first().waitFor();assert.equal(await custom.count(),1);assert.equal(await custom.locator('.couple-bed-layer').count(),0);assert.equal(await custom.locator('img').count(),1);
 await p.locator('[data-furniture-command="photo"]').click();assert.equal(await p.locator('.furniture-photo-grid button').count(),1);await p.locator('.furniture-photo-dialog').getByRole('button',{name:'닫기',exact:true}).click();
 await p.locator('.home-tools-header').getByRole('button',{name:'가구 추가',exact:true}).click();await p.locator('[data-furniture-photo-library]').click();await p.locator('.furniture-photo-dialog select').selectOption('커플 침대');await p.locator('.furniture-photo-grid button').click();await p.locator('.furniture-photo-dialog').waitFor({state:'detached'});assert.equal(await custom.count(),2);
 await p.screenshot({path:'tmp/furniture489.png'});
 await p.locator('.home-tools-header').getByRole('button',{name:'편집 완료',exact:true}).click();await p.reload();await p.waitForFunction(()=>window.photoQA);assert.equal(await p.evaluate(async()=>{const {state}=await import('/state.js?v=20260909dev305');return state.furniturePhotos.length}),1);
 await p.evaluate(async()=>{const {state}=await import('/state.js?v=20260909dev305');const {openFurniturePhotos}=await import('/furniture-photos.js');window.sharedPhotoPatch=null;openFurniturePhotos({world:state,upload:async data=>{if(!data.startsWith('data:image/'))throw Error('missing upload');return 'https://example.invalid/shared-bed.webp'},apply:async patch=>{window.sharedPhotoPatch=JSON.parse(JSON.stringify(patch))}})});
 await p.locator('.furniture-photo-dialog select').selectOption('커플 침대');await p.locator('.furniture-photo-grid button').first().click();await p.locator('.furniture-photo-dialog').waitFor({state:'detached'});assert.equal(await p.evaluate(()=>window.sharedPhotoPatch.image),'https://example.invalid/shared-bed.webp');
 assert(await p.evaluate(async()=>{const {normalizeFurniturePlacement}=await import('/furniture-layout.js?v=20260909dev305');const {roomFurnitureMarkup}=await import('/views.js?v=20260909dev305');const saved=normalizeFurniturePlacement({...window.sharedPhotoPatch,id:'remote'});const html=roomFurnitureMarkup('home','room',{furniturePlacements:[saved]},false);return html.includes('https://example.invalid/shared-bed.webp')&&!html.includes('couple-bed-layer')}));
 console.log('PASS shared upload URL, serialized placement and viewer rendering');
 console.log('PASS furniture upload, flat bed render, reusable gallery and reload persistence');
