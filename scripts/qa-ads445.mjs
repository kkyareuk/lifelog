...
try{
 ...
 let physical={width:384,height:832};await page.exposeFunction('reserveNative',async({height})=>{await page.setViewportSize({width:physical.width,height:physical.height-height-(height?28:0)})});
 ...
 await page.screenshot({path:out+'/ads445-landscape.png'});
 physical={width:384,height:832};await page.setViewportSize(physical);await page.waitForTimeout(500);
 await page.evaluate(()=>document.querySelectorAll('dialog[open]').forEach(d=>d.close()));await page.screenshot({path:out+'/ads445-phone.png'});
 ...
 await page.screenshot({path:out+'/card445-phone.png'});
 // Allowed information screens retain the native banner; arbitrary dialogs still hide it.
 for(const cls of ['plaza-games-dialog','building-detail-dialog','room-editor-dialog']){
  await page.evaluate(cls=>{const d=document.createElement('dialog');d.className=cls;d.id='qa-info';document.body.append(d);d.showModal()},cls);
  await page.waitForTimeout(70);assert(await page.evaluate(()=>document.documentElement.classList.contains('has-game-banner')));
  await page.evaluate(()=>document.querySelector('#qa-info').remove());
 }
 const policy=await page.evaluate(async()=>{const {bannerScreenAllowed}=await import('/banner-placement.js');return ['shop','groups','plaza'].map(tab=>{const doc=document.implementation.createHTMLDocument();doc.documentElement.dataset.activeTab=tab;return bannerScreenAllowed(doc)})});assert.deepEqual(policy,[true,true,true]);
 await page.evaluate(async()=>{window.DRAWER_VILLAGE_PLAZA_ENABLED=true;window.PARALLEL_CITY_CONFIG.ads.testing=false;const ads=await import('/native-ads.js');window.qaReal=ads.adOptions('banner')});assert.equal(await page.evaluate(()=>qaReal.isTesting),false,'plaza does not force demo ads');
 ...
 await page.waitForFunction(()=>innerHeight===832);assert.equal(await page.evaluate(()=>document.documentElement.style.getPropertyValue('--game-ad-height')),'0px','premium releases banner space');
 console.log('PASS445 mock native SDK: adaptive width, retained failure slot, retry and tablet landscape;: banner reserved area, no duplicate load, dialog hide/restore, one interstitial per game');
