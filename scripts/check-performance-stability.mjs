...
const checks=[
  [media.includes("export function stringifyLocalMediaState")&&state.includes('stringifyLocalMediaState(state,{characterSettingsView:"hub"})'),"저장 상태를 복제하지 않는 단일 순회 직렬화"],
  [state.includes("now-lastRecoveryBackupAt<60_000")&&state.includes("lastRecoveryBackupSerialized"),"중복 복구 백업 쓰기 제한"],
  [simulation.includes("const signatureCache=new WeakMap()")&&simulation.includes("cached?.character===c&&cached.revision===revision"),"인원 제한 없는 시뮬레이션 서명 재사용"],
  ...
  [simulation.includes("const applyEntries=nextEntries=>")&&simulation.includes("if(changed){day.entries=nextEntries;")&&simulation.includes("sceneBatch.revision++;save(false,false)"),"동일 공동 장면의 중복 저장 차단"],
  ...
  [Number(gradle.match(/versionCode\s+(\d+)/)?.[1])>=190&&/versionName\s+["']1\.0\.\d+(?:\.\d+)?["']/.test(gradle),"개발 빌드 190 이상"]
];
