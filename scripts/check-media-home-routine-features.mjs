...
const checks=[
  ...
  [state.includes('floorMaterial:"cream"')&&state.includes('wallMaterial:"same"')&&state.includes("export function setRoomFloorImage")&&views.includes('floorMaterial==="custom"?"":wallSurfaceImage')&&views.includes('room-custom-floor')&&css.includes('.room.room-custom-floor .room-wall-shell::before{display:none!important')&&app.includes('pickImage("roomFloor"'),"직접 그린 바닥을 방 전체에 표시하고 해당 방의 벽면을 숨김"],
