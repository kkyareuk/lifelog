...
(async()=>{
 ...
 const patch={familyArchive:{clanName:'Clan',heritages:[{label:'Compass',successorId:'b',kind:'object'}]},a:'a',b:'b',type:'가족',details:{origin:'3'},referenceId:'a',roleLinks:[{from:'a',to:'b',role:'maternalAunt',blood:false}],cohabit:true,cohabitHomeId:'ha'};
 ...
 const response=await service.respond('op',{groupId:'g',proposalId:'family-op',accept:true});assert.equal(response.status,'accepted');assert.equal(data.get('groups/g/residents/b').sharedHomeId,'ha');assert(!data.has('groups/g/homes/hb'));assert.deepEqual(data.get('groups/g/relationships/accepted-family').roleLinks,patch.roleLinks);assert.equal(data.get('groups/g/relationships/accepted-family').familyArchive.heritages[0].successorId,'b');
