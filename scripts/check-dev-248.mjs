...
globalThis.localStorage={
  ...
  getItem:key=>stored.has(key)?stored.get(key):null,setItem:(key,value)=>stored.set(key,String(value)),removeItem:key=>stored.delete(key)
};
globalThis.window={addEventListener(){}};
globalThis.document={addEventListener(){},visibilityState:"visible"};
const {resolveHomeRoomForActivity,roomAllowsCharacter}=await import("../simulation.js");

const person=id=>({id,homeId:"home",residences:[{homeId:"home",sleepRoomId:id==="b"?"bedroom-upstairs":"bedroom"}]});
...
assert.equal(roomAllowsCharacter(person("b"),home,home.rooms.bath),false);
assert.equal(resolveHomeRoomForActivity(person("b"),home,"bath",{title:"욕실에서 씻는 중",minute:420},date),"bath-upstairs");
assert.equal(resolveHomeRoomForActivity(person("b"),home,"bedroom",{title:"자는 중",minute:30},date),"bedroom-upstairs");
assert.equal(resolveHomeRoomForActivity(person("c"),home,"bath",{title:"욕실에서 씻는 중",minute:420},date),"living");
...
assert.match(gradle,/versionCode\s+248\b/);assert.match(gradle,/versionName\s+"1\.0\.224"/);
assert.ok(prepare.includes('DRAWER_VILLAGE_NATIVE_BUILD="20260906dev248"'));
console.log("PASS dev 248: room access, room owners, multi-room selection and sleeping floors");
