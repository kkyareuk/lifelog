...
try{
 ...
 for(let i=1;i<=20;i++){await cdp.send('Input.dispatchTouchEvent',{type:'touchMove',touchPoints:[{x:start.x+(end.x-start.x)*i/20,y:start.y+(end.y-start.y)*i/20}]});await page.waitForTimeout(16)}
 await cdp.send('Input.dispatchTouchEvent',{type:'touchEnd',touchPoints:[]});await page.waitForTimeout(300);

 const moved=await page.evaluate(()=>g.state.homes[g.active().homeId].rooms[dragKeys[1]].furniturePlacements.some(p=>p.id===dragId));console.log('PERSONAL TOUCH DRAG',moved);assert(moved);await page.screenshot({path:resolve(out,'drag369.png')});
 ...
 const sharedSource=await item.boundingBox(),sharedDest=await page.locator(`.room[data-room-key="${keys[0]}"] .room-furniture-layer`).boundingBox();
 ...
 for(let i=1;i<=20;i++){await cdp.send('Input.dispatchTouchEvent',{type:'touchMove',touchPoints:[{x:start2.x+(end2.x-start2.x)*i/20,y:start2.y+(end2.y-start2.y)*i/20}]});await page.waitForTimeout(16)}
 await cdp.send('Input.dispatchTouchEvent',{type:'touchEnd',touchPoints:[]});await page.waitForTimeout(300);
 assert(await page.evaluate(()=>sharedWrites.some(w=>w.layout.rooms[dragKeys[0]].furniturePlacements.some(p=>p.id===dragId))));console.log('SHARED TOUCH DRAG PASS');
