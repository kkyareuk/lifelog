...
const root=resolve('.'),out=resolve('qa-discovery328');await mkdir(out,{recursive:true});
...
try{for(const language of ['ko','en','ja'])for(const width of [384,1180]){
 ...
 await p.evaluate(()=>{document.querySelectorAll('dialog[open]').forEach(d=>d.close());game.state.activeTab='character';localStorage.removeItem('drawer-discovery-request:guest');discovery.considerDiscovery(c,{title:'걷는 중'});});
 const request=p.locator('[data-discovery-tools] button').first();await request.click();await p.locator('.character-discovery-dialog').waitFor();await p.locator('.character-discovery-dialog>button').last().click();assert(await request.isDisabled());
 await p.screenshot({path:out+`/request-${width}-${language}.png`});
 await p.locator('[data-discovery-tools] button').last().click();await p.locator('.discovery-group-lock input').first().check();await p.screenshot({path:out+`/groups-${width}-${language}.png`});await p.locator('.character-discovery-dialog>button').first().click();assert(await p.evaluate(()=>Object.keys(rules.DISCOVERY_AXES).every(f=>rules.discoveryLocked(c,f))));
 await p.evaluate(()=>window.DrawerVillageNavigation.go('statistics'));await p.locator('.discovery-statistics').waitFor();assert.equal(await p.locator('.discovery-statistics tr').count(),26);await p.locator('.discovery-statistics').screenshot({path:out+`/statistics-${width}-${language}.png`});if(language==='en'&&width===384)console.log(await p.locator('.discovery-statistics').innerText());
 await p.evaluate(()=>window.DrawerVillageNavigation.go('character'));await p.locator('[data-open-full-character-settings]:visible').first().click();await p.locator('.character-book-v9-menu>summary').click();await p.locator('[data-character-pane=personality]:visible').first().click();assert.equal(await p.locator('.character-field-fold').count(),0);assert(await p.locator('.discovery-score').count()>0);await p.screenshot({path:out+`/scores-${width}-${language}.png`});
