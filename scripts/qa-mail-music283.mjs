...
const {chromium}=require(playwrightPath),output=resolve(root,"qa-output-283");
...
try{
  ...
  await page.evaluate(async()=>{
    window.qaAudio=[];window.Audio=class{constructor(src){this.src=src;this.paused=true;window.qaAudio.push(this)}play(){this.paused=false;return Promise.resolve()}pause(){this.paused=true}};
    const audio=await import('/background-music.js?qa=283');window.qaAudioState={backgroundMusicVolume:35,backgroundMusicMuted:false};window.qaSync=audio.syncBackgroundMusic;
  ...
  assert.equal(await page.evaluate(()=>qaAudio[0].volume),0.35);
  ...
  assert.equal(await page.evaluate(()=>qaAudio.length),1);
  assert.equal(await page.evaluate(()=>qaAudio[0].volume),0.7);
