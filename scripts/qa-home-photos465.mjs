...
try{
 ...
 await p.evaluate(()=>{document.querySelectorAll('dialog[open]').forEach(d=>d.close());window.fixtureUid='me';window.ParallelCityAuth.getInfo=()=>({ready:true,user:{uid:fixtureUid}});window.snap={activeGroupId:'group',group:{id:'group',ownerUid:'me',towns:[{id:'town',name:'Town',places:[]}]},homes:[{id:'shared',ownerUid:'me',townId:'town',layoutRevision:0,layoutJson:JSON.stringify(g.state.homes[hid])}],residents:[],members:[],loadedCollections:['group','homes','residents']};window.uploads=0;window.savedLayouts=[];window.DrawerVillageGroups={getSnapshot:()=>snap,uploadHomeMemberImage:async()=>{uploads++;return location.origin+'/assets/home-ui/home.png'},saveHomeLayout:async input=>{savedLayouts.push(input);return {revision:input.revision+1}}};window.photoPatch=null;photoQA.openBuildingShapeDialog(hid,'home',async patch=>{photoPatch=patch;return true})});
