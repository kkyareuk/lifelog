...
const {chromium}=require(playwrightPath),output=resolve(root,"qa-town355");
...
try{
 ...
 const cycle=async()=>{for(const tab of ['town','home','observe']){await page.evaluate(tab=>{document.querySelectorAll('dialog[open]').forEach(d=>d.close());window.DrawerVillageNavigation.go(tab)},tab);await page.waitForTimeout(70);}};
