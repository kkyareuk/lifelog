...
const {chromium,webkit}=require(playwrightPath),output=resolve(root,"tmp/qa-games425");
...
try{
 ...
 for(const lang of ['ko','en','ja']){await page.evaluate(lang=>s.state.uiLanguage=lang,lang);await page.evaluate(g=>paint(g)(),engine.view(raw,'qa'));assert(await page.locator('.mp-loose-tool').count()>0);await page.locator('.mp-loose-tool').first().click();assert.equal(await page.evaluate(()=>sent.kind),'takeTool');await page.screenshot({path:output+'/preparation-'+lang+'.png'});}
 ...
 raw.phase='voteResult';raw.voteResults=[{voter:'p0',target:'p1'},{voter:'p2',target:'p1'}];await page.evaluate(g=>paint(g)(),engine.view(raw,'qa'));assert.equal(await page.locator('.mp-vote-results span').count(),2);await page.screenshot({path:output+'/votes.png'});
 raw.phase='move';await page.evaluate(g=>paint(g)(),engine.view(raw,'qa'));await page.waitForSelector('[data-camera-ready=true]');const camera=await page.locator('.mp-village').evaluate(n=>{n.scrollTop=60;n.scrollLeft=20;n.dispatchEvent(new Event('scroll'));return {x:n.scrollLeft,y:n.scrollTop}});raw.phase='walk';await page.evaluate(g=>paint(g)(),engine.view(raw,'qa'));await page.waitForSelector('[data-camera-ready=true]');console.log('CAMERA',camera,await page.locator('.mp-village').evaluate(n=>({x:n.scrollLeft,y:n.scrollTop,w:n.clientWidth,sw:n.scrollWidth,plane:n.firstChild.style.cssText})));assert.deepEqual(await page.locator('.mp-village').evaluate(n=>({x:n.scrollLeft,y:n.scrollTop})),camera);
 ...
 console.log('PASS425 UI: KO/EN/JA compact meeting, grounds menus, sequential reactions, no overlap/scroll, spectator map, finished dawn result.');
