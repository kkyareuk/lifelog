...
try{
 ...
 await page.locator('[name=capacity]').selectOption('6');assert.equal(await page.locator('[name=place]:checked').count(),3);await page.locator('[name=start]').check();await page.locator('form.plaza-setup button').click();assert.equal(await page.evaluate(()=>setupChoice.start),true);
