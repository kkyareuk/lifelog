...
(async()=>{const {db,data}=fixture(),api=require('../functions/career-world')({db}),now=()=>new Date(2026,0,25).getTime(),money=require('../functions/character-money')({db,clock:now});const career={id:'custom-clerk',name:'서기',payDay:25,departments:['행정부'],ranks:[{id:'junior',name:'수습',salaryMeals:230,duties:[{name:'문서 정리',description:'문서를 정리하고 있어요.'}]}]};
 ...
 console.log('PASS473 server membership, host rule, creator ownership, revision conflicts, personal ownership, salary canonical lookup, department and frequency persistence');
