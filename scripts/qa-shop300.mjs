...
try{
 ...
 for(const [w,h] of [[1280,800],[1024,768],[768,1024],[360,840]]){await page.setViewportSize({width:w,height:h});await page.waitForTimeout(300);const b=await page.locator('.drawer-shop-stage').boundingBox();assert.ok(b.width<=w+1);const cards=await page.locator('.drawer-shop-product').evaluateAll(es=>es.map(e=>{const b=e.getBoundingClientRect();return {x:b.x,right:b.right,y:b.y}}));assert.equal(cards.length,3);if(w>=900){const seller=await page.locator(".drawer-shop-seller").boundingBox();assert.ok(seller.height>400&&seller.x<w*.5);assert.ok(cards[0].x>w*.5);assert.ok(cards.every(c=>Math.abs(c.y-cards[0].y)<2))}assert.ok(cards.every(c=>c.x>=0&&c.right<=w));await page.screenshot({path:resolve(output,'shop300-'+w+'.png')})}
 await page.locator('[data-drawer-shop-tab="skin"]').click();assert.equal(await page.locator('.drawer-shop-coming').count(),1);await page.locator('.drawer-shop-back').click();assert.equal(await page.locator('.drawer-shop-stage').count(),0);await page.setViewportSize({width:1280,height:800});await page.evaluate(()=>{window.PARALLEL_CITY_CONFIG.iosPreview=false;window.PARALLEL_CITY_CONFIG.iosApp=false;location.hash='tab=shop'});await page.locator('[data-drawer-shop-tab="base"]').click();assert.equal(await page.locator('.drawer-shop-product').count(),4);const android=await page.locator('.drawer-shop-product').evaluateAll(es=>es.map(e=>e.getBoundingClientRect().y));assert.equal(android[0],android[2]);assert.ok(android[3]>android[0]);await page.screenshot({path:resolve(output,'shop300-android.png')});assert.deepEqual(errors,[]);console.log('PASS tablet three-column shop, phone/portrait bounds, tabs and back');
