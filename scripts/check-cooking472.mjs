...
const complete=advanceSharedLife(snapshot,job.endsAt+1),done=JSON.parse(complete[0].lifeJson);assert.equal(done.cooking.inventory.kimchi_jjigae,1);assert(done.cooking.experience>0);assert.equal(done.cooking.experienceVersion,2);snapshot.residents[0].lifeJson=complete[0].lifeJson;
