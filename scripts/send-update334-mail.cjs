...
const PROJECT='lifelog-98fff',DISPATCH='update-314-334-20260911-v1',GROUP='_drawer_village_announcements',SENDER='_drawer_village_operator';
async function main(){
 if(process.argv.includes('--send')){const draft=JSON.parse(fs.readFileSync(path.join(__dirname,'../docs/update-mail334.json'),'utf8'));if(draft._delivery?.status!=='approved')throw Error('Mail has not been approved.')}
 ...
 const copy=JSON.parse(fs.readFileSync(path.join(__dirname,'../docs/update-mail334.json'),'utf8'));
 ...
 await dispatch.set({...result,status:'complete'},{merge:true});fs.writeFileSync(path.join(__dirname,'../docs/update-mail334-result.json'),JSON.stringify({dispatch:DISPATCH,...result},null,2)+'\n');console.log(JSON.stringify(result));
}
