...
let baseline=true;const baselineViews=execFileSync("git",["show","6dbc3be:views.js"],{encoding:"utf8",maxBuffer:20*1024*1024});const baselineApp=execFileSync("git",["show","6dbc3be:app.js"],{encoding:"utf8",maxBuffer:10*1024*1024});
...
for(const mode of ['before','after']){
 ...
 const result=await page.evaluate(async()=>{qaRenders=0;qaSceneCalls=0;const samples=[];for(const tab of ['observe','character','town','home','observe']){const start=performance.now();window.DrawerVillageNavigation.go(tab);samples.push({tab,ms:Math.round(performance.now()-start)});await new Promise(r=>setTimeout(r,500))}return {samples,sceneCalls:qaSceneCalls,renders:qaRenders,characters:g.state.order.length}});
 const {profile}=await cdp.send('Profiler.stop');await writeFile(resolve(output,'cpu350.json'),JSON.stringify(profile));const counts=new Map();for(const n of profile.nodes){const key=n.callFrame.functionName+' '+n.callFrame.url.split('/').at(-1)+':'+n.callFrame.lineNumber;counts.set(key,(counts.get(key)||0)+(n.hitCount||0))}console.log([...counts].sort((a,b)=>b[1]-a[1]).slice(0,25));
