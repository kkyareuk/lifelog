...
for(const asset of ["assets/shop/drawer-shop-base.png","assets/shop/drawer-shop-bundle.png"]){
  const info=await stat(new URL(`../${asset}`,import.meta.url));
  assert.ok(info.size>100_000&&info.size<1_000_000,`${asset}는 배포용 시안 이미지 크기를 유지해야 합니다.`);
}
assert.match(index,/shop\.css\?v=20260828shop179b/);
assert.match(prepareApp,/"shop\.css"/);
assert.match(views,/drawer-shop-base\.png/);
assert.match(views,/drawer-shop-bundle\.png/);
...
assert.match(views,/open_celebration_bundle/);
...
assert.match(config,/open_celebration_bundle:"open_celebration_bundle"/);
assert.match(backend,/PRODUCTS=new Set\(\["open_celebration_bundle"/);
assert.doesNotMatch(backend,/CONSUMABLE_PRODUCTS=new Set\(\[[^\]]*open_celebration_bundle/);
assert.match(backend,/productId==="open_celebration_bundle"/);
assert.match(backend,/next\.characterSlotPacks=.*\+2\*count/);
assert.match(backend,/next\.townSlotPacks=.*\+2\*count/);
