...
try{
  ...
  const checks=await page.evaluate(async()=>{
    const game=await import('/state.js?v=20260909dev295'),transfer=await import('/settings-transfer.js?v=20260909dev295'),c=game.state.characters[game.state.activeId],home=game.state.homes[c.homeId];
  ...
  await page.evaluate(async()=>{
    ...
    (await import('/groups.js?v=20260909dev295')).showMultiplayerList();location.hash='tab=groups';
  ...
  await page.evaluate(async()=>{window.ParallelCityAuth.savePublicProfile=async input=>{window.qaProfileInput=input.name;return {name:input.name,photoURL:''}};const p=await import('/user-profile.js?v=20260909dev295');p.openUserProfile()});
