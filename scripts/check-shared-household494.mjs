...
life=command({kind:'food',characterId:'a',dishId:'meal',action:'eat'});assert.equal(life.household.active.action,'eat');assert.equal(life.cooking.dishes.length,0);assert.equal(life.household.active.endsAt-life.household.active.startedAt,30000);assert(life.scene.meetingJourney);
life=command(null,life.household.active.startedAt);assert.equal(life.scene.needKey,'hunger');
life=command(null,life.household.active.endsAt);assert.equal(life.lifeNeeds.hunger,100);assert(life.household.returning||!life.household.active);
