...
for(const mode of ['before','after']){
 baseline=mode==='before';const page=await browser.newPage({viewport:{width:384,height:854}});const errors=[];page.on('pageerror',e=>{errors.push(e.message);console.log('ERROR',e.message)});await page.route('**/*',r=>r.request().url().startsWith(origin)?r.continue():r.abort());await page.goto(origin);await page.waitForFunction(()=>window.qaRender);
 ...
 const result=await page.evaluate(async()=>{qaRenders=0;qaSceneCalls=0;const samples=[];for(const tab of ['shop','character','town','home','town','home','character']){const start=performance.now();window.DrawerVillageNavigation.go(tab);if(tab==='character'){g.state.characterSettingsView='full';g.state.characterPane='profile';qaRender()}samples.push({tab,ms:Math.round(performance.now()-start)});await new Promise(r=>setTimeout(r,500))}return {samples,sceneCalls:qaSceneCalls,renders:qaRenders,characters:g.state.order.length}});
 const {profile}=await cdp.send('Profiler.stop');await writeFile(resolve(output,'cpu351.json'),JSON.stringify(profile));const counts=new Map();for(const n of profile.nodes){const key=n.callFrame.functionName+' '+n.callFrame.url.split('/').at(-1)+':'+n.callFrame.lineNumber;counts.set(key,(counts.get(key)||0)+(n.hitCount||0))}console.log([...counts].sort((a,b)=>b[1]-a[1]).slice(0,25));
