...
try{
 ...
 for(const size of [{width:402,height:820},{width:393,height:798}]){
  await page.setViewportSize(size);
  await page.evaluate(()=>qaRender());
  for(const field of ['morality','aggressionLevel']){
   const control=page.locator('.book-form-grid [data-field="'+field+'"]').first();
   await control.scrollIntoViewIfNeeded();
   console.log(field,await control.evaluate(el=>{const r=el.getBoundingClientRect(),g=el.closest('.book-form-grid');return {y:r.y,bottom:r.bottom,gridHeight:g.clientHeight,scroll:g.scrollTop,touch:getComputedStyle(g).touchAction}}));
   await page.evaluate(()=>document.querySelectorAll('dialog.page-guide[open]').forEach(d=>d.close()));
   await control.tap();
   await page.locator('.selection-popup-options button').last().click();
   const expected=await control.inputValue();
   assert.equal(await page.evaluate(field=>g.state.characters[g.state.activeId][field],field),expected);
   await page.evaluate(()=>qaRender());
   assert.equal(await page.locator('.book-form-grid [data-field="'+field+'"]').first().inputValue(),expected);
  }
 }
 ...
 console.log('PASS454 reported morality/aggression selection, title, pinch and dropdown continuity in '+(useWebKit?'WebKit':'Chromium'));
