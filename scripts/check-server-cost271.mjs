...
const game=await import('../state.js?v=20260909dev284');
...
let unchanged=0;
for(let minute=0;minute<minutes;minute++){const result=await service.advance('u',{groupId:'g'});unchanged+=count-(result.changedCount||0);for(const row of expected)assert.equal(data.get('groups/g/residents/'+row.id).lifeJson,row.lifeJson,'Every persisted scene and log matches the unmodified engine');now+=60000;}
