...
await page.evaluate(async()=>{window.profileModule=await import('/group-member-profile.js');window.savedProfiles=[];profileModule.openMemberProfile({profile:{displayName:'방A의 이름',photoURL:''},onSave:async v=>savedProfiles.push(v)});});
await page.locator('.member-profile-dialog input[name=name]').fill('방A의 새 이름');
await page.locator('.member-profile-dialog [type=submit]').click();
assert.equal(await page.locator('.member-profile-dialog').count(),0);
assert.equal(await page.evaluate(()=>savedProfiles[0].name),'방A의 새 이름');
for(const lang of ['ko','en','ja']){
 await page.evaluate(lang=>profileModule.openMemberProfile({language:lang,profile:{displayName:'Room B'},onSave:async v=>savedProfiles.push(v)}),lang);
 await page.screenshot({path:resolve(output,'member-profile347-'+lang+'.png')});
 const box=await page.locator('.member-profile-dialog').boundingBox();assert(box.x>=0&&box.x+box.width<=384);
 await page.locator('.member-profile-dialog [data-cancel]').click();
}
assert.equal(await page.evaluate(()=>savedProfiles.length),1);
assert(!errors.length,errors.join('\n'));console.log('PASS profile save, cancel, KO/EN/JA phone layout');
