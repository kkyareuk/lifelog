...
const {stageTownEdit,commitTownEdit,withTownEditDraft,townEditDraft}=await import('../town-edit-draft.js?v=20260908dev282');
...
const game=await import('../state.js?v=20260908dev282');game.resetAll();game.state.catalog={food:[],drink:[]};for(let i=0;i<40;i++){assert.ok(game.addCatalogItem('food',{name:'F'+i}));assert.ok(game.addCatalogItem('drink',{name:'D'+i}))}assert.equal(game.addCatalogItem('book',{name:'Overflow'}),null);
