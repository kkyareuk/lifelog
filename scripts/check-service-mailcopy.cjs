...
(async()=>{
 ...
 data.set('groups/g/members/op',{role});data.set('users/op/sync/core',{state:{order:[role],towns:[{id:'personal-town'}]}});
 const result=await relations.requestResidence('op',{groupId:'g',requestId:'admin-'+role,resident:{sourceCharacterId:role,name:'Admin character',townId:'t',profileJson:'{}',scheduleJson:'{}'}});
 assert.equal(result.status,'accepted');assert.ok(data.has('groups/g/residents/op_'+role));
 const proposal=data.get('groups/g/proposals/admin-'+role);assert.equal(proposal.recipientUid,'op');assert.equal(proposal.appliedByAuthority,true);
 assert.ok(![...data.entries()].some(([key,value])=>key.includes('outbox')&&value.proposalId==='admin-'+role));
 }
 ...
 data.set('groups/source',{ownerUid:'member',towns:[{id:'source-town'}]});data.set('groups/source/members/member',{role:'owner'});
 data.set('groups/source/residents/cross',{name:'Cross',ownerUid:'member',sourceCharacterId:'cross-personal',townId:'source-town',profileJson:'{"name":"Latest"}',scheduleJson:'{}'});
 data.set('users/member/characterTransfers/cross-personal',{personalId:'cross-personal',location:'group',groupId:'source',revision:1});
 const crossRequest={groupId:'g',requestId:'cross-request',kind:'admission',sourceGroupId:'source',sourceResidentId:'cross',townId:'t'};
 await relations.requestResidence('member',crossRequest);assert.ok(data.has('groups/source/residents/cross'),'Pending move remains at source');
 await relations.respond('host',{groupId:'g',proposalId:'cross-request',accept:false});assert.ok(data.has('groups/source/residents/cross'),'Rejected move remains at source');
 await relations.requestResidence('member',{...crossRequest,requestId:'cross-accept'});
 data.set('groups/source/residents/cross',{...data.get('groups/source/residents/cross'),profileJson:'{"name":"Edited while pending"}'});
 await relations.respond('host',{groupId:'g',proposalId:'cross-accept',accept:true});assert.ok(!data.has('groups/source/residents/cross'));assert.equal(JSON.parse(data.get('groups/g/residents/member_cross-personal').profileJson).name,'Edited while pending');
 assert.equal(data.get('users/member/characterTransfers/cross-personal').groupId,'g');
 console.log('PASS cross-group move keeps source while pending/declined, transfers latest profile atomically on acceptance');
 ...
 data.set('groups/g/members/manager',{role:'manager'});
 await relations.sendMail('host',{groupId:'g',requestId:'merged-admin',audience:'announcement',recipientScope:'role:manager',subject:'Administrators'});
 assert.equal(data.get('groups/g/mail/merged-admin-op').recipientUid,'op');
 assert.equal(data.get('groups/g/mail/merged-admin-manager').recipientUid,'manager');
 console.log('PASS administrator announcements include legacy operators and managers');
