...
const slotField=slotProduct==='diamonds_100'?'diamondPaid':slotProduct==='character_slot_1'?'characterSingleSlots':['town_slot_1','town_slots_5'].includes(slotProduct)?'townSlotPacks':slotProduct==='green_tea'?'teaSupportCount':'characterSlotPacks';
const grant=slotProduct==='diamonds_100'?100:slotProduct==='town_slots_5'?5:1;
...
const db={collection:name=>({doc:id=>({key:name+'/'+id,get:async()=>({data:()=>structuredClone(rows.get(name+'/'+id))})})}),runTransaction:fn=>{const job=queue.then(()=>fn({get:async ref=>({exists:rows.has(ref.key),data:()=>structuredClone(rows.get(ref.key))}),set:(ref,data,options)=>rows.set(ref.key,options?.merge?{...rows.get(ref.key),...data}:data)}));queue=job.catch(()=>{});return job}};
let revoked=false;
const notificationPayload='header.'+Buffer.from(JSON.stringify({data:{environment}})).toString('base64url')+'.signature';
const app=express();app.use(express.json());
installAppleBilling(app,{db,signedInUser:async req=>{if(req.get('Authorization')!=='Bearer ok')throw Object.assign(Error(),{status:401});return {uid}},nextEntitlements:(old,id,n)=>({...old,[slotField]:(old?.[slotField]||0)+n*grant}),serverTimestamp:()=>1,services:()=>({environment,api:{getTransactionInfo:async()=>({signedTransactionInfo:'signed'})},verifier:{verifyAndDecodeTransaction:async()=>({...purchase,...(revoked?{revocationDate:1}:{})}),verifyAndDecodeNotification:async()=>({notificationType:'REFUND',data:{signedTransactionInfo:'signed'}})}})});
...
assert.equal((await post('entitlements',{},'bad')).status,401);
...
await Promise.all(Array.from({length:8},()=>post('verify',{transactionId:'123'})));assert.equal(rows.get('appleSandboxAccounts/buyer').appleSandboxEntitlements[slotField],grant);assert.equal(rows.get('appleSandboxAccounts/buyer').entitlements,undefined);
assert.equal((await (await post('entitlements')).json()).sandboxEntitlements[slotField],grant);assert.equal(rows.has('users/buyer'),false);
...
let buy=0,finish=0,verified=true,loggedIn=true,cancel=false,history=[],syncError='',ledgerCount=1,ledgerUid=uid;
const bridge={getDiagnostics:async()=>({sandboxReceipt:true}),getProducts:async()=>({products:[{productId:purchase.productId,formattedPrice:slotProduct==='character_slot_1'?'₩1,000':'₩1,200',regularPaidOffer:true}]}),purchase:async()=>{buy++;if(cancel)throw {code:'PURCHASE_CANCELLED'};return {transactionId:'123'}},finishPurchase:async()=>finish++,restorePurchases:async()=>({purchases:history,syncError}),addListener:()=>{}};
const window={Capacitor:{isNativePlatform:()=>true,getPlatform:()=> 'ios',Plugins:{AppleBilling:bridge}},PARALLEL_CITY_CONFIG:{appleBilling:{enabled:true,backendUrl:'https://billing.test',products:{[slotProduct]:purchase.productId}}},ParallelCityAuth:{getIdToken:async()=>loggedIn?'token':null,getInfo:()=>({user:{uid}}),refreshEntitlements:async()=>{},setAppleSandboxEntitlements:(value,account)=>{assert.equal(account,uid);assert.equal(value.characterSlotPacks,ledgerCount)}},addEventListener:()=>{}};
vm.runInNewContext(fs.readFileSync(require('node:path').join(__dirname,'../apple-billing-client.js'),'utf8'),{window,document:{documentElement:{lang:'ko'}},setTimeout,clearTimeout,CustomEvent:class{},AbortSignal,fetch:async url=>({ok:true,json:async()=>url.endsWith('entitlements')?{uid:ledgerUid,sandboxEntitlements:{characterSlotPacks:ledgerCount}}:url.endsWith('prepare')?{appAccountToken:accountToken(uid),products:productMap}:{verified,entitlementApplied:verified}})});
...
syncError='ASDErrorDomain:500';const recovered=await billing.restorePurchases();assert.equal(recovered.appleCheckFailed,true);assert(recovered.warning.includes('ASDErrorDomain:500'));assert.equal(recovered.accountChecked,true);
ledgerCount=0;await assert.rejects(()=>billing.restorePurchases(),/ASDErrorDomain:500/);
history=[{transactionId:'123'}];assert.equal((await billing.restorePurchases()).restored,1,'Local unfinished purchase verified even when Apple sync fails');
history=[];ledgerUid='different-account';await assert.rejects(()=>billing.restorePurchases(),/APPLE_ACCOUNT_MISMATCH/);ledgerUid=uid;syncError='';
