...
const {chromium}=require(playwrightPath),output=resolve(root,"qa-supporters380");
...
try{
 ...
 await page.route('**/supporter/public',r=>r.fulfill({json:{entries:[]}}));
 await page.evaluate(()=>credits.openSupporterCredits());await page.waitForTimeout(300);
 assert.equal(await page.locator('.supporter-name').filter({hasText:'모리모'}).count(),1);
 await page.evaluate(()=>{window.DRAWER_VILLAGE_VERSION_CODE='377';credits.openSupporterCredits({remote:false})});
 assert.equal(await page.locator('.supporter-name').filter({hasText:'모리모'}).count(),0);
 assert.deepEqual(errors,[]);console.log('PASS bundled credits offline, five internal names, remote refresh preserves previews, public build excludes previews, KO/EN/JA and mobile bounds');
