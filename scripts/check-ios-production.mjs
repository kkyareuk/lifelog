...
assert.equal(source.commit,'06d36e4');assert.equal(source.androidCode,304);
...
assert.equal(release.sourceAndroidCode,304);assert.equal(release.sourceAndroidVersion,'1.0.271');
...
console.log('PASS production 304 game hashes, shared game parity and reviewed platform changes, explicit eligible upload without review submission');
