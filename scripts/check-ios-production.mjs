...
assert.equal(source.commit,'53a5d87');assert.equal(source.androidCode,305);
...
assert.equal(release.sourceAndroidCode,305);assert.equal(release.sourceAndroidVersion,'1.0.272');
...
console.log('PASS production 305 game hashes, shared game parity and reviewed platform changes, explicit eligible upload without review submission');
