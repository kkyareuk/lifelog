...
const source=fs.readFileSync(process.env.QA_AUTH_SOURCE||new URL('../auth.js',import.meta.url),'utf8').replace(/^import .*;\r?\n/gm,'');
...
memory.set('drawer-account:C:drawer-village-game-v1',JSON.stringify({schema:36,characters:{},order:[],lastSaved:1}));
...
const accountOnly=game.createCharacter();await game.save(true,false);
await auth.logout();const guestNext=game.createCharacter();await game.save(true,false);
await callback({uid:'C',email:'c@test'});
assert(!game.state.characters[guestNext],'Automatic session restoration must not adopt guest data');
await auth.logout();await auth.login();
assert(game.state.characters[guestNext],'Explicit sign-in also retains guest data when account already has characters');
assert(game.state.characters[accountOnly],'Existing account character must survive guest adoption');
console.log('PASS explicit populated-account sign-in preserves both saves; automatic sign-in stays isolated');

const preserved=JSON.stringify(game.cloneState());stallImage=true;await callback({uid:"C",email:"c@test"});assert.equal(auth.getInfo().startupError,"sync/request-timeout:image-epoch");assert.equal(game.state.characters[guestFirst]?.name,"첫 로그인 전 캐릭터");assert(memory.has("drawer-account:C:drawer-village-game-v1"));console.log("PASS stalled account read exits into recoverable error without deleting account records");
