...
const game=await import('../state.js?v=20260909dev301');game.createCharacter();const profile=structuredClone(game.state.characters[game.state.activeId]);
...
try{
 ...
 await pages[0].evaluate(async()=>{qaSnapshot={...qaSnapshot,activeGroupId:'',group:null};const g=await import('/state.js?v=20260909dev301');window.qaGame=g;window.DrawerVillageNavigation.go('character');document.querySelectorAll('dialog[open]').forEach(d=>d.close())});
