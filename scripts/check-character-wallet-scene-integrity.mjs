...
const checks=[
  ...
  [css.includes('.character-draft-back{')&&css.includes('top:3.9dvh!important')&&css.includes('width:17vw!important')&&css.includes('height:7.63dvh!important'),"뒤로가기 원화 비율을 유지한 축소와 상향 배치"],
  ...
  [css.includes('linear-gradient(180deg,#fff9eb 0%,#f3e5cf 52%,#795941 100%)')&&css.includes('z-index:30!important')&&css.includes('z-index:32!important'),"선택 아이콘 앞·세로로 짙어지는 캐릭터 목록"],
  [css.includes('.character-wallet-selected>b{')&&css.includes('z-index:4!important')&&css.includes('top:8.72dvh!important'),"선택됨 글자판을 아이콘 바로 아래에 여백 없이 연결"],
  [css.includes('.character-wallet-selected{')&&css.includes('top:14.35dvh!important')&&css.includes('.character-wallet-roster{')&&css.includes('top:22.75dvh!important'),"선택 아이콘·글자판·팝업을 같은 간격으로 위쪽 이동"],
  [views.includes('rosterVisibleRows=Math.min(6,Math.max(1,mobileCharacterIds.length))')&&views.includes('data-roster-count="${mobileCharacterIds.length}"')&&css.includes('var(--roster-visible,3) * 7.85dvh')&&css.includes('max-height:65dvh!important'),"캐릭터 수에 따라 여섯 명까지 길어지고 추가 인원은 스크롤하는 목록"],
  [css.includes('.mobile-character-profile-draft{')&&css.includes('z-index:20!important')&&css.includes('.character-wallet-roster{')&&css.includes('z-index:30!important')&&css.includes('background:linear-gradient(180deg,#fff9eb 0%,#f3e5cf 52%,#795941 100%)'),"선택 팝업의 부모 스택까지 연필보다 위로 올리고 불투명 레이어로 표시"],
  [css.includes('aspect-ratio:1206/1806!important')&&css.includes('transform:rotate(-2.8deg)!important')&&css.includes('background:#d9c6aa!important'),"모자이크 종이를 찌그러뜨리지 않은 기울어진 배경 장식으로 배치"],
  [css.includes('--character-pill-cap:calc(3.25dvh * .484)')&&css.includes('min-height:29px!important'),"내보내기·저장·삭제 알약 버튼의 세로 크기 확대"],
  [css.includes('.character-roster-reorder{')&&css.includes('z-index:5!important')&&css.includes('text-decoration:none!important')&&css.includes('text-shadow:0 1px 2px #2b1c13!important'),"위치 바꾸기를 목록 아래에 항상 보이게 표시하고 밑줄 제거"],
