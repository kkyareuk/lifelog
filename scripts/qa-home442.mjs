...
try{
 ...
 console.log(bounds);assert(bounds.canvas.width>=768);assert(bounds.scroll>bounds.client);
 await page.screenshot({path:out+'/house442.png'});
 await page.evaluate(()=>{const p=document.querySelector('.home-canvas-viewport');p.scrollLeft=150;p.scrollTop=200});await page.waitForTimeout(50);await page.evaluate(()=>qaRender());await page.waitForTimeout(100);
 assert(await page.evaluate(()=>document.querySelector('.home-canvas-viewport').scrollTop>0),'pan restored');
 ...
 await page.goto(origin);await page.waitForFunction(()=>window.DrawerVillageNavigation);
 ...
 console.log('PASS442 house canvas dimensions, preserved layout, restored scroll, tap cycling and independent attached-object drag');
