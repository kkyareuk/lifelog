...
try{
 ...
 await page.screenshot({path:out+'/ads442-landscape.png'});
 physical={width:384,height:832};await page.setViewportSize(physical);await page.waitForTimeout(500);
 await page.evaluate(()=>document.querySelectorAll('dialog[open]').forEach(d=>d.close()));await page.screenshot({path:out+'/ads442-phone.png'});
 ...
 console.log('PASS442 mock native SDK: adaptive width, retained failure slot, retry and tablet landscape;: banner reserved area, no duplicate load, dialog hide/restore, one interstitial per game');
