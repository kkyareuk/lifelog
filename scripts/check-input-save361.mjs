...
let task,delay=200,writes=0;const context={saveEpoch:0,isolatedWorldDepth:0,editorPersonalState:null,timer:null,pendingNotify:false,document:{querySelector:()=>null,activeElement:null},clearTimeout(){},setTimeout:(fn)=>{task=fn;return 1},inputIdleDelay:()=>delay,clearDeferredTextSave(){},queueSnapshotWrite:()=>{writes++;return true}};vm.createContext(context);vm.runInContext(code,context);
