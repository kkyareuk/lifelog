...
try{const page=await browser.newPage({viewport:{width:384,height:820},deviceScaleFactor:3,hasTouch:true,serviceWorkers:'block'});await page.route('**/*',r=>r.request().url().startsWith(origin)?r.continue():r.abort());await page.goto(origin);await page.waitForFunction(()=>window.DrawerVillageNavigation);
 ...
 await page.evaluate(async()=>{
  const c=g.state.characters[testId],other=Object.values(g.state.characters).find(x=>x.id!==c.id),scene={home:true,room:'living',title:'함께 앉아 이야기를 나누는 중',sharedFurnitureKey:'living:sofa',interactionId:'fixture-social',groupInteraction:true},room={name:'거실'};
 ...
 assert.equal(await page.locator('.room-activity-labels .home-person-status:visible').count(),1);assert.equal(await page.locator('.room-activity-labels .home-person-status:visible b').textContent(),'크로 · 네리네');
