...
const checks=[
  [views.includes('class="town-native-header"')&&views.includes('class="town-native-title" data-open-town-switcher')&&css.includes('.native-app[data-active-tab="town"] .native-sub-header{display:none!important}')&&css.includes('var(--home-ui-pill-middle)'),"마을 상단바를 집과 같은 조합형으로 통일하고 제목·로고에 마을 이동 연결"],
  ...
  [views.includes('data-mobile-town-layout-mode')&&views.includes('data-mobile-town-decoration-mode')&&views.includes('data-mobile-building-edit-mode')&&app.includes('setMobileTownMode("town")')&&app.includes('setMobileTownMode("decorations")')&&app.includes('setMobileTownMode("buildings")'),"마을 정보·장식·건물 편집 모드 분리"],
  [app.includes('currentTownMode()!=="buildings"')&&css.includes('.decorations-editing .town-map-scroll :is([data-place],[data-home-map]){pointer-events:none')&&css.includes('.buildings-editing .town-map-scroll [data-town-decoration]{pointer-events:none'),"일반 화면과 장식 모드에서 건물 편집·이동 차단"],
  [views.includes('data-decoration-search')&&views.includes('data-decoration-category')&&css.includes('.town-decoration-results{display:flex'),"하단형 장식 검색·카테고리·가로 목록"],
