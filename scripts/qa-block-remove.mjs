...
const {chromium}=require(playwrightPath),output=resolve(root,"qa-output-block-remove");
...
try{
 ...
 await page.evaluate(()=>{const b=document.createElement('button');b.dataset.safetyManage='';b.textContent='차단 관리';document.body.append(b)});await page.locator('[data-safety-manage]').last().click();await page.getByRole('button',{name:'차단 해제',exact:true}).click();await page.waitForFunction(()=>qaBlocks.length===0);assert.ok((await page.locator('.user-safety-dialog').innerText()).includes('차단한 계정이 없어요'));await page.screenshot({path:resolve(output,'account-safety295.png')});await page.getByRole('button',{name:'닫기',exact:true}).click();
 await page.locator('[data-user-safety]').evaluate(b=>b.dataset.safetyMode='report');await page.locator('[data-user-safety]').click();assert.equal(await page.locator('.user-safety-dialog form').count(),1);assert.equal(await page.getByRole('button',{name:'계정 차단',exact:true}).count(),0);await page.getByRole('button',{name:'닫기',exact:true}).click();
 await page.locator('[data-user-safety]').evaluate(b=>b.dataset.safetyMode='block');await page.locator('[data-user-safety]').click();assert.equal(await page.locator('.user-safety-dialog form').count(),0);assert.equal(await page.getByRole('button',{name:'신고하기',exact:true}).count(),0);assert.equal(await page.getByRole('button',{name:'계정 차단',exact:true}).count(),1);assert.deepEqual(errors,[]);console.log('PASS mobile report receipt, account block, different group/new character hiding, unblock and dialog bounds');

 await page.getByRole('button',{name:'닫기',exact:true}).click();
 await page.evaluate(()=>{window.qaRemovalInputs=[];DrawerVillageGroups.getSnapshot=()=>({group:{id:'g',ownerUid:'a'},members:[{uid:'a',role:'owner'},{uid:'b',role:'member'}]});DrawerVillageGroups.setUserBlock=async input=>{qaRemovalInputs.push(input);return {blocked:[{uid:'b'}]}}});
 await page.locator('[data-user-safety]').click();
 assert.equal(await page.getByRole('button',{name:'차단하고 퇴장시키기',exact:true}).count(),1);
 await page.getByRole('button',{name:'차단하고 퇴장시키기',exact:true}).click();
 await page.waitForFunction(()=>qaRemovalInputs.length===1);
 assert.equal(await page.evaluate(()=>qaRemovalInputs[0].removeMember),true);
 await page.evaluate(()=>{DrawerVillageGroups.getSnapshot=()=>({group:{id:'g',ownerUid:'host'},members:[{uid:'a',role:'member'}]})});
 await page.locator('[data-user-safety]').click();
 assert.equal(await page.getByRole('button',{name:'차단하고 퇴장시키기',exact:true}).count(),0);
 assert.deepEqual(errors,[]);console.log('PASS owner block-and-remove choice and ordinary-member restriction');
