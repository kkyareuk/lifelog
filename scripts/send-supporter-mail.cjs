...
const PROJECT='lifelog-98fff',DISPATCH='supporter-name-20260910-v1',GROUP='_drawer_village_announcements',SENDER='_drawer_village_operator';
async function main(){
 if(process.argv.includes('--send')){const draft=JSON.parse(fs.readFileSync(path.join(__dirname,'../docs/supporter-mail-20260910.json'),'utf8'));if(draft._delivery?.status!=='approved')throw Error('Mail has not been approved.')}
 ...
 const copy=JSON.parse(fs.readFileSync(path.join(__dirname,'../docs/supporter-mail-20260910.json'),'utf8'));
 ...
 await dispatch.set({...result,status:'complete'},{merge:true});fs.writeFileSync(path.join(__dirname,'../docs/supporter-dispatch-result-20260910.json'),JSON.stringify({dispatch:DISPATCH,...result},null,2)+'\n');console.log(JSON.stringify(result));
}
