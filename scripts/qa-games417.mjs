...
const {chromium,webkit}=require(playwrightPath),output=resolve(root,"tmp/qa-games417");
...
try{
 ...
 for(const lang of ['ko','en','ja']){await page.evaluate(lang=>s.state.uiLanguage=lang,lang);raw.phase='discussion';raw.meetingEndsAt=Date.now()+300000;raw.currentClaim=null;await page.evaluate(g=>paint(g)(),engine.view(raw,'qa'));assert.equal(await page.locator('[data-meeting-action]').count(),5);assert(await page.locator('.mp-public-record').isVisible());assert.equal(await page.locator('.mp-notebook').count(),0);await page.locator('[data-meeting-action=accuse]').click();await page.locator('.mp-meeting-choices button').first().click();await page.locator('.mp-meeting-choices button').first().click();assert.equal(await page.evaluate(()=>sent.kind),'accuse');
 raw.phase='claim';raw.currentClaim={kind:'accuse',speaker:'p1',target:'p0'};await page.evaluate(g=>paint(g)(),engine.view(raw,'qa'));assert.equal(await page.locator('[data-meeting-action]').count(),3);assert(await page.locator('[data-meeting-action=oppose]').isEnabled());await page.locator('[data-meeting-action=oppose]').click();await page.locator('.mp-meeting-choices button').first().click();assert.equal(await page.evaluate(()=>sent.kind),'oppose');await page.evaluate(g=>paint(g)(),engine.view(raw,'qa'));await page.screenshot({path:output+'/meeting-'+lang+'.png'});
 }
 console.log('PASS417 Chrome/WebKit: KO/EN/JA five own-turn and three response controls, baseless opposition, public record.');
