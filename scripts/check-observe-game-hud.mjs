...
const checks=[
  ...
  [css.includes('-webkit-text-stroke:1.35px #17110d!important')&&css.includes('background:transparent!important;color:#fff!important')&&css.includes('-webkit-line-clamp:4!important')&&css.includes('height:auto!important;max-height:34dvh!important')&&css.includes('top:1.45dvh!important'),"현재 순간 무배경 외곽선 글자·한 줄 제목·설명 4줄·내용 맞춤 펼침 카드"],
  [css.includes('top:5.23dvh!important')&&css.includes('.game-hud-profile-copy small::before')&&css.includes('inset:-.22dvh 0 0 0!important')&&views.includes('<small><em>${esc(c.jobTitle'),"시간 기준선과 프로필을 침범하지 않는 직업 길이별 갈색 영역"],
