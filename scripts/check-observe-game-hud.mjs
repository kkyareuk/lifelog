...
const checks=[
  ...
  [css.includes('@media(min-width:721px)')&&css.includes('.game-hud-side{width:76px!important}')&&css.includes('width:70px!important;height:70px!important')&&css.includes('width:62px!important;height:62px!important')&&css.includes('.game-hud-side button>small{width:94px!important}')&&css.includes('width:88px!important;height:88px!important')&&css.includes('.game-hud-dock button>small{width:84px!important}'),"태블릿 가로 화면에서도 홈 메뉴 원화와 글자판이 화면 비율을 따라 거대해지지 않는 크기 상한"],
  ...
  [css.includes('.game-hud-profile-copy b{')&&css.includes('left:22.57vw!important;top:1.745dvh!important')&&css.includes('color:#fff!important;-webkit-text-fill-color:#fff!important;-webkit-text-stroke:1.2px #000!important')&&css.includes('.game-hud-top>time{')&&css.includes('top:5.998dvh!important')&&css.includes('-webkit-text-stroke:1.1px #000!important')&&css.includes('.game-hud-date{')&&css.includes('top:4.035dvh!important'),"이름·시간 흰 글자와 검은 외곽선·동일 하단 정렬 및 위쪽 날짜"],
  ...
  [views.includes('ja:{character:"人物",catalog:"辞典",relationship:"関係",routine:"予定"')&&views.includes('ko:{character:"캐릭터",catalog:"사전",relationship:"관계",routine:"일정"')&&views.includes('en:{character:"People",catalog:"Dictionary"'),"사전 명칭과 한국어·영어·일본어 모바일 메뉴 표기"],
  ...
  ,[views.includes('data-home-layout-fill')&&views.includes('class="home-layout-ld-frame-guide"')&&app.includes('const canvasRect=(mode==="ld"?layer:canvas).getBoundingClientRect()')&&app.includes('persistCharacterSceneLayout(characterId,"ld"'),"실제 비율 회색 영역 안 LD 끌기·크기 조절·화면 채우기 프리셋"]
  ...
  ,[views.includes('class="mobile-character-profile-draft"')&&views.includes('class="character-wallet-selected"')&&views.includes('class="character-registration-card"')&&views.includes('data-character-ui-version="5"')&&views.includes('data-open-quick-character-settings')&&views.includes('data-open-full-character-settings')&&css.includes('assets/character-ui/wallet.png')&&!views.includes('character-pane-ribbon'),"캐릭터 화면을 카드지갑 허브와 빠른·전체 설정 구조로 재배치"]
