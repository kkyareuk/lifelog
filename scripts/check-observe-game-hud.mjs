...
const checks=[
  ...
  [views.includes('data-game-hud-moment')&&views.includes('class="game-hud-moment-title" data-toggle-game-hud-moment')&&views.includes('game-hud-button game-hud-moment-toggle')&&views.includes('class="game-hud-moment-body" data-toggle-game-hud-moment')&&views.includes('data-expand-label')&&app.includes("card.classList.toggle('expanded')")&&app.includes("card.querySelectorAll('[data-toggle-game-hud-moment]')")&&app.includes('event.key!=="Enter"'),"지금 이 순간 글자·버튼·내용 카드 전체의 펼치기·접기 동작"],
  ...
  [css.includes('-webkit-text-stroke:1.5px #000!important')&&css.includes('background-color:transparent!important')&&css.includes('-webkit-text-fill-color:#fff!important')&&css.includes('font-size:clamp(18px,5.2vw,24px)!important')&&css.includes('border:1.5px solid #000!important;border-radius:30px!important')&&interfaceCss.includes(':not(.game-hud-moment-title)')&&css.includes('-webkit-line-clamp:4!important')&&views.includes('game-hud-button game-hud-moment-toggle')&&css.includes('background:transparent url("./assets/home-ui/red-tape.png")')&&fs.existsSync(path.join(root,"assets/home-ui/red-tape.png")),"현재 순간 무배경 흰 외곽선 글자·4줄 설명·검은 카드 테두리와 빨간 테이프"],
  [css.includes('.game-hud-profile-copy small::before')&&css.includes('display:none!important;content:none!important')&&css.includes('max-width:52vw!important')&&css.includes('background:transparent url("./assets/home-ui/pill.png")')&&views.includes('<small><em>${esc(c.jobTitle'),"프로필에서 분리되고 화면 밖으로 나가지 않는 독립 직업 캡슐"],
  ...
  ,[views.includes('class="mobile-character-profile-draft"')&&views.includes('class="character-draft-sheet"')&&views.includes('class="character-draft-actions"')&&css.includes('.mobile-character-pane-grid button:nth-child(6)')&&css.includes('background:#f5ecd3'),"캐릭터 화면을 프로필 용지와 색상별 설정 띠 구조로 재배치"]
