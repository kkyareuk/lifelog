...
const checks=[
  ...
  [css.includes('-webkit-text-stroke:1.5px #000!important')&&css.includes('background-color:transparent!important')&&css.includes('-webkit-text-fill-color:#fff!important')&&css.includes('font-size:clamp(18px,5.2vw,24px)!important')&&css.includes('border:1.5px solid #000!important;border-radius:30px!important')&&interfaceCss.includes(':not(.game-hud-moment-title)')&&css.includes('-webkit-line-clamp:4!important')&&views.includes('game-hud-button game-hud-moment-toggle')&&css.includes('--home-ui-red-tape')&&fs.existsSync(path.join(root,"assets/home-ui/red-tape.png")),"현재 순간 무배경 흰 외곽선 글자·4줄 설명·검은 카드 테두리와 빨간 테이프"],
