...
const cdp=await page.context().newCDPSession(page);await cdp.send('Emulation.setCPUThrottlingRate',{rate:4});await cdp.send('Profiler.enable');await cdp.send('Profiler.start');const timing=await page.evaluate(async()=>{
 ...
 return times;
 });const {profile}=await cdp.send('Profiler.stop');const totals=new Map();for(let i=0;i<(profile.samples||[]).length;i++){const node=profile.nodes.find(n=>n.id===profile.samples[i]);const key=node.callFrame.functionName+' '+node.callFrame.url.split('/').pop()+':'+node.callFrame.lineNumber;totals.set(key,(totals.get(key)||0)+(profile.timeDeltas[i]||0));}console.log([...totals].sort((a,b)=>b[1]-a[1]).slice(0,18));console.log(JSON.stringify(timing));assert(!errors.length,errors.join('\n'));
