...
const PROJECT='lifelog-98fff',DISPATCH='update-351-381-20260913-v1',GROUP='_drawer_village_announcements',SENDER='_drawer_village_operator';
async function main(){
 if(process.argv.includes('--send')){const draft=JSON.parse(fs.readFileSync(path.join(__dirname,'../docs/update-mail381.json'),'utf8'));if(draft._delivery?.status!=='approved')throw Error('Mail has not been approved.')}
 ...
 for(const trigger of triggers){const document=trigger.filters?.find(f=>f.attribute==='document')?.value;if(trigger.type?.includes('firestore')&&document!=='notificationOutbox/{eventId}'&&document!=='moderationReports/{reportId}'&&document!=='playerFeedback/{reportId}')throw Error('Unreviewed Firestore trigger: '+trigger.name)}
 if(!process.argv.includes('--send'))return;
 const copy=JSON.parse(fs.readFileSync(path.join(__dirname,'../docs/update-mail381.json'),'utf8'));
 ...
 await db.runTransaction(async tx=>{
  ...
  if(!prior.exists){tx.set(root,{name:'서랍마을 운영 공지',ownerUid:SENDER,systemAnnouncements:true,createdAt:Date.now()});tx.set(root.collection('members').doc(SENDER),{displayName:'서랍마을 운영팀',role:'owner'});tx.create(dispatch,{contentHash,createdAt:Date.now(),recipientCount:recipients.length,status:'sending',pushNotification:false})}
 });
 ...
 await dispatch.set({...result,status:'complete'},{merge:true});fs.writeFileSync(path.join(__dirname,'../docs/update-mail381-result.json'),JSON.stringify({dispatch:DISPATCH,...result},null,2)+'\n');console.log(JSON.stringify(result));
}
