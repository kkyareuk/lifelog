...
export const FURNITURE_PROPS=Object.freeze(["책","화분","향수","액자","컵","인형","수집품","조명","화분 2"]);
...
export const FURNITURE_FOOTPRINTS=Object.freeze({
  ...
  "냉장고":footprint(1,2),"카운터":footprint(1,2),"인덕션":footprint(1,1),"화분 2":footprint(1,2),"조리대":footprint(3,1),"식탁":footprint(3,2),"오븐":footprint(1,2),"커피포트":footprint(1,1),"커피머신":footprint(1,1),"식기세척기":footprint(1,2),"에스프레소 머신":footprint(1,1),"티 세트":footprint(1,1),"제빵 도구":footprint(2,1),"칵테일 바":footprint(2,2),"와인 냉장고":footprint(1,2),"향신료 선반":footprint(2,1),"요리책 선반":footprint(2,2),
  ...
  "변기":footprint(1,2),"1인 침대":footprint(2,3),"샤워부스":footprint(2,2),"욕조":footprint(3,1),"세면대":footprint(2,1),"세탁기":footprint(1,2),"건조기":footprint(1,2),"입욕제 선반":footprint(2,1),"향수 선반":footprint(2,1),"스킨케어 선반":footprint(2,1),
...
const PROP_ICONS={"화분 2":"🌹",인덕션:"🍳",책:"📕",화분:"🪴",향수:"🧴",액자:"🖼️",컵:"☕",인형:"🧸",수집품:"🏺",조명:"💡"};
const PROP_LABELS={
  en:{"화분 2":"Potted flowers",인덕션:"Induction cooktop",책:"Book",화분:"Plant",향수:"Perfume",액자:"Frame",컵:"Cup",인형:"Doll",수집품:"Collectible",조명:"Lamp"},
  ja:{"화분 2":"花の鉢植え",인덕션:"IHコンロ",책:"本",화분:"鉢植え",향수:"香水",액자:"写真立て",컵:"カップ",인형:"人形",수집품:"コレクション",조명:"照明"}
};
const PROP_SURFACE_PATTERN=/(선반|책장|진열|수납장|신발장|찬장|와인장|책상|테이블|협탁|화장대|조리대|작업대|식탁|카운터(?: 연결형| 상판)?|바)$/;
...
export const FURNITURE_CATALOG=Object.freeze({
  ...
  kitchen:["냉장고","카운터","인덕션","조리대","식탁","오븐","커피포트","커피머신","식기세척기","에스프레소 머신","티 세트","제빵 도구","칵테일 바","와인 냉장고","향신료 선반","요리책 선반"],
  entry:["신발장","전신거울","우산꽂이","반려동물 산책용품","자전거 보관대","운동 장비 선반","캠핑 장비"],
  bath:["변기","샤워부스","욕조","세면대","세탁기","건조기","입욕제 선반","향수 선반","스킨케어 선반"],
  bedroom:["1인 침대","침대","커플 침대","옷장","화장대","협탁","빔프로젝터","독서등","향수 진열대","레코드 플레이어","작은 게임기","봉제인형","수집품 진열장"],
  ...
  balcony:["화분","화분 2","야외 의자","작은 테이블","빨래 건조대","원예 도구","캠핑 의자","천체망원경"],
  storage:["수납장","선반","보관 상자","옷걸이","캠핑 장비","운동 장비","수집품 상자"],
  other:["수납장","의자","작은 테이블","책장","오디오"]
});

const ICONS={"화분 2":"🌹","카운터":"🗄️","인덕션":"🍳",
  "소파":"🛋️","TV":"📺","책장":"📚","오디오":"🔊","안마의자":"🪑","게임기":"🎮","캣타워":"🐈","턴테이블":"💿","보드게임장":"🎲","홈시어터":"🎞️","프로젝터":"📽️","악기 진열장":"🎸","수집품 진열장":"🏺","독서 의자":"🪑","반려동물 장난감":"🧸","러닝머신":"🏃",
  "냉장고":"🧊","조리대":"🍳","식탁":"🍽️","오븐":"♨️","커피포트":"🫖","커피머신":"☕","식기세척기":"🫧","에스프레소 머신":"☕","티 세트":"🫖","제빵 도구":"🥐","칵테일 바":"🍸","와인 냉장고":"🍷","향신료 선반":"🧂","요리책 선반":"📖",
  "신발장":"👞","전신거울":"🪞","우산꽂이":"☂️","반려동물 산책용품":"🦮","자전거 보관대":"🚲","운동 장비 선반":"🏋️","캠핑 장비":"⛺",
  "변기":"🚽","1인 침대":"🛏️","샤워부스":"🚿","욕조":"🛁","세면대":"🧼","세탁기":"🧺","건조기":"♨️","입욕제 선반":"🫧","향수 선반":"🧴","스킨케어 선반":"🧴",
...
export const supportsFurnitureProps=item=>!/^카운터|^협탁$/.test(String(item))&&PROP_SURFACE_PATTERN.test(String(item||"").trim());
...
export function normalizeFurniturePlacement(value,index=0){
  ...
  const item=/^카운터 (상판|연결형)$/.test(legacyItem)?"카운터":legacyItem;
  if(!item)return null;
...
export function normalizeFurniturePlacements(value){
  if(!Array.isArray(value))return [];
  const seen=new Set();
  const migrated=value.flatMap(p=>{if(!p||!(/^카운터|^협탁$/.test(p.item))||!p.props?.length)return [p];return [{...p,props:[]},...p.props.map((prop,i)=>({id:p.id+'-surface-'+i,item:prop.item,x:p.x,y:p.y,scale:.55,surfaceId:p.id,surfaceU:p.item==='협탁'?.5:(i%4+.5)/4,surfaceV:.5}))]});
  return migrated.map(normalizeFurniturePlacement).filter(item=>item&&!seen.has(item.id)&&(seen.add(item.id),true)).slice(0,80);
}
