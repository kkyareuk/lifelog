...
import {normalizeRelationshipDetails,narrativeWeight} from "./official-relationship-details.js?v=20260909dev305";
...
import {normalizeTownProfile,TOWN_ILLUSTRATIONS} from "./town-profile.js?v=20260909dev305";
import {normalizeBuildingLighting} from "./town-lighting.js?v=20260909dev305";
...
import {missingBuildings} from "./building-recovery.js?v=20260909dev305";
import {normalizeSceneImageVariants} from "./character-scene-image.js?v=20260909dev305";
...
const normalizedBodyProfile=value=>{
  ...
  return{
    ...
    appearanceSummaries:Array.isArray(source.appearanceSummaries)?[...new Set(source.appearanceSummaries.map(String))].slice(0,4):[],
    ...
    appearance:{
      ...
      bodyHairLocations:Array.isArray(appearanceSource.bodyHairLocations)?[...new Set(appearanceSource.bodyHairLocations.map(String))].slice(0,16):[],
    ...
    hospitalVisits:Array.isArray(source.hospitalVisits)?source.hospitalVisits.slice(0,12).map(r=>({department:String(r?.department||""),purpose:String(r?.purpose||""),frequency:String(r?.frequency||"")})):[],
...
const fresh=()=>({schema:36,activeTab:"observe",characterPane:"profile",characterOverviewPane:"basic",characterBodyPane:"figure",characterPersonalityPane:"core",characterTastePane:"categories",characterSettingsView:"hub",statisticsTownId:"all",activeId:null,activeHomeId:null,activeTownId:null,homeEditMode:false,homeVisualMode:"sd",homeSdScale:100,homeLdScale:100,homeUiTheme:"drawer-classic",buildingLabelMode:"full",preventInterTownMovement:false,soundMuted:false,soundEffectsVolume:45,backgroundMusicVolume:35,backgroundMusicMuted:false,animationIntensity:"normal",measurementUnits:"metric",routineView:"weekly",routineMonth:"",uiLanguage:"ko",uiFont:"hanbit",uiScale:"normal",colorMode:"light",visualTheme:"drawer-default",ownerName:"",personalTownLabel:"내 마을",characterNotificationsEnabled:false,characterNotificationConsent:"unknown",characterNotificationSettings:defaultCharacterNotificationSettings(),achievements:{version:1,progress:{},unlockedAt:{},lastGooglePlaySync:{}},lastSaved:0,characters:{},order:[],homes:{},relationships:{},relationshipDevelopment:{},characterGroups:[],deletedCharacterIds:[],deletedRelationshipIds:[],deletedRelationshipKeys:[],deletedHomeIds:[],deletedRoutineIds:[],deletedMonthlyRoutineIds:[],anniversaries:[],characterViews:{},routines:{},monthlyRoutines:{},dailyPlans:{},interactions:[],dailyQuestion:null,scheduledChoices:[],characterDirectives:{},catalog:defaultCatalog(),towns:[],world:{name:"서랍마을",bg:"world-assets/owner-forest-town.webp",illustrationId:"owner-forest",photo:"",townType:"생활 중심",townSubtype:"골목 생활권",density:"여유로움",urbanization:"소도시",reputation:"평판 정보 없음",fameLevel:"거의 알려지지 않음",size:"보통 마을",terrain:"평야",transportModes:["일반 도로","시외버스"],travelAllowed:true,description:"",places:[
...
function normalizeHomes(x){
  ...
  x.backgroundMusicMuted=Boolean(x.backgroundMusicMuted);
  x.backgroundMusicVolume=Number.isFinite(+x.backgroundMusicVolume)?Math.max(0,Math.min(100,+x.backgroundMusicVolume)):35;
  ...
  x.achievements={
    version:1,
    progress:achievementSource.progress&&typeof achievementSource.progress==="object"?achievementSource.progress:{},
    unlockedAt:achievementSource.unlockedAt&&typeof achievementSource.unlockedAt==="object"?achievementSource.unlockedAt:{},
    lastGooglePlaySync:achievementSource.lastGooglePlaySync&&typeof achievementSource.lastGooglePlaySync==="object"?achievementSource.lastGooglePlaySync:{}
  };
  delete x.userMods;
  ...
  x.personalTownLabel=String(x.personalTownLabel||"내 마을").trim().slice(0,24)||"내 마을";
  ...
  x.preventInterTownMovement=Boolean(x.preventInterTownMovement);
  x.characterGroups=Array.isArray(x.characterGroups)?x.characterGroups.filter(group=>group&&typeof group==="object"&&!Array.isArray(group)).map(group=>({
    id:String(group.id||uid()),name:String(group.name||"새 그룹").trim().slice(0,40)||"새 그룹",
    memberIds:Array.isArray(group.memberIds)?[...new Set(group.memberIds.map(String).filter(id=>x.characters[id]))]:[]
  ...
  x.characterDirectives=x.characterDirectives&&typeof x.characterDirectives==="object"&&!Array.isArray(x.characterDirectives)
    ?Object.fromEntries(Object.entries(x.characterDirectives).filter(([characterId,item])=>x.characters?.[characterId]&&item&&typeof item==="object"&&!Array.isArray(item)).map(([characterId,item])=>[characterId,{
      ...
      room:String(item.room||"living"),placeId:String(item.placeId||""),homeId:x.homes?.[item.homeId]?String(item.homeId):"",targetId:x.characters?.[item.targetId]?String(item.targetId):"",
      subjectId:x.characters?.[item.subjectId]?String(item.subjectId):"",withIds:[...new Set((Array.isArray(item.withIds)?item.withIds:[]).map(String).filter(id=>x.characters?.[id]))].slice(0,4),
      topic:String(item.topic||"").slice(0,120),copy:item.copy&&typeof item.copy==="object"&&!Array.isArray(item.copy)?item.copy:{}
    }]))
    :{};
  ...
  relationList.filter(Boolean).forEach(relation=>{
    ...
    if(!relation.a||!relation.b||String(relation.a)===String(relation.b))return;
  ...
  const hadAuthoritativeTownList=x.towns.length>0;
  ...
  x.towns.forEach((town,index)=>{
    ...
    // With a towns array, each town's own place list is authoritative. `world` is
    // only the active-town mirror and must never be unioned back during restore.
    const activePlaces=!hadAuthoritativeTownList&&town.id===x.activeTownId?x.world.places:[];
  ...
  Object.values(x.homes||{}).forEach((h,homeIndex)=>{
    ...
    h.iconPreset=String(h.iconPreset&&h.iconPreset!=="drawer-home"?h.iconPreset:"red-roof-home");
  ...
  Object.values(x.characters||{}).forEach(c=>{
    ...
    x.routines[c.id]=Array.isArray(x.routines[c.id])?x.routines[c.id].filter(r=>r&&typeof r==="object"&&!Array.isArray(r)&&!x.deletedRoutineIds.includes(String(r.id||""))).map(r=>{
      ...
      return {
        ...
        notes:r.notes||"",dressCode:normalizeDressCode(r.dressCode)
      };
    }):[];
    x.monthlyRoutines[c.id]=Array.isArray(x.monthlyRoutines[c.id])?x.monthlyRoutines[c.id].filter(r=>r&&typeof r==="object"&&!Array.isArray(r)&&!x.deletedMonthlyRoutineIds.includes(String(r.id||""))&&/^\d{4}-\d{2}-\d{2}$/.test(String(r.date||""))).map(r=>({
      ...
      title:String(r.title||"새 일정"),placeId:String(r.placeId||""),visitHomeId:x.homes?.[r.visitHomeId]?String(r.visitHomeId):"",withIds:Array.isArray(r.withIds)?r.withIds.map(String).filter(id=>x.characters[id]):[],notes:String(r.notes||""),dressCode:normalizeDressCode(r.dressCode)
    })):[];
    ...
    c.emotionalSensitivity=["매우 둔감함","둔감한 편","보통","예민한 편","매우 예민함"].includes(c.emotionalSensitivity)?c.emotionalSensitivity:"보통";
    c.emotionalContagion=["거의 물들지 않음","가까운 사람에게만 물듦","상황에 따라 물듦","쉽게 물드는 편","매우 쉽게 물듦"].includes(c.emotionalContagion)?c.emotionalContagion:"상황에 따라 물듦";
    ...
    ["favoriteAnimals","favoriteElectronics","favoriteWeapons","favoriteBooks"].forEach(field=>c[field]=Array.isArray(c[field])?[...new Set(c[field])]:[]);
    ["dislikedStoryGenres","dislikedFoodPreferences","dislikedDrinks","dislikedMusicGenres","dislikedVideoGenres","dislikedGameGenres","dislikedScentNotes","dislikedAnimals","dislikedElectronics","dislikedWeapons","dislikedBooks"].forEach(field=>c[field]=Array.isArray(c[field])?[...new Set(c[field])]:[]);
    ...
    c.dislikes=c.dislikes&&typeof c.dislikes==="object"&&!Array.isArray(c.dislikes)?c.dislikes:{};
    Object.entries(c.dislikes).forEach(([kind,ids])=>c.dislikes[kind]=Array.isArray(ids)?[...new Set(ids.map(String))]:[]);
    Object.keys(defaultsCatalog).forEach(kind=>c.dislikes[kind]=Array.isArray(c.dislikes[kind])?[...c.dislikes[kind]]:[]);
    ...
    c.homeSceneLayout=normalizeHomeSceneLayout(c.homeSceneLayout);
    ...
    rawResidences.forEach(item=>{
      ...
      const legacyStayPattern=String(item.stayPattern||"");
      residenceMap.set(homeId,{
        homeId,
        role:["주거지","본가","별채","주말집","업무용 숙소","연인의 집","친척집","기타"].includes(item.role)?item.role:"주거지",
        stayPattern:legacyStayPattern==="명절·기념일"?"필요할 때 방문":["상시 거주","평일 중심","주말 중심","요일 지정","필요할 때 방문"].includes(legacyStayPattern)?legacyStayPattern:"상시 거주",
...
function load(){
  ...
  const world=normalizeHomes(fresh());
  world.uiLanguage=initialLanguage(globalThis.window?.navigator);
  return world;
}
...
export function createCharacter(limit=5){
  if(state.order.length>=Math.max(0,Number(limit)||0))return null;
  const id=uid();
  state.deletedCharacterIds=(state.deletedCharacterIds||[]).filter(value=>value!==id);
  state.characters[id]={id,name:"새 캐릭터",aggressionLevel:"공격적인 반응이 거의 없음",discovery:{version:2,locks:{},answers:{}},socialStyle:"조용히 어울림",perceptionStyle:"균형형",decisionStyle:"균형형",planningStyle:"상황에 따라",neatness:"보통",energyRhythm:"상황에 따라",interference:"적당히 관여",createdAt:Date.now(),ageGroup:"성인",gender:"설정하지 않음",speechStyle:"자동 · 성격에 맞춤",attractedGenders:[],touchReaction:"상황에 따라 자연스럽게 받아들임",appearanceLevel:"보통",appearanceInterest:"보통",appearanceTags:[],attractionTraits:[],dislikedAttractionTraits:[],personalityTypes:[],characterTraits:[],traitExpressions:[],traitNotes:"",traitNotesInScripts:false,bodyProfile:defaultBodyProfile(),timelineResetAt:0,job:"무직",jobTitle:"",workplaceId:"",educationLevel:"설정하지 않음",lifeAdaptation:"설정하지 않음",birthday:"",driverLicense:"면허 없음",smokingStatus:"설정하지 않음",alcoholTolerance:"설정하지 않음",photo:"",icon:"",ldImage:"",sceneImageVariants:normalizeSceneImageVariants(),characterMenuTheme:"drawer-default",homeUiTheme:state.homeUiTheme||"drawer-classic",homeVisualMode:"sd",homeVisualScale:100,homeSdScale:100,homeLdScale:100,homeSceneLayout:defaultHomeSceneLayout(),wake:"07:30",wakeHabit:"알람을 듣고 천천히 일어남",sleep:"00:30",sleepHabit:"이불을 단정히 덮고 잠",foodHabit:"규칙적으로 식사함",walkingStyle:"보통 속도로 자연스럽게",income:"필요한 만큼 소비",wealth:"평범한 형편",spiceTolerance:2,sweetPreference:2,socialEnergy:3,sensingIntuition:3,thinkingFeeling:3,perceivingJudging:3,fashionSense:"무난하게 입음",accessoryPreference:"상황에 따라",humorStyle:"건조한 농담만 함",emotionalExpression:"상황에 따라 표현함",impulseControl:"가끔 욱하지만 멈춤",emotionalBaseline:"현실적인 편",moodVolatility:"상황에 따라 달라짐",moodPersistence:"보통",positiveMoodResponse:"미소와 말로 표현함",stressMoodResponse:"잠시 거리를 둠",moodRecoveryStyle:"혼자 정리하며 회복",angerResponse:"차분히 이유를 확인함",flirtResponse:"알아도 모른 척함",emotionalSensitivity:"보통",emotionalContagion:"상황에 따라 물듦",savedOutfits:[],theme:{primary:"#176b60",secondary:"#6fd0ae",gradient:true},tastes:[],interests:[],hobbies:[],musicGenres:[],foodTypes:[],foodPreferences:[],drinks:[],favoriteAnimals:[],favoriteElectronics:[],favoriteWeapons:[],favoriteBooks:[],dislikedStoryGenres:[],dislikedFoodPreferences:[],dislikedDrinks:[],dislikedMusicGenres:[],dislikedVideoGenres:[],dislikedGameGenres:[],dislikedScentNotes:[],dislikedAnimals:[],dislikedElectronics:[],dislikedWeapons:[],dislikedBooks:[],favorites:{},dislikes:{},inventory:{},homeId:id,sleepRoomId:"bedroom",residences:[{homeId:id,role:"주거지",stayPattern:"상시 거주",visitDays:[],visitDates:"",notes:"",isPrimary:true,sleepRoomId:"bedroom"}]};
  state.characters[id].appearanceCareLevel="기본적으로 단정하게";
  state.characters[id].accessoryUse="착용하지 않음";
  delete state.characters[id].accessoryPreference;
...
export const touchCharacterTimelines=ids=>{
...
export function setCharacterBodyChoices(characterId,path,values,persist=true){
  const character=state.characters?.[characterId],parts=String(path||"").split(".").filter(Boolean);
  if(!character||!parts.length)return false;
  const bodyProfile=normalizedBodyProfile(structuredClone(character.bodyProfile||{})),last=parts.pop();
...
export function addRoutine(characterId){
  ...
  const id=uid(),routine={id,seriesId:id,day:1,start:"09:00",end:"10:00",type:"개인 일정",title:"새 일정",placeId:"",visitHomeId:"",withIds:[],notes:"",dressCode:normalizeDressCode()};
...
function invalidateDeletedSchedules(characterId,routineIds,tombstoneKey){
  ...
  const affectedCharacterIds=new Set([characterId]);
  Object.values(state.characters||{}).forEach(character=>{
    ...
    if(affected){character.timelineResetAt=changedAt;affectedCharacterIds.add(character.id)}
  });
  affectedCharacterIds.forEach(id=>delete state.dailyPlans?.[id]);
}
...
export function setCharacterSceneImage(id,key,mode,data){
  const c=state.characters[id];if(!c)return;
  c.sceneImageVariants=normalizeSceneImageVariants(c.sceneImageVariants);
  if(!c.sceneImageVariants[key])return;
  c.sceneImageVariants[key][mode==="icon"?"icon":"ldImage"]=String(data||"");
  save(true);
}
...
const DIRECTIVE_COPY={
  ...
  nap:{ko:["잠깐 눈을 붙이는 중","편한 자리를 골라 짧게 쉬며 기운을 회복하고 있어요."],en:["Taking a short nap","They found a comfortable spot and are resting briefly to recharge."],ja:["少し昼寝をしているところ","楽な場所で短く休み、元気を取り戻しています。"],room:"bedroom",minutes:45},
  ...
  music:{ko:["좋아하는 음악을 즐기는 중","듣고 싶던 곡을 골라 음악에 천천히 빠져들고 있어요."],en:["Enjoying some favorite music","They picked a song they wanted to hear and are settling into the music."],ja:["好きな音楽を楽しんでいるところ","聴きたかった曲を選び、ゆっくり音楽に浸っています。"],room:"living",minutes:55},
  game:{ko:["게임에 집중하는 중","하고 싶던 게임을 켜고 한 판에 차분히 집중하고 있어요."],en:["Focused on a game","They started a game they had wanted to play and are concentrating on a round."],ja:["ゲームに集中しているところ","遊びたかったゲームを始め、一戦に集中しています。"],room:"living",minutes:65},
  art:{ko:["손으로 무언가 만드는 중","그리거나 만들고 싶던 것을 꺼내 자기 방식대로 표현하고 있어요."],en:["Making something by hand","They brought out something they wanted to draw or make and are expressing it their way."],ja:["手を動かして何かを作っているところ","描いたり作ったりしたかったものを出し、自分らしく表現しています。"],room:"study",minutes:70},
  walk:{ko:["가볍게 산책하는 중","바람을 쐬며 천천히 걸어 머릿속을 환기하고 있어요."],en:["Taking a gentle walk","They are walking at an easy pace, getting some air, and clearing their head."],ja:["軽く散歩しているところ","風に当たりながらゆっくり歩き、気分転換をしています。"],room:"living",minutes:55},
  work:{ko:["해야 할 업무에 집중하는 중","미뤄 둔 일을 펼쳐 하나씩 차분히 처리하고 있어요."],en:["Focusing on work","They opened the work they had put off and are handling it one task at a time."],ja:["仕事に集中しているところ","後回しにしていた仕事を開き、一つずつ落ち着いて進めています。"],room:"study",minutes:80},
...
function baseSocialDirectiveCopy(kind,actor,target,subject,topic,options={}){
  ...
  const copy={
    ...
    affection:{ko:[`${names.target}와 둘만의 시간을 보내는 중`,'서로의 마음을 확인하고 잠시 둘만의 시간을 보내고 있어요.'],en:[`Sharing private time with ${names.target}`,'They check in with each other and spend some private time together.'],ja:[`${names.target}と二人だけの時間を過ごすところ`,'互いの気持ちを確かめ、二人だけの時間を過ごしています。']},
...
export function addRoom(homeId,floor){
  ...
  h.rooms[key]={name:"새 방",type:"other",size:"보통 방",order:nextOrder,floor:Math.max(1,Math.min(h.floorCount||1,Number(floor)||h.activeFloor||1)),image:"",imageFit:"cover",floorMaterial:"natural",wallMaterial:"same",floorImage:"",interiorStyle:"설정하지 않음",titleTone:"light",furniture:[...FURNITURE_CATALOG.other],furniturePlacements:[]};
  save(true);
  return key;
}
...
export function addFurniturePlacement(homeId,roomKey,item){
  ...
  const placements=normalizeFurniturePlacements(room.furniturePlacements),id=`furniture-${uid()}`;
  placements.push(newFurniturePlacement(id,item,placements.length));
  room.furniturePlacements=placements;
  room.furniture=Array.isArray(room.furniture)?[...new Set([...room.furniture,String(item)])]:[String(item)];
  touchCharacterTimelines(Object.values(state.characters).filter(c=>(c.residences||[]).some(entry=>entry.homeId===homeId)).map(c=>c.id));
  save(true);return id;
}
export function updateFurniturePlacement(homeId,roomKey,placementId,patch,persist=true){
...
export function assignFurnitureBed(homeId,roomKey,placementId,characterId,assigned=true){
  const room=state.homes[homeId]?.rooms?.[roomKey],character=state.characters[characterId];if(!room||!character)return false;
  const placements=normalizeFurniturePlacements(room.furniturePlacements),index=placements.findIndex(item=>item.id===placementId);
  if(index<0||!isBedFurniture(placements[index].item))return false;
  const current=placements[index],capacity=furnitureCapacity(current.item),ids=new Set(current.assignedCharacterIds||[]);
  if(assigned){
    if(!ids.has(characterId)&&ids.size>=capacity)return false;
    // 한 집에서는 한 캐릭터가 한 침대만 사용한다. 다른 침대의 지정은 해제한다.
    placements.forEach((placement,placementIndex)=>{
      if(placementIndex===index||!isBedFurniture(placement.item)||!placement.assignedCharacterIds?.includes(characterId))return;
      placements[placementIndex]=normalizeFurniturePlacement({...placement,assignedCharacterIds:placement.assignedCharacterIds.filter(id=>id!==characterId)},placementIndex);
    });
    ids.add(characterId);
  }else ids.delete(characterId);
  placements[index]=normalizeFurniturePlacement({...current,assignedCharacterIds:[...ids]},index);
  room.furniturePlacements=placements;touchCharacterTimelines([characterId]);save(true);return placements[index];
}
...
export function deleteCatalogItem(kind,id){
  ...
  Object.values(state.characters).forEach(c=>{if(c.dislikes?.[kind])c.dislikes[kind]=c.dislikes[kind].filter(x=>x!==id)});
...
export function toggleFavorite(characterId,kind,itemId,persist=true){
  ...
  c.favorites[kind]=list.includes(itemId)?list.filter(x=>x!==itemId):[...list,itemId];c.timelineResetAt=Date.now();if(persist)save(true);
}
export function toggleDislike(characterId,kind,itemId,persist=true){
  const c=state.characters[characterId];if(!c)return;
  c.dislikes=c.dislikes||{};const list=Array.isArray(c.dislikes[kind])?[...c.dislikes[kind]]:[];
  c.dislikes[kind]=list.includes(itemId)?list.filter(x=>x!==itemId):[...list,itemId];c.timelineResetAt=Date.now();if(persist)save(true);
}
...
export function relationshipViewDefaults(type,temporalStatus="current",orderLength=state.order.length){
  ...
  if(temporalStatus==="past")return {overall:"그저 그런 사람",importance:"비중 없음",awareness:"자기 감정을 분명히 자각함",mutualAwareness:"상대의 마음을 전혀 모름",trust:"조심스럽게 지켜봄",fear:"설정하지 않음",closeness:"거리감 있음",comfort:"어색하지만 필요한 대화는 무난함",annoyance:"가끔 성가심",attention:"관심 없음",jealousy:"질투하지 않음",conflictIntensity:"가끔 부딪힘",expectation:"언제든 끝날 수 있다고 생각함",touchIntensity:"신체 접촉 없음",aggression:"공격 충동 없음",aggressionAction:"행동으로 옮기지 않음"};
  if(["연인","약혼","부부"].includes(type))return {overall:"연애 감정으로 좋아함",importance:"비중 없음",awareness:"자기 감정을 분명히 자각함",mutualAwareness:"서로의 마음을 확인함",trust:"어느 정도 믿음",fear:"설정하지 않음",closeness:"가까운 사이",comfort:"편안하고 농담과 장난이 잘 통함",annoyance:"전혀 귀찮거나 성가시지 않음",attention:"자주 살핌",jealousy:"가끔 신경 쓰임",conflictIntensity:"갈등이 거의 없음",expectation:"오래 함께할 거라 기대함",touchIntensity:"포옹·기대기까지",aggression:"공격 충동 없음",aggressionAction:"행동으로 옮기지 않음"};
  if(["부모·자녀","형제·자매"].includes(type))return {overall:"소중하게 여김",importance:"비중 없음",awareness:"자기 감정을 분명히 자각함",mutualAwareness:"상대가 느끼는 감정을 알고 있음",trust:"어느 정도 믿음",fear:"설정하지 않음",closeness:"가까운 사이",comfort:"편안하고 농담과 장난이 잘 통함",annoyance:"가끔 성가심",attention:"종종 신경 씀",jealousy:"질투하지 않음",conflictIntensity:"가끔 부딪힘",expectation:"평생 이어질 관계라고 믿음",touchIntensity:"포옹·기대기까지",aggression:"공격 충동 없음",aggressionAction:"행동으로 옮기지 않음"};
  if(["친구","소꿉친구","학창 시절 친구들","친구 모임"].includes(type))return {overall:"친구로 좋아함",importance:"비중 없음",awareness:"자기 감정을 분명히 자각함",mutualAwareness:"상대가 느끼는 감정을 알고 있음",trust:"어느 정도 믿음",fear:"설정하지 않음",closeness:"편한 사이",comfort:"편안하고 농담과 장난이 잘 통함",annoyance:"전혀 귀찮거나 성가시지 않음",attention:"종종 신경 씀",jealousy:"질투하지 않음",conflictIntensity:"갈등이 거의 없음",expectation:"오래 함께할 거라 기대함",touchIntensity:"인사·부축 같은 의례적 접촉만",aggression:"공격 충동 없음",aggressionAction:"행동으로 옮기지 않음"};
  if(type==="혐관")return {overall:"매우 싫어함",importance:"비중 없음",awareness:"자기 감정을 분명히 자각함",mutualAwareness:"상대가 느끼는 감정을 알고 있음",trust:"전혀 믿지 않음",fear:"설정하지 않음",closeness:"거리감 있음",comfort:"함께 있으면 매우 불편하고 대화도 전혀 통하지 않음",annoyance:"보기만 해도 피곤함",attention:"종종 신경 씀",jealousy:"질투하지 않음",conflictIntensity:"자주 충돌함",expectation:"언제든 끝날 수 있다고 생각함",touchIntensity:"신체 접촉 없음",aggression:"거친 말을 하고 싶은 충동",aggressionAction:"대부분 참지만 가끔 거친 말이 나옴"};
  return {overall:"그저 그런 사람",importance:"비중 없음",awareness:"자기 감정을 분명히 자각함",mutualAwareness:"상대의 마음을 전혀 모름",trust:"보통",fear:"설정하지 않음",closeness:"보통",comfort:"어색하지만 필요한 대화는 무난함",annoyance:"전혀 귀찮거나 성가시지 않음",attention:"필요할 때만 봄",jealousy:"질투하지 않음",conflictIntensity:"갈등이 거의 없음",expectation:"정하지 않음",touchIntensity:"신체 접촉 없음",aggression:"공격 충동 없음",aggressionAction:"행동으로 옮기지 않음"};
}
function withoutOrphanedGeneratedView(explicit,relations){
  ...
  const types=["연인","약혼","부부","친구","소꿉친구","학창 시절 친구들","친구 모임","부모·자녀","형제·자매","혐관","동거인","기타"];
...
export function characterViewFor(sourceId,targetId,indexedRelations=null){
  ...
  let defaults={overall:"낯선 사람으로 여김",importance:"비중 없음",awareness:"자기 감정을 분명히 자각함",mutualAwareness:"상대의 마음을 전혀 모름",trust:"조심스럽게 지켜봄",fear:"설정하지 않음",closeness:"낯선 사이",comfort:"긴장하고 대화도 조심스러움",annoyance:"전혀 귀찮거나 성가시지 않음",attention:"관심 없음",jealousy:"질투하지 않음",conflictIntensity:"갈등이 거의 없음",expectation:"정하지 않음",touchIntensity:"신체 접촉 없음",aggression:"공격 충동 없음",aggressionAction:"행동으로 옮기지 않음"};
  if(currentRelations.length){
    if(currentRelations.some(relation=>["연인","약혼","부부"].includes(relation.type)))defaults={...defaults,overall:"연애 감정으로 좋아함",mutualAwareness:"서로의 마음을 확인함",trust:"어느 정도 믿음",closeness:"가까운 사이",comfort:"편안하고 농담과 장난이 잘 통함",attention:"종종 신경 씀",touchIntensity:"포옹·기대기까지"};
  ...
  if(currentRelations.some(r=>(r.type==="부부"&&["1","2"].includes(r.details?.origin)||r.type==="약혼"&&r.details?.origin==="2"))){defaults={...defaults,overall:"그저 그런 사람",mutualAwareness:"상대의 마음을 전혀 모름",trust:"보통",closeness:"보통",comfort:"어색하지만 필요한 대화는 무난함",attention:"필요할 때만 봄",touchIntensity:"신체 접촉 없음"};}
  // Resolve an age-aware default rather than persisting an opt-in over a
  // user's directional contact limit. Past relationships never grant it.
  if(currentRelations.some(r=>["연인","약혼","부부"].includes(r.type))&&[sourceId,targetId].every(id=>isAdultAge(state.characters[id]?.ageGroup)))defaults.touchIntensity="성인 간 친밀한 접촉까지";
  ...
  return {...defaults,...cleanExplicit,importance:narrativeWeight(cleanExplicit.importance),fear:migratedFear,comfort:migratedComfort||defaults.comfort};
}
...
export function addRelationship(data){
  data={
    ...
    details:normalizeRelationshipDetails(data?.type,data?.details),
...
export function updateRelationship(id,data){
  ...
  if((data.a||relation.a)===(data.b||relation.b))return;
...
export function addPlace(){
  ...
  state.world.places.push({id:uid(),townId:state.activeTownId||"",name,type:"기타",subtype:"",emoji:"🏬",image:"",interiorImage:"",imageScale:1,stock:[],priceRange:"보통",audiences:[],spicy:0,sweet:0,x:50,y:50,color:"#8ecbc0",createdAt:Date.now(),userCreated:true});touchCharacterTimelines(Object.values(state.characters).filter(c=>c.townId===state.activeTownId).map(c=>c.id));save(true);
}
