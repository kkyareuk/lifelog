...
export function openCharacterMoney(pane='wallet',characterId=null,homeId=null){
 ...
 const mutate=async(action,payload={})=>{
  ...
  try{
   if(snapshot){const result=await window.DrawerVillageGroups.characterMoney({id:c.id,action,...payload});if(!valid())return;c.wallet=result.wallet;if(result.walletSharing){world.walletSharing=result.walletSharing;snapshot.group.walletSharing=result.walletSharing;bindWalletAccounts(world);}if(result.wealth!==undefined)c.wealth=result.wealth;if(result.income!==undefined)c.income=result.income;const record=snapshot.residents.find(r=>r.id===c.id);if(record){record.lifeJson=JSON.stringify({...JSON.parse(record.lifeJson||'{}'),wallet:result.wallet});record.profileJson=JSON.stringify({...JSON.parse(record.profileJson||'{}'),wealth:c.wealth,income:c.income});}if(payload.homeId&&result.commonWallet){world.homes[payload.homeId].commonWallet=result.commonWallet;const h=snapshot.homes.find(h=>h.id===payload.homeId);if(h)h.commonWallet=result.commonWallet}}
 ...
 function draw(){
  ...
  const balance=document.createElement('h3');balance.textContent=displayMoney(walletBalance(c.wallet),c,world.uiLanguage);if(!isWork)d.append(balance);const content=document.createElement('section');d.append(content);
  ...
  if(page==='settings'){
   ...
   const choice=(label,values,current)=>{const l=document.createElement('label'),select=document.createElement('select');l.textContent=label;select.setAttribute('aria-label',label);select.disabled=!canEdit;for(const [ko,en,ja] of values)select.add(new Option(tr(ko,en,ja),ko,false,ko===current));l.append(select);content.append(l);return select};
   ...
   if(c.wallet?.poolId){wealth.disabled=true;const note=document.createElement('p');note.textContent=tr('재산 유형은 공유를 해제한 뒤 변경할 수 있어요.','End sharing before changing wealth type.','資産の種類は共有解除後に変更できます。');content.append(note);}
   ...
   action(tr('저장','Save','保存'),()=>{const changed=wealth.value!==c.wealth;if(changed&&!window.confirm(tr('재산 유형을 바꾸면 현재 잔액을 선택한 유형의 금액으로 바꿉니다. ','Changing wealth type replaces the current balance with ','資産の種類を変更すると現在の残高を変更します：')+displayMoney(INITIAL_MONEY[wealth.value],c,world.uiLanguage)+tr('으로 변경할까요?',' . Continue?','。変更しますか？')))return;void mutate('settings',{patch:{datePayment:payment.value,datePayFrequency:Number(frequency.value),wealth:wealth.value,income:spending.value,confirmWealthReset:changed}})});
  renderWalletSharing({container:content,world,c,snapshot,uid,canEdit,save,valid,onUpdate:draw});
  }else{
   const homes=Object.values(world.homes).filter(h=>c.homeId===h.id||c.residences?.some(r=>r.homeId===h.id));
   for(const home of homes.filter(h=>h.commonWallet?.members?.includes(c.id)&&h.commonWallet.balance>0&&(!homeId||homeId===h.id))){
