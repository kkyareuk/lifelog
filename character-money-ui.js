...
export function openCharacterMoney(pane='wallet',characterId=null,homeId=null){
 ...
 function draw(){
  ...
  if(page==='settings'){
   const s=moneySettings(c),unit=field(tr('재화 단위 (직접 입력)','Currency name','通貨名（自由入力）'),'text',s.unit);unit.maxLength=20;
   const meal=field(tr('평범한 밥 한 끼 가격','Price of an ordinary meal','普通の食事1回の価格'),'number',s.mealPrice);meal.min='0.0001';meal.step='any';
   const help=document.createElement('p');help.textContent=tr('단위와 한 끼 가격을 바꿔도 실제 재산 가치는 유지돼요.','Changing the unit or meal price preserves the value of your money.','単位や食事の価格を変えても資産の価値は変わりません。');content.append(help);
   ...
   action(tr('저장','Save','保存'),()=>mutate('settings',{patch:{unit:unit.value,mealPrice:Number(meal.value),datePayment:payment.value,datePayFrequency:Number(frequency.value),wealth:wealth.value,income:spending.value}}));
  }else if(page==='work'){
   const text=document.createElement('p');text.textContent=(c.jobTitle||c.job||tr('무직','Unemployed','無職'))+' · '+(world.world.places.find(p=>p.id===c.workplaceId)?.name||tr('직장 미지정','No workplace','職場未設定'));content.append(text);
   const wage=field(tr('근무 1회 완료 시 급여','Pay per completed shift','勤務1回あたりの給与'),'number',moneySettings(c).wage/10000*moneySettings(c).mealPrice);wage.min='0';wage.step='any';
   const hint=document.createElement('p');hint.textContent=tr('퇴근할 때 입금돼요. 무직·학생에게는 자동 급여가 없어요.','Paid when work ends. Unemployed characters and students receive no automatic salary.','退勤時に入金します。無職・学生には自動給与がありません。');content.append(hint);action(tr('급여 저장','Save pay','給与を保存'),()=>{try{void mutate('settings',{patch:{wage:moneyFromDisplay(wage.value,c)}})}catch(e){d.querySelector('[role=status]').textContent=errorText(e,world.uiLanguage)}});
  }else{
   ...
   const history=document.createElement('ol');for(const entry of c.wallet?.entries||[]){const li=document.createElement('li');const label=entry.kind==='wage'?tr('급여','Wages','給与'):entry.kind==='expense'?tr('이용료','Expense','利用料'):tr('공동지갑 이체','Shared-wallet transfer','共同財布への振替');li.textContent=label+' · '+displayMoney(entry.amount,c,world.uiLanguage);history.append(li)}content.append(history);
  }
