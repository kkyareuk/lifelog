...
(()=>{
 ...
 function installHomeEditor(){
  ...
  panel.querySelector('#houseRoomCount').onchange=()=>{const c=normalize(active()),count=Number(panel.querySelector('#houseRoomCount').value);c.roomCount=count;c.rooms=roomDefaults(count).map((r,i)=>({...r,...((c.rooms||[])[i]||{})}));renderRooms(c)};
  panel.querySelector('#saveHouseEdit').onclick=()=>{const c=normalize(active());c.homeLayout=panel.querySelector('#houseHomeType').value;c.homeType=c.homeLayout;c.roomCount=Number(panel.querySelector('#houseRoomCount').value);c.sleepArrangement=panel.querySelector('#houseSleepArrangement').value;c.bedroomCount=Math.max(1,c.rooms.filter(room=>room.type==='bedroom').length);c.bedCount=c.bedroomCount;syncFrom(c);save();renderRooms(c);window.ParallelCityHome?.render();toast('집 설정을 저장했습니다.')};
 }
 ...
 function fillHouseEditor(){
  ...
  panel.querySelector('#houseHomeType').value=c.homeLayout||c.homeType||'studio';panel.querySelector('#houseRoomCount').value=String(c.roomCount);panel.querySelector('#houseSleepArrangement').value=c.sleepArrangement||'oneBed';renderRooms(c);
 }
 ...
 function collect(){
  const c=normalize(active());c.bedtime=document.querySelector('#charBedtime')?.value||'00:30';c.wakeTime=document.querySelector('#charWakeTime')?.value||'07:30';c.hometown=document.querySelector('#charHometown')?.value.trim()||'';c.petName=document.querySelector('#charPetName')?.value.trim()||'';syncFrom(c);
 }
