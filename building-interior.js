...
export function openBuildingInterior(placeId,{snapshot=null,bindRoomGeometry,editBuilding=()=>{},toast=()=>{}}={}){
 ...
 const home=buildingInterior(place,town.id,world.uiLanguage),selection={homeEditMode:false,floors:{},uiHidden:false};world.homes={...world.homes,[home.id]:home};world.activeHomeId=home.id;
 ...
 const valid=()=>personal===state&&(window.ParallelCityAuth?.getInfo?.()?.user?.uid||'')===account&&(groupId?window.DrawerVillageGroups?.getSnapshot?.()?.activeGroupId===groupId:state.activeTownId===town.id);
 ...
 function render(){
  ...
  const root=document.createElement('section');root.className='home-page home-native-page building-interior-page'+(selection.uiHidden?' home-ui-hidden':'');
  ...
  button(tr('건물 편집','Building','建物編集'),async()=>{if(!canEdit)return;await queue.done;if(saveError)return;d.close();editBuilding(placeId,()=>{if(valid())openBuildingInterior(placeId,{snapshot:groupId?window.DrawerVillageGroups.getSnapshot():null,bindRoomGeometry,editBuilding,toast})})}).disabled=!canEdit;
  ...
  button(tr('직원','Staff','従業員'),()=>showBuildingStaff(panel,{world,home,place,groupId,account,valid,refresh:refreshWorld,toast,staffCopy})).dataset.openHomeFeature='members';
  ...
  const hide=document.createElement('button');hide.className='home-native-pill home-native-ui-toggle';hide.innerHTML='<span></span>';hide.querySelector('span').textContent=selection.uiHidden?tr('UI 보기','Show UI','UI表示'):tr('UI 숨김','Hide UI','UI非表示');hide.setAttribute('aria-pressed',String(selection.uiHidden));hide.onclick=()=>{const hidden=root.classList.toggle('home-ui-hidden');selection.uiHidden=hidden;hide.setAttribute('aria-pressed',String(hidden));hide.querySelector('span').textContent=hidden?tr('UI 보기','Show UI','UI表示'):tr('UI 숨김','Hide UI','UI非表示')};root.append(hide);
