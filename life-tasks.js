...
export function lifeCopy(task){
 ...
 return {ko:[title,details[task.id]||(task.hobby?`${task.labels[0]} 취미에 몰두하며 자기 시간을 보내고 있어요.`:`${task.labels[0]}에 집중하며 시간을 보내고 있어요.`)],en:[task.labels[1],`Taking time to ${task.labels[1].toLowerCase()}.`],ja:[task.labels[2],`${task.labels[2]}時間を過ごしています。`]};
}
