...
(()=>{
 ...
 function pet(c){
  if(!c.pet||c.pet==='none')return'';const code=c.pet==='cat'?c.catBreed:c.dogBreed;
  const breed=({korean:'코리안 쇼트헤어',persian:'페르시안',ragdoll:'랙돌',siamese:'샴',british:'브리티시 쇼트헤어',mainecoon:'메인쿤',scottish:'스코티시 폴드',mixed:'믹스견',maltese:'말티즈',poodle:'푸들',retriever:'리트리버',shiba:'시바견',corgi:'웰시코기',jindo:'진돗개'})[code]||(c.pet==='cat'?'고양이':'강아지');
 ...
 function render(){
  ...
  root.innerHTML=groups().map(people=>{
   ...
   const layout=owner.homeLayout||owner.homeType||'apartment',layoutName={studio:'원룸',apartment:'아파트',apartmentA:'아파트',apartmentB:'아파트',house:'주택',mansion:'대저택'}[layout]||'우리 집';
   return`<article class="home-card"><header><div><h3>🏠 ${esc(owner.homeName||`${owner.name}의 집`)}</h3><p>${esc(layoutName)} · ${people.map(p=>esc(p.name)).join(' · ')} 함께 거주 중</p></div><b>${inside.length}명 귀가</b></header><div class="photo-room-grid layout-${esc(layout)}">${cards}</div></article>`;
