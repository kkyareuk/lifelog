...
export function discoveryChoices(c,q,random=Math.random){
 const available=q.choices.map((choice,index)=>({choice,index})).filter(({choice})=>!choice.tattoo||!discoveryLocked(c,'bodyProfile.tattoos'));
...
export function discoveryCandidates(c,scene){
 ...
 return DISCOVERY_EVENTS.filter(q=>discoveryEligible(c,q)&&(q.field||q.choices.some(o=>(o.tattoo&&!discoveryLocked(c,'bodyProfile.tattoos'))||Object.keys(o.effects).some(f=>!discoveryLocked(c,f)))));
}
