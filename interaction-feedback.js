...
// Click feedback needs only these small settings, never a backup of the world.
const settings=()=>window.ParallelCity?.getInteractionSettings?.()||{};
const volume=state=>{const s=audioSettings(state);return s.soundMuted?0:Math.max(0,Math.min(1,(Number(s.soundEffectsVolume)||0)/100))};
export function playInteractionSound(kind='ui-select',state=settings()){
 const v=volume(state),now=performance.now();if(!v||document.visibilityState==='hidden'||last.kind===kind&&now-last.at<60)return;
 last={kind,at:now};for(const a of voices.values()){a.pause();a.currentTime=0}
 let a=voices.get(kind);if(!a){a=new Audio('./assets/audio/'+kind+'.mp3');a.preload='auto';a.playsInline=true;voices.set(kind,a)}a.volume=v;a.play().catch(()=>{});
}
...
document.addEventListener('click',event=>{
 ...
 const state=settings();playInteractionSound(interactionSound(control,state),state);
