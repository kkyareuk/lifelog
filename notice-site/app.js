...
function controls(){el('new-draft').disabled=!identity||busy;const enabled=!!identity&&!busy&&!current?.publishedAt;for(const id of ['subject','body','save','review'])el(id).disabled=!enabled;el('send').disabled=!enabled;}
...
el('new-draft').onclick=()=>{if(!identity||busy)return;const draft={id:'notice-'+crypto.randomUUID(),subject:'',body:'',publishedAt:0};drafts.unshift(draft);choose(draft)};
