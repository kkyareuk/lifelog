import {bindSharedHome} from './shared-home-editor.js?v=20260908dev278';
import {residentText} from './shared-residents.js?v=20260908dev278';
import {sharedSelection,withSharedWorld,decodeShared,buildSharedWorld} from './shared-world.js?v=20260908dev278';
import {state} from './state.js?v=20260908dev278';
import {renderGroupRelations} from './groups.js?v=20260908dev278';
...
export function bindSharedUi({bindRoomGeometry,render,toast:notify,setMode,setPanel,setPlacement,openMap,openShape,openRelation,openGroup,openRoutine,openMonthly,newRoutine,newMonthly}){
 ...
 root.addEventListener('click',e=>{
  ...
  if(el.matches('[data-add-routine],[data-add-routine-day],[data-add-monthly-routine],[data-edit-routine],[data-edit-monthly-routine]')){stop(e);withSharedWorld(snapshot(),()=>{const c=state.characters[state.activeId];if(!owned(c.id)){toast(residentText('내 캐릭터를 선택해서 제안해 주세요.','Select your own character to propose a schedule.','自分のキャラクターを選んで提案してください。'));return}const monthly=el.matches('[data-add-monthly-routine],[data-edit-monthly-routine]'),id=el.dataset.editRoutine||el.dataset.editMonthlyRoutine||'',draft=id?null:(monthly?newMonthly():newRoutine());if(draft&&el.dataset.addRoutineDay!==undefined)draft.day=Number(el.dataset.addRoutineDay);(monthly?openMonthly:openRoutine)(id,draft,(patch,targetId,close)=>enqueue(async()=>{const result=await api().propose({kind:'schedule',patch,targetId});close();toast(result.status==='accepted'?residentText('일정을 설정했어요.','Schedule saved.','予定を設定しました。'):residentText('일정 제안을 보냈어요. 우편함에서 답변을 확인해 주세요.','Schedule proposed. Check your mailbox for responses.','予定を提案しました。返答は郵便箱で確認できます。'));render()},toast));const dialog=document.querySelector('.routine-sheet-backdrop');const label=()=>{const others=[...dialog.querySelectorAll('[name=withId]:checked')].some(x=>!owned(x.value));dialog.querySelector('[data-routine-save]').textContent=others?residentText(id?'수정 제안하기':'일정 제안하기',id?'Propose changes':'Propose schedule',id?'変更を提案':'予定を提案'):residentText('설정하기','Save schedule','予定を設定')};dialog.addEventListener('change',label);label()});return}
  ...
  if(el.matches('[data-resident-apply]')){stop(e);select.residentForm='admission';select.admissionCandidates=Object.values(state.characters).filter(c=>!s.residents.some(r=>r.ownerUid===uid()&&r.sourceCharacterId===c.id)).map(c=>({id:c.id,name:c.name}));render();return}
