...
export function bindSceneZoom(root=document){
 for(const viewport of root.querySelectorAll('.home-canvas-viewport,.town-map-scroll')){
  ...
  viewport.dataset.zoomBound='1';viewport.style.setProperty('touch-action','none','important');viewport.style.setProperty('overflow','hidden','important');viewport.style.overflowAnchor='none';
  ...
  // Restore the saved transform before geometry/labels are measured.
  scene.style.transformOrigin='0 0';scene.style.transform=`translate(${pose.x}px,${pose.y}px) scale(${pose.scale})`;
  const eventRoot=viewport.closest('.home-page')||viewport;
  const points=new Map();let gesture=null,pending=null,consumed=false,frame=0,rect=null;
  const draw=()=>{frame=0;if(!viewport.isConnected)return;const w=viewport.clientWidth,h=viewport.clientHeight;const sceneWidth=scene.offsetWidth,sceneHeight=scene.offsetHeight;pose.x=Math.max(Math.min(0,w-sceneWidth*pose.scale),Math.min(0,pose.x));pose.y=Math.max(Math.min(0,h-sceneHeight*pose.scale),Math.min(0,pose.y));viewport.scrollLeft=0;viewport.scrollTop=0;scene.style.transformOrigin='0 0';scene.style.transform=`translate(${pose.x}px,${pose.y}px) scale(${pose.scale})`;};
  ...
  viewport.addEventListener('click',e=>{if(consumed){e.preventDefault();e.stopImmediatePropagation();consumed=false}},true);frame=requestAnimationFrame(draw);
 }
}
