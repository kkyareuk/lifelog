...
import {trackTitleImages} from './title-loading.js';
export function mountTitleScreen({getState,onEnter}){
 ...
 let entered=false,last='',working=false,loaded=0,total=1;
 ...
 const update=()=>{
  ...
  const failed=auth?.startupError||window.DrawerVillageAuthStartupError,accountReady=!configured||auth?.ready&&!auth.startupSyncing,ready=accountReady&&loaded===total;
  const returning=!!auth?.user||!!state.order?.length,key=[ready,returning,failed,working,state.uiLanguage,loaded,total,accountReady].join(':');
  if(key===last)return;last=key;actions.replaceChildren();progress.max=total+1;progress.value=loaded+(accountReady?1:0);progress.hidden=ready;status.textContent=ready?'':`${copy[4]} ${progress.value}/${progress.max}`;
 ...
 const timer=setInterval(update,500);window.addEventListener('drawer-village-auth-busy',update);trackTitleImages(root,(done,count)=>{loaded=done;total=count;update()});update();
 return root;
}
