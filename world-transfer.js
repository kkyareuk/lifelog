...
import {state,personalState,cloneState,replaceState,save} from './state.js?v=20260909dev291';
...
export async function worldTransferDialog({homeId='',kind='town',render,toast,limits}){
 ...
 node('h2',text('공유 코드 · 마을 이전','Sharing codes · Town transfer','共有コード・村の移転'));const close=node('button',text('닫기','Close','閉じる'));close.onclick=()=>d.close();
 const mode=node('select');for(const [id,label] of [['home',text('집 · 방 · 인테리어','Home, rooms and interior','家・部屋・インテリア')],['relationships',text('관계 전체','All relationships','関係全体')],['town',text('마을 전체','Entire town','村全体')]]){const option=node('option',label,mode);option.value=id}mode.value=homeId?'home':kind;
 const select=node('select'),fill=()=>{select.replaceChildren();for(const item of mode.value==='home'?Object.values(source.homes):source.towns){const option=node('option',item.name,select);option.value=item.id}select.hidden=mode.value==='relationships'};fill();if(homeId)select.value=homeId;mode.onchange=fill;
 const info=node('p',text('코드 가져오기는 복사입니다. 관계만 가져올 때는 연결할 캐릭터를 직접 고릅니다. 마을 전체는 캐릭터와 관계·집·배치를 함께 복사합니다.','Code imports create copies. Match characters when importing relationships. An entire town includes characters, relationships, homes and layout.','コードからの読み込みはコピーです。関係のみの場合は人物を指定します。村全体には人物・関係・家・配置が含まれます。'));
 ...
 read.onclick=()=>task(read,async()=>{const result=await api.readWorldCode(code.value);same();const pack=result.package;panel.replaceChildren();node('h3',pack.name,panel);node('p',counts(pack),panel);const mapping={};
...
function errorText(message){const errors={'destination-town-not-empty':['대상 멀티 마을이 비어 있어야 해요.','The destination town must be empty.','移転先は空の村を選んでください。'],'choose-distinct-characters':['서로 다른 캐릭터를 모두 연결해 주세요.','Match every person to a different character.','全員を異なる人物に接続してください。'],'relationship-already-exists':['이미 관계가 있는 인물 조합이에요. 기존 관계를 확인해 주세요.','A relationship already exists for this set of characters.','この人物の組み合わせには既に関係があります。'],'character-slot-required':['캐릭터 슬롯이 부족해요.','Not enough character slots.','キャラクター枠が足りません。'],'town-slot-required':['마을 슬롯이 부족해요.','Not enough town slots.','村の枠が足りません。'],'catalog-limit':['사전은 전체 80개까지예요.','The dictionary is limited to 80 items total.','辞典は合計80件までです。'],'catalog-id-conflict':['같은 ID의 사전 물품 내용이 달라요. 먼저 사전을 확인해 주세요.','A dictionary ID conflicts with an existing item.','同じIDの辞典項目の内容が異なります。'],'personal-world-required':['내 마을에서 불러와 주세요.','Import from your personal world.','自分の村で読み込んでください。']};return errors[message]?text(...errors[message]):text('처리하지 못했어요. 다시 확인해 주세요.','Could not complete the operation. Please check and retry.','処理できませんでした。確認して再試行してください。')+' ('+message+')'}
