...
export function importWorldPackage(pack,{mapping={},characterLimit=5,townLimit=2}={}){
 ...
 if(pack.kind==='town'){
  ...
  for(const [kind,rows] of Object.entries(copy.catalog||{})){next.catalog[kind]||=[];for(const item of rows){const old=next.catalog[kind].find(x=>x.id===item.id);if(old&&JSON.stringify(old)!==JSON.stringify(item))throw Error('catalog-id-conflict');if(!old){next.catalog[kind].push(item);added++}}}if(count+added>100)throw Error('catalog-limit');
 }
...
function errorText(message){const errors={'wrong-sharing-kind':['이 화면에서 다루는 종류의 공유 코드를 입력해 주세요.','Enter a sharing code matching this screen.','この画面の種類に合う共有コードを入力してください。'],'destination-town-not-empty':['대상 멀티 마을이 비어 있어야 해요.','The destination town must be empty.','移転先は空の村を選んでください。'],'choose-distinct-characters':['서로 다른 캐릭터를 모두 연결해 주세요.','Match every person to a different character.','全員を異なる人物に接続してください。'],'relationship-already-exists':['이미 관계가 있는 인물 조합이에요. 기존 관계를 확인해 주세요.','A relationship already exists for this set of characters.','この人物の組み合わせには既に関係があります。'],'character-slot-required':['캐릭터 슬롯이 부족해요.','Not enough character slots.','キャラクター枠が足りません。'],'town-slot-required':['마을 슬롯이 부족해요.','Not enough town slots.','村の枠が足りません。'],'catalog-limit':['사전은 전체 100개까지예요.','The dictionary is limited to 100 items total.','辞典は合計100件までです。'],'catalog-id-conflict':['같은 ID의 사전 물품 내용이 달라요. 먼저 사전을 확인해 주세요.','A dictionary ID conflicts with an existing item.','同じIDの辞典項目の内容が異なります。'],'personal-world-required':['내 마을에서 불러와 주세요.','Import from your personal world.','自分の村で読み込んでください。']};return errors[message]?text(...errors[message]):text('처리하지 못했어요. 다시 확인해 주세요.','Could not complete the operation. Please check and retry.','処理できませんでした。確認して再試行してください。')+' ('+message+')'}
