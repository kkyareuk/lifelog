...
export function storyQuestions(c,characters={},now=Date.now()){
 ...
 const recentTopics=new Set((c.storyResponses||[]).filter(r=>now-r.at<7*86400000).map(r=>r.id.startsWith('story:home:')?'home':r.id.startsWith('story:cooking:')?'cooking':r.id));
 return questions.filter(q=>!answered.has(q.id)&&!recentTopics.has(q.id.startsWith('story:home:')?'home':q.id.startsWith('story:cooking:')?'cooking':q.id));
}
