...
export function bindSharedHome(root,s,render,toast,bindRoomGeometry,space=null,prepareImage){
 ...
 root.addEventListener('click',e=>{
  ...
  if(b.matches('[data-home-edit]')){stop(e);if(!canEdit)return;if(selection.homeEditMode){const button=b;button.disabled=true;commit().then(()=>{selection.homeEditMode=false;delete selection.homeDrafts?.[home.id];render()}).catch(error=>{button.disabled=false;toast(error.message)})}else{selection.homeEditMode=true;render()}return}
 ...
 if(space?.choosePhoto){const photo=document.createElement('button');photo.type='button';photo.textContent=mt('방 사진 고르기','Choose room photo','部屋の写真を選ぶ');photo.onclick=async()=>{photo.disabled=true;try{const image=await space.choosePhoto();if(image){change(()=>updateRoom(home.id,roomKey,{image,floorImage:image,usePhoto:true,floorMaterial:'custom'}));d.close()}}catch(e){toast(e.message)}finally{photo.disabled=false}};d.append(photo);const clear=document.createElement('button');clear.type='button';clear.textContent=mt('방 사진 지우기','Remove room photo','部屋の写真を削除');clear.onclick=()=>{change(()=>updateRoom(home.id,roomKey,{image:'',floorImage:'',usePhoto:false,floorMaterial:'natural'}));d.close()};d.append(clear)}
 if(!space&&prepareImage)addSharedRoomPhotos(d,{room,prepareImage,language:state.uiLanguage,toast,apply:async patch=>{Object.assign(room,patch);await commit()}});
 ...
 const appearance=document.createElement('section');appearance.className='room-appearance-options';for(const [field,label] of [['usePhoto',mt('벽지·바닥 대신 사진 사용','Use photo instead of wallpaper and floor','壁紙・床の代わりに写真を使う')],['hideFurniture',mt('가구 숨기기','Hide furniture','家具を隠す')]]){if(field==='usePhoto'&&space?.allowRoomPhotos===false)continue;const l=document.createElement('label'),check=document.createElement('input');l.className='check';check.type='checkbox';check.name=field;check.checked=Boolean(room[field]);check.onchange=()=>change(()=>updateRoom(home.id,roomKey,{[field]:check.checked}));l.append(check,document.createTextNode(label));appearance.append(l)}const appearanceHelp=document.createElement('small');appearanceHelp.textContent=mt('가구 그림만 숨겨요. 사용과 배치는 유지되며 편집할 때 다시 보여요.','Only furniture artwork is hidden. Furniture stays usable and reappears while editing.','家具の絵だけを隠します。配置と使用は維持され、編集時には再び表示されます。');appearance.append(appearanceHelp);d.append(appearance);
