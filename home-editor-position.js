...
export function bindEditorPosition(root,homeId,language='ko'){
 ...
 const clamp=()=>{const p=positions.get(homeId),r=dock.getBoundingClientRect();dock.style.left=Math.max(4,Math.min(p?.x??8,innerWidth-r.width-4))+'px';dock.style.top=Math.max(4,Math.min(p?.y??innerHeight-r.height-12,innerHeight-r.height-4))+'px'};
