...
export function showDiscoveryAdChoice(language,onReady){
 ...
 watch.onclick=async()=>{
  ...
  try{const result=await watchDiscoveryAd();if(!valid())return;
   ...
   status.textContent=result==='test'?t('테스트 광고 시청 완료. 테스트 보상은 승인된 테스트 계정에서만 받을 수 있어요.','Test ad completed. Test rewards require an authorized test account.','テスト広告の視聴完了。テスト報酬は承認済みのテストアカウント限定です。'):result==='cancelled'?t('시청을 취소했어요. 무료 대기시간은 그대로예요.','Ad cancelled. Your free countdown is unchanged.','視聴を中止しました。無料の待ち時間は変わりません。'):t('보상을 확인하고 있어요. 확인되면 질문을 열어 드릴게요.','Verifying your reward. Your question will open when confirmed.','報酬を確認しています。確認でき次第、質問を開きます。');
