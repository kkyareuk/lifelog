import {state,beginCharacterEditor,endCharacterEditor,characterEditorActive,emptyWorld,runIsolatedWorld,createCharacter} from './state.js?v=20260909dev304';
import {buildSharedWorld} from './shared-world.js?v=20260909dev304';
...
export function bindSharedCharacters(render){
 ...
 document.querySelectorAll('[data-new]').forEach(button=>{if(!snapshot?.activeGroupId)return;const create=button.onclick;button.onclick=async()=>{leaveSharedCharacterEditor();await window.DrawerVillageGroups.select('');state.activeTab='character';state.characterSettingsView='hub';create?.();render()}});
