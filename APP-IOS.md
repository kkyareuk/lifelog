# 2026-09-16 내부 1.0.368 (420)

419 침대·집 정보·재질·놀이 모집 오류 수정에 이어 집 외형의 흰 네모와 이름표 제거. Actions35087097095 서명·검증·업로드 성공, TestFlight420 테스트 준비 완료·기존 내부 초대1명 확인. 공개 심사와 운영main 유지. docs/release419.md 및 docs/release420.md 참고.

# 2026-09-16 내부 1.0.365 (417)

문맥 메뉴 전체 행동·1인 침대·욕구 회복·참여형 마피아 회의. Actions35069105388 Mac 서명·검증·업로드 성공. TestFlight417 테스트 준비 완료 및 기존 나만 테스트 그룹 417 설치됨 표시 확인. 공개411 회신과는 별도이며 4.8 로그인 대응은 미완료. docs/release417.md 참고.

# 2026-09-16 내부 1.0.364 (416)

전체 할 일 복구·새 가구·욕구 행동·게임플레이 관계 설정·추리 화면. Actions35058302469 서명·Apple 검증·업로드 성공. TestFlight 416 테스트 준비 완료와 기존 내부 그룹 나만 테스트의 416 설치됨 표시 확인. 실기기 조작 QA는 별도. 공개 심사 변경 없음. docs/release416.md 참고.

# 2026-09-15 내부 1.0.358 (410)

내 마을 마피아, 일과 단축, 의문 추천·지적 집중 장면, 기록 보드. Actions34973923354 Mac 서명·업로드 성공. TestFlight 준비 완료와 기존 내부 그룹410 제공 확인. docs/release410.md 참고.

# 2026-09-15 내부 1.0.357 (409)

장소별 멀티 로그 저장 검증, 마피아 연결 회의·모집·실내·선택 피드백. Actions34968177686 Mac 서명·검증·업로드 성공. TestFlight 처리 완료, 기존 내부 그룹에409 제공 확인. 자세한 기록 docs/release409.md.

# 2026-09-15 내부 1.0.356 (408)

마피아45초 선택·연출 분리, SVG 추리 화면, 자동 알리바이·지적 예약, 팝업 화면 이탈 수정. Actions34963559344 Mac 서명·검증·업로드 성공. TestFlight408 테스트 준비 완료 및 기존 내부 그룹 제공 확인. 공개405 심사는 변경하지 않음. docs/release408.md 참고.

# 2026-09-15 내부 1.0.355 (407)

장소별 생활로그·이동/대기 표시와 마피아 이동60초/실내행동15초 수정. Actions34952166788에서 Mac 서명·IPA 검증·업로드 성공. App Store Connect 처리 완료, TestFlight 기존 내부 그룹에 407 제공 및 설치됨 표시 확인. 공개405 심사는 유지. 최종 상태 docs/release407.md.

# 서랍마을 iOS — 공개 1.0.353(405) / 내부 1.0.354(406)

## 2026-09-15 최종 배포

- 공개405는 광장 준비 중, 내부406은 활동 활성화. 소스는 각각 7e85e81 / e017e4b.
- Mac Actions34940842211 / 34940995345 서명·IPA 검증·업로드 성공. App Store Connect 처리 완료.
- TestFlight406은 기존 내부 그룹에서 제공·설치됨 확인. 실기기 조작 QA를 수행한 것은 아님.
- 공개405는 기존391 제출을 교체하여 2026-09-15 16:38 KST 심사 제출 완료. 승인 후 자동으로 전체 공개하도록 설정.
- 프로필/대명사·홈 선택 버튼·우편 디자인·말투·멀티 생성 개선. 신규 문구 EN/JA100%, 전체 앱 번역률 미측정.
- 자세한 검증과 최종 심사 상태: docs/release405-406.md. 아래는 과거 기록.

# 서랍마을 iOS — v1.0.204 / build 10 준비 · 최근 업로드 build 5

## 2026-09-05 build 10 준비 (아직 업로드하지 않음)

- 마을 전체 산책 캐릭터의 발소리가 들리며, 걸음걸이 설정에 맞춰 이동 속도·보폭 애니메이션·발소리 간격이 함께 달라진다.
- 신규 걸음걸이 `그림자처럼 매우 민첩하게`를 추가했다.
- 법적 등록 여부를 부부, 부모·자녀, 형제·자매 관계에서 각각 설정할 수 있다.
- 태블릿 가로 화면은 기존 캐릭터 목록과 2열 디자인을 유지하면서, 오른쪽에서 스마트폰과 같은 7개 캐릭터 설정 탭과 저장 상태를 공유한다.
- 새 사용자 문구의 영어·일본어 번역을 함께 포함한다. 전체 UI 정적 번역률은 영어 2082/2770(75.2%), 일본어 2081/2770(75.1%)다.
- Android 1.0.204 / code 219와 공통 소스를 맞춘 로컬 준비판이다. Mac 서명·IPA·TestFlight 업로드와 App Store 심사는 요청하거나 실행하지 않았다.

## 2026-09-05 build 9 준비 (아직 업로드하지 않음)

- 분수처럼 한 장식에 방문자가 모일 때 캐릭터를 장식 둘레의 서로 다른 자리로 분산해 겹침을 줄였다.
- 장식 방문 빈도를 조정하고, 마을 특성 장면의 캐릭터가 집이나 한 지점 주변만 돌지 않고 지도 곳곳을 잇는 여러 산책 동선을 따라 이동하게 했다.
- 캐릭터마다 동선 시작 지점·속도·출발 시간을 다르게 해 같은 경로에서도 한 덩어리로 겹쳐 걷는 현상을 줄였다.
- 새 사용자 문구는 없으며 기존 “마을 산책 중”의 영어·일본어 번역을 그대로 사용한다. 관계 편집 build 8의 신규 문구는 영어·일본어 모두 포함되어 있다.
- Android 1.0.204 / code 219와 공통 소스를 맞춘 로컬 준비판이다. Mac 서명·IPA·TestFlight 업로드와 App Store 심사는 요청하거나 실행하지 않았다.

## 2026-09-05 build 8 준비 (아직 업로드하지 않음)

- 관계 편집 화면을 고정 좌표 배치에서 세로 스크롤 카드형 배치로 바꿔, 캐릭터가 많아도 선택 목록과 다른 설정이 겹치지 않게 했다.
- 관계에 포함한 모든 캐릭터를 목록으로 보여 주고 각 행의 위·아래 버튼으로 전체 표시 순서를 바꿀 수 있다.
- 부부의 법적 등록 여부는 “관계가 밖에서 다뤄지는 방식” 선택지와 분리된 별도 체크 항목으로 저장한다. 기존 부부 데이터도 새 구조로 옮긴다.
- 이번 변경의 새 문구는 한국어·영어·일본어로 함께 제공한다. 전체 UI 정적 번역률은 영어 2086/2774(75.2%), 일본어 2085/2774(75.2%)다.
- Android 1.0.202 / code 217과 공통 소스를 맞춘 로컬 준비판이다. Mac 서명·IPA·TestFlight 업로드와 App Store 심사는 요청하거나 실행하지 않았다.
