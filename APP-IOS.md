# 서랍마을 iOS 준비 — v1.0.198 / iOS build 5 준비

현재 상태: **build 4는 사용자 iPad 설치·실행 및 사진 표시 확인. 제공된 화면에서 태블릿 배치 문제 발견. build 5는 해당 UI 수정과 로컬 자산 준비 단계이며 Mac 빌드·TestFlight 업로드·실기기 재검증 전이다. App Store 제출 가능 상태 아님.**

## build 5 태블릿 배치 수정 (미배포)
...
- 이전 빌드 4에 수정이 자동 적용되지 않는다. 빌드 5 업로드 요청 파일은 별도 사용자 승인 전까지 변경하지 않는다.
...
- `.github/ios-testflight-request.json`은 기존 build 4 업로드 기록 그대로 유지한다. 준비판 검사만 `node scripts/check-ios-testflight.mjs --prepared` 사용. 실제 업로드 검사는 일치 조건을 유지한다.
