...
(()=>{
 ...
 function cleanliness(owner,people){
  ...
  owner.homeCleanliness=Number(owner.homeCleanliness)||82;return owner.homeCleanliness;
 }
 ...
 function render(){
  ...
  root.innerHTML=groups().map(people=>{
   ...
   const cards=list.map((r,index)=>{const fallback=['living','kitchen','bath','bedroom','study'].includes(r.type)?r.type:'study',background=r.photo?`linear-gradient(#fff3,#fff3),url('${r.photo}')`:`linear-gradient(#fff1,#fff1),url('./icons/room-${fallback}.svg')`;return`<section class="photo-room type-${esc(r.type)} room-${index+1} ${r.photo?'has-photo':''}" style="background-image:${background}"><div class="photo-room-title">${esc(r.name)}</div><div class="photo-room-people">${(placed.get(r.id)||[]).map(c=>person(c,action(c,r,people,clean))).join('')}</div>${r.type==='living'?pet(owner):''}</section>`}).join('');
