...
export function homeMemberMenu(home,characters,locale){
  ...
  return `<section class="home-feature-panel home-design-page home-members" data-home-feature="members"><header class="home-design-head"><button type="button" class="home-design-back" data-close-home-feature aria-label="${c.back}"></button><h2>${c.members}</h2></header>${[
    ["resident",c.members,characters.map(p=>`<article class="home-resident-entry">${card("resident",p,p.photo||p.icon)}<button type="button" class="home-resident-remove" data-home-remove-resident="${escape(p.id)}" data-home-id="${escape(home.id)}" aria-label="${escape(p.name)} · ${c.removeResident}">${c.removeResident}</button></article>`).join("")],
...
export function fitCoupleBedOccupants(root){
  ...
  const layout=()=>people.forEach(person=>{
    ...
    const side=bed.dataset.bedSide==='true',faceSize=side?Math.max(24,Math.min(56,paintedHeight*.36*(Number(style.getPropertyValue('--furniture-scale'))||1))):Math.max(underCover?46:36,Math.min(underCover?76:56,paintedWidth*(underCover?.38:.28)*(Number(style.getPropertyValue('--furniture-scale'))||1)));
