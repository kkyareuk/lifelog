...
export function readSettingsFile(text){
  ...
  if(file.format==='drawer-village-catalog'){
    ...
    for(const [kind,items] of Object.entries(file.catalog))if(!kinds.includes(kind)||!Array.isArray(items)||items.length>100||items.some(x=>!x||typeof x.name!=='string'))throw Error('사전은 전체 100개까지 불러올 수 있어요.');
  }
  return file;
}
...
export function mergeCatalogFile(file){
  ...
  for(const [kind,items] of Object.entries(file.catalog)){
    ...
    for(const source of items){
      ...
      if(existing)Object.assign(existing,item);else{if(Object.entries(next).reduce((n,[k,items])=>n+items.filter(i=>k!=='fashion'||!i.ownerId).length,0)>=100)throw Error('사전은 전체 100개까지 불러올 수 있어요.');next[kind].push(item)}
    }
  }
