...
export function mountRelationshipDetails(form,old,language='ko'){
 ...
 const copy=({ko:['관계의 배경과 일상','정하지 않음','기억에 남은 첫 만남','직접 선택한 사건만 회상에 사용해요. 현재의 감정은 각자의 시선과 관계 단계에 따라 달라져요.'],en:['Relationship history and daily life','Unspecified','A remembered first meeting','Only selected events enter memories. Present feelings depend on each perspective and the relationship stage.'],ja:['関係の背景と日常','未設定','記憶に残る初対面','選んだ出来事だけを回想に使います。今の感情は各自の視線と関係段階で変わります。']}[language]||[]);
 ...
 const render=()=>{if(slot.children.length)capture();type=form.type.value;const spec=OFFICIAL_RELATIONSHIP_DETAILS[type],value=drafts.get(type)||{};
  ...
  const fields=[...spec.fields,{id:'firstMeeting',label:{ko:copy[2],en:copy[2],ja:copy[2]},options:FIRST_MEETINGS}];
  slot.innerHTML=`<legend>${escape(copy[0])}</legend>${fields.map(f=>`<div class="relationship-detail-row"><div class="relationship-field-heading"><label for="relationship-detail-${f.id}">${escape(detailText(f.label,language))}</label>${relationshipInfo(f.id,language)}</div><select id="relationship-detail-${f.id}" data-detail="${f.id}"><option value="">${escape(copy[1])}</option>${f.options.map(o=>`<option value="${o.id}" ${value[f.id]===o.id?'selected':''}>${escape(detailText(o.label,language))}</option>`).join('')}</select></div>`).join('')}`;
 };
