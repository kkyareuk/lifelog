...
export function advanceSharedLife(snapshot,now,command=null){
  ...
  return runIsolatedWorld(world,()=>withSimulationBatch(()=>{
    ...
    return world.order.map(id=>{
      ...
      let json=JSON.stringify({coffeeInventory:c.coffeeInventory||{},lifeNeeds:c.lifeNeeds||null,storyMemory:c.storyMemory||[],storyDays:c.storyDays||{},storyLastScene:c.storyLastScene||"",scene:scenes[id],timelineResetAt:c.timelineResetAt||0,days,directive:world.characterDirectives?.[id]||null});
      if(Buffer.byteLength(json)>120000){for(const d of Object.values(days)){delete d.signature;d.entries=d.entries.slice(-30)}json=JSON.stringify({coffeeInventory:c.coffeeInventory||{},lifeNeeds:c.lifeNeeds||null,storyMemory:c.storyMemory||[],storyDays:c.storyDays||{},storyLastScene:c.storyLastScene||"",scene:scenes[id],timelineResetAt:c.timelineResetAt||0,days,directive:world.characterDirectives?.[id]||null})}
