...
export function meetingScene(scene,directive,characterId,now=Date.now(),language='ko'){
 ...
 return {...scene,title:text[0]||scene.title,desc:text[1]||scene.desc,copy:undefined,home:Boolean(where.home),visitHomeId:where.homeId||'',room:where.room||'',placeId:where.placeId||'',townId:where.townId,mood:directive.jealous?'질투':directive.privacyStartled?'놀람':arrived?(scene.mood||'평온'):'평온',transit:actor&&!arrived&&!where.home,meetingKind:directive.kind,privacyStartled:!!directive.privacyStartled,jealous:!!directive.jealous,contactRejected:!!directive.contactRejected,contactTone:arrived?(directive.copy?.[language]||directive.copy?.ko)?.contactTone:undefined,meetingFurniture:arrived?directive.furniture:null,meetingActorId:j.actorId,participantOrder:scene.participantOrder||j.participantOrder,groupInteraction:arrived&&Boolean(directive.targetId),withId:arrived?directive.targetId:undefined,withIds:arrived?(directive.withIds||[]).filter(id=>id!==characterId):[],meetingJourney:actor&&!arrived&&segment?{...segment,arrivesAt:j.arrivesAt}:null,meetingWaiting:!actor&&!arrived,meetingLocation:nearby,manualDirective:true};
}
