...
export function considerDiscovery(c,scene,context={}){
 ...
 const button=document.createElement('button');button.type='button';button.className='game-hud-button discovery-rail-button';const icon=document.createElement('i');icon.className='discovery-question-icon';icon.setAttribute('aria-hidden','true');const label=document.createElement('em');button.append(icon,label);if(rail.matches('[data-home-discovery-slot],[data-web-discovery-rail]')){const caption=document.createElement('span');caption.textContent=t('심리테스트','Personality test','心理テスト');button.append(caption);}
