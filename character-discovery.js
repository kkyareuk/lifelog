...
export function considerDiscovery(c,scene,context={}){
 ...
 const paint=()=>{if(!bar.isConnected)return;const wait=discoveryRemaining();button.disabled=requesting||(wait>0&&!adsAvailable());label.textContent=`${Math.floor(Math.ceil(wait/1000)/60)}:${String(Math.ceil(wait/1000)%60).padStart(2,'0')}`;label.className='discovery-countdown';button.setAttribute('aria-label',t('질문받기','Get a question','質問を受ける')+(wait>0?` (${Math.ceil(wait/60000)} min)`:''));button.title=discoveryPremium()?t('광고 제거 혜택 · 계정 전체에서 1분마다 질문 하나','Ad-free benefit: one question per minute across your account','広告削除特典：アカウント全体で1分ごとに質問を1つ'):t('계정 전체에서 10분마다 질문 하나','One question every 10 minutes across your account','アカウント全体で10分ごとに質問を1つ');requestTimer=setTimeout(paint,1000);};
