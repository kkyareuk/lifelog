...
import {discoveryAccountLast,refreshDiscoveryAccess,consumeDiscoveryAccess,discoveryRemaining,discoveryPremium} from './discovery-access.js';
import {adsAvailable} from './native-ads.js';
import {showDiscoveryAdChoice} from './discovery-ad-dialog.js';
import {variedDiscoveryPool} from './discovery-question-pool.js';
import {recordSaveFailure,saveFailureMessage} from './save-status.js?v=20260909dev305';
...
export function showDiscovery(c,scene,question,context={}){
 ...
 discoveryChoices(c,question).forEach(({choice,index})=>{const b=document.createElement('button');b.type='button';b.textContent=choice.text[state.uiLanguage]||choice.text.ko;b.onclick=async()=>{
  ...
  try{await new Promise(resolve=>requestAnimationFrame(()=>setTimeout(resolve,0)));if(!await applyDiscoveryChoice(c,question,index,{...context,selectedValue})){status.textContent=t('질문을 연 뒤 설정이 바뀌었거나 이미 답한 질문이에요. 닫고 다시 확인해 주세요.','Settings changed after this question opened, or it was already answered. Close it and check again.','質問を開いた後に設定が変わったか、すでに回答済みです。閉じて再度ご確認ください。');return;}tutorialEvent('question-success');if(pending===d)close();}catch(error){recordSaveFailure(error,`question:${question.id}`);if(select&&!select.value){select.focus();return;}if(!valid()){close();return}choices.querySelectorAll('button').forEach(b=>b.disabled=false);status.textContent=saveFailureMessage(state.uiLanguage);}
 };choices.append(b)});
 ...
 const skip=document.createElement('button');skip.type='button';skip.textContent=t('지금은 넘기기','Skip for now','今は見送る');skip.onclick=()=>{tutorialEvent('question-skip');close()};d.append(heading,art,name,prompt);if(editor)d.append(editor.root);d.append(choices,hint,feedback,status,skip);d.onclose=()=>{d.remove();if(pending===d){pending=null;pendingCheck=null}};document.body.append(d);timeOperation('question-modal',()=>d.showModal());
}
...
export function considerDiscovery(c,scene,context={}){
 ...
 const last=discoveryAccountLast;let requesting=false;
 void refreshDiscoveryAccess().catch(()=>{});
 const paint=()=>{if(!bar.isConnected)return;const wait=discoveryRemaining();button.disabled=requesting||Boolean(window.ParallelCityAuth?.getInfo?.()?.startupSyncing)||(wait>0&&!adsAvailable());button.classList.toggle('is-cooling-down',wait>0);label.textContent=`${Math.floor(Math.ceil(wait/1000)/60)}:${String(Math.ceil(wait/1000)%60).padStart(2,'0')}`;label.className='discovery-countdown';button.setAttribute('aria-label',t('질문받기','Get a question','質問を受ける')+(wait>0?` (${Math.ceil(wait/60000)} min)`:''));button.title=discoveryPremium()?t('광고 제거 혜택 · 계정 전체에서 1분마다 질문 하나','Ad-free benefit: one question per minute across your account','広告削除特典：アカウント全体で1分ごとに質問を1つ'):t('계정 전체에서 10분마다 질문 하나','One question every 10 minutes across your account','アカウント全体で10分ごとに質問を1つ');requestTimer=setTimeout(paint,1000);};
 button.onclick=()=>timeOperation('question-open',async()=>{if(requesting||pending||document.querySelector('dialog[open]'))return;if(discoveryRemaining()){showDiscoveryAdChoice(state.uiLanguage,()=>button.onclick());return;}const id=observedCharacterId();const current=context.groupId?sharedDiscoveryCharacter(context.groupId,id):state.characters[id];if(!current||current.id!==c.id)return;
 const people=context.groupId?Object.fromEntries((window.DrawerVillageGroups.getSnapshot().residents||[]).map(r=>[r.id,r])):state.characters;const events=storyQuestions(current,people).filter(q=>discoveryChoices(current,q).some(({choice})=>Object.keys(choice.targets).some(f=>!discoveryLocked(current,f))));let candidates=variedDiscoveryPool(current,events,discoveryCandidates(current,{title:'질문받기'}));if(!candidates.length){const again=repeatDiscoveryCandidates(current);candidates=again.filter(q=>!current.discovery?.recent?.includes(q.id));if(!candidates.length)candidates=again;}if(!candidates.length){if(DISCOVERY_FIELDS.every(f=>discoveryLocked(current,f)))showDiscoveryGroups(current,context);else{const message=document.createElement('dialog');message.className='character-discovery-dialog';const text=document.createElement('p');text.textContent=t('지금 바꿀 수 있는 설정이 잠겨 있어요. 전체설정에서 바꾸고 싶은 항목의 잠금을 풀면 질문을 받을 수 있어요.','The available settings are locked. Unlock a setting in full settings to receive questions.','変更できる設定がロックされています。詳細設定で変更したい項目のロックを外すと質問を受けられます。');const done=document.createElement('button');done.textContent=t('닫기','Close','閉じる');done.onclick=()=>message.close();message.append(text,done);message.onclose=()=>message.remove();document.body.append(message);message.showModal();}return;}
 try{requesting=true;button.disabled=true;if(!await consumeDiscoveryAccess()){clearTimeout(requestTimer);paint();return;}}catch{label.textContent=t('연결을 확인하고 다시 눌러 주세요','Check your connection and try again','接続を確認して再度お試しください');return;}finally{requesting=false;button.disabled=discoveryRemaining()>0&&!adsAvailable();}
...
export function bindDiscoveryLocks(){
 ...
 for(const field of DISCOVERY_FIELDS){const numeric=DISCOVERY_AXES[field]?.numeric;for(const control of document.querySelectorAll(`[data-discovery-container="${field}"],[data-field="${field}"]${field.startsWith('bodyProfile.')?`,[data-body-field="${field.slice(12)}"],[data-body-measurement="${field.slice(12)}"]`:''}${numeric?`,[data-field="${numeric}"]`:''}${field==='touchReaction'?', [data-touch-reactions]':''}`)){
  ...
  b.onclick=async e=>{if(fixedDiscoveryKnown(c,field))return;e.preventDefault();e.stopPropagation();parent.querySelector('[data-lock-error]')?.remove();const before=c.discovery;c.discovery={...before,locks:{...before?.locks,...Object.fromEntries(groupFields(field).map(f=>[f,!groupLocked(field)]))}};b.disabled=true;
   try{if(state.sharedContext){const gid=state.sharedContext.groupId;if(!sharedDiscoveryCharacter(gid,c.id))throw Error();await window.DrawerVillageGroups.saveResident({groupId:gid,id:c.id,profile:{...c}});}else if(!await save(true))throw Error();if(groupLocked(field))tutorialEvent('trait-locked');}
