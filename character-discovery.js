...
export function showDiscovery(c,scene,question,context={}){
 ...
 discoveryChoices(c,question).forEach(({choice,index})=>{const b=document.createElement('button');b.type='button';b.textContent=choice.text[state.uiLanguage]||choice.text.ko;b.onclick=async()=>{
  ...
  try{await new Promise(resolve=>requestAnimationFrame(()=>setTimeout(resolve,0)));if(!await applyDiscoveryChoice(c,question,index,{...context,selectedValue}))throw Object.assign(Error('Question settings changed'),{name:'QuestionStateError'});if(pending===d)close();}catch(error){recordSaveFailure(error,`question:${question.id}`);if(select&&!select.value){select.focus();return;}if(!valid()){close();return}choices.querySelectorAll('button').forEach(b=>b.disabled=false);status.textContent=saveFailureMessage(state.uiLanguage);}
 };choices.append(b)});
