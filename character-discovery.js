import {discoveryAccountLast,refreshDiscoveryAccess,consumeDiscoveryAccess} from './discovery-access.js';
...
export function considerDiscovery(c,scene,context={}){
 ...
 const paint=()=>{if(!bar.isConnected)return;const wait=discoveryWait(last());button.disabled=requesting||wait>0;label.textContent=`${Math.floor(Math.ceil(wait/1000)/60)}:${String(Math.ceil(wait/1000)%60).padStart(2,'0')}`;label.className='discovery-countdown';button.setAttribute('aria-label',t('질문받기','Get a question','質問を受ける')+(wait>0?` (${Math.ceil(wait/60000)} min)`:''));button.title=t('계정 전체에서 10분마다 질문 하나','One question every 10 minutes across your account','アカウント全体で10分ごとに質問を1つ');requestTimer=setTimeout(paint,1000);};
 button.onclick=()=>timeOperation('question-open',async()=>{if(requesting||pending||document.querySelector('dialog[open]')||discoveryWait(last()))return;const id=observedCharacterId();const current=context.groupId?sharedDiscoveryCharacter(context.groupId,id):state.characters[id];if(!current||current.id!==c.id)return;
 ...
 try{requesting=true;button.disabled=true;if(!await consumeDiscoveryAccess()){clearTimeout(requestTimer);paint();return;}}catch{label.textContent=t('연결을 확인하고 다시 눌러 주세요','Check your connection and try again','接続を確認して再度お試しください');return;}finally{requesting=false;button.disabled=discoveryWait(last())>0;}
