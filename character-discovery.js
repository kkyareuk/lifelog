...
export function considerDiscovery(c,scene,context={}){
 ...
 const storageKey=['drawer-discovery-request',uid()||'guest',context.groupId||'personal',c.id].map(encodeURIComponent).join(':');
 ...
 button.onclick=()=>timeOperation('question-open',()=>{if(pending||document.querySelector('dialog[open]')||discoveryWait(last()))return;const id=observedCharacterId();const current=context.groupId?sharedDiscoveryCharacter(context.groupId,id):state.characters[id];if(!current||current.id!==c.id)return;
