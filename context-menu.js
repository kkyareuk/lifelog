...
const words={ko:{actor:'행동할 캐릭터',close:'닫기',more:'다른 행동',busy:'이동·행동을 준비하고 있어요…',failed:'지금은 실행할 수 없어요. 출입 권한이나 사용 중인 자리를 확인해 주세요.',empty:'이 마을에서 조작할 내 캐릭터가 없어요.'},en:{actor:'Acting character',close:'Close',more:'Other activities',busy:'Preparing the activity…',failed:'Unavailable now. Check access and occupied furniture.',empty:'You have no controllable character here.'},ja:{actor:'行動するキャラクター',close:'閉じる',more:'ほかの行動',busy:'行動を準備しています…',failed:'今は実行できません。入室権限や使用中の家具を確認してください。',empty:'この村に操作できるキャラクターがいません。'}};
export function installContextMenu({world,execute,enabled}){
 ...
 document.addEventListener('click',e=>{
  ...
  const initial=()=>{paint(recommendedContextActions(target,w,actors.find(c=>c.id===select.value)));const heading=document.createElement('small');heading.textContent=({ko:'추천 행동',en:'Suggested actions',ja:'おすすめの行動'})[lang]||'추천 행동';list.prepend(heading);const extra=document.createElement('button');extra.textContent=copy.more;extra.disabled=!actors.length;extra.onclick=categories;list.append(extra)};select.onchange=initial;initial();
