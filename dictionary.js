import {state,addCatalogItem,updateCatalogItem,deleteCatalogItem,save} from './state.js?v=20260902relationship202';
  ...
  shell.addEventListener('click',async event=>{
    ...
    if(b.hasAttribute('data-dict-photo')){
      ...
      d.querySelectorAll('[data-source]').forEach(e=>e.onclick=()=>{
        ...
        else{if(!commit())return; e.dataset.source==='app'?actions.illustration(id,kind):actions.upload('catalogImage',id,kind)}
      });
    }
  });
}
