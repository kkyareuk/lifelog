import {state,addCatalogItem,updateCatalogItem,deleteCatalogItem,save} from './state.js?v=20260908dev278';
import {initializeLocalMediaState} from './local-media.js?v=20260908dev278';
...
function refreshResults(){const r=document.querySelector('[data-dict-results]');if(!r)return;r.innerHTML=results();document.querySelector('[data-dict-count]').textContent=`${tr('총')} ${matches().length}${ui.kind?' / 80':' · '+({ko:'종류별 최대 80개',en:'Up to 80 per category',ja:'カテゴリごとに80件まで'}[state.uiLanguage]||'종류별 최대 80개')}`;actions?.translate?.(r)}
export function mountDictionary(callbacks){
  ...
  shell.addEventListener('click',async event=>{
    ...
    if(b.hasAttribute('data-dict-add')){
      const add=kind=>{const id=addCatalogItem(kind,{name:tr('새 항목'),category:cfg.categories[kind]?.[0]||'기타'});if(!id){actions.toast('종류별로 80개까지 추가할 수 있어요.');return}open(kind,id)};
    ...
    if(b.hasAttribute('data-dict-copy')){collect();const {id,...copy}=ui.draft;const newId=addCatalogItem(ui.editing.kind,{...copy,name:`${copy.name} (${tr('복제')})`});if(newId)open(ui.editing.kind,newId);else actions.toast('종류별로 80개까지 추가할 수 있어요.')}
