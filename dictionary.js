...
function foodPricePreview(item){
 const cost=catalogFoodCost(item),home=Math.round(cost.home*10000).toLocaleString(),out=Math.round(cost.out*10000).toLocaleString();
 return ({ko:`자동 가격 · 재료비 ${home}원 / 외식 ${out}원`,en:`Automatic price · Ingredients ${home} won / Dining out ${out} won`,ja:`自動価格 · 材料費 ${home}ウォン / 外食 ${out}ウォン`})[state.uiLanguage]||'';
}
function editor(){
...
function bindFields(shell){
  ...
  shell.querySelectorAll('[data-dict-field]').forEach(el=>el.addEventListener('input',()=>{
    collect();
    const pricePreview=shell.querySelector('[data-food-price-preview]');if(pricePreview)pricePreview.textContent=foodPricePreview(ui.draft);
