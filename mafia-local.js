...
export async function localGames(action,input,owner){
 ...
 const participant=(c,delegated)=>({id:c.id,name:c.name,ownerUid:owner,icon:c.icon||c.iconImage||'',photo:c.photo||'',speechStyle:c.speechStyle||'',personalityTypes:c.personalityTypes||[],voteTraits:engine.voteTraits(c),gameSkills:Object.fromEntries(["deceptionSkill", "intuitionSkill", "stealthSkill", "intelligenceSkill", "observationSkill", "composureSkill"].map(k=>[k,Math.max(0,Math.min(100,Number(c[k]??50)))])),traits:JSON.stringify([c.personalityTypes,c.personality,c.temperament,c.emotionalBaseline,c.angerResponse]),hobbies:JSON.stringify(c.hobbies||[]),homeId:c.homeId||'',sleep:c.sleep||'23:00',alive:true,delegated});
