...
export async function localGames(action,input,owner){
 ...
 const participant=(c,delegated)=>({id:c.id,name:c.name,ownerUid:owner,icon:c.icon||c.iconImage||'',photo:c.photo||'',speechStyle:c.speechStyle||'',personalityTypes:c.personalityTypes||[],shoeClue:/부츠/.test(c.shoeStyle||'')?'boots':/구두|하이힐/.test(c.shoeStyle||'')?'heels':'unknown',heightClue:Number(c.bodyProfile?.heightCm||c.heightCm)>180?'tall':Number(c.bodyProfile?.heightCm||c.heightCm)>0&&Number(c.bodyProfile?.heightCm||c.heightCm)<160?'short':'unknown',voteTraits:engine.voteTraits(c),gameSkills:Object.fromEntries(["deceptionSkill", "intuitionSkill", "stealthSkill", "intelligenceSkill", "observationSkill", "composureSkill"].map(k=>[k,Math.max(0,Math.min(100,Number(c[k]??50)))])),traits:JSON.stringify([c.personalityTypes,c.personality,c.temperament,c.emotionalBaseline,c.angerResponse]),hobbies:JSON.stringify(c.hobbies||[]),homeId:c.homeId||'',sleep:c.sleep||'23:00',alive:true,delegated});
 if(action==='setGameConsent'){if(input.characters.some(id=>!people.some(c=>c.id===id)))throw Error('character-owner-required');data.consent=[...new Set(input.characters)];}
  ...
  const selected=new Set(input.autoStage?locations.filter(l=>l.townId===(input.townId||townId)).slice(0,8).map(l=>l.id):input.locations);if(selected.size<engine.targetCount(input.capacity,4)||selected.size>8||[...selected].some(id=>!locations.some(l=>l.id===id)))throw Error('game-locations');
  g={id:input.gameId,name:String(input.name).slice(0,60),hostUid:owner,type:'mafia',rulesVersion:4,drama:true,notebook:true,meetingControls:2,nightCycle:true,preparationRules:1,capacity:input.capacity,mafiaCount:Math.max(1,Math.min(Math.floor((input.capacity-1)/2),Number(input.mafiaCount)||1)),moveSeconds:[15,30,45,60,90,120].includes(input.moveSeconds)?input.moveSeconds:45,actionSeconds:[15,30,45,60,90,120].includes(input.actionSeconds)?input.actionSeconds:45,meetingSeconds:[60,120,180,300,600].includes(input.meetingSeconds)?input.meetingSeconds:300,status:'recruiting',createdAt:now,players:[],mode:'live',durationMs:45000,seed:crypto.randomUUID(),map:{townId,name:town.name,bg:world.photo||world.bg||'./world-assets/owner-forest-town.webp'},locations:locations.map(l=>({...l,selected:selected.has(l.id)}))};data.games.unshift(g);data.games=data.games.slice(0,20);
 }else{
  ...
  if(action==='joinGame'){const c=people.find(c=>c.id===input.characterId);if(g.status!=='recruiting'||!c)throw Error('game-invalid-action');if(g.players.some(p=>!p.delegated))throw Error('game-one-character');g.players.push(participant(c,false));}
   ...
   if(g.players.length<4)throw Error('game-not-enough-players');g.voteRelations={};g.bias={};for(const [source,targets] of Object.entries(s.characterViews||{}))for(const [target,view] of Object.entries(targets)){g.voteRelations[source+':'+target]=engine.voteRelation(view);const terms=JSON.stringify(view);g.bias[source+':'+target]=/원한|증오|혐오/.test(terms)?6:/사랑|연인/.test(terms)?-6:/미워|싫어|질투|불편|경계/.test(terms)?3:/신뢰|친구|소중|편안/.test(terms)?-3:0;}g.deadlineAt=now+(g.moveSeconds||45)*1000;engine.start(g);
