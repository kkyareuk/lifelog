...
export function carePriority(world,c,other){
 const official=Object.values(world.relationships||{}).filter(r=>r.temporalStatus!=='past'&&(r.groupMembers?.length?r.groupMembers:[r.a,r.b]).includes(c.id)&&(r.groupMembers?.length?r.groupMembers:[r.a,r.b]).includes(other.id));
 const v=world.characterViews?.[c.id]?.[other.id]||{},text=Object.values(v).filter(x=>typeof x==='string').join(' ');
 return (official.length?1000:0)+(dislikesPerson(world,c,other)?-2000:0)+(/사랑|연모|소중|가장|매우.*좋/.test(text)?100:0)+(/신뢰|편안|좋아|호감/.test(text)?30:0)-(/중요하지|무관심|그저 그런/.test(text)?20:0);
}
export function personalChoices(world,c){const choices=[],adult=isAdultAge(c.ageGroup),traits=[...values(c.personalityTypes),c.neatness,c.interference,c.conflictStyle,c.activityTempo,c.humorStyle].join(' '),others=Object.values(world.characters||{}).filter(x=>x.id!==c.id&&x.townId===c.townId).sort((a,b)=>carePriority(world,c,b)-carePriority(world,c,a));
