...
const TOPIC_LINES={
  ko:{
    ...
    worries:[['젠장, 생각만 해서는 안 끝나겠군. 지금 할 수 있는 쪽부터 고른다.','선택지가 둘이다. 어느 쪽이 쓸모 있는지 말해 봐.'],['선택지를 검토하고 있습니다. 당장 실행할 수 있는 쪽이 합리적이겠지요.','결정할 일이 있습니다. 각각의 이득과 손해를 분명히 해 두고 싶습니다.']],
    ...
    home:[['{home} 정리 중이다. 망할, 어지러운 건 그냥 못 넘기겠군.','{home}에 있다. 필요한 것부터 정리하고 있어.'],['{home}을 정리하고 있습니다. 필요한 물건은 제자리에 두어야겠지요.','{home}에서 잠시 머무르고 있습니다. 그동안 주변을 정돈해 두겠습니다.']],
    work:[['일을 나누는 중이다. 급한 것부터 끝낸다.','망할 일도 끝은 내야지. 쓸데없이 시간 끌 생각은 없어.'],['업무 순서를 검토하고 있습니다. 우선순위를 명확히 할 필요가 있습니다.','남은 일과 시간을 확인했습니다. 불필요한 지연은 피하고 싶습니다.']],
  ...
  en:{
    ...
    worries:[['Damn. Thinking alone will not finish it. Pick something we can do now.'],['I am weighing the options. An actionable choice would be preferable.']],
    ...
    home:[['Tidying {home}. What a damn mess.'],['I am putting {home} in order. Necessary things should be in their proper places.']],
  ...
  ja:{
    ...
    worries:[['ちっ、考えるだけでは終わらないな。今できるほうを選ぶ。'],['選択肢を検討しています。今実行できるほうが合理的でしょう。']],
...
export function characterMomentSpeech(character,neutral,{topic='moments',context={},language='ko',seed=0}={}){
  ...
  if(style==='과묵한 직설체'||style==='냉정한 격식체'){
    ...
    return authoredSelf(pool[Math.abs(seed)%pool.length],character,language).replace(/\{(\w+)\}/g,(_,key)=>String(context[key]||''));
  }
