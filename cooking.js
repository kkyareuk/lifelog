import {catalogRecipes} from './catalog-recipes.js';
...
export const recipeById=(id,world)=>RECIPES.find(r=>r.id===id)||catalogRecipes(world).find(r=>r.id===id);
export const savedRecipeById=(id,world,saved)=>saved?.recipeSnapshot||recipeById(id,world)||LEGACY_RECIPES.find(r=>r.id===id);
...
export function cookingProgress(job,now=Date.now()){
 const recipe=job?.recipeSnapshot||(job?.recipeVersion===472?recipeById(job.recipeId):LEGACY_RECIPES.find(r=>r.id===job?.recipeId));if(!recipe)return null;
...
export function canStartCooking(world,c,recipeId,now=Date.now()){
 const r=recipeById(recipeId,world);if(!r||!recipeAllowed(characterTown(world,c),r))return 'cooking-era';
...
export function startCooking(world,c,directive,recipeId,requestId,now){
 const r=recipeById(recipeId,world),error=canStartCooking(world,c,recipeId,now);if(error)return false;
...
export function finishCooking(world,c,now){
 ...
 const recipe=savedRecipeById(job.recipeId,world,job);if(!recipe)return false;
