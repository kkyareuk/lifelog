import {state,active} from "./state.js?v=20260801f";
import {eventFor,visibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260801f";
...
const CATALOG_ICONS={food:"🍽️",drink:"🥤",fashion:"👗",music:"🎵",idol:"🎤",book:"📚",movie:"🎬",game:"🎮",perfume:"🧴",hobby:"🎨"};
...
function sceneImage(c,entry){
  ...
  return place?.interiorImage||place?.image||catalogItem(entry.itemId)?.image||"";
}
function logTitle(entry){const image=sceneImage(active(),entry);return `${image?`<img class="catalog-sticker" src="${esc(image)}" alt="">`:""}<b>${esc(entry.title)}</b>`}
...
function observe(){
  ...
  return `${roster()}${townSwitcher}${sleepGate}<div class="observe"><section><div class="world-hud"><div><small>현재 시각</small><b>${new Date().toLocaleString("ko-KR",{month:"long",day:"numeric",weekday:"short",hour:"2-digit",minute:"2-digit"})}</b></div><div><small>관찰 중</small><b>${esc(c.name)} · ${esc(e.title)}</b></div></div><div class="viewport"><div class="world"><img src="${state.world.bg}" class="world-bg">${state.world.places.map(placeCard).join("")}${state.order.map(id=>personCard(state.characters[id])).join("")}</div></div></section><aside class="detail-column"><div class="detail panel"><div class="hero">${c.photo?`<img src="${c.photo}" alt="">`:avatar(c)}</div><h2>${esc(c.name)}</h2><p>${esc(c.jobTitle||c.job)}</p><div class="scene"><small>CURRENT SCENE</small><h3>${esc(e.title)}</h3><p>${esc(e.desc)}</p><b>${place?`📍 ${esc(place.name)} · ${esc(state.world.name)}`:"🏠 집 안"}</b>${currentImage?`<img class="place-photo" src="${esc(currentImage)}" alt="">`:""}</div></div><section class="panel life-log"><div class="title"><h2>오늘의 생활 로그</h2><small>현재 시각까지 자동 기록</small></div><ol>${logs.map((x,i)=>`<li role="button" tabindex="0" data-log-detail="${i}" class="${x===logs.at(-1)?"now":""}"><time>${esc(x.time)}</time><span>${logTitle(x)}<small>${esc(x.desc)}</small></span></li>`).join("")}</ol></section></aside></div><div id="log-modal" class="log-modal" hidden></div>`;
}
...
function homeCard(id,chars){
  ...
  const petStatus=pet=>{
    ...
    if(!activeHours[pet.species])return "자기 자리에서 잠들어 있어요.";
    if(inside.length)return `${inside[(hour+pet.name.length)%inside.length].name}와 함께 시간을 보내고 있어요.`;
    return pet.species==="인공지능"?"집 안을 살피며 대기하고 있어요.":"집 안에서 혼자 놀고 있어요.";
  };
  const petCards=pets.map(p=>`<article class="pet-card">
    ...
    ${edit?`<div class="pet-edit"><label>이름<input data-pet-field="name" data-home-id="${id}" data-pet-id="${p.id}" value="${esc(p.name)}"></label><label>종류<select data-pet-field="species" data-home-id="${id}" data-pet-id="${p.id}">${petKinds.map(x=>`<option ${x===p.species?"selected":""}>${x}</option>`).join("")}</select></label><label>품종<input data-pet-field="breed" data-home-id="${id}" data-pet-id="${p.id}" value="${esc(p.breed)}" placeholder="유저가 직접 입력"></label><label>성별<select data-pet-field="sex" data-home-id="${id}" data-pet-id="${p.id}">${["모름","수컷","암컷"].map(x=>`<option ${x===p.sex?"selected":""}>${x}</option>`).join("")}</select></label><label class="check"><input type="checkbox" data-pet-field="neutered" data-home-id="${id}" data-pet-id="${p.id}" ${p.neutered?"checked":""}> 중성화 완료</label><div class="pet-actions"><button data-pet-image="photo" data-home-id="${id}" data-pet-id="${p.id}">원형 사진</button><button data-pet-image="icon" data-home-id="${id}" data-pet-id="${p.id}">투명 아이콘</button><button class="danger" data-delete-pet="${p.id}" data-home-id="${id}">삭제</button></div></div>`:""}
  </article>`).join("");
