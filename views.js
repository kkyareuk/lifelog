...
function relationship(){
  ...
  const cards=all.map(r=>{
    if(r.groupId){
      if(shownGroups.has(r.groupId))return"";
      shownGroups.add(r.groupId);
      const group=all.filter(item=>item.groupId===r.groupId),memberIds=[...new Set(group.flatMap(item=>[item.a,item.b]))];
...
function townInformationScreen(character){
  ...
  const illustrations=townIllustrationsFor(state.world),selectedIllustration=illustrations.find(item=>item.src===state.world.bg)||illustrations.find(item=>item.pack==="base");
  const illustrationCards=illustrations.map((item,index)=>`<button type="button" class="town-illustration-card ${item.src===state.world.bg?"on":""} ${item.pack!=="base"?"is-locked":""}" ${item.pack==="base"?`data-town-illustration="${esc(item.src)}"`:`data-town-illustration-locked="${esc(item.id)}"`}><span>${item.src?`<img src="${esc(item.src)}" alt="">`:`<i aria-hidden="true">${item.id.includes("snow")?"❄️":item.id.includes("tropical")?"🌴":"🏜️"}</i>`}${index===0?`<em>${t("추천","추천")}</em>`:""}</span><b>${t(item.name,item.name)}</b><small>${item.pack==="base"?t("기본 제공","기본 제공"):t("기후 확장 DLC","기후 확장 DLC")}</small></button>`).join("");
  ...
  const art=selectedIllustration?.src||state.world.bg;
  return `<aside class="town-information-screen" aria-label="${esc(t("마을 정보 편집","마을 정보 편집"))}">
    ...
    <div class="town-information-content">
      <button type="button" class="town-information-hero" data-town-illustration-open aria-label="${esc(t("마을 일러스트 선택","마을 일러스트 선택"))}"><img src="${esc(art)}" alt="${esc(state.world.name)}"><span>${t("마을 일러스트 선택","마을 일러스트 선택")}</span></button>
    ...
    <dialog class="town-illustration-dialog" data-town-illustration-dialog><form method="dialog"><header><span><small>TOWN ILLUSTRATION</small><b>${t("마을 일러스트 선택","마을 일러스트 선택")}</b><p>${t("마을 설정과 잘 맞는 순서로 보여 줘요. 선택하면 마을 전체 배경이 바뀝니다.","마을 설정과 잘 맞는 순서로 보여 줘요. 선택하면 마을 전체 배경이 바뀝니다.")}</p></span><button value="close" aria-label="${esc(t("닫기","닫기"))}">×</button></header><div class="town-illustration-grid">${illustrationCards}</div></form></dialog>
  </aside>`;
}
function townMobile(){
