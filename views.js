...
function nativeCharacterHub(c){
  ...
  const tabletSummary=`<section class="tablet-character-summary" aria-label="${esc(c.name)} ${esc(t("캐릭터 정보","캐릭터 정보"))}"><div class="tablet-character-summary-paper"><header><span class="tablet-character-summary-avatar">${selectedIcon}</span><span><small>${t("선택한 캐릭터","선택한 캐릭터")}</small><h2>${esc(c.name)}</h2><p>${esc(c.jobTitle||c.job||unknown)}</p></span><i class="tablet-character-theme" style="--primary:${esc(c.theme?.primary||"#176b60")};--secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")}" aria-hidden="true"></i></header><dl>${draftRows.map(({label,value})=>`<div><dt>${t(label,label)}</dt><dd>${esc(value)}</dd></div>`).join("")}</dl><div class="tablet-character-setting-actions"><button type="button" data-open-quick-character-settings><b>${t("캐릭터 프로필","캐릭터 프로필")}</b><small>${t("기본 모습을 정하고, 플레이의 선택으로 성향을 쌓아요.","Set the basics. Shape traits through choices in play.","基本を決め、プレイ中の選択で性格を育みます。")}</small></button><button type="button" data-open-full-character-settings><b>${t("전체설정","전체설정")}</b><small>${t("성향을 직접 다듬고, 자물쇠로 변화를 잠가요.","Edit traits. Lock them to prevent changes and questions.","傾向を編集し、鍵で変化と質問を止めます。")}</small></button></div><nav><button type="button" data-export-profile>${t("프로필 내보내기","프로필 내보내기")}</button><button type="button" data-save>${t("캐릭터 저장","캐릭터 저장")}</button><button type="button" class="danger" data-delete-character="${c.id}">${t("캐릭터 삭제","캐릭터 삭제")}</button></nav></div></section>`;
  return `<div class="editor character-editor character-editor-hub-only" style="--character-own:${esc(c.theme?.primary||"#176b60")};--character-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")};--character-accent:${esc(c.theme?.primary||"#176b60")};--character-accent-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")}"><aside class="panel desktop-character-list"><div class="title"><h2>${t("캐릭터 목록","캐릭터 목록")}</h2><button data-new ${state.order.length>=limit?"disabled":""}>${slotLabel}</button></div>${list}</aside><section class="panel form character-hub-shell">${tabletSummary}<section class="mobile-character-dashboard" data-character-ui-version="8" style="--character-own:${esc(c.theme?.primary||"#176b60")};--character-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")};--character-accent:${esc(c.theme?.primary||"#176b60")};--character-accent-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")}"><img class="character-wood-background" src="./assets/character-ui/character-wood-background.png" alt="" aria-hidden="true">${mobileProfile}<p class="character-slot-count">${esc(slotText(state,window.ParallelCityAuth?.getInfo?.()).text)}</p>${hubActions}${draftActions}</section>${quickSettings}${characterRenderOptions.reorder?`<dialog class="mobile-character-reorder-dialog" data-mobile-character-reorder-dialog><form method="dialog"><div class="mobile-editor-head"><span><small>CHARACTER ORDER</small><b>${t("캐릭터 위치 바꾸기","캐릭터 위치 바꾸기")}</b></span><button value="close">×</button></div><p>${t("화살표를 눌러 홈과 캐릭터 목록의 순서를 바꿔요.","화살표를 눌러 홈과 캐릭터 목록의 순서를 바꿔요.")}</p><div>${reorderRows}</div><button class="primary" value="close">${t("완료","완료")}</button></form></dialog>`:""}</section></div>`;
}
function character(){
...
Object.assign(UI_TEXT.en,{
  "캐릭터 슬롯 1개가 결제 즉시 계정에 영구 적용됩니다. 6개월 뒤에 사라지는 상품이 아닙니다. 결제일부터 최소 6개월간 이용을 보장하며, 이후에도 서비스 운영 기간 동안 유지됩니다.":"One character slot is permanently applied to the account immediately after payment. This does not expire after six months. Access is guaranteed for at least six months from the payment date and continues while the service remains in operation.",
  "마을 슬롯 1개가 결제 즉시 계정에 영구 적용됩니다. 6개월 뒤에 사라지는 상품이 아닙니다. 결제일부터 최소 6개월간 이용을 보장하며, 이후에도 서비스 운영 기간 동안 유지됩니다.":"One town slot is permanently applied to the account immediately after payment. This does not expire after six months. Access is guaranteed for at least six months from the payment date and continues while the service remains in operation."
});
Object.assign(UI_TEXT.ja,{
  "캐릭터 슬롯 1개가 결제 즉시 계정에 영구 적용됩니다. 6개월 뒤에 사라지는 상품이 아닙니다. 결제일부터 최소 6개월간 이용을 보장하며, 이후에도 서비스 운영 기간 동안 유지됩니다.":"キャラクター枠1人分が決済直後にアカウントへ永続的に適用されます。6か月後に失効する商品ではありません。決済日から最低6か月間の利用を保証し、その後もサービス運営期間中は維持されます。",
  "마을 슬롯 1개가 결제 즉시 계정에 영구 적용됩니다. 6개월 뒤에 사라지는 상품이 아닙니다. 결제일부터 최소 6개월간 이용을 보장하며, 이후에도 서비스 운영 기간 동안 유지됩니다.":"村スロット1つが決済直後にアカウントへ永続的に適用されます。6か月後に失効する商品ではありません。決済日から最低6か月間の利用を保証し、その後もサービス運営期間中は維持されます。"
});
Object.assign(UI_TEXT.en,{
...
const SHOP_PRODUCTS={
  ...
  character_slot_1:{label:"캐릭터 슬롯",title:"캐릭터 1명 추가",description:"캐릭터 슬롯 1개가 결제 즉시 계정에 영구 적용됩니다. 6개월 뒤에 사라지는 상품이 아닙니다. 결제일부터 최소 6개월간 이용을 보장하며, 이후에도 서비스 운영 기간 동안 유지됩니다.",price:1000},
  town_slots_5:{label:"마을 슬롯",title:"마을 5개 추가",description:"마을 슬롯 5개가 계정에 추가됩니다. 기존 구매 슬롯은 그대로 유지됩니다.",price:9300},
  town_slot_1:{label:"마을 슬롯",title:"마을 1개 추가",description:"마을 슬롯 1개가 결제 즉시 계정에 영구 적용됩니다. 6개월 뒤에 사라지는 상품이 아닙니다. 결제일부터 최소 6개월간 이용을 보장하며, 이후에도 서비스 운영 기간 동안 유지됩니다.",price:1900},
