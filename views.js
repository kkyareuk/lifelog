...
function character(){
  ...
  const overviewLife=`<section class="character-overview-basic character-overview-life" aria-label="${esc(t("개요 생활 설정","개요 생활 설정"))}">
    ...
    <label class="overview-field overview-life-habit"><b>${t("생활습관","생활습관")}</b>${overviewSelect("activityTempo",["한 가지씩 차분히","잠깐 쉬고 다음 일","상황에 따라","생각나면 바로 움직임","부산스럽게 여러 일을 오감","허둥대며 주의가 자주 옮겨감"],c.activityTempo||"상황에 따라")}</label>
    <label class="overview-field overview-food-habit"><b>${t("식습관","식습관")}</b>${overviewSelect("foodHabit",["끼니를 자주 거름","배고플 때만 먹음","규칙적으로 식사함","조금씩 자주 먹음","천천히 오래 먹음","빠르게 먹는 편"],c.foodHabit||"규칙적으로 식사함")}</label>
  ...
  const lowerBookmark=`<button type="button" class="character-book-v8-shape ${fullActivePane==="visual"?"on":""}" data-character-pane="visual" ${fullActivePane==="visual"?'aria-current="page"':""}><b>${t("모양","모양")}</b></button>`;
