import {state,active,characterViewFor,explicitCharacterViewFor} from "./state.js?v=20260820contactsettings1";
import {eventFor as simulateEventFor,visibleTimeline as simulateVisibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260820contactsettings1";
...
function roomStyle(h,key,layout,mobileLayout){
  ...
  const resolvedMobile=manual||mobileLayout||{x:0,y:0,w:100,h:100},image=room.image,parts=[
    ...
    `--room-image-fit:${room.imageFit==="cover"?"cover":"contain"}`
  ];
...
function character(){
  ...
  const contactCard=pendingQuestion?`<section class="character-contact-inbox"><span>${avatar(c)}</span><div><small>TODAY'S CONTACT</small><h2>${esc(c.name)}에게 오늘의 질문이 도착했어요</h2><p>갑자기 팝업으로 띄우지 않아요. 준비됐을 때 여기서 열고 선택하면 실제 생활 일정으로 이어집니다.</p></div><button type="button" class="primary" data-open-daily-question>질문 열기</button></section>`:`<section class="character-contact-inbox is-empty"><span>${avatar(c)}</span><div><small>TODAY'S CONTACT</small><h2>오늘의 연락</h2><p>질문과 선택은 팝업 대신 이곳에 조용히 도착해요.</p></div></section>`;
