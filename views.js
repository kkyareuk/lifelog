...
import {eventFor as simulateEventFor,visibleTimeline as simulateVisibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260810g";
...
function header(){
  ...
  return `<header><div class="brand"><span class="logo"><img src="./icons/drawer-village-logo.png" alt="서랍마을"></span><div><h1>서랍마을</h1><small>서랍 속 캐릭터 생활 관찰 게임</small></div>${previewMode()?`<span class="preview-badge">${esc(previewConfig().label||"사전 체험")}</span>`:""}</div><nav>${tabs.map(([k,n,icon])=>`<button data-tab="${k}" class="${state.activeTab===k?"on":""}"><span class="tab-icon" aria-hidden="true">${icon}</span><span>${n}</span></button>`).join("")}</nav><span id="save-state">기기에 저장됨</span></header>${nativeBar}`;
}
const NATIVE_MENU_TABS=[["home","집","⌂"],["character","캐릭터","♙"],["catalog","취향 사전","◇"],["relationship","관계","∞"],["routine","주간 루틴","▦"],["town","마을","▧"],["shop","상점","♢"],["settings","설정","⚙"]];
...
function nativeGameMenu(){
  return `<nav class="native-game-menu" aria-label="앱 메뉴">${NATIVE_MENU_TABS.map(([key,label,icon],index)=>`<button type="button" data-tab="${key}" class="${index<4?"native-menu-left":"native-menu-right"}" style="--native-menu-row:${index%4}"><span aria-hidden="true">${icon}</span><small>${label}</small></button>`).join("")}</nav>`;
}
...
function nativeScenePresentation(c,entry){
  ...
  const lineupHtml=companions.length?`<span class="native-scene-lineup ${coResidentConversation?"is-conversation":""}" style="--scene-count:${sceneParticipants.length}" aria-label="${esc(sceneParticipants.map(person=>person.name).join(", "))}">${sceneParticipants.map((person,index)=>{
    ...
    return `<span class="native-scene-lineup-person ${person.id===c.id?"is-current":""}" style="--scene-index:${index};--scene-delay:${delay}s;--scene-duration:${duration}s">${avatar(person,"native-scene-lineup-avatar")}${actionProp}${tone==="sleep"?'<b class="native-character-sleep-mark" aria-hidden="true">ZZ</b>':tone==="drowsy"?'<b class="native-character-drowsy-mark" aria-hidden="true">z</b>':""}${tone==="date-overwhelmed"&&person.id===c.id?'<b class="native-character-sweat" aria-hidden="true">💧</b>':""}<small>${esc(person.name)}</small></span>`;
  }).join("")}</span>`:"";
...
function observe(){
  ...
  if(!localId)return `${nativeGameMenu()}<section class="native-observe-home native-observe-empty"><div><span>🏙️</span><h1>이 마을에 사는 캐릭터가 없어요</h1><p>캐릭터 화면에서 생활하는 마을을 지정해 주세요.</p><button class="primary" data-tab="character">캐릭터 설정 열기</button></div></section><div class="standard-observe-view">${roster()}${townSwitcher}<div class="observe"><section><div class="viewport"><div class="world"><img src="${TOWN_BACKGROUND}" class="world-bg">${state.world.places.map(placeCard).join("")}</div></div></section><aside class="panel empty"><h2>이 마을에 사는 캐릭터가 없어요</h2><p>캐릭터 프로필에서 생활하는 마을을 지정할 수 있어요.</p></aside></div></div>`;
  ...
  const locationBackground=e.home
  ...
  const stageClasses=`${presentation.partner?"has-scene-companion":""} ${presentation.lineupHtml?"has-scene-lineup":""} ${presentation.pet?"has-scene-pet":""}`;
  const sceneActors=`${avatar(c,"native-main-character")}${presentation.sleepMarkHtml}${presentation.lineupHtml}${presentation.conversationHtml}${presentation.actionHtml}${presentation.companionHtml}${presentation.petHtml}<i></i>${itemOrbit}`;
  const desktopScene=`<section class="desktop-observe-scene native-app" aria-label="${esc(c.name)}의 지금 이 순간"><div class="desktop-scene-canvas scene-tone-${presentation.tone} scene-action-${presentation.actionKind}" style="--native-own:${esc(c.theme?.primary||"#176b60")};--native-own-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")}"><div class="native-observe-backdrop" style="background-image:url(&quot;${esc(nativeBackground)}&quot;)"></div><div class="native-observe-shade"></div><div class="native-scene-atmosphere" aria-hidden="true"></div>${presentation.effects}<div class="desktop-scene-copy"><small>지금 이 순간</small><h1>${esc(c.name)} · ${esc(e.title)}</h1><p>${esc(e.desc)}</p><b>${location}</b></div><div class="native-character-stage ${stageClasses}" aria-label="${esc(c.name)} 현재 장면">${sceneActors}</div></div></section>`;
  const nativeHome=`${nativeGameMenu()}<section class="native-observe-home scene-tone-${presentation.tone} scene-action-${presentation.actionKind}" style="--native-own:${esc(c.theme?.primary||"#176b60")};--native-own-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")}"><div class="native-observe-backdrop" style="background-image:url(&quot;${esc(nativeBackground)}&quot;)"></div><div class="native-observe-shade"></div><div class="native-scene-atmosphere" aria-hidden="true"></div>${presentation.effects}<div class="native-observe-top"><span><b>${esc(c.name)}</b><small>${esc(c.jobTitle||c.job||"생활 중")}</small></span><span class="native-observe-clock"><time>${new Date().toLocaleTimeString("ko-KR",{hour:"2-digit",minute:"2-digit"})}</time><button type="button" class="native-scene-refresh" data-refresh-observe aria-label="현재 장면 새로고침" title="현재 장면 새로고침">↻</button></span></div><div class="native-character-stage ${presentation.partner?"has-scene-companion":""} ${presentation.lineupHtml?"has-scene-lineup":""} ${presentation.pet?"has-scene-pet":""}" aria-label="${esc(c.name)} 현재 장면">${avatar(c,"native-main-character")}${presentation.sleepMarkHtml}${presentation.lineupHtml}${presentation.conversationHtml}${presentation.actionHtml}${presentation.companionHtml}${presentation.petHtml}<i></i>${itemOrbit}</div><div class="native-character-picker" aria-label="관찰 캐릭터 선택">${state.order.map(id=>{const person=state.characters[id];return `<button type="button" data-home-character="${id}" class="${id===c.id?"on":""}" title="${esc(person.name)}" aria-label="${esc(person.name)}">${avatar(person)}</button>`}).join("")}</div><article class="native-status-card"><div class="native-status-card-head"><small>지금 이 순간</small><button type="button" data-toggle-native-moment aria-expanded="false">펼치기</button></div><h1>${esc(e.title)}</h1><p>${esc(e.desc)}</p><b>${location}</b></article><section class="native-log-card" data-open-native-log-card role="button" tabindex="0" aria-label="오늘의 기록 전체 보기"><div><b>오늘의 기록 <small class="native-log-open-hint">눌러서 펼쳐 보기 ↗</small></b><span><button type="button" data-open-native-log>전체 보기</button><button type="button" data-tab="home">집 보기</button></span></div><ol>${nativeLog||"<li><span><b>아직 기록이 없어요</b><small>조금 뒤 새로운 생활 장면이 나타납니다.</small></span></li>"}</ol></section>${nativeFullLog}</section>`;
...
function character(){
  ...
  const paneMeta={profile:["프로필","기본 정보·끌림","👤"],body:["신체","외형·건강·접근성","✦"],personality:["성격","성향·서사·인지","◈"],taste:["취향 선택","취미·음식·콘텐츠","♡"],worldTaste:["세계관 선호","최애·소지품","⌂"],manage:["사진·아이콘·테마","이미지·색상·파일","✎"]};
  ...
  const managePane=`<section class="character-manage-pane" style="--own:${esc(c.theme?.primary||"#176b60")};--own-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")}"><div class="traits-pane-heading"><h2>${esc(c.name)}의 사진·아이콘·테마·파일</h2><p>프로필 사진과 투명 아이콘은 서로 다른 파일로 보관합니다. 한쪽을 등록해도 다른 쪽에는 복사되지 않아요.</p></div><div class="character-manage-grid"><section><span>${c.photo?`<img class="profile-photo-fallback" src="${esc(c.photo)}" alt="${esc(c.name)} 프로필 사진">`:`<span class="character-image-empty-preview"><i>사진</i><small>미등록</small></span>`}</span><div><h3>프로필 사진 설정</h3><p>일반 프로필 자리에서 여백 없이 동그랗게 잘라 보여요. 지도 아이콘으로 저장되지는 않습니다.</p><div class="image-actions"><button type="button" data-image="photo">사진 파일</button><button type="button" data-image-url="photo" data-id="${c.id}">사진 링크</button></div></div></section><section><span>${c.icon?`<img class="sprite" src="${esc(c.icon)}" alt="${esc(c.name)} 투명 아이콘">`:`<span class="character-image-empty-preview icon"><i>PNG</i><small>아이콘 미등록</small></span>`}</span><div><h3>투명 지도 아이콘 설정</h3><p>별도로 등록했을 때만 사용해요. PNG 전체가 잘리지 않도록 투명 여백과 원본 비율을 유지합니다.</p><div class="image-actions"><button type="button" data-image="icon">투명 PNG 파일</button><button type="button" data-image-url="icon" data-id="${c.id}">아이콘 링크</button></div></div></section><section class="character-manage-theme"><h3>테마색 설정</h3><p>버튼 색을 고르거나 색상 선택기와 HEX 값으로 직접 입력할 수 있어요.</p>${palette("primary","대표 테마색")}${palette("secondary","그라데이션 보조색")}<label class="check"><input type="checkbox" data-gradient ${c.theme?.gradient?"checked":""}> 보조색으로 그라데이션 사용</label></section><section class="character-manage-files"><h3>캐릭터 삭제</h3><p>삭제 전 경고를 확인한 뒤 이 캐릭터와 연결된 기록을 정리해요.</p><button type="button" class="danger" data-delete-character="${c.id}">캐릭터 삭제</button></section></div></section>`;
