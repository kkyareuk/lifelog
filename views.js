...
import {state,active,characterViewFor,explicitCharacterViewFor} from "./state.js?v=20260821characterui8";
import {eventFor as simulateEventFor,visibleTimeline as simulateVisibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260821characterui8";
...
const I18N={
  ...
  ja:{brandName:"ひきだし村",observe:"観察",mailbox:"郵便箱",home:"家",character:"人物",catalog:"好み",relationship:"関係",routine:"予定",statistics:"統計",town:"村",shop:"店",settings:"設定",saved:"端末に保存済み",brandTagline:"引き出しの中のキャラクター生活観察ゲーム",currentMoment:"今この瞬間",todayLog:"今日の記録",expand:"開く",collapse:"閉じる",viewAll:"すべて見る",viewHome:"家を見る",gridEdit:"グリッド編集",language:"言語",languageHelp:"日本語は基本画面に対応し、生活シーンや関係文もアップデートごとに翻訳を増やしています。",languageNote:"日本語ベータ・基本画面と一部の生活シーンに対応。翻訳範囲を継続して拡大します。",mailArrived:"手紙が届きました",mailReady:"準備ができたら手紙を開いてください。選択は実際の生活予定に反映されます。",mailEmpty:"届いた手紙はまだありません",mailEmptyHelp:"キャラクターからの質問・選択・悩み・近況はここに届きます。",mailboxHelp:"キャラクターからの手紙をここでまとめて確認できます。",openLetter:"手紙を開く",characterPicker:"観察する人物を選ぶ",currentTownResidents:"この村の人物",moveToAnotherTown:"別の村へ移動",close:"閉じる",noSleepingRoom:"その他・なし（宿泊しない）",locationExterior:"現在の建物の外観",inTransit:"移動中",outAndAbout:"外出中"}
};
...
Object.assign(UI_TEXT.en,{"세계관 설정":"Worldview settings","캐릭터 프로필":"Character profile","돌아가기":"Back","미설정":"Not set","나이":"Age","생일":"Birthday","키·몸무게":"Height · weight"});
Object.assign(UI_TEXT.ja,{"세계관 설정":"世界観設定","캐릭터 프로필":"キャラクタープロフィール","돌아가기":"戻る","미설정":"未設定","나이":"年齢","생일":"誕生日","키·몸무게":"身長・体重"});
const UI_TEXT_MORE={
  en:{
...
const BUILDING_PRESET_SOURCES={"type-generic":"world-assets/building-types/generic.png","type-cafe":"world-assets/building-types/cafe.png","type-restaurant":"world-assets/building-types/restaurant-handdrawn.png","type-hospital":"world-assets/building-types/hospital.png","type-office":"world-assets/building-types/office.png","type-shop":"world-assets/building-types/shop.png","type-school":"world-assets/building-types/school.png","type-lodging":"world-assets/building-types/lodging.png","type-library":"world-assets/building-types/library.png","type-theater":"world-assets/building-types/theater.png","type-park":"world-assets/building-types/park.png","type-home":"world-assets/building-types/home.png","drawer-building":"world-assets/drawer-building.png","drawer-home":"world-assets/drawer-home.png","medieval-castle":"world-assets/medieval-castle.svg","medieval-tavern":"world-assets/medieval-tavern.svg","medieval-market":"world-assets/medieval-market.svg"};
...
function header(){
  const tabs=[["observe",t("observe","관찰"),"◉"],["mailbox",t("mailbox","우편함"),"✉"],["home",t("home","집"),"⌂"],["character",t("character","캐릭터"),"♙"],["catalog",t("catalog","취향 사전"),"◇"],["relationship",t("relationship","관계"),"∞"],["routine",t("routine","일정"),"▦"],["statistics",t("statistics","통계"),"▥"],["town",t("town","마을"),"▧"],["shop",t("shop","상점"),"♢"],["settings",t("settings","설정"),"⚙"]];
...
const GAME_HUD_SIDE_TABS={
  left:[
    {key:"character",labelKey:"character",label:"캐릭터",art:"./assets/home-ui/profile-placeholder.png"},
    {key:"catalog",labelKey:"catalog",label:"취향 사전",art:"./assets/home-ui/catalog.png"},
    {key:"relationship",labelKey:"relationship",label:"관계",art:"./assets/home-ui/relationship.png"}
  ],
  right:[
    {key:"routine",labelKey:"routine",label:"일정",art:"./assets/home-ui/routine.png"},
    {key:"statistics",labelKey:"statistics",label:"통계",art:"./assets/home-ui/statistics.png"},
    {key:"settings",labelKey:"settings",label:"설정",art:"./assets/home-ui/settings.png"}
  ]
};
const GAME_HUD_LABELS={
  ko:{character:"캐릭터",catalog:"취향 사전",relationship:"관계",routine:"일정",statistics:"통계",settings:"설정",mailbox:"우편함",home:"집",todayLog:"기록물",shop:"상점",town:"마을"},
  en:{character:"People",catalog:"Tastes",relationship:"Bonds",routine:"Schedule",statistics:"Stats",settings:"Settings",mailbox:"Mail",home:"Home",todayLog:"Log",shop:"Shop",town:"Town"},
  ja:{character:"人物",catalog:"好み",relationship:"関係",routine:"予定",statistics:"統計",settings:"設定",mailbox:"郵便",home:"家",todayLog:"記録",shop:"店",town:"村"}
};
...
function gameHudDock(){
  return `<nav class="game-hud-dock" aria-label="${esc(t("주요 메뉴","주요 메뉴"))}"><button type="button" class="game-hud-button game-hud-home-button" data-tab="home"><img src="./assets/home-ui/home.png" alt=""><small>${gameHudLabel("home","집")}</small></button><button type="button" class="game-hud-button game-hud-mailbox-button" data-tab="mailbox"><img src="./assets/home-ui/mailbox.png" alt=""><small>${gameHudLabel("mailbox","우편함")}</small></button><button type="button" class="game-hud-button game-hud-log-button" data-open-native-log><img src="./assets/home-ui/ink.png" alt=""><small>${gameHudLabel("todayLog","기록물")}</small></button><button type="button" class="game-hud-button game-hud-shop-button" data-tab="shop"><img src="./assets/home-ui/shop.png" alt=""><small>${gameHudLabel("shop","상점")}</small></button><button type="button" data-tab="town" class="game-hud-button game-hud-town-button"><img src="./assets/home-ui/town.png" alt=""><small>${gameHudLabel("town","마을")}</small></button></nav>`;
}
...
function observe(){
  ...
  if(!localId){
    if(nativeHome)return `<section class="game-observe-hud game-observe-empty"><div class="native-observe-backdrop" style="background-image:url(&quot;${esc(state.world?.bg||TOWN_BACKGROUND)}&quot;)"></div><div class="native-observe-shade"></div>${gameHudSideMenu("left")}${gameHudSideMenu("right")}<div class="game-observe-empty-copy"><span>🏙️</span><h1>이 마을에 사는 캐릭터가 없어요</h1><p>캐릭터 화면에서 생활하는 마을을 지정해 주세요.</p><button class="primary" data-tab="character">캐릭터 설정 열기</button></div>${gameHudDock()}</section>${buildingDetailDialogs()}`;
  ...
  const location=`<span class="game-hud-location">${locationExterior?`<img src="${esc(locationExterior)}" alt="${esc(t("locationExterior","현재 건물 외관"))}">`:`<i aria-hidden="true">${e.transit?"🚌":"📍"}</i>`}<b>${esc(locationText)}</b></span>`;
  ...
  if(!nativeHome){
    ...
    const desktopLocation=sceneHome?`🏠 ${esc(sceneHome.name||"집")} · ${esc(sceneHome.rooms?.[e.room]?.name||"집 안")}`:e.transit?`🚌 ${esc(t("inTransit","이동 중"))}`:place?`📍 ${esc(place.name)} · ${esc(townForEntry(e).name)}`:`📍 ${esc(t("outAndAbout","외출 중"))}`;
  ...
  const hudRoster=`<details class="game-hud-profile"><summary class="game-hud-profile-toggle" data-open-game-hud-roster aria-label="${esc(t("characterPicker","관찰 캐릭터 바꾸기"))}"><span class="game-hud-profile-frame">${profileAvatar(c,"game-hud-current-profile")}<img class="game-hud-profile-ring" src="./assets/home-ui/profile-ring.png" alt=""></span><span class="game-hud-profile-copy"><b>${esc(c.name)}</b><small><em>${esc(c.jobTitle||c.job||t("생활 중","생활 중"))}</em></small></span></summary><section class="game-hud-roster-drawer"><small class="game-hud-roster-kicker">${t("currentTownResidents","이 마을의 캐릭터")}</small><div class="game-hud-roster-options" aria-label="${esc(t("characterPicker","관찰 캐릭터 선택"))}">${localIds.map(id=>{const person=state.characters[id];return `<button type="button" data-home-character="${id}" class="game-hud-button character-picker-button ${id===c.id?"on":""}" style="--picker-theme:${esc(person.theme?.primary||"#176b60")}" title="${esc(person.name)}" aria-label="${esc(person.name)}">${profileAvatar(person)}<small>${esc(person.name)}</small></button>`}).join("")}</div>${otherTowns.length?`<div class="game-hud-town-jump"><b>${t("moveToAnotherTown","다른 마을로 이동")}</b><div>${otherTowns.map(town=>`<button type="button" data-observe-town="${town.id}">${esc(town.name)}</button>`).join("")}</div></div>`:""}</section></details>`;
  const drawerLogLabel=({ko:"서랍 로그",en:"Drawer Log",ja:"ひきだしログ"}[state.uiLanguage||"ko"]||"서랍 로그");
  const statusCard=`<article class="game-hud-moment" data-game-hud-moment><div class="game-hud-moment-head"><button type="button" class="game-hud-moment-title" data-toggle-game-hud-moment aria-expanded="false" aria-label="${esc(drawerLogLabel)} · ${esc(t("expand","펼치기"))}">${esc(drawerLogLabel)}</button><button type="button" class="game-hud-button game-hud-moment-toggle" data-toggle-game-hud-moment aria-expanded="false" data-expand-label="${esc(t("expand","펼치기"))}" data-collapse-label="${esc(t("collapse","접기"))}"><span>${t("expand","펼치기")}</span></button></div><div class="game-hud-moment-body" data-toggle-game-hud-moment role="button" tabindex="0" aria-expanded="false" aria-label="${esc(drawerLogLabel)} · ${esc(t("expand","펼치기"))}"><h1>${esc(e.title)}</h1><p>${esc(e.desc)}</p>${location}</div></article>`;
  const gameHud=`<section class="game-observe-hud scene-tone-${presentation.tone} scene-action-${presentation.actionKind}" data-native-hud-version="4" style="--native-own:${esc(c.theme?.primary||"#176b60")};--native-own-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")};--log-theme:${logTheme}" aria-label="${esc(c.name)}의 관찰 화면"><div class="native-observe-backdrop" style="background-image:url(&quot;${esc(nativeBackground)}&quot;)"></div><div class="native-observe-shade"></div><div class="native-scene-atmosphere atmosphere-${presentation.atmosphere}" aria-hidden="true"></div>${presentation.effects}<div class="game-hud-top">${hudRoster}<time>${new Date().toLocaleTimeString(uiLocale(),{hour:"2-digit",minute:"2-digit"})}</time></div>${gameHudSideMenu("left")}${gameHudSideMenu("right")}<div class="game-hud-stage"><div class="native-character-stage ${stageClasses}" style="--home-visual-scale:${visualScale};${sceneLayoutVars(c,visualMode)}" aria-label="${esc(c.name)} 현재 장면">${sceneActors}</div></div>${statusCard}${gameHudDock()}</section>`;
...
function roomStyle(h,key,layout,mobileLayout){
  ...
  const resolvedMobile=manual||mobileLayout||{x:0,y:0,w:100,h:100},image=room.image,parts=[
    `--room-x:${layout?.x||1}`,
    `--room-y:${layout?.y||1}`,
    `--room-w:${layout?.w||1}`,
    `--room-h:${layout?.h||1}`,
...
function character(){
  ...
  const paneMeta={profile:["프로필","사진·기본 정보·생활 습관","👤"],body:["신체","외형·건강·접근성","✦"],personality:["성격","성향·서사·인지","◈"],taste:["취향 선택","취미·음식·콘텐츠","♡"],worldTaste:["세계관 선호","최애·소지품","⌂"],manage:["사진·색상·배치","이미지·테마색·홈 배치","🎨"]};
  const mobilePaneLabels={worldTaste:"세계관 설정",manage:"사진·색상·배치"};
  const paneButtons=Object.entries(paneMeta).map(([key,[label,help,icon]])=>`<button type="button" data-open-character-pane="${key}" class="character-pane-ribbon ${state.characterPane===key?"on":""}"><span>${icon}</span><b>${t(mobilePaneLabels[key]||label,mobilePaneLabels[key]||label)}</b><small>${t(help,help)}</small></button>`).join("");
  ...
  const draftRows=[["이름",c.name],["나이",c.ageGroup||unknown],["생일",c.birthday||unknown],["성별",c.gender||unknown],["직업 종류",c.jobTitle||c.job||unknown],["키·몸무게",[c.bodyProfile?.height&&`${c.bodyProfile.height}cm`,c.bodyProfile?.weight&&`${c.bodyProfile.weight}kg`].filter(Boolean).join(" · ")||unknown]];
  const mobileProfileDraft=`<section class="mobile-character-profile-draft"><header><button type="button" class="character-draft-back" data-tab="observe" aria-label="${esc(t("돌아가기","돌아가기"))}"></button><b>${t("캐릭터 프로필","캐릭터 프로필")}</b></header><div class="character-draft-sheet"><section class="character-draft-identity">${avatar(c)}<strong>${esc(c.name)}</strong><small>${esc(c.ageGroup||unknown)} · ${esc(c.jobTitle||c.job||unknown)}</small></section><dl>${draftRows.map(([label,value])=>`<div><dt>${t(label,label)}</dt><dd>${esc(value)}</dd></div>`).join("")}</dl></div><nav class="character-draft-actions"><button type="button" class="character-draft-action" data-export-profile>${t("프로필 내보내기","프로필 내보내기")}</button><button type="button" class="character-draft-action" data-save>${t("캐릭터 저장","캐릭터 저장")}</button><button type="button" class="character-draft-action danger" data-delete-character="${c.id}">${t("캐릭터 삭제","캐릭터 삭제")}</button></nav></section>`;
...
function catalog(){
  ...
  return `<section class="panel form catalog-shell"><div class="title"><div><h1>세계관 취향 도감</h1><p>아이콘을 누르면 세부 정보와 편집 항목이 열려요.</p></div><button type="button" class="primary" data-catalog-save>도감 저장</button></div>${sections}</section>`;
}
...
function relationship(){
  ...
  const characterGroups=(state.characterGroups||[]).map(group=>{const members=(group.memberIds||[]).map(id=>state.characters[id]).filter(Boolean);return `<article class="character-group-card"><div>${members.length?members.slice(0,5).map(member=>avatar(member)).join(""):'<span class="avatar">＋</span>'}</div><span><b>${esc(group.name)}</b><small>${members.length?members.map(member=>esc(member.name)).join(" · "):"구성원 없음"}</small></span><button type="button" data-edit-character-group="${esc(group.id)}">편집</button><button type="button" class="danger" data-delete-character-group="${esc(group.id)}">삭제</button></article>`}).join("");
...
Object.assign(UI_TEXT.ja,{
  ...
  "질문·고민·안부와 실제 생활로그가 낮 시간에 도착해요. 알림을 눌러도 갑작스러운 팝업은 열리지 않고, 캐릭터 화면의 ‘오늘의 연락’에서 확인합니다.":"質問・悩み・気遣いと具体的な生活ログが昼間に届きます。通知を押しても突然ポップアップは開かず、キャラクター画面の「今日の連絡」で確認できます。","연락 설정 정리하기":"連絡設定を整理","1. 연락받을 캐릭터":"1. 連絡を受けるキャラクター","2. 빈도와 시간":"2. 頻度と時間","3. 받고 싶은 연락":"3. 受け取りたい連絡","📣 새 버전 업데이트 소식":"📣 新バージョンのお知らせ","업데이트 뒤 한 번, 선택한 캐릭터의 이미지와 이름으로 변경 내용을 알려줘요.":"更新後に一度、選んだキャラクターの名前と画像で変更内容を知らせます。",
...
function townMobile(){
  ...
  return `<section class="mobile-town-shell ${mobileTownEditing?"editing":""} ${panelType?`sheet-open ${panelType}-panel`:""}">${desktopTabs}<div class="mobile-town-hud"><span><small>현재 마을</small><b>${esc(state.world.name)}</b></span><div class="mobile-town-hud-actions">${mobileTownEditing?'<button type="button" data-add-town>+ 새 마을</button>':""}<button type="button" data-mobile-town-edit-toggle class="${mobileTownEditing?"on":""}">${mobileTownEditing?"편집 종료":"편집 모드"}</button></div></div>${mobileTownEditing?`<div class="mobile-town-tools"><button type="button" data-mobile-town-settings>마을 설정</button><button type="button" data-add-place>+ 건물</button></div>`:""}<div class="town-edit"><div class="town-map-scroll"><div class="world"><img src="${state.world.bg}" class="world-bg">${state.world.places.map(placeCard).join("")}${townHomes().map(homeMapCard).join("")}${state.world.places.map(peopleAtPlaceCard).join("")}${townHomes().map(peopleAtHomeCard).join("")}</div></div>${editorPanel}</div>${characterCard}${mobileSwitcher}${buildingDetailDialogs()}</section>`;
}
function view(){
