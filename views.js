import {state,active,characterViewFor} from "./state.js?v=20260804j";
import {eventFor,visibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260804j";
...
function homeCard(id,chars){
  ...
  const mobileRoomEditors=edit?`<section class="mobile-room-editors"><h3>방 꾸미기</h3><p>방을 고쳐도 위의 집 배치가 그대로 유지돼요.</p>${roomKeys.map(key=>{const room=h.rooms?.[key]||{},furniture=FURNITURE[key]||[];return `<details><summary>${esc(room.name||key)}</summary><div class="mobile-room-editor-body"><label>방 이름<input data-room-name="${key}" data-home-id="${id}" value="${esc(room.name||key)}"></label><div class="mobile-room-image-actions"><button data-room-bg="${id}" data-home-id="${id}" data-room="${key}">사진 선택</button><button data-image-url="room" data-id="${id}" data-room="${key}">이미지 주소</button>${room.image?`<button data-clear-room-bg data-home-id="${id}" data-room="${key}">사진 지우기</button>`:""}</div><div class="mobile-room-furniture">${furniture.map(item=>`<button data-furniture="${item}" data-home-id="${id}" data-room="${key}" class="${(room.furniture||[]).includes(item)?"on":""}">${item}</button>`).join("")}</div></div></details>`}).join("")}</section>`:"";
