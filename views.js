import {state,active} from "./state.js?v=20260801a";
import {eventFor,visibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260801a";
...
function placeCard(p){
  return `<button class="place ${p.image?"has-art":""}" style="left:${p.x}%;top:${p.y}%;--place:${p.color};--place-scale:${p.imageScale||1}" data-place="${p.id}">${p.image?`<img class="building-art" src="${esc(p.image)}" alt=""><span class="building-name">${esc(p.name)}</span>`:`<i>${p.emoji}</i><span class="place-label"><b>${esc(p.name)}</b><small>${esc(p.type)}</small></span>`}</button>`;
}
