...
import {state,active,characterViewFor,explicitCharacterViewFor} from "./state.js?v=20260820settings13";
import {eventFor as simulateEventFor,visibleTimeline as simulateVisibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260820settings13";
...
function gameHudSideMenu(side){
  return `<nav class="game-hud-side game-hud-side-${side}" aria-label="${esc(t(side==="left"?"캐릭터와 관계 메뉴":"일정과 설정 메뉴",side==="left"?"캐릭터와 관계 메뉴":"일정과 설정 메뉴"))}">${GAME_HUD_SIDE_TABS[side].map(([key,labelKey,label,icon])=>`<button type="button" data-tab="${key}"><span data-menu-icon="${key}" aria-hidden="true">${icon}</span><small>${t(labelKey,label)}</small></button>`).join("")}</nav>`;
}
function gameHudDock(){
  return `<nav class="game-hud-dock" aria-label="${esc(t("주요 메뉴","주요 메뉴"))}"><button type="button" data-tab="observe" class="on"><span data-menu-icon="observe" aria-hidden="true">◉</span><small>${t("observe","관찰")}</small></button><button type="button" data-tab="home"><span data-menu-icon="home" aria-hidden="true">⌂</span><small>${t("home","집")}</small></button><button type="button" data-open-native-log><span data-menu-icon="log" aria-hidden="true">▤</span><small>${t("todayLog","오늘의 기록")}</small></button><button type="button" data-tab="shop"><span data-menu-icon="shop" aria-hidden="true">♢</span><small>${t("shop","상점")}</small></button><button type="button" data-tab="town" class="game-hud-town-button"><img src="./icons/town-map.svg" alt=""><small>${t("town","마을")}</small></button></nav>`;
}
...
function observe(){
  ...
  const hudRoster=`<details class="game-hud-profile"><summary aria-label="${esc(t("관찰 캐릭터 바꾸기","관찰 캐릭터 바꾸기"))}">${avatar(c)}<span><b>${esc(c.name)}</b><small>${esc(c.jobTitle||c.job||t("생활 중","생활 중"))}</small></span></summary><div class="game-hud-roster" aria-label="${esc(t("관찰 캐릭터 선택","관찰 캐릭터 선택"))}">${state.order.map(id=>{const person=state.characters[id];return `<button type="button" data-home-character="${id}" class="character-picker-button ${id===c.id?"on":""}" style="--picker-theme:${esc(person.theme?.primary||"#176b60")}" title="${esc(person.name)}" aria-label="${esc(person.name)}">${avatar(person)}<small>${esc(person.name)}</small></button>`}).join("")}</div></details>`;
  const statusCard=`<article class="game-hud-moment" data-toggle-native-moment-card role="button" tabindex="0" aria-expanded="false"><div class="game-hud-moment-head"><small>${t("currentMoment","지금 이 순간")}</small><button type="button" data-toggle-native-moment aria-expanded="false" data-label-expand="${t("expand","펼치기")}" data-label-collapse="${t("collapse","접기")}">${t("expand","펼치기")}</button></div><h1>${esc(e.title)}</h1><p>${esc(e.desc)}</p><b>${location}</b></article>`;
  const gameHud=`<section class="game-observe-hud scene-tone-${presentation.tone} scene-action-${presentation.actionKind}" style="--native-own:${esc(c.theme?.primary||"#176b60")};--native-own-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")};--log-theme:${logTheme}" aria-label="${esc(c.name)}의 관찰 화면"><div class="native-observe-backdrop" style="background-image:url(&quot;${esc(nativeBackground)}&quot;)"></div><div class="native-observe-shade"></div><div class="native-scene-atmosphere atmosphere-${presentation.atmosphere}" aria-hidden="true"></div>${presentation.effects}<div class="game-hud-top">${hudRoster}<time>${new Date().toLocaleTimeString(uiLocale(),{hour:"2-digit",minute:"2-digit"})}</time></div>${gameHudSideMenu("left")}${gameHudSideMenu("right")}<div class="game-hud-stage"><div class="native-character-stage ${stageClasses}" style="--home-visual-scale:${visualScale};${sceneLayoutVars(c,visualMode)}" aria-label="${esc(c.name)} 현재 장면">${sceneActors}</div></div>${statusCard}${gameHudDock()}</section>`;
...
function character(){
  ...
  const pendingQuestion=state.dailyQuestion&&!state.dailyQuestion.answered&&state.dailyQuestion.characterId===c.id?state.dailyQuestion:null;
  const contactShortcut=`<button type="button" class="character-contact-shortcut ${pendingQuestion?"has-contact":""}" data-open-character-contact aria-label="오늘의 연락 열기"><span aria-hidden="true">${pendingQuestion?"💌":"✉"}</span><small>연락</small>${pendingQuestion?'<i aria-label="새 연락 있음"></i>':""}</button>`;
  const contactDialog=`<dialog class="character-contact-dialog" data-character-contact-dialog><form method="dialog"><div class="mobile-editor-head"><span><small>TODAY'S CONTACT</small><b>오늘의 연락</b></span><button value="close" aria-label="닫기">×</button></div>${pendingQuestion?`<section class="character-contact-inbox"><span>${avatar(c)}</span><div><h2>${esc(c.name)}에게 오늘의 질문이 도착했어요</h2><p>준비됐을 때 열고 선택하면 실제 생활 일정으로 이어집니다.</p></div><button type="button" class="primary" data-open-daily-question>질문 열기</button></section>`:`<section class="character-contact-inbox is-empty"><span>${avatar(c)}</span><div><h2>아직 도착한 연락이 없어요</h2><p>새 질문과 선택은 이 메뉴에 조용히 도착해요.</p></div></section>`}</form></dialog>`;
  ...
  return `<div class="editor character-editor" style="--character-own:${esc(c.theme?.primary||"#176b60")};--character-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")}">
    ...
    <section class="panel form">
      <section class="mobile-character-dashboard" style="--character-own:${esc(c.theme?.primary||"#176b60")};--character-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")}">
        <div class="mobile-character-top"><div class="mobile-character-strip">${mobileStrip}</div><div>${contactShortcut}<button type="button" data-new ${state.order.length>=limit?"disabled":""}>＋</button><button type="button" data-open-character-reorder>위치 바꾸기</button></div></div>
