...
function character(){
  ...
  const bodyAccessibilityPane=()=>`<section class="character-body-svg-page character-body-accessibility-page" aria-label="${esc(c.name)} ${esc(t("건강·접근성 설정","건강·접근성 설정"))}">
    ...
    ${bodyChoiceDialog}
    <nav class="character-book-page-controls body-controls" aria-label="${esc(t("신체 페이지 이동","신체 페이지 이동"))}"><button type="button" data-character-body-pane="appearance" aria-label="${esc(t("이전 페이지","이전 페이지"))}">◀</button><b>${7+(isCourtWorld()?1:0)}</b><button type="button" data-character-pane="wardrobe" aria-label="${esc(t("다음 페이지","다음 페이지"))}">▶</button></nav>
  </section>`;
  ...
  const fullPane=fullActivePane==="visual"?characterFullOverview(c):fullActivePane==="profile"?profileOverviewPane():fullActivePane==="body"?bodyPane():fullActivePane==="wardrobe"?wardrobePane():fullActivePane==="personality"?personalityBookPane():fullActivePane==="secrets"?secretsPane(state,c,12+(isCourtWorld()?1:0)):fullActivePane==="closet"?closetBookPane():tasteBookPane();
  ...
  const fullPaneMeta=(profileBook?[["visual","사진·색상·배치"],["profile","개요"]]:[["visual","사진·색상·배치"],["profile","개요"],["body","신체"],["wardrobe","의상 취향"],["personality","성격"],["secrets",secretText(state.uiLanguage,"비밀 설정","Secrets","秘密の設定")],["taste","취향·소지품"],["closet","추가 LD 사진"]]);
  ...
  const fullPageEntries=[
    ...
    {key:"secrets",pane:"secrets",get html(){return secretsPane(state,c,12+(isCourtWorld()?1:0))}},
