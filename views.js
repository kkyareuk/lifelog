...
const LD_FIELDS=["ldNeutral","ldJoy","ldSad","ldAngry","ldTired"];
const hasLdArt=c=>LD_FIELDS.some(field=>Boolean(c?.[field]));
const hasSdArt=c=>Boolean(c?.icon||c?.photo);
function ldFieldForTone(tone="neutral"){
  if(tone==="sleep"||tone==="drowsy")return"ldTired";
  if(tone==="sad"||tone==="date-broken"||tone==="date-overwhelmed")return"ldSad";
  if(String(tone).startsWith("fight-")||tone==="interaction-tense")return"ldAngry";
  if(["date-romantic","interaction-playful","interaction-warm","crush-mutual","crush-one-sided"].includes(tone))return"ldJoy";
  return"ldNeutral";
}
function sceneAvatar(c,cls="",tone="neutral",mode="sd"){
  if(mode==="ld"&&hasLdArt(c)){
    const preferred=ldFieldForTone(tone);
    const src=c[preferred]||c.ldNeutral||LD_FIELDS.map(field=>c[field]).find(Boolean);
...
const jobExpansionCard=()=>`<article class="premium-product one-time-product coming-soon-product" data-product-id="job_expansion"><div class="premium-product-heading"><span>출시 준비 중</span><div><small>직업 확장팩</small><h2>더 넓은 직업의 하루</h2></div><b>가격 미정</b></div><p>공연·방송·법조·운송·특수직 등 직업별 전용 일정, 근무 장소와 생활 장면을 준비하고 있어요.</p><button class="premium-buy" type="button" disabled>출시 준비 중</button></article>`;
