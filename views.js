...
function character(){
  ...
  const mobileStrip=state.order.filter(id=>id!==c.id).map(id=>{const x=state.characters[id];return `<button type="button" data-mobile-character-select="${id}" class="character-roster-entry" style="--own:${x.theme.primary}">${avatar(x)}<small>${esc(x.name)}</small></button>`}).join("");
