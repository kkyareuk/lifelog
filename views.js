...
import {state,active,characterViewFor,explicitCharacterViewFor} from "./state.js?v=20260822characterwallet1";
import {eventFor as simulateEventFor,visibleTimeline as simulateVisibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260822characterwallet1";
...
Object.assign(UI_TEXT.en,{"세계관 설정":"Worldview settings","캐릭터 프로필":"Character profile","돌아가기":"Back","미설정":"Not set","나이":"Age","생일":"Birthday","월":"month","일":"day","나이 / 생일":"Age / birthday","키·몸무게":"Height · weight","서랍마을 주민등록증":"Drawer Village Resident Card","이곳에 사진을 넣어주세요":"Place a photo here","선택됨":"Selected","선택된 캐릭터 바꾸기":"Change selected character","위치 바꾸기":"Reorder","새 캐릭터 만들기":"Create character","연도 없이 월과 일을 골라 주세요. 생일 당일 생활과 달력에 반영돼요.":"Choose the month and day without a year. It appears in that day's life and calendar."});
Object.assign(UI_TEXT.ja,{"세계관 설정":"世界観設定","캐릭터 프로필":"キャラクタープロフィール","돌아가기":"戻る","미설정":"未設定","나이":"年齢","생일":"誕生日","월":"月","일":"日","나이 / 생일":"年齢／誕生日","키·몸무게":"身長・体重","서랍마을 주민등록증":"ひきだし村 住民登録証","이곳에 사진을 넣어주세요":"ここに写真を入れてください","선택됨":"選択中","선택된 캐릭터 바꾸기":"選択中の人物を変更","위치 바꾸기":"並べ替え","새 캐릭터 만들기":"人物を作成","연도 없이 월과 일을 골라 주세요. 생일 당일 생활과 달력에 반영돼요.":"年を入れずに月日を選んでください。誕生日当日の生活とカレンダーに反映されます。"});
const UI_TEXT_MORE={
  en:{
...
const GAME_HUD_SIDE_TABS={
  left:[
    ...
    {key:"catalog",labelKey:"catalog",label:"취향 사전",asset:"catalog.png"},
...
function observe(){
  ...
  if(!localId){
    if(nativeHome){
      ...
      return `<section class="game-observe-hud game-observe-empty" data-native-hud-version="4" data-home-ui-theme="${theme.id}" style="${homeUiThemeStyle()}"><div class="native-observe-backdrop" style="background-image:url(&quot;${esc(state.world?.bg||TOWN_BACKGROUND)}&quot;)"></div><div class="native-observe-shade"></div><div class="game-hud-top game-hud-empty-top"><div><b>${esc(state.world?.name||t("town","마을"))}</b><small>이 마을에 사는 캐릭터가 없어요</small></div><time>${new Date().toLocaleTimeString(uiLocale(),{hour:"2-digit",minute:"2-digit"})}</time></div>${gameHudSideMenu("left")}${gameHudSideMenu("right")}<div class="game-observe-empty-copy"><span>🏙️</span><h1>이 마을에 사는 캐릭터가 없어요</h1><p>캐릭터 화면에서 생활하는 마을을 지정해 주세요.</p><button class="primary" data-tab="character">캐릭터 설정 열기</button></div>${gameHudDock()}</section>${buildingDetailDialogs()}`;
    }
...
function character(){
  ...
  const paneButtons=Object.entries(paneMeta).map(([key,[label,help]])=>`<button type="button" data-open-character-pane="${key}" class="character-pane-ribbon ${state.characterPane===key?"on":""}"><b>${t(mobilePaneLabels[key]||label,mobilePaneLabels[key]||label)}</b><small>${t(help,help)}</small></button>`).join("");
  ...
  const draftRows=[["이름",c.name],["나이 / 생일",`${c.ageGroup||unknown} / ${birthdayLabel}`],["성별",c.gender||unknown],["직업 종류",c.jobTitle||c.job||unknown],["키·몸무게",[c.bodyProfile?.height&&`${c.bodyProfile.height}cm`,c.bodyProfile?.weight&&`${c.bodyProfile.weight}kg`].filter(Boolean).join(" · ")||unknown]];
  const selectedIcon=c.icon?`<img src="${esc(c.icon)}" alt="${esc(c.name)}">`:"";
  ...
  const mobileProfileDraft=`<section class="mobile-character-profile-draft">
    ...
    <button type="button" class="character-wallet-selected" data-toggle-character-roster aria-label="${esc(t("선택된 캐릭터 바꾸기","선택된 캐릭터 바꾸기"))}"><span>${selectedIcon}</span><b>${t("선택됨","선택됨")}</b></button>
    <div class="character-wallet-art" aria-hidden="true"></div>
    <section class="character-registration-card">
      <div class="character-registration-photo">${profilePhoto}</div>
    ...
    <div class="character-wallet-roster" data-character-roster hidden><div class="mobile-character-strip">${mobileStrip}</div><span><button type="button" data-new ${state.order.length>=limit?"disabled":""}>＋</button><button type="button" data-open-character-reorder>${t("위치 바꾸기","위치 바꾸기")}</button></span></div>
  </section>`;
  return `<div class="editor character-editor" style="--character-own:${esc(c.theme?.primary||"#176b60")};--character-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")}">
    ...
    <section class="panel form">
      <section class="mobile-character-dashboard" style="--character-own:${esc(c.theme?.primary||"#176b60")};--character-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")}">
        ...
        <div class="mobile-character-pane-grid">${paneButtons}<button type="button" class="character-pane-add" data-new ${state.order.length>=limit?"disabled":""} aria-label="${esc(t("새 캐릭터 만들기","새 캐릭터 만들기"))}"></button></div>
      </section>
...
function mailbox(){
  ...
  const giftCopy={
    en:{title:"Send a gift",help:"Choose a character and an item from the taste dictionary. The recipient receives it and both characters remember the exchange.",target:"Recipient",item:"Gift",send:"Send gift",noTarget:"Add another character first",noItem:"Add an item to the taste dictionary first"},
    ja:{title:"プレゼントを送る",help:"相手と好み図鑑の品物を選びます。相手の持ち物に追加され、二人の記録にも残ります。",target:"送り先",item:"プレゼント",send:"プレゼントを送る",noTarget:"ほかのキャラクターを追加してください",noItem:"好み図鑑に品物を追加してください"},
    ko:{title:"선물 보내기",help:"받을 캐릭터와 취향 사전의 물건을 고르세요. 상대의 소지품에 추가되고 두 캐릭터의 기록에도 남아요.",target:"받을 캐릭터",item:"선물할 물건",send:"선물 보내기",noTarget:"다른 캐릭터를 먼저 추가해 주세요",noItem:"취향 사전에 물건을 먼저 추가해 주세요"}
  }[state.uiLanguage||"ko"];
