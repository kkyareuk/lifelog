...
function roomFurnitureMarkup(homeId,roomKey,room,edit,bedStates=new Map()){
  ...
  }).join("")}${placements.filter(p=>furnitureSprite(p)?.frame).map(p=>`<span class="chair-frame-overlay" data-chair-frame="${esc(p.id)}" style="${furniturePlacementStyle(p,furnitureFootprint(p.item))}"><img class="furniture-sprite" src="${furnitureSprite(p).frame}" alt=""></span>`).join("")}</div>`;
}
...
function homeLifePersonMarkup(character,event,agent,room,roomKey,index,bedSlot=-1,options={}){
  ...
  return `<div class="home-person ${scene.remote?'is-remote-contact':''} ${!walking&&/씻|샤워|목욕|washing|shower|bath|洗|入浴/i.test(title)?"is-washing":""} ${lifeClass} ${scene.contactRejected?'is-contact-declined':(scene.meetingKind==='hug'||/포옹|안아|hug|抱き|抱擁/i.test(scene.title||''))?'is-hugging':scene.meetingKind==='affection'?'is-affection':''} scene-action-${esc(actionKind)} ${sleeping?"is-sleeping":""}" data-sleep-style="${sleeping?sleepMotion.style:""}" style="--sleep-duration:${sleepMotion.duration}s;--sleep-delay:-${(now/1000+index*.71)%sleepMotion.duration}s;${lifeStyle}--home-float-delay:${-(index%4)*.37}s" role="button" tabindex="0" aria-label="${esc(`${character.name} · ${title}`)}" data-seat-id="${agent?.phase==="using"&&["의자","소파"].includes(agent?.item)?esc(agent.furnitureId):""}" data-couple-bed-id="${esc(options.bedPlacement?.id||"")}" data-bed-slot="${bedSlot}" data-home-person="${esc(character.id)}" data-home-occupant="character" data-character-id="${esc(character.id)}" data-occupant-name="${esc(character.name)}" data-occupant-title="${esc(title)}" data-occupant-desc="${esc(scene?.desc||"")}" data-occupant-room="${esc(room.name||roomKey)}"><span class="home-person-visual">${avatar(character)}${actionProp}${scene.remote?'<i class="remote-contact-prop" aria-hidden="true">📱</i>':''}${scene.gossipReaction?`<i class="home-person-chat-bubble" aria-hidden="true">${scene.gossipReaction==='angry'?'💢':scene.gossipReaction==='surprised'?'!':'…'}</i>`:speech}${sleeping&&!quietBedSleep?'<i class="room-sleep-mark" aria-hidden="true">ZZ</i>':""}</span>${hideStatus?"":`<span class="home-person-status"><b>${esc(character.name)}</b><small>${esc(activity)}</small></span>`}</div>`;
}
