import {state,active,characterViewFor,explicitCharacterViewFor} from "./state.js?v=20260820layoutroot1";
import {eventFor as simulateEventFor,visibleTimeline as simulateVisibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260820layoutroot1";
...
function homeCard(id,chars){
  ...
  const roomHtml=visibleRoomKeys.map(key=>{
    ...
    return `<div class="room room-${esc(room.type||key)} ${edit?"room-edit-target":""}" ${roomStyle(h,key,packedRooms.items[key],mobileRooms[key])} ${editAttributes}>
      ...
      <div class="room-people">${shownPeople.map(c=>{const e=sceneFor(c),title=e?.title||"집에서 시간을 보내는 중",text=`${title} ${e?.desc||""} ${e?.mood||""}`,presentation=nativeScenePresentation(c,e,"sd"),sleeping=presentation.actionKind==="sleep",actionProp=nativeSceneActionProp(c,e,presentation.actionKind,text,true);return `<div class="home-person scene-action-${esc(presentation.actionKind)} ${sleeping?"is-sleeping":""}" role="button" tabindex="0" aria-label="${esc(`${c.name} · ${title}`)}" data-home-occupant="character" data-character-id="${c.id}" data-occupant-name="${esc(c.name)}" data-occupant-title="${esc(title)}" data-occupant-desc="${esc(e?.desc||"")}" data-occupant-room="${esc(room.name||key)}">${avatar(c)}${actionProp}${sleeping?'<i class="room-sleep-mark" aria-hidden="true">ZZ</i>':""}<span class="home-person-status"><b>${esc(c.name)}</b><small>${esc(title)}</small></span></div>`}).join("")}</div>
...
function character(){
  ...
  const themeColorField=(field,label,value)=>`<label class="theme-custom-color"><b>${label}</b><input type="color" data-color="${field}" value="${esc(value)}" aria-label="${label} 색상표"><input type="text" data-theme-hex="${field}" value="${esc(value)}" maxlength="7" inputmode="text" aria-label="${label} HEX 코드"></label><div class="character-theme-palette"><small>${label} 빠른 색상</small><div>${themePalette.map(color=>`<button type="button" class="theme-swatch ${String(value).toUpperCase()===color?"selected":""}" data-theme-swatch="${field}" data-color-value="${color}" style="--swatch:${color};background:${color}!important;background-image:linear-gradient(${color},${color})!important" aria-label="${color}"></button>`).join("")}</div></div>`;
  const homeVisualChoice=`<section class="character-manage-theme"><div><h3>캐릭터 테마색</h3><p>색상표에서 직접 고르거나 6자리 HEX 코드를 입력해요. 캐릭터 선택 효과와 강조색에 함께 적용됩니다.</p></div>${themeColorField("primary","주 색상",c.theme?.primary||"#176B60")}${themeColorField("secondary","보조 색상",c.theme?.secondary||"#D4A373")}<label class="theme-gradient-toggle"><input type="checkbox" data-gradient ${c.theme?.gradient!==false?"checked":""}> 두 색상을 그라데이션으로 사용</label></section>`;
...
function routine(){
  ...
  const calendarCells=Array.from({length:Math.ceil((firstDay+lastDate)/7)*7},(_,index)=>{const date=index-firstDay+1;if(date<1||date>lastDate)return '<section class="monthly-day is-empty" aria-hidden="true"></section>';const dateKey=`${monthKey}-${String(date).padStart(2,"0")}`,dayItems=monthItems.filter(item=>item.date===dateKey);return `<section class="monthly-day"><h3><b>${date}</b><small>${days[index%7]}</small></h3>${dayItems.map(item=>`<article class="routine-block"><b>${esc(item.start)}–${esc(item.end)}</b><strong>${esc(item.title)}</strong><small>${esc(item.type)}</small><div class="routine-actions"><button data-edit-monthly-routine="${item.id}">편집</button><button class="danger" data-delete-monthly-routine="${item.id}">삭제</button></div></article>`).join("")||'<small class="monthly-empty">일정 없음</small>'}</section>`}).join("");
  const monthly=`<div class="monthly-heading"><button type="button" data-routine-month-step="-1" aria-label="이전 달">‹</button><h2>${year}년 ${month}월</h2><button type="button" data-routine-month-step="1" aria-label="다음 달">›</button></div><div class="monthly-calendar">${calendarCells}</div>`;
  const monthlyView=state.routineView==="monthly";
  return `<section class="panel form routine-shell"><div class="title"><div><h1>${monthlyView?"월간 일정":"주간 일정"}</h1><p>${monthlyView?"날짜가 정해진 약속과 한 번만 있는 일정을 달력에 기록해요.":"반복되는 회사 일정, 수업, 데이트와 개인 일정을 요일별로 지정해요."}</p></div><button class="primary" ${monthlyView?"data-add-monthly-routine":"data-add-routine"}>+ 일정 추가</button></div>${toolbar}<div class="routine-view-tabs"><button type="button" data-routine-view="weekly" class="${monthlyView?"":"on"}">주간 일정</button><button type="button" data-routine-view="monthly" class="${monthlyView?"on":""}">월간 일정</button></div>${monthlyView?monthly:table}</section>`;
}
...
function settingsContent(){
  ...
  const notifications=`<section class="setting-card character-notification-card"><div class="notification-setting-head"><span><small>CHARACTER CONTACT</small><h2>캐릭터 연락 알림</h2><p>질문·고민·안부와 실제 생활로그가 낮 시간에 도착해요. 알림을 눌러도 갑작스러운 팝업은 열리지 않고, 캐릭터 화면의 ‘오늘의 연락’에서 확인합니다.</p></span><button type="button" class="${state.characterNotificationsEnabled?"danger":"primary"}" data-character-notification-toggle>${state.characterNotificationsEnabled?"알림 끄기":"알림 켜기"}</button></div><p class="notification-permission-state" data-character-notification-status>${state.characterNotificationsEnabled?"알림 사용 중":state.characterNotificationConsent==="granted"?"알림 꺼짐 · 권한은 유지됨":state.characterNotificationConsent==="denied"?"휴대폰에서 알림이 거부됨":"아직 알림을 요청하지 않음"}</p><details class="notification-settings-detail" ${state.characterNotificationsEnabled?"open":""}><summary>연락 설정 정리하기</summary><div class="notification-settings-body"><h3>1. 연락받을 캐릭터</h3><p>여러 명을 고르면 같은 캐릭터가 연달아 나오지 않게 번갈아 연락해요.</p><div class="notification-character-grid">${noticeCharacters}</div><h3>2. 빈도와 시간</h3><div class="notification-setting-grid"><label>연락 빈도<select data-character-notification-setting="frequency"><option value="light" ${noticeSettings.frequency==="light"?"selected":""}>가끔 · 주 3회</option><option value="daily" ${noticeSettings.frequency!=="light"&&noticeSettings.frequency!=="lively"?"selected":""}>하루 1회</option><option value="lively" ${noticeSettings.frequency==="lively"?"selected":""}>활발하게 · 하루 2회</option></select></label><label>알림 말투<select data-character-notification-setting="voiceMode"><option value="mixed" ${noticeSettings.voiceMode!=="character"&&noticeSettings.voiceMode!=="concise"?"selected":""}>섞어서 · 말투와 담백한 문장</option><option value="character" ${noticeSettings.voiceMode==="character"?"selected":""}>캐릭터 말투를 적극 반영</option><option value="concise" ${noticeSettings.voiceMode==="concise"?"selected":""}>담백하게 · 말투 연출 최소화</option></select></label><label>연락 시작 시간<select data-character-notification-setting="startHour">${noticeHourOptions(9,16,noticeSettings.startHour||10)}</select></label><label>연락 종료 시간<select data-character-notification-setting="endHour">${noticeHourOptions(12,21,noticeSettings.endHour||18)}</select></label></div><h3>3. 받고 싶은 연락</h3><div class="notification-kind-grid">${noticeKinds.map(([kind,label])=>`<label class="${(noticeSettings.contentKinds||[]).includes(kind)?"on":""}"><input type="checkbox" data-character-notification-kind="${kind}" ${(noticeSettings.contentKinds||[]).includes(kind)?"checked":""}><span>${label}</span></label>`).join("")}</div><label class="notification-update-option"><input type="checkbox" data-character-update-notices ${noticeSettings.updateNotices!==false?"checked":""}><span><b>📣 새 버전 업데이트 소식</b><small>업데이트 뒤 한 번, 선택한 캐릭터의 이미지와 이름으로 변경 내용을 알려줘요.</small></span></label><p>생활로그는 관찰 기록 문장 그대로 보내고 말투를 입히지 않아요. 그 밖의 연락은 선택한 말투 설정을 따르며, 같은 문구와 캐릭터가 짧은 기간에 반복되지 않게 조절해요.</p><button type="button" data-character-notification-test ${state.characterNotificationsEnabled?"":"disabled"}>5초 뒤 시험 알림 보내기</button></div></details><small>알림의 작은 상태표시 아이콘은 Android 규칙상 앱 아이콘이며, 본문 옆 큰 아이콘에는 연락한 캐릭터의 등록 이미지를 표시해요. 처음 켤 때 설명 뒤 Android 공식 허용창이 한 번 나타나며 언제든 여기나 휴대폰 설정에서 끌 수 있어요.</small></section>`;
  ...
  return `<section class="panel form settings-shell"><h1>${t("settings","설정")}</h1>${sync}${notifications}${colorMode}${visualThemeSettings()}${fontSettings()}${ownerNameSettings()}${homeCharacterDisplay}${map}${language}${backup}${feedback}${guide}${appInfo}<button data-reset>모든 데이터 초기화</button></section>`;
}
Object.assign(UI_TEXT.en,{
...
Object.assign(UI_TEXT.ja,{
  ...
  "주간 일정":"週間予定","월간 일정":"月間予定","날짜가 정해진 약속과 한 번만 있는 일정을 달력에 기록해요.":"日付のある約束や一度だけの予定をカレンダーに記録します。","반복되는 회사 일정, 수업, 데이트와 개인 일정을 요일별로 지정해요.":"繰り返す仕事・授業・デート・個人予定を曜日ごとに設定します。","월간 일정 편집":"月間予定を編集","날짜":"日付","이전 달":"前の月","다음 달":"次の月","일정 없음":"予定なし",
...
function townMobile(){
  ...
  return `<section class="mobile-town-shell ${mobileTownEditing?"editing":""} ${panelType?`sheet-open ${panelType}-panel`:""}">${desktopTabs}<div class="mobile-town-hud"><span><small>현재 마을</small><b>${esc(state.world.name)}</b></span><div class="mobile-town-hud-actions"><button type="button" data-add-town>+ 새 마을</button><button type="button" data-mobile-town-edit-toggle class="${mobileTownEditing?"on":""}">${mobileTownEditing?"편집 종료":"편집 모드"}</button></div></div>${mobileTownEditing?`<div class="mobile-town-tools"><button type="button" data-mobile-town-settings>마을 설정</button><button type="button" data-add-place>+ 건물</button></div>`:""}<div class="town-edit"><div class="town-map-scroll"><div class="world"><img src="${state.world.bg}" class="world-bg">${state.world.places.map(placeCard).join("")}${townHomes().map(homeMapCard).join("")}${state.world.places.map(peopleAtPlaceCard).join("")}${townHomes().map(peopleAtHomeCard).join("")}</div></div><aside class="panel form town-editor-panel"><div class="mobile-town-sheet-head"><span><small>${panelType==="world"?"TOWN SETTINGS":"BUILDING SETTINGS"}</small><b>${panelType==="world"?esc(state.world.name):esc(selectedPlace?.name||"건물 편집")}</b></span><button type="button" data-mobile-town-close aria-label="편집 창 닫기">×</button></div>${generalEditor}<div class="place-editor">${placeEditors}</div></aside></div>${characterCard}${mobileSwitcher}${buildingDetailDialogs()}</section>`;
}
function view(){
