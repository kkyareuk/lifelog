...
import {state,active,characterViewFor,explicitCharacterViewFor} from "./state.js?v=20260822emptytownhud1";
import {eventFor as simulateEventFor,visibleTimeline as simulateVisibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260822emptytownhud1";
...
function observe(){
  ...
  if(!localId){
    if(nativeHome)return `<section class="game-observe-hud game-observe-empty" data-native-hud-version="4" aria-label="${esc(t("observe","관찰"))}"><div class="native-observe-backdrop" style="background-image:url(&quot;${esc(state.world?.bg||TOWN_BACKGROUND)}&quot;)"></div><div class="native-observe-shade"></div>${gameHudSideMenu("left")}${gameHudSideMenu("right")}<div class="game-observe-empty-copy"><span>🏙️</span><h1>${t("emptyTownTitle","이 마을에 사는 캐릭터가 없어요")}</h1><p>${t("emptyTownHelp","캐릭터 화면에서 생활하는 마을을 지정해 주세요.")}</p><button class="primary" data-tab="character">${t("openCharacterSettings","캐릭터 설정 열기")}</button></div>${gameHudDock()}</section>${buildingDetailDialogs()}`;
