...
import {state,active,characterViewFor,explicitCharacterViewFor} from "./state.js?v=20260901wardrobe188";
import {renderDictionary,itemArt} from "./dictionary.js?v=20260901wardrobe188";
import {PLACEMENTS,characterPlacement,orderAnimationCharacters} from "./character-placement.js?v=20260901wardrobe188";
import {characterMood} from "./character-mood.js?v=20260901wardrobe188";
import {createContactMailbox} from "./notification-mail.js?v=20260901wardrobe188";
import {dictionaryCopy} from "./dictionary-copy.js?v=20260901wardrobe188";
import {eventFor as simulateEventFor,visibleTimeline as simulateVisibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260901wardrobe188";
import {SPEECH_STYLE_OPTIONS} from "./speech-styles.js?v=20260901wardrobe188";
import {furnitureFootprint,furnitureIcon,furnitureLabel,furniturePropIcon,normalizeFurniturePlacements,supportsFurnitureProps} from "./furniture-layout.js?v=20260901wardrobe188";
import {homeSurfaceImage,normalizeHomeSurface,normalizeWallSurface,wallSurfaceImage} from "./home-surfaces.js?v=20260901wardrobe188";
import {TOWN_TYPE_SUBTYPES,TOWN_TYPES,TOWN_REPUTATIONS,TOWN_FAME_LEVELS,TOWN_TERRAINS,TOWN_TRANSPORTS} from "./town-profile.js?v=20260901wardrobe188";
import {normalizeBuildingLighting,buildingLightsOn,scheduleTownLighting} from "./town-lighting.js?v=20260901wardrobe188";
...
function wardrobeSceneArt(c,entry,field){
  ...
  const mood=characterMood(c,scene,state).label,moodTag=/유혹/.test(copy)?"유혹적임":/만족/.test(mood)?"만족함":/들뜸|신남/.test(mood)?"들뜸":/기분 좋|기쁨/.test(mood)?"기분 좋음":/지루/.test(mood)?"지루함":/긴장/.test(mood)?"긴장함":/화/.test(mood)?"화남":/슬픔|가라앉/.test(mood)?"슬픔":/피곤/.test(mood)?"피곤함":"평온함";
  const month=new Date().getMonth()+1,warmth=[12,1,2].includes(month)?["따뜻함","매우 따뜻함"]:[6,7,8].includes(month)?["시원함","매우 시원함"]:["보통","따뜻함","시원함"];
  const scheduled=[...(state.routines?.[c.id]||[]),...(state.monthlyRoutines?.[c.id]||[])].find(value=>String(value.id)===String(scene.routineId)),place=(state.world?.places||[]).find(value=>value.id===scene.placeId),codes=[place?.dressCode,scheduled?.dressCode].filter(code=>code?.enabled),dress={colors:[...new Set(codes.flatMap(code=>code.colors||[]))],materials:[...new Set(codes.flatMap(code=>code.materials||[]))],flairs:[...new Set(codes.flatMap(code=>code.flairs||[]))],formality:codes.map(code=>code.formality).find(value=>value&&value!=="지정 안 함")||"",requiredUniform:codes.some(code=>code.requiredUniform)};
...
function sceneAvatar(c,cls="",tone="neutral",mode="sd",entry=null){
  ...
  return `<img class="sprite ${cls}" src="${esc(src)}" alt="${esc(c.name)} 아이콘">`;
}
function header(){
...
function buildingDressCodeControls(p){
  const code=p.dressCode||{enabled:false,colors:[],materials:[],flairs:[],formality:"지정 안 함",requiredUniform:false};
  ...
  return `<details class="building-dress-code"><summary>${t("드레스코드","드레스코드")} · ${code.enabled?t("사용 중","사용 중"):t("사용 안 함","사용 안 함")}</summary><div><label class="book-check-field"><span>${t("이 건물의 드레스코드 사용","이 건물의 드레스코드 사용")}</span><input type="checkbox" data-place-dress-enabled="${p.id}" ${code.enabled?"checked":""}></label><label>${t("격식 정도","격식 정도")}<select data-place-dress-formality="${p.id}">${["지정 안 함","매우 편한 차림","캐주얼","단정한 차림","격식 있음","최고 격식"].map(value=>`<option ${value===code.formality?"selected":""}>${t(value,value)}</option>`).join("")}</select></label><label class="book-check-field"><span>${t("필수 유니폼","필수 유니폼")}</span><input type="checkbox" data-place-dress-uniform="${p.id}" ${code.requiredUniform?"checked":""}></label><h5>${t("허용 색 · 여러 개 선택","허용 색 · 여러 개 선택")}</h5>${group("colors",["검정","흰색","아이보리","회색","갈색","베이지","빨강","노랑","초록","파랑","남색","보라","분홍"])}<h5>${t("권장 재질 · 여러 개 선택","권장 재질 · 여러 개 선택")}</h5>${group("materials",["면","데님","니트","울","가죽","실크","린넨","벨벳","레이스"])}<h5>${t("분위기 · 여러 개 선택","분위기 · 여러 개 선택")}</h5>${group("flairs",["미니멀","단정함","편안함","캐주얼","스포티","빈티지","러블리","우아함","화려함","정장","유니폼","파티용"])}</div></details>`;
}
...
function nativeScenePresentation(c,entry,visualMode="sd"){
  ...
  const lineupHtml=companions.length?`<span class="native-scene-lineup ${coResidentConversation?"is-conversation":""} ${sceneParticipants.length===2?"is-pair":""}" style="--scene-count:${sceneParticipants.length}" aria-label="${esc(sceneParticipants.map(person=>person.name).join(", "))}">${sceneParticipants.map((person,index)=>{
    ...
    return `<span class="native-scene-lineup-person ${walkStyleClassFor(person)} ${pairSlot} ${person.id===c.id?"is-current is-selected":""} ${visualMode==="ld"&&hasLdArt(person,personEntry)?"is-ld":""}" style="--scene-index:${index};--scene-delay:${delay}s;--scene-duration:${duration}s;--person-visual-scale:${personVisualScale};${sceneLayoutVars(person,visualMode)}">${sceneAvatar(person,"native-scene-lineup-avatar",tone,visualMode,personEntry)}${actionProp}${sleepBadge}${tone==="date-overwhelmed"&&person.id===c.id?'<b class="native-character-sweat" aria-hidden="true">💧</b>':""}<small>${esc(person.name)}</small></span>`;
  }).join("")}</span>`:"";
...
function townActionPresentation(entry,place=null){
  ...
  if(/청소|씻|욕실|정돈/.test(copy))return {kind:"clean",icon:"🫧",label:entry?.title||"정리 중"};
...
function characterHomeLayoutEditor(c){
  ...
  const sdArt=previewSd
    ?`<img src="${esc(previewSd)}" alt="${esc(c.name)} SD 미리보기">`
