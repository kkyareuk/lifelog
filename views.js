...
function discoveryStatistics(characters){
 const c=characters.find(c=>c.id===state.statisticsCharacterId)||characters.find(c=>c.id===state.activeCharacterId)||characters[0];if(!c)return '';
 const rows=Object.entries(DISCOVERY_AXES).map(([field,axis])=>{const m=discoveryMetric(c,field);const value=m.value===null?t('직접 설정','Manual setting','直接設定'):Math.round(m.value)+(m.categorical?'%':' / 100');
 const summary=m.categorical?t('현재 주로 나타나는 반응: ','Current response: ','現在よく現れる反応：')+t(m.label,m.label):t('현재 성향: ','Current tendency: ','現在の傾向：')+t(m.label,m.label);
 return `<tr><th>${esc(t(axis.label,axis.label))}</th><td>${value}</td><td>${esc(summary)}</td></tr>`;}).join('');
 return `<section class="statistics-card discovery-statistics"><h2>${t('캐릭터 성향 점수','Character trait scores','キャラクターの傾向スコア')}</h2><select data-discovery-stat-character aria-label="${t('캐릭터 선택','Choose character','キャラクターを選択')}">${characters.map(person=>`<option value="${esc(person.id)}" ${c.id===person.id?'selected':''}>${esc(person.name)}</option>`).join('')}</select><p>${t('숫자는 성향의 방향을 나타내며 좋고 나쁨의 점수가 아니에요. 반응 유형은 선택 비중(%)으로 표시해요.','Scores show a direction, not a rating of good or bad. Response types show their share (%).','数値は傾向の方向で、良し悪しの評価ではありません。反応の種類は割合（%）で表示します。')}</p><table><tbody>${rows}</tbody></table></section>`;
}
