...
Object.assign(UI_TEXT.en,{"식재료":"Ingredients","출퇴근 이동수단 · 여러 개 선택 가능":"Commute methods · select multiple","자차":"Own car","대중교통":"Public transit","버스":"Bus","지하철":"Subway","택시":"Taxi","도보":"Walking","자전거":"Bicycle","차 소유주":"Car owner","가족 공동 차량":"Shared family car","지도 외관 이미지 바꾸기":"Change map exterior image","집 설정 열기":"Open home settings"});
Object.assign(UI_TEXT.ja,{"식재료":"食材","출퇴근 이동수단 · 여러 개 선택 가능":"通勤手段・複数選択可","자차":"自家用車","대중교통":"公共交通機関","버스":"バス","지하철":"地下鉄","택시":"タクシー","도보":"徒歩","자전거":"自転車","차 소유주":"車の所有者","가족 공동 차량":"家族共用車","지도 외관 이미지 바꾸기":"地図の外観画像を変更","집 설정 열기":"家の設定を開く"});
Object.assign(UI_TEXT.en,{
...
function observe(){
  ...
  const movementClass=e.transit?`is-scene-moving ${e.movementKind==="jog"?"is-scene-jogging":"is-scene-transit"}`:"";
...
function character(){
  ...
  const overviewPane=["basic","life","attraction"].includes(state.characterOverviewPane)?state.characterOverviewPane:"basic";
  ...
  const overviewSubtabs=`<nav class="character-overview-subtabs" aria-label="${esc(t("개요 설정 분류","개요 설정 분류"))}">${[["basic","기본"],["life","생활"],["attraction","이끌림"]].map(([key,label])=>`<button type="button" data-character-overview-pane="${key}" class="${overviewPane===key?"on":""}" ${overviewPane===key?'aria-current="page"':""}><span>${t(label,label)}</span></button>`).join("")}</nav>`;
  const overviewContent=overviewPane==="basic"?overviewBasic:overviewPane==="life"?`<section class="character-overview-scroll-pane">${townAssignment(c)}${profile}<section class="settings-complete-group">${profileAdvancedFields}<section class="setting-card character-lifestyle-settings"><h2>${t("운전·흡연·주량","운전·흡연·주량")}</h2><div class="fields lifestyle-profile-fields">${lifestyleSelect("운전면허·운전 경험","driverLicense",["면허 없음","면허만 있음 · 운전하지 않음","초보운전","가끔 운전함","운전에 익숙함","장거리·야간 운전도 익숙함"],c.driverLicense||"면허 없음")}${lifestyleSelect("흡연 여부","smokingStatus",["설정하지 않음","비흡연","금연 중","가끔 흡연","전자담배 사용","흡연"],c.smokingStatus||"설정하지 않음")}${lifestyleSelect("주량","alcoholTolerance",["설정하지 않음","마시지 않음","한두 모금","매우 약함","약한 편","보통","강한 편","매우 강함"],c.alcoholTolerance||"설정하지 않음")}</div>${commuteModes}</section></section></section>`:`<section class="character-overview-scroll-pane">${profileAttractionSettings(c)}</section>`;
  const profileOverviewPane=`<section class="character-profile-overview-page">${overviewSubtabs}${overviewContent}</section>`;
