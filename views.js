...
function personCard(c){
  const e=eventFor(c);if(e.home||(e.townId&&e.townId!==state.activeTownId))return"";
  ...
  return `<button class="person ${state.mapCharacterLabelMode==="name"?"show-name":"icon-only"}" data-person="${c.id}" title="${esc(c.name)} · ${esc(e.title)}" style="left:calc(${p.x}% + ${off[0]/12}%);top:calc(${p.y}% + ${off[1]/6.76}%)">${avatar(c)}${state.mapCharacterLabelMode==="name"?`<span>${esc(c.name)}</span>`:""}</button>`;
}
function observe(){
...
function homeCard(id,chars){
  ...
  const residentEditor=edit?`<section class="resident-editor"><h3>함께 사는 캐릭터</h3><div>${state.order.map(cid=>{const c=state.characters[cid],on=c.homeId===id;return `<div class="resident-setting"><button data-home-resident="${cid}" data-home-id="${id}" class="${on?"on":""}">${avatar(c)} ${esc(c.name)}</button>${on?`<label>집이 있는 마을<select data-home-town="${cid}">${state.towns.map(t=>`<option value="${t.id}" ${t.id===c.townId?"selected":""}>${esc(t.name)}</option>`).join("")}</select></label>`:""}</div>`}).join("")}</div><small>거주 마을은 집 편집에서 정해요. 여러 명을 선택할 수 있고 취향은 서로 섞이지 않아요.</small></section>`:"";
...
function character(){
  ...
  const movieGenres=[...new Set(Object.values(VIDEO_GENRES).flat())],gameGenres=DETAIL_OPTIONS.game;
  const storyGenres=["로맨스","코미디","액션","판타지","SF","스릴러","공포","미스터리","범죄","드라마","시대극","일상","청춘","가족","모험"];
  const taste=`<h2>${esc(c.name)}의 취향 선택</h2><p>작품과 물건의 단순 카테고리가 아니라 실제로 좋아하는 세부 취향을 골라 주세요.</p>${chips("관심사",INTERESTS,c.interests||[],"interests")}${chips("취미",HOBBIES,c.hobbies||[],"hobbies")}${chips("음식",FOOD_PREFERENCES,c.foodPreferences||[],"foodPreferences")}${chips("좋아하는 음료",DRINKS,c.drinks||[],"drinks")}${chips("좋아하는 장르",storyGenres,c.favoriteStoryGenres||[],"favoriteStoryGenres")}${chips("좋아하는 음악 장르",MUSIC,c.musicGenres||[],"musicGenres")}${chips("좋아하는 패션 스타일",DETAIL_OPTIONS.fashion,c.favoriteFashionStyles||[],"favoriteFashionStyles")}${chips("좋아하는 영상 장르",movieGenres,c.favoriteVideoGenres||[],"favoriteVideoGenres")}${chips("좋아하는 게임 장르",gameGenres,c.favoriteGameGenres||[],"favoriteGameGenres")}${chips("좋아하는 향 계열",PERFUME_NOTES,c.favoriteScentNotes||[],"favoriteScentNotes")}`;
  ...
  const profileWithLicense=`<section class="profile-license">${profile}<label class="check"><input type="checkbox" data-character-check="${c.id}" data-field="driverLicense" ${c.driverLicense?"checked":""}> 운전면허 있음</label></section>`;
...
function relationship(){
  ...
  const cards=all.map(r=>{
    if(r.groupId){
      ...
      return `<article class="relation group-relation"><h2>${members.map(member=>esc(member.name)).join(" · ")}</h2><p>${esc(r.type)} · ${members.length}명 그룹 관계</p><p>친밀도 ${r.intimacy??75} · 갈등도 ${r.conflict??20}</p><button class="danger" data-delete-group="${r.groupId}">그룹 관계 삭제</button></article>`;
    }
    const a=state.characters[r.a],b=state.characters[r.b];
    return a&&b?`<article class="relation" style="--a:${a.theme.primary};--b:${b.theme.primary}"><h2>${esc(a.name)} × ${esc(b.name)}</h2><p>${esc(r.type)} · ${r.cohabit?"함께 거주":"따로 거주"}</p><p>친밀도 ${r.intimacy??75} · 갈등도 ${r.conflict??20}</p><button data-edit-rel="${r.id}">편집</button><button class="danger" data-delete-rel="${r.id}">삭제</button></article>`:"";
  }).join("");
