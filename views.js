...
export function buildingDetailDialogs(selectedKey=""){
  ...
  const homeDialogs=townHomes().filter(home=>`home:${home.id}`===selectedKey).map(home=>{
    ...
    const interior=interiorImage?`<img src="${imageEsc(interiorImage)}" alt="${esc(home.name)} 대표 실내">`:`<div class="building-interior-placeholder"><span>🏠</span><b>대표 실내 이미지가 아직 없어요</b><small>집 화면에서 집이나 방 사진을 등록할 수 있어요.</small></div>`;
...
function characterFullOverview(c){
  ...
  return `<section class="character-full-svg-overview" aria-label="${esc(c.name)} ${t("이미지","이미지")}"><div class="character-full-content-group">
    ...
    <button type="button" class="character-full-image-slot ld ${hasLdArt(c)?"has-image":"is-empty"}" data-image="ldImage">${hasLdArt(c)?`<img src="${imageEsc(ldArtSource(c))}" alt="${esc(c.name)} LD 일러스트">`:emptyPhotoSlot("LD")}</button>
