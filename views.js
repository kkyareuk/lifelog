...
import {state,active,characterViewFor,explicitCharacterViewFor} from "./state.js?v=20260827townlayout164";
import {eventFor as simulateEventFor,visibleTimeline as simulateVisibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260827townlayout164";
import {SPEECH_STYLE_OPTIONS} from "./speech-styles.js?v=20260827townlayout164";
import {furnitureFootprint,furnitureIcon,furnitureLabel,furniturePropIcon,normalizeFurniturePlacements,supportsFurnitureProps} from "./furniture-layout.js?v=20260827townlayout164";
import {homeSurfaceImage,normalizeHomeSurface,normalizeWallSurface,wallSurfaceImage} from "./home-surfaces.js?v=20260827townlayout164";
...
function character(){
  ...
  const bookListDialog=`<dialog class="character-body-choice-dialog character-book-list-dialog" data-book-list-dialog><form method="dialog"><header><span><small>MULTI SELECT</small><b data-book-list-title>${t("여러 개 선택 가능","여러 개 선택 가능")}</b></span><button type="submit" value="close" aria-label="${esc(t("닫기","닫기"))}">×</button></header><p>${t("분류별로 살펴보고 여러 개를 선택할 수 있어요.","분류별로 살펴보고 여러 개를 선택할 수 있어요.")}</p>${Object.entries(bookListSources).map(([field,source])=>{const path=source.path||field,selected=new Set(valueAtPath(c,path)||[]);return `<section class="character-body-choice-panel book-list-group-panel" data-book-list-panel="${field}" data-book-list-title-value="${field}" hidden>${Object.entries(source.groups).map(([group,options])=>`<div class="book-choice-group"><h3>${esc(t(group,group))}</h3><div>${options.map(value=>`<button type="button" data-book-list-choice="${field}" data-book-list-path="${esc(path)}" data-value="${esc(value)}" class="${selected.has(value)?"on":""}"><span>${esc(t(value,value))}</span><i aria-hidden="true">✓</i></button>`).join("")}</div></div>`).join("")}</section>`}).join("")}<footer><button type="submit" value="close">${t("선택 완료","선택 완료")}</button></footer></form></dialog>`;
...
function townMobile(){
  ...
  if(mobileTownPanel&&mobileTownPanel!=="world"&&!selectedPlace&&!selectedHome)mobileTownPanel="";
  const panelType=mobileTownPanel==="world"?"world":selectedPlace?"place":selectedHome?"home":"";
  const localIds=state.order.filter(id=>visibleTownId(state.characters[id])===state.activeTownId);
  ...
  const townHeader=`<header class="town-native-header"><button type="button" class="home-native-back town-native-back" data-tab="observe" aria-label="${esc(t("메인 화면으로 돌아가기","메인 화면으로 돌아가기"))}"><img src="${esc(homeUiAsset(character||active(),"back.png"))}" alt=""></button><div class="town-native-title"><i style="--town-color:${esc(character?.theme?.primary||"#f1efe9")}" aria-hidden="true"></i><b>${esc(state.world.name)}</b></div><button type="button" class="town-native-info" data-mobile-town-open-settings>${t("마을 정보","마을 정보")}</button><nav aria-label="${esc(t("마을 편집","마을 편집"))}"><button type="button" class="home-native-pill" data-mobile-town-open-settings><span>${t("마을 편집","마을 편집")}</span></button><button type="button" class="home-native-pill" data-mobile-building-edit-mode><span>${t("건물 편집","건물 편집")}</span></button></nav></header>`;
  ...
  const generalEditor=mobileTownEditing&&panelType==="world"?`<div class="town-general-editor"><div class="title"><h2>마을 편집</h2><button class="primary" data-town-save>저장</button></div><section class="inline-guide"><b>마을을 만드는 순서</b><ol><li>마을 이름과 배경을 고르세요.</li><li>건물이나 장식을 추가하세요.</li><li>촘촘한 격자에서 끌어 위치를 정하세요.</li></ol></section><label>마을 이름<input data-world-name value="${esc(state.world.name)}"></label><label>마을 시대<select data-world-era><option value="modern" ${state.world.era!=="medieval"?"selected":""}>현대</option><option value="medieval" ${state.world.era==="medieval"?"selected":""}>중세</option></select><small>시대에 맞는 생활 표현을 적용합니다.</small></label><label>기본 배경<select data-world-bg>${backgroundOptions()}</select></label><button data-add-place>+ 건물 추가</button>${decorationCatalog}</div>`:"";
  const placeEditors=mobileTownEditing&&selectedPlace?townPlaceEditor(selectedPlace,items,audiences,true):mobileTownEditing&&selectedHome?townHomeEditor(selectedHome,true):"";
  const selectedBuildingName=selectedPlace?.name||selectedHome?.name||"건물 편집";
  const editorPanel=mobileTownEditing&&panelType?`<aside class="panel form town-editor-panel"><div class="mobile-town-sheet-head"><span><small>${panelType==="world"?"TOWN SETTINGS":"BUILDING SETTINGS"}</small><b>${panelType==="world"?esc(state.world.name):esc(selectedBuildingName)}</b></span><button type="button" data-mobile-town-close aria-label="편집 창 닫기">×</button></div>${generalEditor}<div class="place-editor">${placeEditors}</div></aside>`:"";
  return `<section class="mobile-town-shell ${mobileTownEditing?"editing":""} ${panelType?`sheet-open ${panelType}-panel`:""}" style="${homeUiThemeStyle(character||active())}">${desktopTabs}${townHeader}<div class="town-edit"><div class="town-map-scroll"><div class="world"><img src="${state.world.bg}" class="world-bg">${state.world.places.map(placeCard).join("")}${townHomes().map(homeMapCard).join("")}${townDecorationsMarkup()}${state.world.places.map(peopleAtPlaceCard).join("")}${townHomes().map(peopleAtHomeCard).join("")}</div></div>${editorPanel}</div>${townPlacementToolbar()}${buildingDetailDialogs()}</section>`;
}
Object.assign(UI_TEXT.en,{
