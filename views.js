...
function dailyLogItems(entries,c){
  ...
  return uniqueDisplayedMoments(entries).map(x=>{
    ...
    return `<li class="${importantEntry(x)?"important":""} ${x===entries.at(-1)?"now":""}" style="--log-theme:${logTheme}"><time>${esc(x.time)}</time><span>${[...new Set([x.withId,...(x.withIds||[])])].filter(id=>id&&id!==c.id&&state.characters[id]).map(id=>`<span class="log-companion">${avatar(state.characters[id])}<b>${esc(state.characters[id].name)}</b></span>`).join("")}<b>${esc(x.title)}</b><small>${esc(x.desc)}</small></span></li>`;
  }).join("");
}
...
const SHOP_PRODUCTS={
  character_slots_5:{label:"캐릭터 슬롯",title:"캐릭터 5명 추가",description:"캐릭터 슬롯 5개가 결제 즉시 계정에 적용됩니다. 기존 구매 슬롯은 그대로 유지됩니다.",price:1200},
...
function nativePlayShop({browseOnly=false,webBilling=false}={}){
  ...
  const icons={character_slots_5:`👤`,character_slot_1:`👤`,town_slot_1:"🏘️",storage_50mb:"🗄️",green_tea:"🎁"};
