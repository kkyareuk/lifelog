...
function character(){
  ...
  const wardrobe=`<div class="wardrobe-head"><div><h2>${esc(c.name)}의 옷장</h2><p>취향 사전에 등록한 옷·신발·가방·액세서리를 골라 옷장을 채워 주세요.</p></div><button data-tab="catalog">+ 옷 도감에 등록</button></div><div class="wardrobe-board">${wardrobeItems.map(item=>`<button class="wardrobe-item ${ownedWardrobe.has(item.id)?"on":""}" data-owned-kind="fashion" data-owned-id="${item.id}">${item.image?`<img src="${esc(item.image)}" alt="">`:`<span>👕</span>`}<b>${esc(item.name)}</b><small>${esc([item.category,item.material,item.color,item.flair].filter(Boolean).join(" · ")||"세부 정보 없음")}</small></button>`).join("")||"<div class='empty-mini'><b>아직 옷 도감이 비어 있어요.</b><p>취향 사전의 옷·패션 항목에서 옷을 먼저 등록해 주세요.</p></div>"}</div>`;
