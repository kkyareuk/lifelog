...
function observe(){
  ...
  const hudRoster=`<div class="game-hud-profile"><button type="button" class="game-hud-profile-toggle" data-toggle-game-hud-roster aria-expanded="false" aria-label="${esc(t("관찰 캐릭터 바꾸기","관찰 캐릭터 바꾸기"))}">${avatar(c)}<span><b>${esc(c.name)}</b><small>${esc(c.jobTitle||c.job||t("생활 중","생활 중"))}</small></span></button><div class="game-hud-roster" hidden aria-label="${esc(t("관찰 캐릭터 선택","관찰 캐릭터 선택"))}">${state.order.map(id=>{const person=state.characters[id];return `<button type="button" data-home-character="${id}" class="game-hud-button character-picker-button ${id===c.id?"on":""}" style="--picker-theme:${esc(person.theme?.primary||"#176b60")}" title="${esc(person.name)}" aria-label="${esc(person.name)}">${avatar(person)}<small>${esc(person.name)}</small></button>`}).join("")}</div></div>`;
