...
import {state,active,characterViewFor,explicitCharacterViewFor} from "./state.js?v=20260820desktoprestore1";
import {eventFor as simulateEventFor,visibleTimeline as simulateVisibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260820desktoprestore1";
...
function header(){
  const tabs=[["observe",t("observe","관찰"),"◉"],["mailbox",t("mailbox","우편함"),"✉"],["home",t("home","집"),"⌂"],["character",t("character","캐릭터"),"♙"],["catalog",t("catalog","취향 사전"),"◇"],["relationship",t("relationship","관계"),"∞"],["routine",t("routine","주간 루틴"),"▦"],["statistics",t("statistics","통계"),"▥"],["town",t("town","마을"),"▧"],["shop",t("shop","상점"),"♢"],["settings",t("settings","설정"),"⚙"]];
...
const GAME_HUD_SIDE_TABS={
  ...
  right:[
    {key:"routine",labelKey:"routine",label:"주간 루틴",icon:"▦"},
...
function gameHudDock(){
  return `<nav class="game-hud-dock" aria-label="${esc(t("주요 메뉴","주요 메뉴"))}"><button type="button" class="game-hud-button game-hud-mailbox-button" data-tab="mailbox"><img src="./icons/mailbox.png" alt=""><small>${t("mailbox","우편함")}</small></button><button type="button" class="game-hud-button" data-tab="home"><span data-menu-icon="home" aria-hidden="true">⌂</span><small>${t("home","집")}</small></button><button type="button" class="game-hud-button" data-open-native-log><span data-menu-icon="log" aria-hidden="true">▤</span><small>${t("todayLog","오늘의 기록")}</small></button><button type="button" class="game-hud-button" data-tab="shop"><span data-menu-icon="shop" aria-hidden="true">♢</span><small>${t("shop","상점")}</small></button><button type="button" data-tab="town" class="game-hud-button game-hud-town-button"><img src="./icons/town-map.png" alt=""><small>${t("town","마을")}</small></button></nav>`;
}
...
function observe(){
  ...
  const statusCard=`<article class="game-hud-moment"><div class="game-hud-moment-head"><small>${t("currentMoment","지금 이 순간")}</small></div><h1>${esc(e.title)}</h1><p>${esc(e.desc)}</p>${location}</article>`;
