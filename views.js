...
import {state,active,characterViewFor,explicitCharacterViewFor} from "./state.js?v=20260820svgfit1";
import {eventFor as simulateEventFor,visibleTimeline as simulateVisibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260820svgfit1";
...
function observe(){
  ...
  const hudRoster=`<details class="game-hud-profile"><summary class="game-hud-profile-toggle" data-open-game-hud-roster aria-label="${esc(t("characterPicker","관찰 캐릭터 바꾸기"))}"><span class="game-hud-profile-frame">${profileAvatar(c,"game-hud-current-profile")}<img class="game-hud-profile-ring" src="./icons/hud-profile-frame.png" alt=""></span><span class="game-hud-profile-copy"><b>${esc(c.name)}</b><small>${esc(c.jobTitle||c.job||t("생활 중","생활 중"))}</small></span></summary><section class="game-hud-roster-drawer"><small class="game-hud-roster-kicker">${t("currentTownResidents","이 마을의 캐릭터")}</small><div class="game-hud-roster-options" aria-label="${esc(t("characterPicker","관찰 캐릭터 선택"))}">${localIds.map(id=>{const person=state.characters[id];return `<button type="button" data-home-character="${id}" class="game-hud-button character-picker-button ${id===c.id?"on":""}" style="--picker-theme:${esc(person.theme?.primary||"#176b60")}" title="${esc(person.name)}" aria-label="${esc(person.name)}">${profileAvatar(person)}<small>${esc(person.name)}</small></button>`}).join("")}</div>${otherTowns.length?`<div class="game-hud-town-jump"><b>${t("moveToAnotherTown","다른 마을로 이동")}</b><div>${otherTowns.map(town=>`<button type="button" data-observe-town="${town.id}">${esc(town.name)}</button>`).join("")}</div></div>`:""}</section></details>`;
...
function routine(){
  ...
  return `<section class="panel form routine-shell"><div class="title"><div><h1>${monthlyView?"월간 일정":"주간 일정"}</h1><p>${monthlyView?"날짜가 정해진 약속·생일·기념일을 달력에서 함께 봐요.":"반복되는 회사 일정, 수업, 데이트와 개인 일정을 요일별로 지정해요."}</p></div><div class="routine-title-actions"><button class="primary" ${monthlyView?"data-add-monthly-routine":"data-add-routine"}>+ 일정 추가</button>${monthlyView?'<button type="button" data-add-anniversary>+ 기념일</button>':""}</div></div>${toolbar}<div class="routine-view-tabs"><button type="button" data-routine-view="weekly" class="${monthlyView?"":"on"}">주간 일정</button><button type="button" data-routine-view="monthly" class="${monthlyView?"on":""}">월간 일정</button></div>${monthlyView?monthly:table}</section>`;
}
