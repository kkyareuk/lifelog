import {state,active} from "./state.js?v=20260803av";
import {eventFor,visibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260803av";
...
let accountEntitlements={backgroundPacks:[],iconPacks:[],dlcPacks:[]};
...
function header(){
  const tabs=[["observe","관찰"],["home","집"],["character","캐릭터"],["wardrobe","옷장"],["catalog","취향 사전"],["relationship","관계"],["routine","주간 루틴"],["town","마을"],["dlc","DLC"],["settings","설정"]];
...
function relationship(){
  ...
  const cards=all.map(r=>{
    if(r.groupId){
      ...
      return `<article class="relation group-relation"><div class="relation-avatars">${members.map(member=>avatar(member)).join("")}</div><h2>${direction}</h2><p>${esc(r.type)} · ${members.length}명이 함께 맺은 관계</p><p class="relation-stage">${esc(r.stage||"편안한 사이")}</p>${r.interactions?.length?`<div class="relation-tags">${r.interactions.map(x=>`<span>${esc(x)}</span>`).join("")}</div>`:""}<button data-edit-rel="${r.id}">구성원·관계 편집</button><button class="danger" data-delete-group="${r.groupId}">그룹 관계 삭제</button></article>`;
    }
...
function view(){
  ...
  return ({observe,home,character,wardrobe,catalog,relationship,routine,town,dlc,settings}[state.activeTab]||observe)();
}
...
export function setAccountEntitlements(value){accountEntitlements={backgroundPacks:Array.isArray(value?.backgroundPacks)?value.backgroundPacks:[],iconPacks:Array.isArray(value?.iconPacks)?value.iconPacks:[],dlcPacks:Array.isArray(value?.dlcPacks)?value.dlcPacks:[]}}
