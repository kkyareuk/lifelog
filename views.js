...
function character(){
  ...
  const manageVisualMode=hasLdArt(c)&&(c.homeVisualMode==="ld"||!hasSdArt(c))?"ld":"sd";
  const homeVisualChoice=`<section class="character-home-visual-choice"><div><h3>홈화면 캐릭터 표현</h3><p>홈화면의 SD·LD 전환은 이곳에서만 바꿔요.</p></div><div class="character-home-visual-buttons" role="group" aria-label="홈화면 캐릭터 표현"><button type="button" data-home-visual-mode="sd" class="${manageVisualMode==="sd"?"on":""}">SD</button><button type="button" data-home-visual-mode="ld" class="${manageVisualMode==="ld"?"on":""}" ${ldSource?"":"disabled"}>LD</button></div></section>`;
