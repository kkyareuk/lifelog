...
import {state,active,characterViewFor,explicitCharacterViewFor} from "./state.js?v=20260827townui167";
import {eventFor as simulateEventFor,visibleTimeline as simulateVisibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260827townui167";
import {SPEECH_STYLE_OPTIONS} from "./speech-styles.js?v=20260827townui167";
import {furnitureFootprint,furnitureIcon,furnitureLabel,furniturePropIcon,normalizeFurniturePlacements,supportsFurnitureProps} from "./furniture-layout.js?v=20260827townui167";
import {homeSurfaceImage,normalizeHomeSurface,normalizeWallSurface,wallSurfaceImage} from "./home-surfaces.js?v=20260827townui167";
...
function character(){
  ...
  const wardrobePane=`<section class="character-book-form-page wardrobe-book-page">
    <div class="book-form-grid">
      ...
      <div class="book-form-stack book-form-combined">${bookField("미용실 방문 빈도","salonFrequency",["자동 · 설정에 맞춤","거의 가지 않음","필요할 때만","두세 달에 한 번","매달","월 2회 이상"],bodyAppearance.salonFrequency||"자동 · 설정에 맞춤")}${bookFieldContinuation("미용실에서 하는 일","salonPurpose",["설정하지 않음","커트","커트·염색","커트·염색·펌","두피·모발 관리까지"],c.salonPurpose||"설정하지 않음")}</div>
...
function townBuildingBrowser(character){
  const entries=townBuildingEntries(),types=["전체","집","사무실","음식점","카페","병원"];
  return `<aside class="town-building-screen town-building-browser-screen" aria-label="${esc(t("건물 정보","건물 정보"))}"><div class="town-building-screen-head"><button type="button" class="town-building-screen-back" data-mobile-town-close aria-label="${esc(t("마을로 돌아가기","마을로 돌아가기"))}"><img src="${esc(homeUiAsset(character||active(),"back.png"))}" alt=""></button><label class="town-building-search"><span class="sr-only">${t("건물 검색","건물 검색")}</span><input type="search" data-building-search placeholder="${esc(t("검색","검색"))}"></label></div><nav class="town-building-town-filters" aria-label="${esc(t("마을 선택","마을 선택"))}"><button type="button" class="on" data-building-town-filter="all">${t("전체","전체")}</button>${(state.towns||[]).map(town=>`<button type="button" data-building-town-filter="${town.id}">${esc(town.name)}</button>`).join("")}</nav><nav class="town-building-type-filters" aria-label="${esc(t("건물 유형","건물 유형"))}">${types.map((type,index)=>`<button type="button" class="${index===0?"on":""}" data-building-type-filter="${esc(type)}">${t(type,type)}</button>`).join("")}</nav><div class="town-building-card-grid" aria-live="polite">${entries.map(entry=>`<button type="button" class="town-building-browser-card" data-building-browser-card data-building-browser-open="${entry.kind==="home"?`home:${entry.id}`:entry.id}" data-building-browser-town="${entry.townId}" data-building-browser-type="${esc(entry.type)}" data-building-browser-name="${esc(entry.name)}"><span><img src="${esc(entry.art)}" alt=""></span><b>${esc(entry.name)}</b><small>${esc(entry.townName)}</small></button>`).join("")}</div><button type="button" class="town-building-add" data-add-place aria-label="${esc(t("건물 추가","건물 추가"))}">＋</button></aside>`;
}
function townBuildingDetailScreen(character,place,home,items,audiences){
  ...
  const editor=place?townPlaceEditor(place,items,audiences,true):home?townHomeEditor(home,true):"";
  return `<aside class="town-building-screen town-building-detail-screen" aria-label="${esc(name)}"><div class="town-building-detail-head"><button type="button" class="town-building-screen-back" data-building-browser-back aria-label="${esc(t("건물 목록으로 돌아가기","건물 목록으로 돌아가기"))}"><img src="${esc(homeUiAsset(character||active(),"back.png"))}" alt=""></button><b>${esc(name)}</b></div><div class="town-building-detail-scroll"><section class="town-building-hero">${interior?`<img class="town-building-interior" src="${esc(interior)}" alt="${esc(name)} ${esc(t("내부 사진","내부 사진"))}">`:`<div class="town-building-interior town-building-interior-empty"><span>${esc(place?.emoji||"🏠")}</span><small>${t("내부 사진을 등록해 주세요","내부 사진을 등록해 주세요")}</small></div>`}<img class="town-building-exterior" src="${esc(exterior)}" alt="${esc(name)} ${esc(t("건물 모양","건물 모양"))}"></section><div class="place-editor town-building-detail-form">${editor}</div></div></aside>`;
}
...
function townMobile(){
  ...
  const residentIds=state.order.filter(id=>{
    const character=state.characters[id],home=state.homes?.[character?.homeId];
    return Boolean(character&&(home?.townId===state.activeTownId||(!home&&character.townId===state.activeTownId)));
  });
  ...
  const decorationCatalog=`<section class="town-decoration-catalog"><header><b>${t("마을 장식 추가","마을 장식 추가")}</b><input type="search" data-decoration-search placeholder="${esc(t("장식 검색","장식 검색"))}" aria-label="${esc(t("장식 검색","장식 검색"))}"></header><nav>${categories.map((category,index)=>`<button type="button" data-decoration-category="${category}" class="${index===0?"on":""}">${t(category,category)}</button>`).join("")}</nav><div class="town-decoration-results">${TOWN_DECORATION_CHOICES.map(([kind,icon,label,category])=>`<button type="button" data-add-town-decoration="${kind}" data-decoration-choice data-decoration-label="${esc(label)}" data-decoration-group="${esc(category)}"><span>${icon}</span><small>${t(label,label)}</small></button>`).join("")}</div></section>`;
  const townInfo=`<div class="town-info-card">${state.world.photo?`<img src="${esc(state.world.photo)}" alt="">`:""}<dl><div><dt>${t("도시화 정도","도시화 정도")}</dt><dd>${t(state.world.urbanization||"소도시",state.world.urbanization||"소도시")}</dd></div><div><dt>${t("마을 평판","마을 평판")}</dt><dd>${t(state.world.reputation||"알려지지 않음",state.world.reputation||"알려지지 않음")}</dd></div><div><dt>${t("마을 시대","마을 시대")}</dt><dd>${state.world.era==="medieval"?t("중세","중세"):t("현대","현대")}</dd></div></dl>${state.world.description?`<p>${esc(state.world.description)}</p>`:""}</div>`;
