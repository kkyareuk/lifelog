import {renderDiamondShop,renderHomeCurrencies} from "./diamond-shop.js?v=20260909dev305";
...
function nativePlayShop({browseOnly=false}={}){
  ...
  const icons={character_slot_1:`👤`,town_slot_1:"🏘️",storage_50mb:"🗄️",green_tea:"🎁"};
...
function shop(){
  ...
  if(window.PARALLEL_CITY_CONFIG?.diamonds?.enabled)return `<section class="panel shop-store">${renderDiamondShop(state.uiLanguage,false)}<article class="diamond-charge"><b>${esc(t("서랍마을 응원 선물 🎁","서랍마을 응원 선물 🎁"))}</b><a href="./payment.html?product=green_tea">₩3,000</a></article>${jobExpansionCard()}</section>`;
  const cart=readCart();
