...
import {state,active,characterViewFor,explicitCharacterViewFor} from "./state.js?v=20260826characterchapters162";
import {eventFor as simulateEventFor,visibleTimeline as simulateVisibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260826characterchapters162";
import {SPEECH_STYLE_OPTIONS} from "./speech-styles.js?v=20260826characterchapters162";
import {furnitureFootprint,furnitureIcon,furnitureLabel,furniturePropIcon,normalizeFurniturePlacements,supportsFurnitureProps} from "./furniture-layout.js?v=20260826characterchapters162";
import {homeSurfaceImage,normalizeHomeSurface,normalizeWallSurface,wallSurfaceImage} from "./home-surfaces.js?v=20260826characterchapters162";
...
function homeMapCard(home){
  ...
  return `<button type="button" class="place has-art map-art-button home-map-place" style="left:${home.mapX}%;top:${home.mapY}%;--place-scale:${home.mapScale||1.08}" data-home-map="${home.id}" data-building-detail-open="home:${home.id}" aria-label="${esc(home.name)} 집 정보 보기"><img class="building-preset-image" src="${esc(home.exteriorImage||"world-assets/drawer-home.png")}" alt=""></button>${label}`;
}
...
function character(){
  ...
  const bodyAccessibilityPane=`<section class="character-body-svg-page character-body-accessibility-page" aria-label="${esc(c.name)} ${esc(t("건강·접근성 설정","건강·접근성 설정"))}">
    ...
    <section class="body-accessibility-device body-hospital"><b>${t("병원 방문 빈도","병원 방문 빈도")}</b>${bodySelect("hospitalVisitFrequency",["자동 · 설정에 맞춤","정기 검진 때만","한 달에 한 번 이하","한 달에 여러 번","주 1회 이상","필요할 때 비정기적으로"],c.bodyProfile?.hospitalVisitFrequency||"자동 · 설정에 맞춤")}<input data-body-field="hospitalVisitPurpose" maxlength="120" value="${esc(c.bodyProfile?.hospitalVisitPurpose||"")}" placeholder="${esc(t("병원에서 할 일(검사/수술)","병원에서 할 일(검사/수술)"))}"></section>
    <section class="body-accessibility-device body-medications"><b>${t("복용중인 약","복용중인 약")}</b><input data-body-field="medication1" maxlength="80" value="${esc(c.bodyProfile?.medication1||"")}" placeholder="${esc(t("약 이름 1","약 이름 1"))}"><input data-body-field="medication2" maxlength="80" value="${esc(c.bodyProfile?.medication2||"")}" placeholder="${esc(t("약 이름 2","약 이름 2"))}"></section>
  ...
  const bookListButton=(label,field,values)=>`<div class="book-form-field"><b>${t(label,label)}</b><button type="button" data-open-book-list="${field}"><span>${bookListSummary(values)}</span><i aria-hidden="true">＋</i></button></div>`;
  const wardrobePane=`<section class="character-book-form-page wardrobe-book-page">
    <div class="book-form-grid">
      <div class="book-form-stack"><label class="book-form-field"><b>${t("화장 정도","화장 정도")}</b>${bodySelect("appearance.makeupLevel",MAKEUP_LEVELS,bodyAppearance.makeupLevel||"하지 않음")}</label>${bookListButton("화장 스타일","makeupStyles",bodyAppearance.makeupStyles||[])}</div>
      <div class="book-form-stack">${bookField("성형·외형 의료 시술","cosmeticSurgery",["설정하지 않음","하지 않음","상담만 받음","비수술 시술","수술 경험 있음"],c.cosmeticSurgery||bodyAppearance.cosmeticSurgery||"설정하지 않음")}${bookListButton("시술 부위","cosmeticSurgeryAreas",bodyAppearance.cosmeticSurgeryAreas||[])}</div>
      ...
      <div class="book-form-stack">${bookField("작업복","workwearType",["설정하지 않음","제복","정장","비즈니스 캐주얼","실용복","슬립"],c.workwearType||"설정하지 않음")}${bookListButton("작업복 의상 태그","workwearTags",c.workwearTags||[])}</div>
      <div class="book-form-stack">${bookField("수영복","swimwearType",["설정하지 않음","래시가드","원피스 수영복","비키니","트렁크","전신 수영복"],c.swimwearType||"설정하지 않음")}${bookListButton("수영복 의상 태그","swimwearTags",c.swimwearTags||[])}</div>
      <div class="book-form-stack">${bookField("잠옷","sleepwearType",["설정하지 않음","평상복","파자마","가운","슬립","속옷","알몸"],c.sleepwearType||"설정하지 않음")}${bookListButton("잠옷 의상 태그","sleepwearTags",c.sleepwearTags||[])}</div>
      <div class="book-form-stack">${bookField("파티복","partywearType",["설정하지 않음","정장","드레스","칵테일 드레스","전통 의상","코스튬"],c.partywearType||"설정하지 않음")}${bookListButton("파티복 의상 태그","partywearTags",c.partywearTags||[])}</div>
  ...
  const personalityCoreFields=[
    ...
    ["남에게 관여하는 정도","interference",["방관자","요청할 때만 도움","적당히 관여","챙기고 확인함","강하게 간섭함"]],
  ...
  const personalityDetailsPane=`<section class="character-book-form-page personality-detail-book-page"><div class="book-form-grid">${bookField("감정 표현의 크기","emotionalExpression",["표정 변화가 거의 없음","감정을 잘 드러내지 않음","상황에 따라 표현함","표현이 풍부함","감정이 바로 드러남"],c.emotionalExpression||"상황에 따라 표현함")}${bookField("충동을 참는 정도","impulseControl",["매우 잘 참음","대체로 참음","가끔 욱하지만 멈춤","쉽게 욱함","거의 참지 않음"],c.impulseControl||"가끔 욱하지만 멈춤")}</div>${bookListButton("서사·인지 특성 선택사항","characterTraits",c.characterTraits||[])}${bookListButton("실제 장면에 반영할 표현","traitExpressions",c.traitExpressions||[])}${bookPageControls(9,'data-character-personality-pane="core"','data-character-pane="taste" data-character-taste-pane="categories"')}</section>`;
  ...
  const bookListSources={
    makeupStyles:["내추럴","포인트 메이크업","풀 메이크업","무대 메이크업","고딕","화려함"],cosmeticSurgeryAreas:["눈","코","입술","윤곽","가슴","피부","치아","기타"],
    favoriteFashionStyles:DETAIL_OPTIONS.fashion,workwearTags:DETAIL_OPTIONS.fashion,swimwearTags:DETAIL_OPTIONS.fashion,sleepwearTags:DETAIL_OPTIONS.fashion,partywearTags:DETAIL_OPTIONS.fashion,
    characterTraits:[...PERSONALITY_TYPES,"ADHD","자폐 스펙트럼","복수 자아·다중 정체성","감각 과민","감각 둔감","불안 경향","우울 경향","강박 경향"],
    traitExpressions:["대화에 바로 드러남","혼자 있을 때 드러남","가까운 사람에게만 드러남","스트레스 상황에 드러남","일할 때 드러남","갈등 중 드러남","기쁠 때 드러남","숨기려 하지만 새어 나옴"],
    interests:INTERESTS,hobbies:HOBBIES,favoriteStoryGenres:storyGenres,skills:["요리","악기","그림","글쓰기","운동","춤","연기","공예","프로그래밍","외국어","정비","의료"],foodPreferences:FOOD_PREFERENCES,drinks:DRINKS,musicGenres:MUSIC,favoriteVideoGenres:videoFormats,favoriteGameGenres:gameGenres,favoriteScentNotes:PERFUME_NOTES,favoriteAnimals:["개","고양이","새","토끼","파충류","어류","말","야생동물"],favoriteElectronics:["스마트폰","컴퓨터","게임기","카메라","오디오","스마트홈","웨어러블"],favoriteWeapons:["검","활","총기","창","도끼","둔기","마법 도구"],favoriteBooks:storyGenres
  };
  const bookListDialog=`<dialog class="character-body-choice-dialog character-book-list-dialog" data-book-list-dialog><form method="dialog"><header><span><small>MULTI SELECT</small><b data-book-list-title>${t("여러 개 선택 가능","여러 개 선택 가능")}</b></span><button type="submit" value="close" aria-label="${esc(t("닫기","닫기"))}">×</button></header><p>${t("여러 개 선택 가능","여러 개 선택 가능")}</p>${Object.entries(bookListSources).map(([field,options])=>`<section class="character-body-choice-panel" data-book-list-panel="${field}" data-book-list-title-value="${field}" hidden>${options.map(value=>`<button type="button" data-chip="${field}" data-value="${esc(value)}" class="${(c[field]||[]).includes(value)?"on":""}"><span>${esc(t(value,value))}</span><i aria-hidden="true">✓</i></button>`).join("")}</section>`).join("")}<footer><button type="submit" value="close">${t("선택 완료","선택 완료")}</button></footer></form></dialog>`;
    ...
    <section class="panel form">
      ...
      <section class="character-book-v8 ${state.characterSettingsView==="full"?"is-open":""}" ${state.characterSettingsView==="full"?"":"hidden"} data-character-full-ui-version="9" data-full-pane="${fullActivePane}" style="--character-accent:${esc(c.theme?.primary||"#176b60")};--character-accent-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")}">
        <div class="character-book-v8-canvas">
          <span class="character-book-v8-stage" aria-hidden="true"><img class="character-book-v8-wood" src="./assets/character-ui/character-book-stage-v12.png" alt=""></span>
...
function townMobile(){
  ...
  const generalEditor=mobileTownEditing&&panelType==="world"?`<div class="town-general-editor"><div class="title"><h2>마을 편집</h2><button class="primary" data-town-save>저장</button></div><section class="inline-guide"><b>마을을 만드는 순서</b><ol><li>마을 이름과 배경을 고르세요.</li><li>건물을 추가하고 유형을 고르세요.</li><li>지도에서 건물을 끌어 위치를 정하세요.</li></ol></section><label>마을 이름<input data-world-name value="${esc(state.world.name)}"></label><label>마을 시대<select data-world-era><option value="modern" ${state.world.era!=="medieval"?"selected":""}>현대</option><option value="medieval" ${state.world.era==="medieval"?"selected":""}>중세</option></select><small>시대에 맞는 생활 표현을 적용합니다.</small></label><label>기본 배경<select data-world-bg>${backgroundOptions()}</select></label><button data-add-place>+ 건물 추가</button></div>`:"";
  const placeEditors=mobileTownEditing&&selectedPlace?townPlaceEditor(selectedPlace,items,audiences,true):"";
  const editorPanel=mobileTownEditing&&panelType?`<aside class="panel form town-editor-panel"><div class="mobile-town-sheet-head"><span><small>${panelType==="world"?"TOWN SETTINGS":"BUILDING SETTINGS"}</small><b>${panelType==="world"?esc(state.world.name):esc(selectedPlace?.name||"건물 편집")}</b></span><button type="button" data-mobile-town-close aria-label="편집 창 닫기">×</button></div>${generalEditor}<div class="place-editor">${placeEditors}</div></aside>`:"";
  return `<section class="mobile-town-shell ${mobileTownEditing?"editing":""} ${panelType?`sheet-open ${panelType}-panel`:""}">${desktopTabs}<div class="mobile-town-hud"><span><small>현재 마을</small><b>${esc(state.world.name)}</b></span><div class="mobile-town-hud-actions">${mobileTownEditing?'<button type="button" data-add-town>+ 새 마을</button>':""}<button type="button" data-mobile-town-edit-toggle class="${mobileTownEditing?"on":""}">${mobileTownEditing?"편집 종료":"편집 모드"}</button></div></div>${!mobileTownEditing?`<p class="mobile-town-hold-guide">건물이나 집을 길게 누르면 바로 편집할 수 있어요.</p>`:""}${mobileTownEditing?`<div class="mobile-town-tools"><button type="button" data-mobile-town-settings>마을 설정</button><button type="button" data-add-place>+ 건물</button></div>`:""}<div class="town-edit"><div class="town-map-scroll"><div class="world"><img src="${state.world.bg}" class="world-bg">${state.world.places.map(placeCard).join("")}${townHomes().map(homeMapCard).join("")}${state.world.places.map(peopleAtPlaceCard).join("")}${townHomes().map(peopleAtHomeCard).join("")}</div></div>${editorPanel}</div>${characterCard}${mobileSwitcher}${buildingDetailDialogs()}</section>`;
}
function view(){
