...
import {state,active,characterViewFor,explicitCharacterViewFor} from "./state.js?v=20260823characterfidelity2";
import {eventFor as simulateEventFor,visibleTimeline as simulateVisibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260823characterfidelity2";
...
function character(){
  ...
  const mobileProfileDraft=`<section class="mobile-character-profile-draft">
    ...
    <div class="character-wallet-roster" data-character-roster data-roster-count="${mobileCharacterIds.length}" style="--roster-visible:${rosterVisibleRows}" hidden><div class="mobile-character-strip">${mobileStrip}</div><span class="character-roster-actions"><button type="button" class="character-roster-add" data-new ${state.order.length>=limit?"disabled":""} aria-label="${esc(t("새 캐릭터 만들기","새 캐릭터 만들기"))}"><img src="./assets/character-ui/add.png" alt=""></button><button type="button" class="character-roster-reorder" data-open-character-reorder aria-label="${esc(t("위치 바꾸기","위치 바꾸기"))}">↕</button><button type="button" class="character-roster-close" data-toggle-character-roster aria-label="${esc(t("닫기","닫기"))}">×</button></span></div>
  </section>`;
  const hubActions=`<section class="character-setting-choices" aria-label="${esc(t("캐릭터 설정 방식","캐릭터 설정 방식"))}"><img class="character-setting-post-it character-setting-post-it-top" src="./assets/character-ui/post-it.png" alt=""><img class="character-setting-post-it character-setting-post-it-bottom" src="./assets/character-ui/post-it.png" alt=""><img class="character-setting-clip" src="./assets/character-ui/clip.png" alt=""><button type="button" class="character-setting-choice character-quick-choice" data-open-quick-character-settings><img src="./assets/character-ui/notebook.png" alt=""><span>${t("빠른 설정 바로가기","빠른 설정 바로가기")}</span></button><button type="button" class="character-setting-choice character-full-choice" data-open-full-character-settings><img src="./assets/character-ui/book.png" alt=""><span class="character-favorite-preview" aria-label="${esc(t("선호 물품 미리보기","선호 물품 미리보기"))}">${favoriteSlots}</span><b>${t("전체 설정 바로가기","전체 설정 바로가기")}</b></button></section>`;
  ...
  return `<div class="editor character-editor" style="--character-own:${esc(c.theme?.primary||"#176b60")};--character-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")}">
    ...
    <section class="panel form">
      <section class="mobile-character-dashboard ${state.characterSettingsView==="full"?"is-hidden":""}" data-character-ui-version="5" style="--character-own:${esc(c.theme?.primary||"#176b60")};--character-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")}">
        ${mobileProfileDraft}
        ${hubActions}
      </section>
