...
function observe(shared=false){
  ...
  const hudRoster=`<details class="game-hud-profile"><summary class="game-hud-profile-toggle" data-open-game-hud-roster aria-label="${esc(t("characterPicker","관찰 캐릭터 바꾸기"))}"><span class="game-hud-profile-frame">${profileAvatar(c,"game-hud-current-profile")}<img class="game-hud-profile-ring" src="${imageEsc(homeUiAsset(c,"profile-ring.png"))}" alt=""></span><span class="game-hud-profile-copy"><b>${esc(c.name)}</b><small><em>${esc(careerCaption(state,c)||t("생활 중","생활 중"))}</em></small></span></summary><section class="game-hud-roster-drawer"><small class="game-hud-roster-kicker">${t("선택한 그룹의 캐릭터","선택한 그룹의 캐릭터")}</small><div class="game-hud-roster-options" aria-label="${esc(t("characterPicker","관찰 캐릭터 선택"))}">${localIds.map(id=>{const person=state.characters[id];return `<button type="button" data-home-character="${id}" class="game-hud-button character-picker-button ${id===c.id?"on":""}" style="--picker-theme:${esc(person.theme?.primary||"#176b60")}" title="${esc(person.name)}" aria-label="${esc(person.name)}">${profileAvatar(person)}<small>${esc(person.name)}</small></button>`}).join("")}</div>${groupJump()}${otherTowns.length?`<div class="game-hud-town-jump"><b>${t("moveToAnotherTown","다른 마을로 이동")}</b><div>${otherTowns.map(town=>`<button type="button" data-observe-town="${town.id}">${esc(town.name)}</button>`).join("")}</div></div>`:""}</section></details>`;
...
function character(){
  ...
  const overviewPane=isCourtWorld()&&state.characterOverviewPane==="court"?"court":state.characterOverviewPane==="basic"?"basic":"life";
  ...
  const overviewBasic=`<section class="character-overview-basic" aria-label="${esc(t("개요 기본 설정","개요 기본 설정"))}">
    ...
    <label class="overview-field overview-job"><b>${t("직업","직업")}</b>${isCourtWorld()?courtJobSelect(c):overviewSelect("job",JOBS,c.job||"무직")}</label>
    ...
    <label class="overview-field overview-job-title"><b>${t("직업명","직업명")}</b><input data-field="${isCourtWorld()?'courtJobTitle':'jobTitle'}" value="${esc((isCourtWorld()?c.courtJobTitle:c.jobTitle)||"")}" maxlength="60" autocomplete="off" autocorrect="off" spellcheck="false"></label>
  ...
  const overviewControlsFor=pane=>pane==="basic"
    ?`<nav class="character-overview-page-controls" aria-label="${esc(t("개요 페이지 이동","개요 페이지 이동"))}"><button type="button" data-character-pane="visual" aria-label="${esc(t("이전 페이지","이전 페이지"))}">◀</button><b>2</b><button type="button" data-character-overview-pane="${isCourtWorld()?'court':'life'}" aria-label="${esc(t("다음 페이지","다음 페이지"))}">▶</button></nav>`
    :`<nav class="character-overview-page-controls" aria-label="${esc(t("개요 페이지 이동","개요 페이지 이동"))}"><button type="button" data-character-overview-pane="${isCourtWorld()?'court':'basic'}" aria-label="${esc(t("이전 페이지","이전 페이지"))}">◀</button><b>3</b><button type="button" data-character-pane="body" aria-label="${esc(t("다음 페이지","다음 페이지"))}">▶</button></nav>`;
  ...
  const profileOverviewBasicPane=()=>{const html=profileOverviewMarkup("basic");return state.characterProfileBook?html.replace(/<button[^>]*data-character-overview-pane="(?:life|court)"[^>]*>[\s\S]*?<\/button>/g,'') : html;};
  const profileOverviewLifePane=()=>profileOverviewMarkup("life");
  const profileOverviewPane=()=>overviewPane==="court"?courtCharacterPage(c):overviewPane==="basic"?profileOverviewBasicPane():profileOverviewLifePane();
