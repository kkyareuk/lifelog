...
const furniturePlacementStyle=(placement,footprint)=>`--furniture-x:${placement.x}%;--furniture-y:${placement.y}%;--furniture-scale:${placement.scale};--furniture-rotation:${placement.item==="커플 침대"?bedPerspective(placement).artRotation:placement.rotation}deg;--furniture-layer:${placement.layer};--furniture-flip:${placement.item==="커플 침대"?bedPerspective(placement).artFlip:placement.flipped?-1:1};--furniture-grid-width:${footprint.columns};--furniture-grid-height:${footprint.rows}`;
...
function character(){
  ...
  const closetBookPane=()=>`<section class="character-book-form-page closet-book-page"><div class="closet-book-toolbar"><label><span class="sr-only">${t("검색","검색")}</span><input type="search" data-closet-search placeholder="${t("옷 검색","옷 검색")}"></label><nav><button type="button" class="on" data-closet-filter="all">${t("전체","전체")}</button><button type="button" data-closet-filter="uniform">${t("유니폼","유니폼")}</button><button type="button" data-closet-filter="formal">${t("격식","격식")}</button></nav></div><div class="closet-book-frame"><div class="closet-book-paper"><div class="closet-book-count"><b>${t("총","총")} ${closetItems.length} / 30</b><small>${t("상황과 드레스코드에 맞춰 자동 선택","상황과 드레스코드에 맞춰 자동 선택")}</small></div><div class="closet-book-grid">${closetItems.map(item=>`<button type="button" data-edit-clothing="${item.id}" data-closet-card data-closet-search-text="${esc(`${item.name} ${(item.occasionTags||[]).join(" ")} ${(item.flairs||[]).join(" ")}`.toLowerCase())}" data-uniform="${item.requiredUniform?"true":"false"}" data-formal="${/격식|정장/.test(item.formality||"")?"true":"false"}"><span class="closet-book-art">${item.iconImage||item.image?`<img src="${imageEsc(item.iconImage||item.image)}" alt="">`:`<i>👕</i>`}</span><b>${esc(item.name)}</b><small>${esc([item.warmth,item.formality,item.comfort].filter(Boolean).join(" · "))}</small></button>`).join("")}<button type="button" class="closet-book-add" ${closetItems.length>=30?"disabled":""} title="${t("옷장은 캐릭터마다 30개까지 등록할 수 있어요.","옷장은 캐릭터마다 30개까지 등록할 수 있어요.")}" data-new-clothing><span>＋</span><b>${t("옷 추가","옷 추가")}</b></button><p class="closet-book-no-results" hidden>${t("검색 결과가 없어요.","검색 결과가 없어요.")}</p></div></div></div>${bookPageControls(11,'data-character-pane="taste"','disabled')}</section>`;
