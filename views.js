...
function placeCard(p){
  return `<button class="place" style="left:${p.x}%;top:${p.y}%;--place:${p.color}" data-place="${p.id}"><i>${p.emoji}</i><b>${esc(p.name)}</b><small>${esc(p.type)}</small></button>`;
}
...
function home(){
  return `<div class="title"><h1>우리 집 생활</h1></div><div class="home-grid">${Object.entries(homeGroups()).map(([id,chars])=>homeCard(id,chars)).join("")}</div>`;
}
function homeCard(id,chars){
  ...
  const people=inside.map(c=>{const e=eventFor(c),same=inside.filter(x=>eventFor(x).room===e.room),i=same.findIndex(x=>x.id===c.id);return `<button class="home-person room-${e.room} pos-${i%3}" data-home-person="${c.id}">${avatar(c)}<span><b>${esc(c.name)}</b><small>${esc(e.title)}</small></span></button>`}).join("");
  return `<article class="home panel" data-home-card="${id}"><div class="title"><div><h2>🏠 ${esc(h.name)}</h2><small>${chars.map(c=>c.name).join(" · ")} 거주 중</small></div><b>${inside.length}명 귀가</b></div><div class="clean">청결도 · 반짝반짝 깨끗함 <i></i></div><div class="rooms">${Object.keys(roomClasses).map(key=>`<div class="room ${roomClasses[key]}" ${roomStyle(h,key)}><b>${esc(h.rooms?.[key]?.name||key)}</b><div class="room-tools"><button data-room-bg="${id}" data-home-id="${id}" data-room="${key}">사진</button>${h.rooms?.[key]?.image?`<button data-clear-room-bg data-home-id="${id}" data-room="${key}">지우기</button>`:""}</div></div>`).join("")}${people}</div></article>`;
}
