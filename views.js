...
function gameHudSideMenu(side,character){
  return `<nav class="game-hud-side game-hud-side-${side}" aria-label="${esc(t(side==="left"?"캐릭터와 관계 메뉴":"일정과 설정 메뉴",side==="left"?"캐릭터와 관계 메뉴":"일정과 설정 메뉴"))}">${GAME_HUD_SIDE_TABS[side].map(({key,labelKey,label,asset,icon})=>`<button type="button" class="game-hud-button" data-tab="${key}" aria-label="${esc(gameHudLabel(labelKey,label))}">${asset?`<img class="game-hud-menu-art" src="${imageEsc(homeUiAsset(character,asset))}" alt="">`:`<span data-menu-icon="${key}" aria-hidden="true">${icon}</span>`}<small><span>${gameHudLabel(labelKey,label)}</span></small></button>`).join("")}</nav>`;
}
function gameHudDock(character){
  const item=(key,label,asset,attrs,classKey=key)=>`<button type="button" class="game-hud-button game-hud-${classKey}-button" ${attrs}><img src="${imageEsc(homeUiAsset(character,asset))}" alt=""><small><span>${gameHudLabel(key,label)}</span></small>${key==="mailbox"&&unreadMailCount()?`<em class="mail-unread-badge">${unreadMailCount()}</em>`:""}</button>`;
...
function observe(shared=false){
  ...
  if(!nativeHome){
    ...
    return `<div class="standard-observe-view" data-observed-character="${esc(c.id)}">${roster()}${townSwitcher}${desktopScene}<div class="web-observe-actions"><div data-web-discovery-rail><button type="button" data-tab="statistics">${esc(t("statistics","통계"))}</button></div><div><button type="button" data-character-command="${esc(c.id)}">${esc(t("할 일 정하기","할 일 정하기"))}</button></div></div><div class="desktop-observe-lower">${desktopLog}<section class="web-village-summary"><img src="./assets/home-ui/town.png" alt=""><h2>${esc(t("town","마을"))}</h2><p>${esc(c.name)} · ${esc(c.jobTitle||c.job||"")}</p><div><button type="button" data-tab="home">${esc(t("viewHome","집 보기"))}</button><button type="button" data-tab="town">${esc(t("town","마을"))}</button><button type="button" data-tab="groups">${esc(GAME_HUD_LABELS[state.uiLanguage]?.groups||"멀티")}</button></div></section></div>${nativeFullLog}${buildingDetailDialogs()}</div>`;
  }
