...
function nativePlayShop({browseOnly=false}={}){
  ...
  const content=section==="base"?`<div class="drawer-shop-products">${["character_slot_1","town_slot_1","storage_50mb","green_tea"].map(productCard).join("")}</div>`:`<div class="drawer-shop-coming" role="status"><span aria-hidden="true">✦</span><b>${esc(comingTitle)}</b><small>${esc(copy.soonDetail)}</small></div>`;
