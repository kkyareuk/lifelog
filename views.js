import {state,active} from "./state.js?v=20260803aq";
import {eventFor,visibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260803aq";
...
function placeCard(p){
  ...
  const homeIcon=p.iconPreset==="drawer-home";
...
function homeCard(id,chars){
  ...
  const petScene=pet=>{
    ...
    const titleMap={강아지:"활기차게 노는 중",고양이:"자기 방식대로 노는 중",새:"횃대에서 활동하는 중",거북이:"천천히 탐색하는 중",호랑이:"영역을 살피는 중",인공지능:"집 안을 지원하는 중",기타:"시간을 보내는 중"};
  ...
  const roomHtml=roomKeys.map(key=>{
    ...
    return `<div class="room ${roomClasses[key]||"custom-room"}" ${roomStyle(h,key)}>
      ...
      <div class="room-pets">${roomPets.map(p=>`<button class="room-pet" title="${esc(petScenes[p.id].desc)}">${p.icon?`<img src="${esc(p.icon)}" alt="">`:p.photo?`<img src="${esc(p.photo)}" alt="">`:`<span>${petEmoji[p.species]||"🐾"}</span>`}<b>${esc(p.name)}</b><small>${esc(petScenes[p.id].title.replace(`${h.rooms?.[key]?.name||"집 안"}에서 `,""))}</small></button>`).join("")}</div>
    </div>`;
  }).join("");
...
function relationship(){
  ...
  const cards=all.map(r=>{
    if(r.groupId){
      ...
      const direction=r.type==="짝사랑"?`${[...new Set(group.map(x=>state.characters[x.admirerId||x.a]?.name).filter(Boolean))].map(esc).join(" · ")} → ${[...new Set(group.map(x=>state.characters[x.targetId||x.b]?.name).filter(Boolean))].map(esc).join(" · ")}`:members.map(member=>esc(member.name)).join(" · ");
...
function dlc(){return `<section class="panel form dlc-store"><div class="title"><div><h1>DLC 상점</h1><p>새로운 마을 배경·건물 모양·꾸미기 팩을 이곳에서 구매할 수 있게 될 예정이에요.</p></div><span class="dlc-badge">준비 중</span></div><div class="empty-mini dlc-empty"><span>🗄️</span><b>아직 판매 중인 DLC 팩이 없어요.</b><p>첫 번째 팩이 준비되면 구성품과 가격을 확인하고 구매할 수 있어요.</p><button disabled>구매 가능한 팩 없음</button></div></section>`;}
