...
import {state,active,characterViewFor,explicitCharacterViewFor} from "./state.js?v=20260823characterrefine1";
import {eventFor as simulateEventFor,visibleTimeline as simulateVisibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260823characterrefine1";
import {SPEECH_STYLE_OPTIONS} from "./speech-styles.js?v=20260823characterrefine1";
...
function character(){
  ...
  const mobileProfileDraft=`<section class="mobile-character-profile-draft">
    ...
    <button type="button" class="character-wallet-selected" data-toggle-character-roster aria-label="${esc(t("선택된 캐릭터 바꾸기","선택된 캐릭터 바꾸기"))}"><span>${selectedIcon}</span><b><i>${t("선택됨","선택됨")}</i></b></button>
  ...
  return `<div class="editor character-editor" style="--character-own:${esc(c.theme?.primary||"#176b60")};--character-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")}">
    ...
    <section class="panel form">
      <section class="mobile-character-dashboard ${state.characterSettingsView==="full"?"is-hidden":""}" data-character-ui-version="7" style="--character-own:${esc(c.theme?.primary||"#176b60")};--character-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")}">
        ${mobileProfileDraft}
        ${hubActions}
      </section>
