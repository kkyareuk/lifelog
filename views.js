import {state,active,characterViewFor,explicitCharacterViewFor} from "./state.js?v=20260819mediafloor1";
import {eventFor as simulateEventFor,visibleTimeline as simulateVisibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260819mediafloor1";
...
function homeCard(id,chars){
  ...
  return `<article class="home panel ${edit?"is-editing":""}" data-home-card="${id}">
    ...
    <div class="rooms ${visibleRoomKeys.length>6?"has-extra":""}" data-room-canvas data-home-id="${id}" data-room-floor="${activeFloor}" style="--room-count:${visibleRoomKeys.length};--room-cols:4;--room-rows:${packedRooms.rows}">${roomHtml||`<button type="button" class="empty-floor-room" data-add-room>+ ${activeFloor}층에 방 추가</button>`}</div>
...
function settingsContent(){
  ...
  const noticeKinds=[["questions","질문과 선택"],["moments","지금 하는 일"],["relationships","관계와 선물"],["home","집과 생활"],["work","일과 학교"],["tastes","취향과 음식"]];
  const notifications=`<section class="setting-card character-notification-card"><div class="notification-setting-head"><span><small>CHARACTER CONTACT</small><h2>캐릭터 연락 알림</h2><p>선택한 캐릭터가 낮 시간에 가끔 소식을 보내거나 선택을 물어요. 답한 내용은 실제 생활 일정에 이어집니다.</p></span><button type="button" class="${state.characterNotificationsEnabled?"danger":"primary"}" data-character-notification-toggle>${state.characterNotificationsEnabled?"알림 끄기":"알림 켜기"}</button></div><p class="notification-permission-state" data-character-notification-status>${state.characterNotificationsEnabled?"알림 사용 중":state.characterNotificationConsent==="granted"?"알림 꺼짐 · 권한은 유지됨":state.characterNotificationConsent==="denied"?"휴대폰에서 알림이 거부됨":"아직 알림을 요청하지 않음"}</p><details class="notification-settings-detail" ${state.characterNotificationsEnabled?"open":""}><summary>누가, 얼마나 자주 연락할지 정하기</summary><div class="notification-settings-body"><h3>연락받을 캐릭터</h3><p>여러 명을 고르면 같은 캐릭터가 연달아 나오지 않게 번갈아 연락해요.</p><div class="notification-character-grid">${noticeCharacters}</div><div class="notification-setting-grid"><label>연락 빈도<select data-character-notification-setting="frequency"><option value="light" ${noticeSettings.frequency==="light"?"selected":""}>가끔 · 주 3회</option><option value="daily" ${noticeSettings.frequency!=="light"&&noticeSettings.frequency!=="lively"?"selected":""}>하루 1회</option><option value="lively" ${noticeSettings.frequency==="lively"?"selected":""}>활발하게 · 하루 2회</option></select></label><label>알림 말투<select data-character-notification-setting="voiceMode"><option value="mixed" ${noticeSettings.voiceMode!=="character"&&noticeSettings.voiceMode!=="concise"?"selected":""}>섞어서 · 말투와 담백한 문장</option><option value="character" ${noticeSettings.voiceMode==="character"?"selected":""}>캐릭터 말투를 적극 반영</option><option value="concise" ${noticeSettings.voiceMode==="concise"?"selected":""}>담백하게 · 말투 연출 최소화</option></select></label><label>연락 시작 시간<select data-character-notification-setting="startHour">${noticeHourOptions(9,16,noticeSettings.startHour||10)}</select></label><label>연락 종료 시간<select data-character-notification-setting="endHour">${noticeHourOptions(12,21,noticeSettings.endHour||18)}</select></label></div><h3>받고 싶은 연락</h3><div class="notification-kind-grid">${noticeKinds.map(([kind,label])=>`<label class="${(noticeSettings.contentKinds||[]).includes(kind)?"on":""}"><input type="checkbox" data-character-notification-kind="${kind}" ${(noticeSettings.contentKinds||[]).includes(kind)?"checked":""}><span>${label}</span></label>`).join("")}</div><p>같은 문구와 같은 캐릭터가 짧은 기간에 반복되지 않도록 최근 연락을 기억해요. ‘섞어서’는 캐릭터 말투를 일부 알림에만 사용합니다.</p><button type="button" data-character-notification-test ${state.characterNotificationsEnabled?"":"disabled"}>5초 뒤 시험 알림 보내기</button></div></details><small>처음 켤 때 서랍마을의 설명을 보여준 뒤 Android의 공식 알림 허용창이 한 번 나타납니다. 언제든 여기서 끄거나 휴대폰 설정에서 차단할 수 있어요.</small></section>`;
...
Object.assign(UI_TEXT.en,{
  ...
  "처음 배치할 때의 기본 크기이며, 집 화면에서는 모서리를 끌어 더 자유롭게 바꿀 수 있어요.":"This is the initial size. Drag a corner on the Home screen to resize it freely.","자동 배치로 되돌리기":"Reset to automatic layout","이 층의 자동 배치 기준으로 되돌렸어요":"This floor was reset to its automatic layout.",
...
Object.assign(UI_TEXT.ja,{
  ...
  "처음 배치할 때의 기본 크기이며, 집 화면에서는 모서리를 끌어 더 자유롭게 바꿀 수 있어요.":"最初の配置サイズです。家画面では角をドラッグして自由に変更できます。","자동 배치로 되돌리기":"自動配置に戻す","이 층의 자동 배치 기준으로 되돌렸어요":"この階を自動配置に戻しました。",
