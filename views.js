...
import {state,active,characterViewFor,explicitCharacterViewFor} from "./state.js?v=20260828relationship174";
import {eventFor as simulateEventFor,visibleTimeline as simulateVisibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260828relationship174";
import {SPEECH_STYLE_OPTIONS} from "./speech-styles.js?v=20260828relationship174";
import {furnitureFootprint,furnitureIcon,furnitureLabel,furniturePropIcon,normalizeFurniturePlacements,supportsFurnitureProps} from "./furniture-layout.js?v=20260828relationship174";
import {homeSurfaceImage,normalizeHomeSurface,normalizeWallSurface,wallSurfaceImage} from "./home-surfaces.js?v=20260828relationship174";
...
function relationship(){
  ...
  const cards=all.map(r=>{
    ...
    return a&&b?`<article class="relation" data-official-card><div class="relation-avatars">${avatar(a)}${avatar(b)}</div><h2>${heading}</h2><p>${esc(displayType(r))} · ${r.cohabit?"함께 거주":"따로 거주"}</p><p class="relation-stage">${r.temporalStatus==="past"?"과거 관계 · ":""}${esc(r.stage||"편안한 사이")}</p><p class="relation-reality">관계 실체 · ${esc(relationshipReality(r.a,r.b,[r]))}</p>${r.temporalStatus==="past"&&r.faultReason&&r.faultReason!=="정하지 않음"?`<p class="relation-fault">관계가 끝난 이유 · ${esc(r.faultReason)}</p>`:""}${relationActivities(r)}<div class="relation-actions"><button data-edit-rel="${r.id}">편집</button><button class="danger" data-delete-rel="${r.id}">삭제</button></div></article>`:"";
  }).join("");
  const map=relationshipMap(all);
  const copy=relationshipScreenCopy();
  const emptyCards=`<div class="empty-mini"><b>${copy.officialEmpty}</b></div>`;
  return `<section class="panel form relationship-page relationship-redesign">${characterViewEditor()}<dialog class="official-relation-dialog relationship-fullscreen-dialog" data-official-relation-dialog><form method="dialog"><header class="relationship-list-head"><button value="close" class="relationship-back-button" aria-label="${copy.back}"><img src="./assets/home-ui/back.png" alt=""></button><span><small>OFFICIAL RELATIONSHIPS</small><h2>${copy.officialTitle}</h2><p>${copy.officialHint}</p></span></header><div class="relationship-card-grid">${cards||emptyCards}</div><div class="relationship-list-pager" ${cards?"":"hidden"}><button type="button" data-official-page="-1">${copy.previous}</button><b data-official-page-label>1 / 1</b><button type="button" data-official-page="1">${copy.next}</button></div><button type="button" data-add-rel class="relationship-list-add">＋ ${copy.setOfficial}</button></form></dialog><dialog class="relationship-map-dialog" data-relationship-map-dialog><form method="dialog"><div class="relationship-map-dialog-head"><span><small>RELATIONSHIP MAP</small><h2>인물 관계도</h2></span><button value="close" aria-label="닫기">×</button></div><p>화살표 색은 각 캐릭터가 상대를 보는 감정 방향을 나타냅니다. 사랑하는 감정의 화살표에는 하트가 표시돼요.</p><div class="relationship-color-legend"><span class="friendly">친구·우호</span><span class="romantic">약한 사랑</span><span class="love">강한 사랑</span></div><div class="relationship-map-actions"><button type="button" data-refresh-relationship-map>현재 설정으로 새로고침</button><button type="button" data-export-relationship-map>PNG로 저장</button></div>${map||"<div class='empty-mini'>표시할 관계가 아직 없어요.</div>"}</form></dialog></section>`;
}
function routine(){
