...
import {state,active,characterViewFor,explicitCharacterViewFor} from "./state.js?v=20260827towneditor166";
import {eventFor as simulateEventFor,visibleTimeline as simulateVisibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260827towneditor166";
import {SPEECH_STYLE_OPTIONS} from "./speech-styles.js?v=20260827towneditor166";
import {furnitureFootprint,furnitureIcon,furnitureLabel,furniturePropIcon,normalizeFurniturePlacements,supportsFurnitureProps} from "./furniture-layout.js?v=20260827towneditor166";
import {homeSurfaceImage,normalizeHomeSurface,normalizeWallSurface,wallSurfaceImage} from "./home-surfaces.js?v=20260827towneditor166";
...
function townPlacementToolbar(){
  ...
  const valid=mobileTownMode==="decorations"?kind==="decoration":["place","home"].includes(kind);
...
function townMobile(){
  ...
  const townHeader=`<header class="town-native-header"><button type="button" class="home-native-back town-native-back" data-tab="observe" aria-label="${esc(t("메인 화면으로 돌아가기","메인 화면으로 돌아가기"))}"><img src="${esc(homeUiAsset(character||active(),"back.png"))}" alt=""></button><button type="button" class="town-native-title" data-open-town-switcher aria-label="${esc(t("마을 이동","마을 이동"))}"><i aria-hidden="true"></i><b>${esc(state.world.name)}</b></button><button type="button" class="town-native-info" data-mobile-town-open-settings>${t("마을 정보","마을 정보")}</button><nav aria-label="${esc(t("마을 편집","마을 편집"))}"><button type="button" class="home-native-pill ${mobileTownMode==="town"?"on":""}" data-mobile-town-layout-mode><span>${t("마을 편집","마을 편집")}</span></button><button type="button" class="home-native-pill ${mobileTownMode==="decorations"?"on":""}" data-mobile-town-decoration-mode><span>${t("마을 장식","마을 장식")}</span></button><button type="button" class="home-native-pill ${mobileTownMode==="buildings"?"on":""}" data-mobile-building-edit-mode><span>${t("건물 편집","건물 편집")}</span></button></nav></header>`;
  ...
  const editorPanel=panelType?`<aside class="panel form town-editor-panel"><div class="mobile-town-sheet-head"><span><small>${panelType==="world"?"TOWN SETTINGS":panelType==="info"?"TOWN INFO":panelType==="decorations"?"TOWN DECORATIONS":"BUILDING SETTINGS"}</small><b>${panelType==="world"?t("마을 정보 편집","마을 정보 편집"):panelType==="info"?esc(state.world.name):panelType==="decorations"?t("마을 장식","마을 장식"):panelType==="buildings"?t("건물 편집","건물 편집"):esc(selectedBuildingName)}</b></span><button type="button" data-mobile-town-close aria-label="${esc(t("편집 창 닫기","편집 창 닫기"))}">×</button></div>${generalEditor}<div class="place-editor">${placeEditors}</div></aside>`:"";
