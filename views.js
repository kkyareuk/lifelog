import {state,active} from "./state.js?v=20260803bl";
import {eventFor,visibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260803bl";
...
function shop(){
  const product=(id,label,title,description,price,ownedCount=0,repeatable=false)=>`<article class="premium-product one-time-product"><div class="premium-product-heading"><span>평생 소장</span><div><small>${label}</small><h2>${title}</h2></div><b>${price.toLocaleString("ko-KR")}원</b></div><p>${description}</p>${ownedCount?`<div class="premium-current"><b>${repeatable?`${ownedCount}회 구매 · 현재 적용 중`:"구매한 상품이에요."}</b><small>계정에 영구 적용됩니다.</small></div>`:""}${repeatable||!ownedCount?`<a class="primary premium-buy" href="./payment.html?product=${id}">${ownedCount?"한 번 더 구매하기":"구매하기"}</a>`:""}</article>`;
  return `<section class="panel form dlc-store shop-store"><div class="title"><div><h1>상점</h1><p>구독 없이 필요한 슬롯과 저장 공간만 한 번 구매해 평생 사용할 수 있어요.</p></div></div><div class="shop-product-grid">${product("character_slots_5","캐릭터 슬롯","캐릭터 5명 추가","기본 7명에 캐릭터 슬롯 5개를 영구적으로 추가합니다.",1200,Number(accountEntitlements.characterSlotPacks)||0,true)}${product("town_slot_1","마을 슬롯","마을 1개 추가","기본 2개에 새로운 마을 슬롯 1개를 영구적으로 추가합니다.",1900,Number(accountEntitlements.townSlotPacks)||0,true)}${product("storage_50mb","사진 저장 공간","총 50MB로 확장","계정 사진 저장 한도를 20MB에서 50MB로 영구 확장합니다.",2900,accountEntitlements.storage50?1:0,false)}</div><section class="shop-coming"><h2>테마 DLC</h2><p>전용 건물·배경·생활 스크립트를 갖춘 콘텐츠를 준비하고 있어요.</p></section><div class="dlc-hidden" hidden>${dlc()}</div></section>`;
}
