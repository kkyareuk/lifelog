...
import {state,active,characterViewFor,explicitCharacterViewFor} from "./state.js?v=20260827townfix169";
import {eventFor as simulateEventFor,visibleTimeline as simulateVisibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260827townfix169";
import {SPEECH_STYLE_OPTIONS} from "./speech-styles.js?v=20260827townfix169";
import {furnitureFootprint,furnitureIcon,furnitureLabel,furniturePropIcon,normalizeFurniturePlacements,supportsFurnitureProps} from "./furniture-layout.js?v=20260827townfix169";
import {homeSurfaceImage,normalizeHomeSurface,normalizeWallSurface,wallSurfaceImage} from "./home-surfaces.js?v=20260827townfix169";
...
function townBuildingBrowser(character){
  ...
  return `<aside class="town-building-screen town-building-browser-screen" aria-label="${esc(t("건물 정보","건물 정보"))}"><div class="town-building-screen-head"><button type="button" class="town-building-screen-back" data-mobile-town-close aria-label="${esc(t("마을로 돌아가기","마을로 돌아가기"))}"><img src="${esc(homeUiAsset(character||active(),"back.png"))}" alt=""></button><label class="town-building-search"><span class="sr-only">${t("건물 검색","건물 검색")}</span><input type="search" data-building-search placeholder="${esc(t("검색","검색"))}"></label></div>${pinnedFilter("town",townFilters)}${pinnedFilter("type",typeFilters)}<div class="town-building-card-grid" aria-live="polite">${entries.map(entry=>`<button type="button" class="town-building-browser-card" data-building-browser-card data-building-browser-open="${entry.kind==="home"?`home:${entry.id}`:entry.id}" data-building-browser-town="${entry.townId}" data-building-browser-type="${esc(entry.type)}" data-building-browser-name="${esc(entry.name)}"><span><img src="${esc(entry.art)}" alt=""></span><b>${esc(entry.name)}</b><small>${esc(entry.townName)}</small></button>`).join("")}</div><button type="button" class="town-building-add" data-add-place aria-label="${esc(t("건물 추가","건물 추가"))}">＋</button></aside>`;
}
