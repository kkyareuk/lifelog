...
import {state,active,characterViewFor,explicitCharacterViewFor} from "./state.js?v=20260827tablet165";
import {eventFor as simulateEventFor,visibleTimeline as simulateVisibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260827tablet165";
import {SPEECH_STYLE_OPTIONS} from "./speech-styles.js?v=20260827tablet165";
import {furnitureFootprint,furnitureIcon,furnitureLabel,furniturePropIcon,normalizeFurniturePlacements,supportsFurnitureProps} from "./furniture-layout.js?v=20260827tablet165";
import {homeSurfaceImage,normalizeHomeSurface,normalizeWallSurface,wallSurfaceImage} from "./home-surfaces.js?v=20260827tablet165";
...
const BUILDING_PRESET_SOURCES={"type-restaurant":"world-assets/building-types/restaurant-handdrawn.png","drawer-building":"world-assets/drawer-building.png","drawer-home":"world-assets/drawer-home.png","medieval-castle":"world-assets/medieval-castle.svg","medieval-tavern":"world-assets/medieval-tavern.svg","medieval-market":"world-assets/medieval-market.svg"};
...
const homeExteriorSource=home=>String(home?.exteriorImage||BUILDING_PRESET_SOURCES["drawer-home"]);
...
function header(){
  ...
  const nativeBar=["observe","home"].includes(state.activeTab)?"":`<div class="native-sub-header"><button type="button" data-tab="observe" aria-label="${esc(t("메인 화면으로 돌아가기","메인 화면으로 돌아가기"))}">‹</button><b>${current[1]}</b><span>${esc(t("brandName","서랍마을"))}</span></div>`;
...
let mobileTownBuildingEditing=false;
let mobileTownPanel="";
...
export function setMobileTownEditing(value){
  ...
  if(mobileTownEditing)mobileTownBuildingEditing=false;
  if(!mobileTownEditing){mobileTownPanel="";mobileTownPlacement=null}
}
export function setMobileTownBuildingEditing(value){
  mobileTownBuildingEditing=Boolean(value);
  if(mobileTownBuildingEditing){mobileTownEditing=false;mobileTownPanel="";mobileTownPlacement=null}
}
...
function placeCard(p,editable=false){
  ...
  const preset=buildingExteriorSource(p);
  ...
  const quickDelete=canDelete&&!mobileTownEditing?`<button type="button" class="place-quick-delete" style="left:${p.x}%;top:${p.y}%" data-delete-place="${p.id}" aria-label="${esc(p.name||"건물")} 삭제" title="건물 삭제">×</button>`:"";
  const selected=mobileTownPlacement?.kind==="place"&&mobileTownPlacement.id===p.id;
  return `<button type="button" class="place has-art map-art-button ${selected?"placement-selected":""}" style="left:${p.x}%;top:${p.y}%;z-index:${Number(p.mapZ)||10};--place:${p.color};--place-scale:${p.imageScale||1};--place-flip:${p.flipX?-1:1}" data-place="${p.id}" data-building-detail-open="${p.id}" aria-label="${esc(p.name)} 건물 정보 보기"><img class="building-preset-image" src="${preset}" alt=""></button>${label}${quickDelete}`;
}
function townHomes(){
...
function homeMapCard(home){
  const mode=state.buildingLabelMode||"full";
  const label=mode==="none"?"":`<span class="map-place-label home-map-label" style="left:${home.mapX}%;top:${home.mapY}%"><b>${esc(home.name)}</b>${mode==="full"?`<small>${esc(`집 · ${home.buildingSubtype||"단독주택"}`)}</small>`:""}</span>`;
  const selected=mobileTownPlacement?.kind==="home"&&mobileTownPlacement.id===home.id;
  return `<button type="button" class="place has-art map-art-button home-map-place ${selected?"placement-selected":""}" style="left:${home.mapX}%;top:${home.mapY}%;z-index:${Number(home.mapZ)||12};--place-scale:${home.mapScale||1.08};--place-flip:${home.mapFlipX?-1:1}" data-home-map="${home.id}" data-building-detail-open="home:${home.id}" aria-label="${esc(home.name)} 집 정보 보기"><img class="building-preset-image" src="${esc(home.exteriorImage||"world-assets/drawer-home.png")}" alt=""></button>${label}`;
}
...
function tabletObserveMap(){
  ...
  return `<section class="tablet-observe-map" aria-label="${esc(t("town","마을"))}"><div class="tablet-observe-world"><img src="${esc(town?.bg||state.world?.bg||TOWN_BACKGROUND)}" class="world-bg" alt="">${state.world.places.map(placeCard).join("")}${townHomes().map(homeMapCard).join("")}${townDecorationsMarkup()}${state.world.places.map(peopleAtPlaceCard).join("")}${townHomes().map(peopleAtHomeCard).join("")}</div></section>`;
}
...
function nativeSceneActionProp(person,entry,actionKind,text,individual=false){
  ...
  const image=hasCatalogPhoto?`<img src="${esc(item.image)}" alt="">`:esc(symbol);
  const title=item?.name?`${person?.name||"캐릭터"} · ${item.name}`:`${person?.name||"캐릭터"} · ${symbol}`;
  return `<span class="${individual?"native-person-action-prop":"native-scene-action-prop"} action-prop-${actionKind}${propVariant}${hasCatalogPhoto?" catalog-food-photo":""}" title="${esc(title)}" aria-hidden="true">${image}</span>`;
}
...
function nativeScenePresentation(c,entry,visualMode="sd"){
  ...
  const lineupHtml=companions.length?`<span class="native-scene-lineup ${coResidentConversation?"is-conversation":""} ${sceneParticipants.length===2?"is-pair":""}" style="--scene-count:${sceneParticipants.length}" aria-label="${esc(sceneParticipants.map(person=>person.name).join(", "))}">${sceneParticipants.map((person,index)=>{
    ...
    return `<span class="native-scene-lineup-person ${walkStyleClassFor(person)} ${pairSlot} ${person.id===c.id?"is-current is-selected":""} ${visualMode==="ld"&&hasLdArt(person)?"is-ld":""}" style="--scene-index:${index};--scene-delay:${delay}s;--scene-duration:${duration}s;--person-visual-scale:${personVisualScale};${sceneLayoutVars(person,visualMode)}">${sceneAvatar(person,"native-scene-lineup-avatar",tone,visualMode)}${actionProp}${sleepBadge}${tone==="date-overwhelmed"&&person.id===c.id?'<b class="native-character-sweat" aria-hidden="true">💧</b>':""}<small>${esc(person.name)}</small></span>`;
  }).join("")}</span>`:"";
...
function townTravelersMarkup(homeId=""){
  ...
  return travelers.map((character,index)=>{
    ...
    const symbol=conversation?"💬":decoration?"✨":scene.movementKind==="jog"?"🏃":scene.transit?"➜":"•",label=conversation?t("마을에서 대화 중","마을에서 대화 중"):decoration?`${decoration.name} · ${scene.title}`:scene.movementKind==="jog"?t("조깅 · 이동 중","조깅 · 이동 중"):scene.transit?t("이동 중","이동 중"):t("마을 산책 중","마을 산책 중");
...
function buildingDetailDialogs(){
  ...
  const homeDialogs=townHomes().map(home=>{
    ...
    return `<dialog class="building-detail-dialog" data-building-detail-dialog="home:${home.id}"><form method="dialog"><header><span><small>HOME</small><h2>${esc(home.name)}</h2></span><button value="close" aria-label="닫기">×</button></header><div class="building-detail-layout"><section class="building-interior-view">${interior}</section><section class="building-detail-info"><dl><div><dt>집 유형</dt><dd>${esc(home.kind||"일반 주거")}</dd></div><div><dt>외관 분위기</dt><dd>${esc(home.exteriorStyle||"설정하지 않음")}</dd></div><div><dt>방 수</dt><dd>${roomEntries.length}개</dd></div><div><dt>거주자로 연결된 캐릭터</dt><dd>${esc(residentNames)}</dd></div></dl><div><h3>지금 집 안에 있는 캐릭터 <small>${current.length}명</small></h3><div class="building-resident-list">${currentCards}</div></div>${home.notes?`<div><h3>집 메모</h3><p>${esc(home.notes)}</p></div>`:""}<div class="building-home-edit-actions"><button type="button" data-home-exterior-image="${home.id}">집 외형 바꾸기</button><button type="button" data-open-home-editor="${home.id}">집 설정 열기</button></div></section></div><button class="primary building-detail-close" value="close">닫기</button></form></dialog>`;
  }).join("");
...
function observe(){
  ...
  if(!localId){
    ...
    return `<div class="standard-observe-view">${roster()}${townSwitcher}<div class="observe desktop-observe-map-only"><section><div class="viewport"><div class="world"><img src="${TOWN_BACKGROUND}" class="world-bg">${state.world.places.map(placeCard).join("")}${townHomes().map(homeMapCard).join("")}${townDecorationsMarkup()}</div></div></section></div>${buildingDetailDialogs()}</div>`;
  }
  ...
  const locationExterior=sceneHome?"world-assets/drawer-home.png":place?buildingExteriorSource(place):"";
  ...
  const sceneActors=`${presentation.lineupHtml?"":sceneAvatar(c,"native-main-character",presentation.tone,visualMode)}${presentation.sleepMarkHtml}${presentation.lineupHtml}${presentation.conversationHtml}${presentation.thoughtHtml}${presentation.actionHtml}${presentation.companionHtml}${presentation.petHtml}${movingBadge}<i></i>`;
  if(!nativeHome){
...
function roomStyle(h,key,layout,mobileLayout){
  ...
  const fullRoomIllustration=floorMaterial==="custom";
...
function homeLifePersonMarkup(character,event,agent,room,roomKey,index,bedSlot=-1,options={}){
  ...
  const now=Date.now(),scheduledWalk=agent?.phase==="walking",walking=scheduledWalk&&now>=Number(agent?.startedAt||0),presentation=nativeScenePresentation(character,scene,"sd"),sleeping=!scheduledWalk&&presentation.actionKind==="sleep",conversing=Boolean(options.conversing);
...
function homeCard(id,chars){
  ...
  const roomHtml=visibleRoomKeys.map(key=>{
    ...
    shownPeople.forEach((character,index)=>{
      ...
      const agent=lifeAgents[character.id],tvPartners=agent?.actionKind==="watch"?(tvGroups.get(agent.furnitureId||`${agent.roomKey}:watch`)||[]):[];
  ...
  const furnitureToolbar=edit?`<nav class="furniture-edit-toolbar" data-furniture-edit-toolbar hidden aria-label="선택한 가구 편집"><strong data-furniture-edit-name>가구</strong><button type="button" data-furniture-command="smaller" aria-label="가구 작게">−</button><button type="button" data-furniture-command="larger" aria-label="가구 크게">＋</button><button type="button" data-furniture-command="rotate" aria-label="가구 회전">↻</button><button type="button" data-furniture-command="back" aria-label="가구 뒤로">↓</button><button type="button" data-furniture-command="front" aria-label="가구 앞으로">↑</button><button type="button" data-furniture-command="props">${propToolbarLabel}</button><button type="button" class="danger" data-furniture-command="delete">삭제</button><button type="button" class="primary" data-furniture-command="done">완료</button></nav>`:"";
...
function characterTraitChoice(c){
  ...
  return `<section class="character-trait-settings">
    ...
    <label class="trait-note-field"><span><b>사용자 정의 특성 표현 · 선택 사항</b><small>위 선택지에 없는 표현만 적어 주세요. 고른 ‘주의가 쉽게 전환됨’ 같은 항목은 메모 없이도 생활 장면 후보에 반영됩니다.</small></span><textarea data-trait-notes maxlength="1200" rows="5" autocomplete="off" autocorrect="off" spellcheck="false" placeholder="한 줄에 한 문장씩 적어 주세요. 예: 대화가 격해지면 창가로 물러나 호흡을 고른다.">${esc(c.traitNotes||"")}</textarea></label>
...
function character(){
  ...
  const overviewControls=overviewPane==="basic"
  ...
  const profileOverviewPane=`<section class="character-profile-overview-page" data-overview-page="${overviewPane}"><div class="character-overview-portrait">${overviewPortrait}</div>${overviewControls}${overviewPane==="basic"?overviewBasic:overviewLife}</section>`;
  ...
  const wardrobeTagButton=(field,values)=>`<div class="book-form-field wardrobe-tag-field"><button type="button" data-open-book-list="${field}" data-book-list-path="${field}"><span>${values?.length?`${t("의상 태그","의상 태그")} · ${values.length}${t("개 선택됨","개 선택됨")}`:t("의상 태그 선택","의상 태그 선택")}</span><i aria-hidden="true">${t("열기","열기")}</i></button></div>`;
  const wardrobeLookField=(label,typeField,typeOptions,currentType,tagField,tags)=>`<div class="book-form-stack wardrobe-look-field">${bookField(label,typeField,typeOptions,currentType)}${wardrobeTagButton(tagField,tags||[])}</div>`;
  const wardrobePane=`<section class="character-book-form-page wardrobe-book-page">
    <div class="book-form-grid">
      <div class="book-form-stack"><label class="book-form-field"><b>${t("화장 정도","화장 정도")}</b>${bodySelect("appearance.makeupLevel",MAKEUP_LEVELS,bodyAppearance.makeupLevel||"하지 않음")}</label>${bookListButton("화장 스타일","makeupStyles",bodyAppearance.makeupStyles||[],"bodyProfile.appearance.makeupStyles")}</div>
      <div class="book-form-stack">${bookField("성형·외형 의료 시술","cosmeticSurgery",["설정하지 않음","하지 않음","상담만 받음","비수술 시술","수술 경험 있음"],c.cosmeticSurgery||bodyAppearance.cosmeticSurgery||"설정하지 않음")}${bookListButton("시술 부위","cosmeticSurgeryAreas",bodyAppearance.cosmeticSurgeryAreas||[],"bodyProfile.appearance.cosmeticSurgeryAreas")}</div>
      <div class="book-form-stack">${bookField("패션","fashionSense",["패션에 전혀 관심 없음","조합을 자주 틀림","무난하게 입음","센스 있게 입음","스타일링에 능숙함"],c.fashionSense||"무난하게 입음")}${bookListButton("의상 태그","favoriteFashionStyles",c.favoriteFashionStyles||[])}</div>
      <div class="book-form-stack">${bookField("미용실 방문 빈도","salonFrequency",["자동 · 설정에 맞춤","거의 가지 않음","필요할 때만","두세 달에 한 번","매달","월 2회 이상"],bodyAppearance.salonFrequency||"자동 · 설정에 맞춤")}${bookField("미용실에서 하는 일","salonPurpose",["설정하지 않음","커트","커트·염색","커트·염색·펌","두피·모발 관리까지"],c.salonPurpose||"설정하지 않음")}</div>
      ...
      ${wardrobeLookField("작업복","workwearType",["설정하지 않음","제복","정장","비즈니스 캐주얼","실용복","보호 작업복"],c.workwearType||"설정하지 않음","workwearTags",c.workwearTags)}
      ${wardrobeLookField("수영복","swimwearType",["설정하지 않음","래시가드","원피스 수영복","비키니","트렁크","전신 수영복"],c.swimwearType||"설정하지 않음","swimwearTags",c.swimwearTags)}
      ${wardrobeLookField("잠옷","sleepwearType",["설정하지 않음","평상복","파자마","가운","슬립","속옷","알몸"],c.sleepwearType||"설정하지 않음","sleepwearTags",c.sleepwearTags)}
      ${wardrobeLookField("파티복","partywearType",["설정하지 않음","정장","드레스","칵테일 드레스","전통 의상","코스튬"],c.partywearType||"설정하지 않음","partywearTags",c.partywearTags)}
  ...
  const personalityBookPane=state.characterPersonalityPane==="details"?personalityDetailsPane:personalityCorePane;
  const tasteCategories=[['관심사','interests'],['취미','hobbies'],['좋아하는 장르','favoriteStoryGenres'],['기술 숙련','skills'],['좋아하는 음식','foodPreferences'],['좋아하는 음료','drinks'],['좋아하는 음악','musicGenres'],['좋아하는 영상','favoriteVideoGenres'],['좋아하는 게임','favoriteGameGenres'],['좋아하는 향','favoriteScentNotes'],['좋아하는 동물','favoriteAnimals'],['좋아하는 전자기기','favoriteElectronics'],['좋아하는 무기','favoriteWeapons'],['좋아하는 책','favoriteBooks']];
  const tasteCategoriesPane=`<section class="character-book-form-page taste-book-page"><div class="book-form-grid">${tasteCategories.map(([label,field])=>bookListButton(label,field,c[field]||[])).join("")}</div>${bookPageControls(10,'data-character-personality-pane="details" data-character-pane="personality"','data-character-taste-pane="catalog"')}</section>`;
  ...
  const tasteCatalogPane=`<section class="character-book-form-page taste-book-page"><div class="book-form-grid">${catalogTasteKinds.map(([label,kind])=>catalogSelectionButton(label,kind,'favorites')).join("")}</div>${bookPageControls(11,'data-character-taste-pane="categories"','data-character-pane="worldTaste"')}</section>`;
  const tasteBookPane=state.characterTastePane==="catalog"?tasteCatalogPane:tasteCategoriesPane;
  const inventoryPane=`<section class="character-book-form-page inventory-book-page"><div class="book-form-grid">${catalogTasteKinds.map(([label,kind])=>catalogSelectionButton(label.replace("좋아하는","보유한"),kind,'inventory')).join("")}</div>${bookPageControls(12,'data-character-taste-pane="catalog" data-character-pane="taste"','disabled')}</section>`;
  const bookListSources={
    ...
    favoriteFashionStyles:{groups:WARDROBE_TAG_GROUPS},workwearTags:{groups:WARDROBE_TAG_GROUPS},swimwearTags:{groups:WARDROBE_TAG_GROUPS},sleepwearTags:{groups:WARDROBE_TAG_GROUPS},partywearTags:{groups:WARDROBE_TAG_GROUPS},
    characterTraits:{groups:{"서사·인지 특성":CHARACTER_TRAITS}},traitExpressions:{groups:{"장면 표현":TRAIT_EXPRESSIONS}},
  ...
  const paneMeta={
    ...
    personality:["성격과 말투","성향·서사·인지·표현","03"],
  ...
  const fullActivePane=["visual","profile","body","wardrobe","personality","taste","worldTaste"].includes(state.characterPane)?state.characterPane:"visual";
  const fullPane=fullActivePane==="visual"?characterFullOverview(c):fullActivePane==="profile"?profileOverviewPane:fullActivePane==="body"?bodyPane:fullActivePane==="wardrobe"?wardrobePane:fullActivePane==="personality"?personalityBookPane:fullActivePane==="taste"?tasteBookPane:inventoryPane;
  ...
  const fullPaneMeta=[["visual","사진·색상·배치"],["profile","개요"],["body","신체"],["wardrobe","옷장"],["personality","성격"],["taste","취향"],["worldTaste","소지품"]];
  ...
  return `<div class="editor character-editor" style="--character-own:${esc(c.theme?.primary||"#176b60")};--character-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")};--character-accent:${esc(c.theme?.primary||"#176b60")};--character-accent-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")}">
    ...
    <section class="panel form">
      ...
      <section class="character-book-v8 ${state.characterSettingsView==="full"?"is-open":""}" ${state.characterSettingsView==="full"?"":"hidden"} data-character-full-ui-version="9" data-full-pane="${fullActivePane}" style="--character-accent:${esc(c.theme?.primary||"#176b60")};--character-accent-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")}">
        <div class="character-book-v8-canvas">
          <span class="character-book-v8-stage" aria-hidden="true"><img class="character-book-v8-wood" src="./assets/character-ui/character-book-stage-v14.png" alt=""></span>
        ...
        ${layoutDialog}${bookListDialog}${bookCatalogDialog}
      </section>
...
function mailbox(){
  const pending=state.dailyQuestion&&!state.dailyQuestion.answered?state.dailyQuestion:null;
...
export function catalogCardMarkup(kind,item){
...
function catalog(){
  const sections=Object.entries(CATALOG_LABELS).map(([kind,label])=>{
    const cards=(state.catalog?.[kind]||[]).map(item=>catalogCardMarkup(kind,item)).join("")||"<p data-catalog-empty>아직 등록된 항목이 없어요.</p>";
    return `<section class="catalog-kind catalog-section" data-catalog-section="${kind}"><div class="title"><h2>${label}</h2><button type="button" data-add-catalog="${kind}">+ 추가</button></div><div class="catalog-dex-grid" data-catalog-grid="${kind}">${cards}</div></section>`;
  }).join("");
...
const characterViewEditor=()=>{
  ...
  const basicFields=`${field(sourceId,targetId,"overall","전체적인 감정","공식 관계와 별개인 이 캐릭터만의 속마음")}${field(sourceId,targetId,"importance","중요도","이 캐릭터의 삶에서 상대가 얼마나 중요한지 정해요.")}${field(sourceId,targetId,"trust","신뢰","좋아하더라도 믿지 않을 수 있어요.")}${field(sourceId,targetId,"closeness","정서적 친밀감","상대를 자기 삶의 얼마나 안쪽 사람으로 느끼는지예요.")}${field(sourceId,targetId,"comfort","함께 있을 때의 편안함","둘이 같은 공간에 있을 때의 편안함과 대화 호흡을 정해요.")}`;
  const advancedFields=`${field(sourceId,targetId,"awareness","감정 자각","자기 마음을 우정·경쟁심·불편함으로 잘못 해석할 수도 있어요.")}${field(sourceId,targetId,"mutualAwareness","상대의 마음을 아는 정도","상대의 감정을 얼마나 파악하고 있는지 정해요.")}${field(sourceId,targetId,"fear","두려움 정도","상대를 얼마나 우습게 보거나 두려워하는지 강도를 정해요.")}${field(sourceId,targetId,"annoyance","성가심","좋아하고 사랑하면서도 많이 귀찮아할 수 있어요.")}${field(sourceId,targetId,"attention","챙기고 신경 쓰는 정도","상태와 일정을 얼마나 살필지 정해요.")}${field(sourceId,targetId,"jealousy","질투·독점욕","사랑과 별개로 정해요.")}${field(sourceId,targetId,"conflictIntensity","갈등 강도","사랑이나 가족애와 별개인 실제 충돌 강도예요.")}${field(sourceId,targetId,"expectation","관계에 대한 기대","이 관계가 얼마나 이어질 거라 생각하는지 정해요.")}${field(sourceId,targetId,"touchIntensity","허용하고 표현하는 스킨십 범위","두 캐릭터의 범위가 다르면 더 낮은 쪽까지만 반영돼요.")}${field(sourceId,targetId,"aggression","공격·위해 충동","충동만으로 실제 공격하지 않아요.")}${field(sourceId,targetId,"aggressionAction","충동을 실제로 표현하는 단계","충동 단계보다 센 행동은 절대 나오지 않아요.")}`;
  ...
  return `<section class="character-view-editor" style="--relationship-own:${esc(source.theme?.primary||"#176b60")};--relationship-own-secondary:${esc(source.theme?.secondary||source.theme?.primary||"#176b60")}"><div class="title"><div><h2>관계와 캐릭터별 시선</h2><p>두 이름을 눌러 누구의 마음이 누구에게 향하는지 고르세요.</p></div></div>
    <div class="relationship-pair-magnet">
      <div class="relationship-pair-core">
        ...
        <button type="button" class="primary" data-open-view-dialog="${sourceId}:${targetId}">관계 설정하기</button>
    ...
    <dialog class="character-view-dialog" data-view-dialog="${sourceId}:${targetId}"><form method="dialog"><div class="title character-view-dialog-title"><div class="character-view-dialog-direction">${avatar(source)}<i>→</i>${avatar(target)}<span><h2>${esc(source.name)} → ${esc(target.name)}</h2><small>${esc(subjectText(source.name))} ${esc(objectText(target.name))} 바라보는 감정과 행동 기준</small></span></div><button value="close" aria-label="닫기">×</button></div><div class="character-view-dialog-context"><b>${officialText?`공식 관계 · ${esc(officialText)}`:"공식 관계 없음 · 이방인"}</b><span>${esc(overall)}</span></div><section class="relationship-basic-settings"><div class="settings-section-heading compact"><span><small>BASIC RELATIONSHIP</small><h3>기본 설정</h3></span><p>감정과 친밀감처럼 자주 쓰는 항목만 먼저 보여요.</p></div><div class="character-view-fields">${basicFields}</div></section><details class="settings-advanced-group relationship-advanced-settings"><summary><span><small>DETAILS</small><b>고급 설정</b><em>감정 자각·질투·갈등·스킨십·행동 안전</em></span><i aria-hidden="true">＋</i></summary><div class="settings-advanced-body"><div class="character-view-fields">${advancedFields}</div></div></details><div class="crop-actions"><button type="button" data-reset-character-view="${sourceId}:${targetId}">이 관계 설정 초기화</button><button class="primary" value="close">편집 완료</button></div></form></dialog>
...
function relationship(){
  ...
  const characterGroups=(state.characterGroups||[]).map(group=>{const members=(group.memberIds||[]).map(id=>state.characters[id]).filter(Boolean);return `<article class="character-group-card"><div class="character-group-avatars">${members.length?members.slice(0,5).map(member=>avatar(member)).join(""):'<span class="avatar">＋</span>'}</div><span class="character-group-copy"><b>${esc(group.name)}</b><small>${members.length?members.map(member=>esc(member.name)).join(" · "):"구성원 없음"}</small></span><div class="character-group-actions"><button type="button" data-edit-character-group="${esc(group.id)}">편집</button><button type="button" class="danger" data-delete-character-group="${esc(group.id)}">삭제</button></div></article>`}).join("");
  const characterGroupSection=`<section class="character-group-manager"><div class="title"><span><small>CHARACTER GROUPS</small><h2>캐릭터 그룹</h2><p>빌런즈처럼 자주 함께 움직이는 인물을 묶으면 일정에서 한 번에 선택할 수 있어요.</p></span><button type="button" class="primary" data-add-character-group>+ 그룹 만들기</button></div><div class="character-group-grid">${characterGroups||'<div class="empty-mini">아직 만든 그룹이 없어요.</div>'}</div></section>`;
  const emptyCards='<div class="empty-mini"><b>아직 설정한 공식 관계가 없어요.</b><p>공식 관계가 없는 캐릭터끼리는 서로 낯선 사람으로 행동해요.</p></div>';
  return `<section class="panel form relationship-page"><div class="title"><h1>관계</h1></div><p>공식 관계와 각 캐릭터의 서로 다른 속마음을 한 화면에서 설정해요. 설정한 시선은 생활 장면의 말투, 접근 방식, 접촉과 갈등에 반영돼요.</p>${characterGroupSection}${characterViewEditor()}<dialog class="official-relation-dialog" data-official-relation-dialog><form method="dialog"><div class="relationship-map-dialog-head"><span><small>OFFICIAL RELATIONSHIPS</small><h2>공식 관계 목록</h2></span><button value="close" aria-label="닫기">×</button></div><p>관계를 눌러 구성과 단계를 편집하거나 삭제할 수 있어요.</p><div class="relationship-card-grid">${cards||emptyCards}</div><button class="primary official-relation-dialog-close" value="close">닫기</button></form></dialog><dialog class="relationship-map-dialog" data-relationship-map-dialog><form method="dialog"><div class="relationship-map-dialog-head"><span><small>RELATIONSHIP MAP</small><h2>인물 관계도</h2></span><button value="close" aria-label="닫기">×</button></div><p>화살표 색은 각 캐릭터가 상대를 보는 감정 방향을 나타냅니다. 사랑하는 감정의 화살표에는 하트가 표시돼요.</p><div class="relationship-color-legend"><span class="friendly">친구·우호</span><span class="romantic">약한 사랑</span><span class="love">강한 사랑</span></div><div class="relationship-map-actions"><button type="button" data-refresh-relationship-map>현재 설정으로 새로고침</button><button type="button" data-export-relationship-map>PNG로 저장</button></div>${map||"<div class='empty-mini'>표시할 관계가 아직 없어요.</div>"}</form></dialog></section>`;
}
function routine(){
  ...
  const monthly=`<div class="monthly-view"><div class="monthly-heading"><button type="button" class="routine-month-back" data-routine-month-step="-1" aria-label="${routineText.previous}"></button><h2>${monthTitle}</h2><button type="button" class="routine-month-next" data-routine-month-step="1" aria-label="${routineText.next}"></button></div><div class="monthly-weekdays">${days.map(day=>`<b>${day}</b>`).join("")}</div><div class="monthly-calendar">${calendarCells}</div></div>`;
...
function screenScaleSettings(){
  ...
  return `<section class="setting-card font-setting-card"><h2>글자와 화면 크기</h2><p>화면에 표시되는 글자의 크기만 조절합니다. 사이트 글꼴은 화면 디자인에 맞는 서랍마을 글꼴로 표시돼요.</p><div class="font-setting-grid"><label>글자 크기<select data-setting="uiScale">${sizes.map(([value,label])=>`<option value="${value}" ${state.uiScale===value?"selected":""}>${label}</option>`).join("")}</select></label></div><div class="font-preview"><b>서랍마을의 오늘</b><span>캐릭터들이 각자의 하루를 보내고 있어요. 긴 생활 로그도 편안하게 읽어 보세요.</span></div></section>`;
}
...
Object.assign(UI_TEXT.en,{
  ...
  "서사·인지 특성 · 선택 사항":"Narrative & cognitive traits · optional","설정 라벨 · 최대 8개":"Setting labels · up to 8","실제 장면에 반영할 표현 · 최대 8개":"Expressions used in scenes · up to 8","사용자 정의 특성 표현 · 선택 사항":"Custom trait expressions · optional","복수 자아·다중 정체성 설정":"Multiple selves · plural identity setting","자폐 스펙트럼 설정":"Autism spectrum setting","해리 경험":"Dissociative experiences","틱·투렛 관련 특성":"Tic · Tourette-related trait","기타 직접 설정":"Other custom setting","주의가 쉽게 전환됨":"Attention shifts easily","익숙한 순서가 바뀌면 힘듦":"Struggles when familiar routines change","자아마다 말투·선호가 다름":"Each self has different speech and preferences","타인의 감정을 직관보다 관찰과 추론으로 파악함":"Understands others' emotions through observation and reasoning rather than intuition","죄책감이나 공감이 낮게 표현됨":"Guilt or empathy is expressed weakly",
...
Object.assign(UI_TEXT.ja,{
  ...
  "서사·인지 특성 · 선택 사항":"物語・認知特性・任意","설정 라벨 · 최대 8개":"設定ラベル・最大8個","실제 장면에 반영할 표현 · 최대 8개":"実際の場面に反映する表現・最大8個","사용자 정의 특성 표현 · 선택 사항":"ユーザー定義の特性表現・任意","복수 자아·다중 정체성 설정":"複数の自己・多重アイデンティティ設定","자폐 스펙트럼 설정":"自閉スペクトラム設定","해리 경험":"解離体験","틱·투렛 관련 특성":"チック・トゥレット関連の特性","기타 직접 설정":"その他の個別設定","주의가 쉽게 전환됨":"注意が移りやすい","익숙한 순서가 바뀌면 힘듦":"慣れた順序が変わるとつらい","자아마다 말투·선호가 다름":"自己ごとに話し方や好みが異なる","타인의 감정을 직관보다 관찰과 추론으로 파악함":"他人の感情を直感より観察と推論で理解する","죄책감이나 공감이 낮게 표현됨":"罪悪感や共感が弱く表現される",
...
function townHomeEditor(home,selected){
  ...
  return `<details class="town-home-building ${selected?"mobile-selected":""}" ${selected?"open":""}><summary><b>🏠 ${esc(home.name)}</b></summary><div class="town-building-editor-card"><label class="town-building-name">${t("건물 이름","건물 이름")}<input data-home-name data-home-id="${home.id}" value="${esc(home.name)}"></label><div class="town-building-grid"><label>${t("건물 유형","건물 유형")}<select aria-label="${esc(t("건물 유형","건물 유형"))}"><option selected>${t("집","집")}</option></select></label><span aria-hidden="true"></span><label>${t("세부 유형","세부 유형")}<select data-home-field="buildingSubtype" data-home-id="${home.id}">${HOME_BUILDING_SUBTYPES.map(value=>`<option ${value===subtype?"selected":""}>${t(value,value)}</option>`).join("")}</select></label><span aria-hidden="true"></span><label class="town-building-action">${t("건물 모양","건물 모양")}<button type="button" data-home-exterior-image="${home.id}">${home.exteriorImage?t("건물 모양 변경","건물 모양 변경"):t("건물 모양 선택","건물 모양 선택")}</button></label><label class="town-building-action">${t("집 선택 사진","집 선택 사진")}<span><button type="button" data-home-bg="${home.id}">${home.image?t("사진 변경","사진 변경"):t("사진 추가","사진 추가")}</button><button type="button" data-image-url="home" data-id="${home.id}">${t("링크","링크")}</button>${home.image?`<button type="button" data-clear-home-bg="${home.id}">${t("지우기","지우기")}</button>`:""}</span></label></div><p class="town-home-editor-help">${t("집의 방·거주 설정은 집 화면에서 이어서 편집할 수 있어요.","집의 방·거주 설정은 집 화면에서 이어서 편집할 수 있어요.")}</p><button type="button" class="town-open-home-details" data-open-home-editor="${home.id}">${t("방·거주 설정 열기","방·거주 설정 열기")}</button></div></details>`;
}
const TOWN_DECORATION_CHOICES=[
  ["bench","🪑","벤치"],["lamp","💡","가로등"],["fountain","⛲","분수"],["tree","🌳","나무"],
  ["flowers","🌸","꽃밭"],["statue","🗿","조각상"],["sign","🪧","안내판"],["vending","🥤","자판기"]
];
function townPlacementToolbar(){
  if(!mobileTownEditing||!mobileTownPlacement)return "";
...
function townMobile(){
  ...
  if(mobileTownPanel&&!["world","decorations"].includes(mobileTownPanel)&&!selectedPlace&&!selectedHome)mobileTownPanel="";
  const panelType=mobileTownPanel==="world"?"world":mobileTownPanel==="decorations"?"decorations":selectedPlace?"place":selectedHome?"home":"";
  const localIds=state.order.filter(id=>visibleTownId(state.characters[id])===state.activeTownId);
  const characterId=localIds.includes(state.activeId)?state.activeId:localIds[0];
  const character=state.characters[characterId];
  const desktopTabs=`<div class="town-tabs">${state.towns.map(t=>`<button data-town-select="${t.id}" class="${t.id===state.activeTownId?"on":""}">🏙️ ${esc(t.name)}</button>`).join("")}${mobileTownEditing?`<button data-add-town>+ 마을 추가</button>${state.towns.length>1?`<button class="danger" data-delete-town="${state.activeTownId}">현재 마을 삭제</button>`:""}`:""}</div>`;
  const townHeader=`<header class="town-native-header"><button type="button" class="home-native-back town-native-back" data-tab="observe" aria-label="${esc(t("메인 화면으로 돌아가기","메인 화면으로 돌아가기"))}"><img src="${esc(homeUiAsset(character||active(),"back.png"))}" alt=""></button><div class="town-native-title"><i aria-hidden="true"></i><b style="color:#fff5dd!important;-webkit-text-fill-color:#fff5dd!important">${esc(state.world.name)}</b></div><button type="button" class="town-native-info" data-mobile-town-open-settings>${t("마을 정보","마을 정보")}</button><nav aria-label="${esc(t("마을 편집","마을 편집"))}"><button type="button" class="home-native-pill ${mobileTownEditing&&panelType!=="decorations"?"on":""}" data-mobile-town-layout-mode><span>${t("마을 편집","마을 편집")}</span></button><button type="button" class="home-native-pill ${mobileTownEditing&&panelType==="decorations"?"on":""}" data-mobile-town-decoration-mode><span>${t("마을 장식","마을 장식")}</span></button><button type="button" class="home-native-pill ${mobileTownBuildingEditing?"on":""}" data-mobile-building-edit-mode><span>${t("건물 편집","건물 편집")}</span></button></nav></header>`;
  const decorationCatalog=`<section class="town-decoration-catalog"><h3>${t("마을 장식 추가","마을 장식 추가")}</h3><p>${t("주민들은 놓아 둔 장식을 보고 쉬거나 사진을 찍는 등 자연스럽게 상호작용해요.","주민들은 놓아 둔 장식을 보고 쉬거나 사진을 찍는 등 자연스럽게 상호작용해요.")}</p><div>${TOWN_DECORATION_CHOICES.map(([kind,icon,label])=>`<button type="button" data-add-town-decoration="${kind}"><span>${icon}</span>${t(label,label)}</button>`).join("")}</div></section>`;
  const generalEditor=panelType==="world"?`<div class="town-general-editor"><div class="title"><h2>${t("마을 정보","마을 정보")}</h2><button class="primary" data-town-save>${t("저장","저장")}</button></div><label>${t("마을 이름","마을 이름")}<input data-world-name value="${esc(state.world.name)}"></label><label>${t("마을 시대","마을 시대")}<select data-world-era><option value="modern" ${state.world.era!=="medieval"?"selected":""}>${t("현대","현대")}</option><option value="medieval" ${state.world.era==="medieval"?"selected":""}>${t("중세","중세")}</option></select></label><label>${t("기본 배경","기본 배경")}<select data-world-bg>${backgroundOptions()}</select></label><button data-add-place>+ ${t("건물 추가","건물 추가")}</button></div>`:panelType==="decorations"?`<div class="town-general-editor town-decoration-editor">${decorationCatalog}</div>`:"";
  const placeEditors=mobileTownBuildingEditing&&selectedPlace?townPlaceEditor(selectedPlace,items,audiences,true):mobileTownBuildingEditing&&selectedHome?townHomeEditor(selectedHome,true):"";
  const selectedBuildingName=selectedPlace?.name||selectedHome?.name||"건물 편집";
  const editorPanel=panelType?`<aside class="panel form town-editor-panel"><div class="mobile-town-sheet-head"><span><small>${panelType==="world"?"TOWN SETTINGS":panelType==="decorations"?"TOWN DECORATIONS":"BUILDING SETTINGS"}</small><b>${panelType==="world"?esc(state.world.name):panelType==="decorations"?t("마을 장식","마을 장식"):esc(selectedBuildingName)}</b></span><button type="button" data-mobile-town-close aria-label="편집 창 닫기">×</button></div>${generalEditor}<div class="place-editor">${placeEditors}</div></aside>`:"";
  return `<section class="mobile-town-shell ${mobileTownEditing?"editing":""} ${mobileTownBuildingEditing?"building-editing":""} ${panelType?`sheet-open ${panelType}-panel`:""}" style="${homeUiThemeStyle(character||active())}">${desktopTabs}${townHeader}<div class="town-edit"><div class="town-map-scroll"><div class="world"><img src="${state.world.bg}" class="world-bg">${state.world.places.map(placeCard).join("")}${townHomes().map(homeMapCard).join("")}${townDecorationsMarkup()}${state.world.places.map(peopleAtPlaceCard).join("")}${townHomes().map(peopleAtHomeCard).join("")}</div></div>${editorPanel}</div>${townPlacementToolbar()}${buildingDetailDialogs()}</section>`;
}
Object.assign(UI_TEXT.en,{
...
Object.assign(UI_TEXT.en,{"배치 조절":"Placement controls","작게":"Smaller","크게":"Larger","좌우반전":"Flip horizontal","뒤로":"Send back","앞으로":"Bring forward","마을 장식 추가":"Add town decorations","주민들은 놓아 둔 장식을 보고 쉬거나 사진을 찍는 등 자연스럽게 상호작용해요.":"Residents naturally interact with decorations by resting, looking around, or taking photos.","벤치":"Bench","가로등":"Streetlight","분수":"Fountain","나무":"Tree","꽃밭":"Flower bed","조각상":"Statue","안내판":"Guide sign","자판기":"Vending machine"});
Object.assign(UI_TEXT.ja,{"배치 조절":"配置調整","작게":"小さく","크게":"大きく","좌우반전":"左右反転","뒤로":"背面へ","앞으로":"前面へ","마을 장식 추가":"村の装飾を追加","주민들은 놓아 둔 장식을 보고 쉬거나 사진을 찍는 등 자연스럽게 상호작용해요.":"住民は置かれた装飾を眺めたり、休んだり、写真を撮ったりして自然に交流します。","벤치":"ベンチ","가로등":"街灯","분수":"噴水","나무":"木","꽃밭":"花壇","조각상":"彫像","안내판":"案内板","자판기":"自動販売機"});
Object.assign(UI_TEXT.en,{"마을 장식":"Town decorations","마을 정보":"Town info","마을 이름":"Town name","마을 시대":"Town era","현대":"Modern","중세":"Medieval","기본 배경":"Background","건물 추가":"Add building","집 유형":"Home type","세부 유형":"Subtype","층":"Floors","방 수":"Rooms","구성원":"Residents","현재 집 안":"Currently home","청결도":"Cleanliness","마을 미지정":"No town","단독주택":"Detached house"});
Object.assign(UI_TEXT.ja,{"마을 장식":"村の装飾","마을 정보":"村情報","마을 이름":"村の名前","마을 시대":"村の時代","현대":"現代","중세":"中世","기본 배경":"背景","건물 추가":"建物を追加","집 유형":"住居タイプ","세부 유형":"詳細タイプ","층":"階数","방 수":"部屋数","구성원":"住人","현재 집 안":"在宅中","청결도":"清潔度","마을 미지정":"村未指定","단독주택":"一戸建て"});
function view(){
