...
function observe(){
  ...
  const hudRoster=`<div class="game-hud-profile"><button type="button" class="game-hud-profile-toggle" data-open-game-hud-roster aria-haspopup="dialog" aria-label="${esc(t("characterPicker","관찰 캐릭터 바꾸기"))}">${profileAvatar(c,"game-hud-current-profile")}<span><b>${esc(c.name)}</b><small>${esc(c.jobTitle||c.job||t("생활 중","생활 중"))}</small></span></button></div>`;
