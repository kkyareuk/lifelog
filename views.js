...
function webShopCart(){
  ...
  return `<section class="web-shop-cart"><h2>${labels.title}</h2>${entries.map(([id,n])=>`<div class="web-shop-cart-line"><span>${esc(t(SHOP_PRODUCTS[id].title,SHOP_PRODUCTS[id].title))}</span><button type="button" data-cart-minus="${id}" aria-label="−">−</button><b>${Number(n)}</b><button type="button" data-cart-plus="${id}" aria-label="＋">＋</button></div>`).join("")||`<p>${labels.empty}</p>`}<p>${labels.limit}</p><b>₩${total.toLocaleString("ko-KR")}</b><a class="primary" ${entries.length&&total<50000?`href="./payment.html?cart=1&lang=${state.uiLanguage}"`:'aria-disabled="true"'}>${labels.pay}</a></section>`;
}
function shop(){
