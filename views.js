...
import {state,active,characterViewFor,explicitCharacterViewFor} from "./state.js?v=20260828layout180";
import {eventFor as simulateEventFor,visibleTimeline as simulateVisibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260828layout180";
import {SPEECH_STYLE_OPTIONS} from "./speech-styles.js?v=20260828layout180";
import {furnitureFootprint,furnitureIcon,furnitureLabel,furniturePropIcon,normalizeFurniturePlacements,supportsFurnitureProps} from "./furniture-layout.js?v=20260828layout180";
import {homeSurfaceImage,normalizeHomeSurface,normalizeWallSurface,wallSurfaceImage} from "./home-surfaces.js?v=20260828layout180";
import {TOWN_TYPE_SUBTYPES,TOWN_TYPES,TOWN_REPUTATIONS,TOWN_TERRAINS,TOWN_TRANSPORTS} from "./town-profile.js?v=20260828layout180";
...
const townBackgroundMarkup=(src,className="world-bg")=>src?`<img src="${esc(src)}" class="${className}" alt="">`:"";
...
function tabletObserveMap(){
  ...
  return `<section class="tablet-observe-map" aria-label="${esc(t("town","마을"))}"><div class="tablet-observe-world world-no-illustration">${townBackgroundMarkup(town?.bg||state.world?.bg)}${state.world.places.map(placeCard).join("")}${townHomes().map(homeMapCard).join("")}${townDecorationsMarkup()}${state.world.places.map(peopleAtPlaceCard).join("")}${townHomes().map(peopleAtHomeCard).join("")}</div></section>`;
}
...
function observe(){
  ...
  if(!localId){
    ...
    return `<div class="standard-observe-view">${roster()}${townSwitcher}<div class="observe desktop-observe-map-only"><section><div class="viewport"><div class="world world-no-illustration">${townBackgroundMarkup(state.world?.bg)}${state.world.places.map(placeCard).join("")}${townHomes().map(homeMapCard).join("")}${townDecorationsMarkup()}</div></div></section></div>${buildingDetailDialogs()}</div>`;
  }
  ...
  return `<div class="editor character-editor" style="--character-own:${esc(c.theme?.primary||"#176b60")};--character-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")};--character-accent:${esc(c.theme?.primary||"#176b60")};--character-accent-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")}">
    ...
    <section class="panel form">
      ...
      <section class="character-book-v8 ${state.characterSettingsView==="full"?"is-open":""}" ${state.characterSettingsView==="full"?"":"hidden"} data-character-full-ui-version="9" data-full-pane="${fullActivePane}" style="--character-accent:${esc(c.theme?.primary||"#176b60")};--character-accent-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")}">
        ...
        ${layoutDialog}${bookListDialog}${bookCatalogDialog}${tasteDialogs}
      </section>
...
const characterViewEditor=()=>{
  ...
  const heroOrder=relationshipAnimationOrder(official[0],[sourceId,targetId],`${sourceId}:${targetId}`),heroLeft=state.characters[heroOrder[0]]||source,heroRight=state.characters[heroOrder[1]]||target;
  const relationshipMotion=relationshipMotionFor(heroLeft.id,heroRight.id,official);
...
const relationshipAnimationOrder=(relation,requestedIds=[],seed="")=>{
  ...
  const weight={"always-left":-20,"prefer-left":-10,random:0,"prefer-right":10,"always-right":20};
  const randomRank=id=>nativeVisualSeed(`${relation.id||relation.name||"relationship"}:${seed}:${id}`)%1000;
  return [...official].sort((a,b)=>{
    const aPlacement=relation.animationPlacement?.[a]||"random",bPlacement=relation.animationPlacement?.[b]||"random";
    const difference=(weight[aPlacement]??0)-(weight[bPlacement]??0);
    if(difference)return difference;
    if(aPlacement==="random"&&bPlacement==="random")return randomRank(a)-randomRank(b);
    return official.indexOf(a)-official.indexOf(b);
  });
};
...
function townInformationScreen(character){
  ...
  return `<aside class="town-information-screen" aria-label="${esc(t("마을 정보 편집","마을 정보 편집"))}">
    ...
    <div class="town-information-content">
      <section class="town-information-hero town-information-hero-empty" aria-label="${esc(t("마을 일러스트 준비 중","마을 일러스트 준비 중"))}"><span aria-hidden="true">✦</span><b>${t("등록된 마을 일러스트가 없어요","등록된 마을 일러스트가 없어요")}</b><small>${t("제공받은 일러스트를 하나씩 추가할 예정이에요.","제공받은 일러스트를 하나씩 추가할 예정이에요.")}</small></section>
...
function nativePlayShop(){
  ...
  return `<section class="drawer-shop-shell" aria-label="${esc(t("shop","상점"))}"><div class="drawer-shop-stage" data-shop-section="${section}"><header class="drawer-shop-hero"><img class="drawer-shop-wood" src="./assets/shop/drawer-shop-wood.jpg" alt=""><img class="drawer-shop-seller" src="./assets/shop/drawer-shop-seller.png" alt=""><button type="button" class="drawer-shop-back" data-tab="observe" aria-label="${esc(copy.back)}"><img src="./assets/home-ui/back.png" alt=""></button><h1>${esc(copy.title)}</h1></header><nav class="drawer-shop-tabs" aria-label="${esc(t("상점 메뉴","상점 메뉴"))}">${tab("bundle",copy.bundle)}${tab("base",copy.base)}${tab("skin",copy.skin)}${tab("expansion",copy.expansion)}</nav><main class="drawer-shop-content">${content}</main><button type="button" class="drawer-shop-restore" data-play-restore>${esc(copy.restore)}</button></div></section>`;
}
function shop(){
