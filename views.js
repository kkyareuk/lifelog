...
import {state,active,characterViewFor,explicitCharacterViewFor} from "./state.js?v=20260822charactergrouplayout1";
import {eventFor as simulateEventFor,visibleTimeline as simulateVisibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260822charactergrouplayout1";
...
function routine(){
  ...
  const monthly=`<div class="monthly-view"><div class="monthly-heading"><button type="button" data-routine-month-step="-1" aria-label="${routineText.previous}"><img src="./assets/home-ui/back.png" alt=""></button><h2>${monthTitle}</h2><button type="button" data-routine-month-step="1" aria-label="${routineText.next}"><img src="./assets/home-ui/back.png" alt=""></button></div><div class="monthly-weekdays">${days.map(day=>`<b>${day}</b>`).join("")}</div><div class="monthly-calendar">${calendarCells}</div></div>`;
  const monthlyView=state.routineView==="monthly";
  return `<section class="routine-shell"><div class="routine-screen-head"><button type="button" class="routine-back-button" data-tab="observe" aria-label="${routineText.back}"><img src="./assets/home-ui/back.png" alt=""></button>${characterSwitcher}</div><div class="routine-title-row"><span><img src="./assets/home-ui/routine.png" alt=""><h1>${routineText.title}</h1></span><div class="routine-title-actions"><button type="button" class="routine-add-button" ${monthlyView?"data-add-monthly-routine":"data-add-routine"}>${routineText.add}</button>${monthlyView?`<button type="button" data-add-anniversary>${routineText.addAnniversary}</button>`:""}</div></div><div class="routine-view-tabs"><button type="button" data-routine-view="weekly" class="${monthlyView?"":"on"}">${routineText.weekly}</button><button type="button" data-routine-view="monthly" class="${monthlyView?"on":""}">${routineText.monthly}</button></div>${monthlyView?monthly:table}</section>`;
}
