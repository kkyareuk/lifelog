import {state,active,characterViewFor,explicitCharacterViewFor} from "./state.js?v=20260809h";
import {eventFor as simulateEventFor,visibleTimeline as simulateVisibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260809h";
...
function nativeScenePresentation(c,entry){
  ...
  const lineupHtml=companions.length?`<span class="native-scene-lineup ${coResidentConversation?"is-conversation":""}" style="--scene-count:${sceneParticipants.length}" aria-label="${esc(sceneParticipants.map(person=>person.name).join(", "))}">${sceneParticipants.map((person,index)=>{
    ...
    return `<span class="native-scene-lineup-person ${person.id===c.id?"is-current":""}" style="--scene-index:${index};--scene-delay:${delay}s;--scene-duration:${duration}s">${avatar(person,"native-scene-lineup-avatar")}${actionProp}${tone==="sleep"?'<b class="native-character-sleep-mark" aria-hidden="true">ZZ</b>':""}${tone==="date-overwhelmed"&&person.id===c.id?'<b class="native-character-sweat" aria-hidden="true">💧</b>':""}<small>${esc(person.name)}</small></span>`;
  }).join("")}</span>`:"";
  ...
  return {
    ...
    pet,
    lineupHtml,
    sleepMarkHtml:tone==="sleep"&&!companions.length?'<b class="native-character-sleep-mark is-main" aria-hidden="true">ZZ</b>':"",
    conversationHtml,
    actionHtml,
    companionHtml:"",
...
function observe(){
  ...
  if(!localId)return `${nativeGameMenu()}<section class="native-observe-home native-observe-empty"><div><span>🏙️</span><h1>이 마을에 사는 캐릭터가 없어요</h1><p>캐릭터 화면에서 생활하는 마을을 지정해 주세요.</p><button class="primary" data-tab="character">캐릭터 설정 열기</button></div></section><div class="standard-observe-view">${roster()}${townSwitcher}<div class="observe"><section><div class="viewport"><div class="world"><img src="${state.world.bg}" class="world-bg">${state.world.places.map(placeCard).join("")}</div></div></section><aside class="panel empty"><h2>이 마을에 사는 캐릭터가 없어요</h2><p>캐릭터 프로필에서 생활하는 마을을 지정할 수 있어요.</p></aside></div></div>`;
...
function catalog(){
  ...
  const primary=/^#[0-9a-f]{6}$/i.test(character?.theme?.primary||"")?character.theme.primary:"#303030";
  const secondary=/^#[0-9a-f]{6}$/i.test(character?.theme?.secondary||"")?character.theme.secondary:"#777777";
  ...
  return `<section class="panel form catalog-shell" style="--catalog-theme:${esc(primary)};--catalog-theme-secondary:${esc(secondary)}"><div class="title"><div><h1>세계관 취향 도감</h1><p>아이콘을 누르면 세부 정보와 편집 항목이 열려요.</p></div><button class="primary" data-catalog-save>도감 저장</button></div>${sections}</section>`;
}
...
function fontSettings(){
  const options=[["system","기기·브라우저 기본 글꼴"],["noto","Noto Sans KR · 가장 무난함"],["nanum","나눔고딕 · 편안한 본문"],["gothica1","Gothic A1 · 또렷한 화면용"],["gowundodum","고운돋움 · 부드럽고 단정함"],["gowunbatang","고운바탕 · 읽기 좋은 명조"],["jua","주아체 · 둥글고 선명함"],["dangam","창원단감둥근체"],["dohyeon","배달의민족 도현체 · 굵고 또렷함"],["corncorn","온글잎 콘콘체 · 편안한 손글씨"],["haeong","국민대학교 해옹 산스 · 부드러운 고딕"],["aggro","SB 어그로체 · 힘 있는 제목체"]];
  const sizes=[["small","작게"],["normal","보통"],["large","크게"],["xlarge","아주 크게"]];
  const ownerName=esc(state.ownerName||"");
  return `<section class="setting-card font-setting-card"><h2>글자와 화면 크기</h2><p>기본값은 기기·브라우저의 평범한 시스템 글꼴입니다. 아래 글꼴들은 외부 사이트가 아니라 서랍마을 파일에 포함되어 있어요.</p><div class="font-setting-grid"><label>글자 크기<select data-setting="uiScale">${sizes.map(([value,label])=>`<option value="${value}" ${state.uiScale===value?"selected":""}>${label}</option>`).join("")}</select></label><label>사용할 글꼴<select data-setting="uiFont">${options.map(([value,label])=>`<option value="${value}" ${state.uiFont===value?"selected":""}>${label}</option>`).join("")}</select></label></div><div class="font-preview"><b>서랍마을의 오늘</b><span>캐릭터들이 각자의 하루를 보내고 있어요. 긴 생활 로그도 편안하게 읽어 보세요.</span></div></section><section class="setting-card owner-name-card"><h2>동기화 표시 이름</h2><p>Google 계정 이름 대신 백업과 동기화 화면에 표시할 이름이에요.</p><label>어떻게 불러드릴까요?<input data-setting="ownerName" maxlength="20" value="${ownerName}" placeholder="예: 꺄륵"></label></section>`;
}
