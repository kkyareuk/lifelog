...
const furniturePlacementStyle=(placement,footprint)=>{if(isLayeredBed(placement))footprint=placement.item==="커플 침대"?{columns:3,rows:4}:{columns:1.5,rows:2};return `--sprite-width:${furnitureSprite(placement)?furnitureSprite(placement).width/527*2/footprint.columns*furnitureSprite(placement).scale:1};--sprite-ratio:${furnitureSprite(placement)?furnitureSprite(placement).width/furnitureSprite(placement).height:1};--furniture-x:${placement.x}%;--furniture-y:${placement.y}%;--furniture-scale:${placement.scale};--furniture-rotation:${isLayeredBed(placement)?bedPerspective(placement).artRotation:furnitureSprite(placement)?0:placement.rotation}deg;--furniture-layer:${placement.layer};--furniture-flip:${furnitureSprite(placement)?.flip??(isLayeredBed(placement)?bedPerspective(placement).artFlip:placement.flipped?-1:1)};--furniture-grid-width:${footprint.columns};--furniture-grid-height:${footprint.rows}`};
...
export function roomFurnitureMarkup(homeId,roomKey,room,edit,bedStates=new Map()){
  ...
  return `<div class="room-furniture-layer" aria-label="${esc(room.name||roomKey)} 가구">${placements.map(placement=>{
    ...
    const boundedSurface=['counter','counter-join','counter-top','nightstand'].includes(sprite?.kind),surfaceProps=boundedSurface&&props.length?`<span class="furniture-surface-props" data-surface-kind="${sprite.kind}" data-surface-full="${props.some(p=>p.item==='인덕션')}">${props.map(prop=>{const art=furnitureSprite({item:prop.item});return `<i>${art?`<img src="${art.src}" alt="">`:furniturePropIcon(prop.item)}</i>`}).join('')}</span>`:'';

    const art=sprite&&!coupleBed?`<span class="room-furniture-art"><img class="furniture-sprite" src="${sprite.src}" alt="">${surfaceProps}</span>`:coupleBed?`<span class="room-furniture-art room-couple-bed-art" data-bed-layer-state="${bedState}" data-bed-side="${bedPerspective(placement).side}" aria-hidden="true">${coupleBedImage("base","",placement)}${bedState!=="under-cover"?coupleBedImage("quilt","",placement):""}${bedState==="default"?coupleBedImage("footboard","",placement):""}</span>`:`<span class="room-furniture-art" aria-hidden="true">${furnitureIcon(placement.item)}</span>`;
    return `<button type="button" class="room-furniture-item ${coupleBed?"is-couple-bed":""}" data-placement-scale="${Number(placement.scale)||1}" data-furniture-kind="${sprite?.kind||""}" data-seat-side="${esc(placement.seatSide||"")}" data-table-id="${esc(placement.tableId||"")}" data-seat-direction="${sprite?.direction||""}" data-bed-single="${coupleBed&&placement.item!=="커플 침대"}" data-bed-direction="${bedPerspective(placement).direction}" data-bed-side="${coupleBed&&bedPerspective(placement).side}" data-furniture-placement="${esc(placement.id)}" data-home-id="${esc(homeId)}" data-room-key="${esc(roomKey)}" data-furniture-name="${esc(label)}" data-furniture-supports-props="${supportsFurnitureProps(placement.item)}" data-furniture-columns="${footprint.columns}" data-furniture-rows="${footprint.rows}" style="${furniturePlacementStyle(placement,footprint)}" aria-label="${esc(label)}${edit?" · 끌어서 이동":""}" ${edit?"":"tabindex=\"-1\""}>${art}${props.length&&!boundedSurface?`<span class="room-furniture-props" aria-hidden="true">${props.map((prop,index)=>`<i style="--prop-slot:${index}">${furniturePropIcon(prop.item)}</i>`).join("")}</span>`:""}${edit?`<small>${esc(label)} · ${footprint.columns}×${footprint.rows}</small>`:""}</button>`;
