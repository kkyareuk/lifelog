...
Object.assign(I18N.en,{"추가 LD 사진":"Additional LD photos","사진 검색":"Search photos","사진 추가":"Add photo","추가 사진은 캐릭터마다 30개까지 등록할 수 있어요.":"Add up to 30 extra photos per character."});
Object.assign(I18N.ja,{"추가 LD 사진":"追加LD画像","사진 검색":"画像を検索","사진 추가":"画像を追加","추가 사진은 캐릭터마다 30개까지 등록할 수 있어요.":"追加画像はキャラクターごとに30枚まで登録できます。"});
...
function roomFurnitureOverlayMarkup(room,bedStates=new Map()){
  const overlays=normalizeFurniturePlacements(room.furniturePlacements).filter(placement=>placement.item==="커플 침대"&&["on-bed","under-cover"].includes(bedStates.get(placement.id))).map(placement=>{
    ...
    return `<span class="room-couple-bed-overlay" data-bed-layer-state="${bedState}" style="${furniturePlacementStyle(placement,footprint)}">${bedState==="under-cover"?coupleBedImage("quilt","",placement):""}${coupleBedImage("footboard","",placement)}</span>`;
  });
...
function character(){
  ...
  const bodyAccessibilityPane=()=>`<section class="character-body-svg-page character-body-accessibility-page" aria-label="${esc(c.name)} ${esc(t("건강·접근성 설정","건강·접근성 설정"))}">
    ...
    <section class="body-accessibility-choice body-health-conditions"><b>${t("만성질환·건강 관리","만성질환·건강 관리")}</b>${bodyInlineChoice("healthConditions",HEALTH_CONDITIONS,c.bodyProfile?.healthConditions||[])}<label>${t("기타 건강 상태","기타 건강 상태")}<input data-body-field="healthOther" maxlength="200" value="${esc(c.bodyProfile?.healthOther||'')}" placeholder="${t("병명·건강 상태 직접 입력","병명·건강 상태 직접 입력")}"></label></section>
