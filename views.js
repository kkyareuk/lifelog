...
function homeDailyLog(chars,h){
  ...
  const entries=chars.flatMap(c=>{
    ...
    const sequence=compactDisplayedTimeline(loggableEntry(current)?[...visible.filter(item=>Number(item.minute)!==Number(current.minute)),current]:visible);
    const own=[];
...
function character(){
  ...
  const palette=(field,label)=>`<div class="character-theme-palette"><b>${label}</b><div>${themePalette.map(color=>`<button type="button" data-theme-swatch="${field}" data-color-value="${color}" class="${String(c.theme?.[field]||"").toUpperCase()===color?"on":""}" style="--swatch:${color}" aria-label="${label} ${color}"></button>`).join("")}</div></div>`;
  const mainTools=`<details class="character-main-tools" style="--own:${esc(c.theme?.primary||"#176b60")};--own-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")}"><summary><span>${avatar(c)}</span><b>사진 · 지도 아이콘 · 테마 · 파일</b><small>캐릭터 탭에서 관리</small></summary><div class="character-main-tools-body"><div class="character-main-media"><span>${avatar(c)}</span><div><b>프로필 사진과 지도 아이콘</b><div class="image-actions"><button type="button" data-image="photo">프로필 사진</button><button type="button" data-image-url="photo" data-id="${c.id}">사진 링크</button><button type="button" data-image="icon">지도 아이콘</button><button type="button" data-image-url="icon" data-id="${c.id}">아이콘 링크</button></div></div></div><div class="character-main-colors">${palette("primary","대표 테마색")}${palette("secondary","그라데이션 보조색")}<label class="check"><input type="checkbox" data-gradient ${c.theme?.gradient?"checked":""}> 보조색으로 그라데이션 사용</label></div><div class="character-main-file-actions"><button type="button" data-export-profile>프로필 내보내기</button><button type="button" class="danger" data-delete-character="${c.id}">캐릭터 삭제</button></div></div></details>`;
