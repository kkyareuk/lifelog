...
function observe(shared=false){
  ...
  if(!nativeHome){
    ...
    return `<div class="standard-observe-view" data-observed-character="${esc(c.id)}">${roster()}${townSwitcher}${desktopScene}<div class="web-observe-actions"><button type="button" data-character-command="${esc(c.id)}">${esc(({ko:"할 일 정하기",en:"Choose activity",ja:"行動を決める"}[state.uiLanguage]||"할 일 정하기"))}</button><div data-web-discovery-rail><button type="button" data-tab="statistics">${esc(t("statistics","통계"))}</button></div><div data-home-discovery-slot></div></div><div class="web-social-shortcuts">${socialHomeDock()}<button type="button" data-home-social="friends">${esc(gameHudLabel("friends",({ko:"친구",en:"Friends",ja:"フレンド"}[state.uiLanguage]||"친구")))}</button></div><div class="desktop-observe-lower">${desktopLog}<section class="web-village-summary"><img src="./assets/home-ui/town.png" alt=""><h2>${esc(t("town","마을"))}</h2><p>${esc(c.name)} · ${esc(c.jobTitle||c.job||"")}</p><div><button type="button" data-tab="home">${esc(t("viewHome","집 보기"))}</button><button type="button" data-tab="town">${esc(t("town","마을"))}</button><button type="button" data-tab="groups">${esc(GAME_HUD_LABELS[state.uiLanguage]?.groups||"멀티")}</button></div></section></div>${nativeFullLog}${buildingDetailDialogs()}</div>`;
  }
  const otherTowns=state.towns.filter(town=>town.id!==state.activeTownId);
  const hudRoster=`<details class="game-hud-profile"><summary class="game-hud-profile-toggle" data-open-game-hud-roster aria-label="${esc(t("characterPicker","관찰 캐릭터 바꾸기"))}"><span class="game-hud-profile-frame">${profileAvatar(c,"game-hud-current-profile")}<img class="game-hud-profile-ring" src="${imageEsc(homeUiAsset(c,"profile-ring.png"))}" alt=""></span><span class="game-hud-profile-copy"><b>${esc(c.name)}</b><small><em>${esc(c.jobTitle||c.job||t("생활 중","생활 중"))}</em></small></span></summary><section class="game-hud-roster-drawer"><small class="game-hud-roster-kicker">${t("선택한 그룹의 캐릭터","선택한 그룹의 캐릭터")}</small><div class="game-hud-roster-options" aria-label="${esc(t("characterPicker","관찰 캐릭터 선택"))}">${localIds.map(id=>{const person=state.characters[id];return `<button type="button" data-home-character="${id}" class="game-hud-button character-picker-button ${id===c.id?"on":""}" style="--picker-theme:${esc(person.theme?.primary||"#176b60")}" title="${esc(person.name)}" aria-label="${esc(person.name)}">${profileAvatar(person)}<small>${esc(person.name)}</small></button>`}).join("")}</div>${groupJump()}${otherTowns.length?`<div class="game-hud-town-jump"><b>${t("moveToAnotherTown","다른 마을로 이동")}</b><div>${otherTowns.map(town=>`<button type="button" data-observe-town="${town.id}">${esc(town.name)}</button>`).join("")}</div></div>`:""}</section></details>`;
...
function nativeCharacterHub(c){
  ...
  const draftRows=[{key:"name",label:"이름",value:c.name},{key:"birthday",label:"생일",value:birthdayLabel},{key:"job",label:"직업 종류",value:isCourtWorld()?courtJobName(c.courtJob,state.uiLanguage):c.jobTitle||c.job||unknown},{key:"age",label:"나이",value:c.ageGroup||unknown},{key:"gender",label:"성별",value:c.gender||unknown}];
...
function character(){
  ...
  const draftRows=[
    ...
    {key:"job",label:"직업 종류",value:isCourtWorld()?courtJobName(c.courtJob,state.uiLanguage):c.jobTitle||c.job||unknown},
