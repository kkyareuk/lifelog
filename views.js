...
import {state,active,characterViewFor,explicitCharacterViewFor} from "./state.js?v=20260820homevisits1";
import {eventFor as simulateEventFor,visibleTimeline as simulateVisibleTimeline,charactersAtPlace,homeGroups} from "./simulation.js?v=20260820homevisits1";
...
const GAME_HUD_SIDE_TABS={
  left:[
    {key:"character",labelKey:"character",label:"캐릭터",art:"./icons/hud-character-card.png"},
    {key:"catalog",labelKey:"catalog",label:"취향 사전",art:"./icons/hud-catalog-book.png"},
    {key:"relationship",labelKey:"relationship",label:"관계",icon:"∞"}
  ],
  right:[
    {key:"routine",labelKey:"routine",label:"일정",icon:"▦"},
    {key:"statistics",labelKey:"statistics",label:"통계",icon:"▥"},
    {key:"settings",labelKey:"settings",label:"설정",icon:"⚙"}
  ]
};
const GAME_HUD_LABELS={
  ko:{character:"캐릭터",catalog:"취향 사전",relationship:"관계",routine:"일정",statistics:"통계",settings:"설정",mailbox:"우편함",home:"집",todayLog:"오늘의 기록",shop:"상점",town:"마을"},
...
function gameHudDock(){
  return `<nav class="game-hud-dock" aria-label="${esc(t("주요 메뉴","주요 메뉴"))}"><button type="button" class="game-hud-button game-hud-mailbox-button" data-tab="mailbox"><img src="./icons/mailbox.png" alt=""><small>${gameHudLabel("mailbox","우편함")}</small></button><button type="button" class="game-hud-button" data-tab="home"><span data-menu-icon="home" aria-hidden="true">⌂</span><small>${gameHudLabel("home","집")}</small></button><button type="button" class="game-hud-button" data-open-native-log><span data-menu-icon="log" aria-hidden="true">▤</span><small>${gameHudLabel("todayLog","오늘의 기록")}</small></button><button type="button" class="game-hud-button" data-tab="shop"><span data-menu-icon="shop" aria-hidden="true">♢</span><small>${gameHudLabel("shop","상점")}</small></button><button type="button" data-tab="town" class="game-hud-button game-hud-town-button"><img src="./icons/town-map.png" alt=""><small>${gameHudLabel("town","마을")}</small></button></nav>`;
}
...
function mailbox(){
  ...
  return `<section class="mailbox-shell panel" style="--character-own:${esc(sender?.theme?.primary||"#176b60")}"><div class="mailbox-heading"><span aria-hidden="true">✉</span><div><small>CHARACTER MAIL</small><h1>${t("mailbox","우편함")}</h1><p>${t("mailboxHelp","캐릭터에게서 온 편지를 한곳에서 확인해요.")}</p></div></div><article class="character-mail-card ${pending?"has-mail":"is-empty"}">${sender?avatar(sender):'<span class="mailbox-empty-icon">✉</span>'}<div><small>${pending?esc(sender?.name||""):t("mailbox","우편함")}</small><h2>${esc(title)}</h2><p>${esc(description)}</p></div>${pending?`<button type="button" class="primary" data-open-daily-question>${t("openLetter","편지 열기")}</button>`:""}</article></section>`;
}
function wardrobe(){
...
function routine(){
  ...
  const routineCopy={
    en:{homeSuffix:"'s home",characterHome:"Character home",returnTitle:"Return home now",returnHelp:"End the current outing and stay home until the next scheduled plan.",currentReturn:`Send ${c.name} home`,allReturn:"Send everyone home"},
    ja:{homeSuffix:"の家",characterHome:"キャラクターの家",returnTitle:"すぐ帰宅",returnHelp:"現在の外出を終え、次の登録予定まで家で過ごします。",currentReturn:`${c.name}を帰宅`,allReturn:"全員を帰宅"},
    ko:{homeSuffix:"의 집",characterHome:"캐릭터의 집",returnTitle:"즉시 귀환",returnHelp:"현재 외출을 끝내고 다음 등록 일정 전까지 집에서 지내게 해요.",currentReturn:`${c.name} 귀환`,allReturn:"모두 귀환"}
  }[state.uiLanguage]||null;
  const routineText=routineCopy||{homeSuffix:"의 집",characterHome:"캐릭터의 집",returnTitle:"즉시 귀환",returnHelp:"현재 외출을 끝내고 다음 등록 일정 전까지 집에서 지내게 해요.",currentReturn:`${c.name} 귀환`,allReturn:"모두 귀환"};
  ...
  const returnPanel=`<section class="routine-return-panel"><span><b>${routineText.returnTitle}</b><small>${routineText.returnHelp}</small></span><div><button type="button" data-force-home="current">${esc(routineText.currentReturn)}</button><button type="button" class="danger" data-force-home="all">${routineText.allReturn}</button></div></section>`;
  const table=`<div class="weekly-scroll"><div class="weekly-table">${days.map((day,index)=>`<section class="routine-day"><h3>${day}요일</h3>${items.filter(item=>item.day===index).map(item=>`<article class="routine-block"><b>${esc(item.start)}–${esc(item.end)}</b><strong>${esc(item.title)}</strong><small>${esc(item.type)}${destinationLabel(item)?` · ${esc(destinationLabel(item))}`:""}${item.withIds?.length?` · ${item.withIds.map(id=>esc(state.characters[id]?.name||"")).filter(Boolean).join(", ")}와 함께`:""}</small><div class="routine-actions"><button data-edit-routine="${item.id}">편집</button><button class="danger" data-delete-routine="${item.id}">삭제</button></div></article>`).join("")||"<small>일정 없음</small>"}</section>`).join("")}</div></div>`;
  ...
  const calendarCells=Array.from({length:Math.ceil((firstDay+lastDate)/7)*7},(_,index)=>{const date=index-firstDay+1;if(date<1||date>lastDate)return '<section class="monthly-day is-empty" aria-hidden="true"></section>';const dateKey=`${monthKey}-${String(date).padStart(2,"0")}`,recurringKey=`${String(month).padStart(2,"0")}${String(date).padStart(2,"0")}`,dayItems=monthItems.filter(item=>item.date===dateKey),birthdays=state.order.map(id=>state.characters[id]).filter(character=>character?.birthday===recurringKey),anniversaries=(state.anniversaries||[]).filter(item=>item.date===recurringKey);const special=[...birthdays.map(character=>`<span class="calendar-special birthday">🎂 ${esc(character.name)} 생일</span>`),...anniversaries.map(item=>`<button type="button" class="calendar-special anniversary" data-edit-anniversary="${item.id}">💌 ${esc(item.title||item.type)}</button>`)];return `<section class="monthly-day"><h3><b>${date}</b><small>${days[index%7]}</small></h3>${special.join("")}${dayItems.map(item=>`<article class="routine-block"><b>${esc(item.start)}–${esc(item.end)}</b><strong>${esc(item.title)}</strong><small>${esc(item.type)}${destinationLabel(item)?` · ${esc(destinationLabel(item))}`:""}</small><div class="routine-actions"><button data-edit-monthly-routine="${item.id}">편집</button><button class="danger" data-delete-monthly-routine="${item.id}">삭제</button></div></article>`).join("")||(!special.length?'<small class="monthly-empty">일정 없음</small>':"")}</section>`}).join("");
  const monthly=`<div class="monthly-view"><div class="monthly-heading"><button type="button" data-routine-month-step="-1" aria-label="이전 달">‹</button><h2>${year}년 ${month}월</h2><button type="button" data-routine-month-step="1" aria-label="다음 달">›</button></div><div class="monthly-calendar">${calendarCells}</div></div>`;
  const monthlyView=state.routineView==="monthly";
  return `<section class="panel form routine-shell"><div class="title"><div><h1>${monthlyView?"월간 일정":"주간 일정"}</h1><p>${monthlyView?"날짜가 정해진 약속·생일·기념일을 달력에서 함께 봐요.":"반복되는 회사 일정, 수업, 데이트와 개인 일정을 요일별로 지정해요."}</p></div><div class="routine-title-actions"><button class="primary" ${monthlyView?"data-add-monthly-routine":"data-add-routine"}>+ 일정 추가</button>${monthlyView?'<button type="button" data-add-anniversary>+ 기념일</button>':""}</div></div>${toolbar}${returnPanel}<div class="routine-view-tabs"><button type="button" data-routine-view="weekly" class="${monthlyView?"":"on"}">주간 일정</button><button type="button" data-routine-view="monthly" class="${monthlyView?"on":""}">월간 일정</button></div>${monthlyView?monthly:table}</section>`;
}
...
function fontSettings(){
  const options=[["system","기기·브라우저 기본 글꼴"],["dangam","창원단감둥근체"],["haeong","국민대학교 해옹 산스 · 부드러운 고딕"],["dohyeon","배달의민족 도현체 · 굵고 또렷함"],["corncorn","온글잎 콘콘체 · 편안한 손글씨"],["griun","그리운 심심체 · 손글씨"],["aggro","SB 어그로체 · 힘 있는 제목체"]];
