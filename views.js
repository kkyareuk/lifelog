...
function observe(){
  ...
  const hudRoster=`<div class="game-hud-profile"><button type="button" class="game-hud-profile-toggle" data-open-game-hud-roster aria-haspopup="dialog" aria-label="${esc(t("characterPicker","관찰 캐릭터 바꾸기"))}"><span class="game-hud-profile-frame">${profileAvatar(c,"game-hud-current-profile")}<img class="game-hud-profile-ring" src="./icons/hud-profile-frame.png" alt=""></span><span class="game-hud-profile-copy"><b>${esc(c.name)}</b><small>${esc(c.jobTitle||c.job||t("생활 중","생활 중"))}</small></span></button></div>`;
  const rosterDialog=`<dialog class="game-hud-roster-dialog" data-game-hud-roster-dialog><form method="dialog"><header><span><small>${t("observe","관찰")}</small><h2>${t("characterPicker","관찰 캐릭터 선택")}</h2></span><button value="close" aria-label="${t("close","닫기")}">×</button></header><div class="game-hud-roster-options" aria-label="${esc(t("characterPicker","관찰 캐릭터 선택"))}">${state.order.map(id=>{const person=state.characters[id];return `<button type="button" data-home-character="${id}" class="game-hud-button character-picker-button ${id===c.id?"on":""}" style="--picker-theme:${esc(person.theme?.primary||"#176b60")}" title="${esc(person.name)}" aria-label="${esc(person.name)}">${profileAvatar(person)}<small>${esc(person.name)}</small></button>`}).join("")}</div></form></dialog>`;
  ...
  return `${gameHud}${rosterDialog}${nativeFullLog}${buildingDetailDialogs()}`;
}
