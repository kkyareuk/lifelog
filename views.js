...
function character(){
  ...
  const catalogCategories=[...new Set(Object.values(state.catalog||{}).flat().map(item=>item.category).filter(Boolean))];
  const taste=`<h2>${esc(c.name)}의 취향 선택</h2><p>취향 사전에 등록한 분류도 이 캐릭터의 선호로 선택할 수 있어요.</p>${chips("취향 사전 선호 카테고리",catalogCategories,c.catalogPreferences||[],"catalogPreferences")}${chips("음식",FOOD_PREFERENCES,c.foodPreferences||[],"foodPreferences")}${chips("좋아하는 음료",DRINKS,c.drinks||[],"drinks")}${chips("좋아하는 음악 장르",MUSIC,c.musicGenres||[],"musicGenres")}${chips("관심사",INTERESTS,c.interests||[],"interests")}${chips("취미",HOBBIES,c.hobbies||[],"hobbies")}`;
