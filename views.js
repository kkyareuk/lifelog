...
import {settingsMenu,settingsAccount,settingsLabels,settingsPaneName} from './settings-layout.js';
...
import {careerCaption,todayCareerDuty,careerHudLabel} from './salary.js';
import {townSettingFields,TOWN_ERAS,TOWN_CULTURES,settingLabel} from './town-setting.js';
import {coffeeInventoryMarkup} from './coffee-crafting.js';
import {isCourtWorld,courtJobSelect,courtCharacterPage,courtJobName} from './court-world-ui.js';
...
import {setDiscoveryEntitlement} from './discovery-access.js';
import {bindBannerAds} from './banner-ads.js';
import {counterArt} from './counter-art.js';
import {notificationCharacters,selectedNotificationIds} from './notification-characters.js';
import {socialHomeDock,socialHomeRight} from './home-social-ui.js?v=20260909dev305';
import {characterLanguageFields} from './character-language-ui.js';
import {logOrderControl,sortLogEntries} from './log-order.js';
...
import {touchReactions} from "./touch-reactions.js?v=20260909dev305";
import {routineScene} from './routine-scenes.js?v=20260909dev305';
import {meetingScene,createEntranceTransitions} from './meeting-journey.js?v=20260909dev305';
import {renderMailbox,unreadMailCount} from './mailbox-center.js?v=20260909dev305';
import {sharedResidentsScreen} from './shared-residents.js?v=20260909dev305';
import {bedPerspective} from './bed-perspective.js?v=20260909dev305';
import {conflictEvidence} from './scene-context.js?v=20260909dev305';
import {localizeLifeLog} from "./life-log-localization.js?v=20260909dev305";
import {MARTIAL_ARTS,MENTAL_HEALTH,CARE_MODES,TRADITIONAL_CLOTHES,HOSPITAL_DEPARTMENTS,hospitalPurposes,creativeCopy} from "./creative-options.js?v=20260909dev305";
import {withSharedWorld} from './shared-world.js?v=20260909dev305';
...
import {dictionaryCopy} from "./dictionary-copy.js?v=20260909dev305";
import {eventFor as simulateEventFor,visibleTimeline as simulateVisibleTimeline,homeGroups,withSimulationBatch} from "./simulation.js?v=20260909dev305";
import {SPEECH_STYLE_OPTIONS} from "./speech-styles.js?v=20260909dev305";
import {furnitureFootprint,furnitureIcon,furnitureLabel,furniturePropIcon,normalizeFurniturePlacements,supportsFurnitureProps} from "./furniture-layout.js?v=20260909dev305";
import {homeSurfaceImage,normalizeHomeSurface,normalizeWallSurface,wallSurfaceImage} from "./home-surfaces.js?v=20260909dev305";
import {TOWN_TYPE_SUBTYPES,TOWN_TYPES,TOWN_REPUTATIONS,TOWN_FAME_LEVELS,TOWN_TERRAINS,TOWN_TRANSPORTS} from "./town-profile.js?v=20260909dev305";
import {normalizeBuildingLighting,buildingLightsOn,scheduleTownLighting,townTimeAppearance} from "./town-lighting.js?v=20260909dev305";
...
Object.assign(I18N.en,{"멀티 정보":"Multiplayer info","멀티 주민":"Residents","방문 가능한 집":"Homes to visit","멀티 규칙":"Multiplayer rules"});
Object.assign(I18N.ja,{"멀티 정보":"マルチ情報","멀티 주민":"マルチの住民","방문 가능한 집":"訪問できる家","멀티 규칙":"マルチのルール"});
...
Object.assign(UI_TEXT.ja,{
  ...
  "어린이":"子ども","청소년":"青少年","성인":"成人","노년":"高齢","설정하지 않음":"未設定","무직":"無職","학생":"学生","회사원":"会社員","의사":"医師","간호사":"看護師","교사":"教師","교수":"教授","바리스타":"バリスタ","정치인":"政治家","기자":"記者","요리사":"料理人","프로그래머":"プログラマー","연구원":"研究員","가수":"歌手","아이돌":"アイドル","예술가":"芸術家","해적":"海賊","군인":"軍人","범죄자":"犯罪者","환경미화원":"清掃員","여관주인":"宿屋の主人","종교인":"宗教者","자영업·직접 입력":"自営業・自由入力",
...
Object.assign(UI_TEXT.en,{
  "7,900원":"KRW 7,900","건물 정보 보기\"":"View building details","기본 실루엣\"":"Default silhouette",
...
Object.assign(UI_TEXT.ja,{
  "7,900원":"7,900ウォン","건물 정보 보기\"":"建物情報を見る","기본 실루엣\"":"基本シルエット",
...
const householdPhaseKey=(c,now)=>[c?.household?.active,c?.household?.returning,state.characterDirectives?.[c?.id]].filter(Boolean).map(task=>{const journey=task.journey;return `${task.id||journey?.startedAt}:${now>=task.endsAt?'done':journey&&now<journey.arrivesAt?'walk'+journey.segments.findIndex(s=>now<s.end):'action'}`}).join('|');
const renderSceneCacheKey=(c,date)=>`${c?.id||""}:${householdPhaseKey(c,date.getTime())}:${Number(c?.timelineResetAt||0)}:${Number(state.lastSaved||0)}:${state.uiLanguage}:${date.getFullYear()}-${date.getMonth()}-${date.getDate()}:${date.getHours()}:${date.getMinutes()}`;
let renderSceneDate=null;
const sharedLifeEntry=(entry,c)=>localizeLifeLog(entry.copy?.[state.uiLanguage]?{...entry,...entry.copy[state.uiLanguage]}:entry,state.uiLanguage,state,c.id);
...
const eventFor=(c,date=renderSceneDate||new Date())=>{
  ...
  if(c&&date===renderSceneDate)projectedRenderScenes.set(c,scene);return scene;
};
const rawViewEvent=(c,date)=>{
  if(state.sharedContext)return c?.sharedScene?routineScene(sharedLifeEntry(c.sharedScene,c),c,state,date.getTime(),state.uiLanguage):{title:t('생활 기록 불러오는 중','생활 기록 불러오는 중'),desc:'',time:'',minute:0,townId:c?.townId};
...
const visibleTimeline=(c,date=renderSceneDate||new Date())=>{
  if(state.sharedContext){const key=`${date.getFullYear()}-${date.getMonth()+1}-${date.getDate()}`;return (c?.days?.[key]?.entries||[]).filter(e=>e&&Number.isFinite(Number(e.minute))&&Number(e.minute)<=date.getHours()*60+date.getMinutes()).map(e=>sharedLifeEntry(e,c));}
  const key=renderSceneCacheKey(c,date);
...
const moneyProfileLabel=value=>({
 ...
 '평범한 형편':t('평범한 형편','Average','普通'),
 '여유 있는 편':t('여유 있는 편','Comfortable','余裕がある'),
 '부유함':t('부유함','Wealthy','裕福'),
 '대부호':t('대부호','Very wealthy','大富豪'),
 '절약 우선':t('절약 우선','Save first','節約優先'),
 '필요한 만큼 소비':t('필요한 만큼 소비','Spend as needed','必要な分だけ使う'),
 '취향에는 아끼지 않음':t('취향에는 아끼지 않음','Spend on favorites','好みには惜しまない'),
 '품질 우선':t('품질 우선','Quality first','品質優先'),
 '가격을 거의 신경 쓰지 않음':t('가격을 거의 신경 쓰지 않음','Rarely mind the price','価格をほぼ気にしない')
}[value]||value);
const moneySettingsLink=c=>`<button type="button" class="career-settings-link" data-character-money-settings="${esc(c.id)}"><b>${t('재산·소비 설정','Wealth & spending','資産・消費設定')}</b><span>${esc(moneyProfileLabel(c.wealth||'평범한 형편'))} · ${esc(moneyProfileLabel(c.income||'필요한 만큼 소비'))}</span></button>`;
const JOBS=["무직","학생","회사원","CEO","의사","간호사","교사","교수","정치인","바리스타","기자","요리사","프로그래머","연구원","가수","아이돌","예술가","해적","군인","범죄자","환경미화원","여관주인","종교인","자영업·직접 입력"];
...
const ROOM_TYPES={living:"거실",kitchen:"주방",entry:"현관",bath:"욕실",bedroom:"침실",study:"서재·취미방",dining:"식당",nursery:"아이방",guest:"손님방",hobby:"취미방",balcony:"베란다",storage:"창고",other:"기타 방"};
...
let accountEntitlements={backgroundPacks:[],iconPacks:[],dlcPacks:[],purchases:[],characterSlotPacks:0,characterSingleSlots:0,townSlotPacks:0,storage50:false};
const previewConfig=()=>window.PARALLEL_CITY_CONFIG?.beta||{};
const previewMode=()=>Boolean(previewConfig().enabled);
const characterLimit=()=>Math.max(0,5+(Math.max(0,Number(accountEntitlements.characterSlotPacks)||0)*5)+Math.max(0,Number(accountEntitlements.characterSingleSlots)||0)-(window.ParallelCityAuth?.getInfo?.().slotUsage?.characters||0));
...
const BUILDING_LIGHT_SOURCES={
  "world-assets/building-types/suitcase-hotel-handdrawn.png":"world-assets/building-types/suitcase-hotel-light.png",
...
const finishedBuildingSource=src=>/^(?:\.\/)?world-assets\/building-types\/generic-building-handdrawn\.png(?:\?.*)?$/.test(src||"")?"world-assets/building-types/office-handdrawn.png":src;
const buildingExteriorSource=place=>finishedBuildingSource(place?.image||state.buildingShapes?.find(shape=>shape.id===place?.iconPreset)?.src||BUILDING_PRESET_SOURCES[place?.iconPreset]||BUILDING_PRESET_SOURCES["drawer-building"]);
const homeExteriorSource=home=>finishedBuildingSource(String(home?.exteriorImage||state.buildingShapes?.find(shape=>shape.id===home?.iconPreset)?.src||BUILDING_PRESET_SOURCES[home?.iconPreset]||BUILDING_PRESET_SOURCES["red-roof-home"]));
...
function wardrobeSceneItem(c,entry,field){
  // Editing base artwork must not run the daily wardrobe simulation or replace
  // the image that the user has just selected with an outfit illustration.
  if(state.activeTab==='character'&&!entry)return null;
...
function sceneAvatar(c,cls="",tone="neutral",mode="sd",entry=null){
  ...
  if(mode==="ld"&&hasLdArt(c,entry)){
    ...
    return `<img class="sprite scene-ld-art ${cls}" src="${imageEsc(src)}" alt="${esc(c.name)} LD 일러스트">`;
  }
  const src=sdArtSource(c,entry);
  if(!src)return `<img class="sprite scene-default-statue ${cls}" src="./assets/home-ui/profile-placeholder.png" alt="${esc(c.name)}">`;
  return `<img class="sprite ${usesProfilePhoto(c,entry)?"profile-photo-fallback":""} ${cls}" src="${imageEsc(src)}" alt="${esc(c.name)}">`;
}
function header(){
  const tabs=[["observe",t("observe","관찰"),"◉"],["mailbox",t("mailbox","우편함"),"✉"],["home",t("home","집"),"⌂"],["character",t("character","캐릭터"),"♙"],["catalog",t("catalog","사전"),"◇"],["relationship",t("relationship","관계"),"∞"],["routine",t("routine","일정"),"▦"],["statistics",t("statistics","통계"),"▥"],["town",t("town","마을"),"▧"],["groups",t("groups","그룹"),"♧"],["shop",t("shop","상점"),"♢"],["settings",t("settings","설정"),"⚙"]];
...
const GAME_HUD_LABELS={
  ko:{character:"캐릭터",catalog:"사전",relationship:"관계",routine:"일정",statistics:"통계",settings:"설정",mailbox:"우편함",home:"집",todayLog:"기록물",shop:"상점",town:"마을",groups:"멀티"},
  en:{character:"People",catalog:"Dictionary",relationship:"Bonds",routine:"Schedule",statistics:"Stats",settings:"Settings",mailbox:"Mail",home:"Home",todayLog:"Log",shop:"Shop",town:"Town",groups:"Multi"},
  ja:{character:"人物",catalog:"辞典",relationship:"関係",routine:"予定",statistics:"統計",settings:"設定",mailbox:"郵便",home:"家",todayLog:"記録",shop:"店",town:"村",groups:"マルチ"}
};
...
export function setSettingsPane(value="home"){
  ...
  settingsPane=["home","gameplay","sound","notifications","display","achievements","account","data","support"].includes(next)?next:"home";
}
...
function gameHudSideMenu(side,character){return side==='right'?socialHomeRight(unreadMailCount()):""}
function gameHudDock(character){return socialHomeDock()}
...
function homeBuildingMapArt(home){
  ...
  return `<span class="building-art" ${light?`data-building-light data-lighting-mode="${config.lightingMode}" data-light-on-time="${config.lightOnTime}" data-light-off-time="${config.lightOffTime}" data-lights-on="${buildingLightsOn(config)}"`:""}><img class="building-preset-image" src="${imageEsc(src)}" alt="">${light?`<img class="building-light building-light-halo" src="${imageEsc(light)}" alt="" aria-hidden="true"><img class="building-light building-light-core" src="${imageEsc(light)}" alt="" aria-hidden="true">`:""}</span>`;
}
...
function placeCard(p,editable=false){
  ...
  const label=mode==="none"?"":`<span class="map-place-label" style="left:${labelX}%;top:${labelY}%;--town-label-scale:${Number(p.imageScale)||1}"><b>${esc(p.name)}</b>${mode==="full"?`<small>${esc(p.subtype?`${p.type} · ${p.subtype}`:p.type)}</small>`:""}</span>`;
...
function homeMapCard(home){
  const mode=state.buildingLabelMode||"full";
  const label=mode==="none"?"":`<span class="map-place-label home-map-label" style="left:${home.mapX}%;top:${home.mapY}%;--town-label-scale:${Number(home.mapScale)||1.08}"><b>${esc(home.name)}</b>${mode==="full"?`<small>${esc(`집 · ${home.buildingSubtype||"단독주택"}`)}</small>`:""}</span>`;
...
function townDecorationCard(item){
  ...
  const art=item.image?`<img class="building-preset-image" src="${imageEsc(item.image)}" alt="">`:`<span class="town-decoration-emoji" aria-hidden="true">${esc(item.emoji||"✨")}</span>`;
...
function tabletObserveMap(){
  ...
  return `<section class="tablet-observe-map" aria-label="${esc(t("town","마을"))}"><div class="tablet-observe-world town-environment" style="--town-brightness:${townTimeAppearance().brightness};--town-dusk:${townTimeAppearance().dusk}" data-town-language="${state.uiLanguage||"ko"}">${townBackgroundMarkup(town?.bg||state.world?.bg)}${state.world.places.map(placeCard).join("")}${townHomes().map(homeMapCard).join("")}${townDecorationsMarkup()}${state.world.places.map(peopleAtPlaceCard).join("")}${townHomes().map(peopleAtHomeCard).join("")}${townTravelersMarkup()}</div></section>`;
}
function multiplayerResidentAvatar(resident,cls=""){
  const source=String(resident?.icon||"").trim();
  return `<span class="multiplayer-resident-avatar ${cls}">${source?`<img src="${imageEsc(source)}" alt="">`:`<b>${esc((resident?.name||"?").slice(0,1))}</b>`}</span>`;
}
function multiplayerObserveCopy(key){
  const text={
    ko:{residents:"이 멀티 마을의 캐릭터",living:"멀티 마을에서 함께 지내는 중",empty:"아직 입주한 캐릭터가 없어요",emptyHelp:"멀티 메뉴에서 내 캐릭터를 이 마을에 입주시켜 주세요.",open:"멀티 설정 열기",location:"멀티 마을",ownerScene:name=>`${name}님의 캐릭터와 같은 마을을 보고 있어요.`},
    en:{residents:"Characters in this multiplayer town",living:"Spending time together in the multiplayer town",empty:"No characters live here yet",emptyHelp:"Open Multiplayer and move one of your characters into this town.",open:"Open multiplayer settings",location:"Multiplayer town",ownerScene:name=>`Viewing ${name}'s character in this shared town.`},
    ja:{residents:"このマルチ村のキャラクター",living:"マルチ村で一緒に過ごしています",empty:"入居中のキャラクターはまだいません",emptyHelp:"マルチ設定から自分のキャラクターをこの村に入居させてください。",open:"マルチ設定を開く",location:"マルチ村",ownerScene:name=>`${name}さんのキャラクターを同じ村で見ています。`}
  };
  return (text[state.uiLanguage]||text.ko)[key];
}
function multiplayerTabletObserveMap(snapshot,town,residents){
  if(!shouldRenderTabletObserveMap())return "";
  const background=town?.previewImage||town?.bg||state.world?.bg||TOWN_BACKGROUND;
  const people=residents.map((resident,index)=>{
    const x=13+(index*23)%72,y=18+(Math.floor(index/4)*24+(index%2)*11)%62;
    return `<button type="button" class="multiplayer-map-resident ${resident.id===snapshot.selectedResidentId?"on":""}" style="--resident-x:${x}%;--resident-y:${y}%" data-multiplayer-resident="${esc(resident.id)}" aria-label="${esc(resident.name)}">${multiplayerResidentAvatar(resident)}<small>${esc(resident.name)}</small></button>`;
  }).join("");
  return `<section class="tablet-observe-map multiplayer-observe-map" aria-label="${esc(town?.name||multiplayerObserveCopy("location"))}"><div class="tablet-observe-world town-environment" style="--town-brightness:${townTimeAppearance().brightness};--town-dusk:${townTimeAppearance().dusk}">${townBackgroundMarkup(background)}<div class="multiplayer-map-residents">${people}</div></div></section>`;
}
function homeTownPicker(){
  const snapshot=window.DrawerVillageGroups?.getSnapshot?.()||{};
  const personal=state.personalTownLabel||({ko:'내 마을',en:'My village',ja:'自分の村'})[state.uiLanguage]||'내 마을';
  const label=({ko:'마을 선택',en:'Choose village',ja:'村を選択'})[state.uiLanguage]||'마을 선택';
  return `<label class="home-town-picker"><select data-multiplayer-dropdown aria-label="${esc(label)}"><option value="">${esc(personal)}</option>${(snapshot.groups||[]).map(g=>`<option value="${esc(g.id)}" ${g.id===snapshot.activeGroupId?'selected':''}>${esc(g.name)}</option>`).join('')}</select><span aria-hidden="true">▼</span></label>`;
}
...
function multiplayerObserve(snapshot,nativeHome){
  return withSharedWorld(snapshot,()=>{
    let markup=observe(true).replaceAll('data-home-character=', 'data-multiplayer-resident=').replaceAll('data-observe-town=','data-multiplayer-town-open=');
    markup=markup.replaceAll('data-open-character-mood','data-shared-mood');
    // Shared actions must go through the authenticated group endpoint.
    markup=markup.replaceAll('data-character-command=','data-shared-command=');
    return markup;
  });
}
...
function sceneEmotionScores(value){
  const text=conflictEvidence(value);
  ...
  add("shock",/큰 충격|충격을 받|충격적|경악|믿기지 않|머리가 하얘|말문이 막|얼어붙었|청천벽력|아연실색|깜짝 놀라|예상하지 못한 (?:사고|비보|이별|폭로|충격)|사고가 나|깨져 버|떨어뜨렸|들켜 버|폭로|비명/,4);
...
function nativeScenePresentation(c,entry,visualMode="sd"){
  ...
  const mirroredPartnerIds=reciprocalSceneCandidates(c.id).filter(id=>{
    ...
    return samePlace&&!otherEntry?.transit&&!entry?.transit&&sameMinute&&(sameInteraction||sameDate)&&reciprocal;
  });
  ...
  const remoteContact=/문자|메시지|전화|편지를|text message|phone|letter|メッセージ|電話|手紙/i.test(text);
  const inferredPartnerIds=remoteContact?[]:mirroredPartnerIds;
  const rawPartnerIds=[...new Set([...declaredPartnerIds,...inferredPartnerIds].filter(id=>{
    ...
    const sameLocation=entry?.home&&otherEntry?.home?(entry.visitHomeId||c.homeId)===(otherEntry.visitHomeId||state.characters[id].homeId)&&entry.room===otherEntry.room:!entry?.home&&!otherEntry?.home&&entry?.placeId&&entry.placeId===otherEntry.placeId&&entry.townId===otherEntry.townId;
    return !remoteContact&&!entry?.transit&&!otherEntry?.transit&&sameLocation&&sleeping===otherSleeping;
  }))];
  ...
  const fighting=Boolean(partner&&/싸움|싸우|말다툼|신경전|충돌|맞받아|목소리.{0,8}높|화를 냈/.test(conflictEvidence(text)));
                ...
                :sceneEmotion==="playful"&&sceneEmotionScore>=2?"interaction-playful"
                  :sceneEmotion==="warm"&&sceneEmotionScore>=1?"interaction-warm"
                    :dating?"date-neutral":partner&&entry?.groupInteraction?"interaction-neutral"
                      :ambientMood==="angry"?"interaction-tense"
                        :ambientMood==="sad"?"sad"
  ...
  const petVisual=pet?(pet.icon?`<img src="${imageEsc(pet.icon)}" alt="${esc(pet.name)}">`:pet.photo?`<img class="photo" src="${imageEsc(pet.photo)}" alt="${esc(pet.name)}">`:`<span>${PET_SCENE_EMOJI[pet.species]||PET_SCENE_EMOJI.기타}</span>`):"";
  ...
  const effectCount=state.animationIntensity==="off"||tone==="sleep"?0:state.animationIntensity==="reduced"?1:tone==="sad"?6:4;
  ...
  const configuredOrder=configuredRelation?relationshipAnimationOrder(configuredRelation,configuredRelation.displayOrder,`${entry?.interactionId||entry?.dateGroup||entry?.title}:${entry?.minute||""}`):Array.isArray(entry?.participantOrder)?entry.participantOrder:[];
  ...
  ])],state.characters,state.relationships,`${entry?.interactionId||entry?.title}:${entry?.minute||''}`,configuredOrder.length>0);
  const sceneParticipants=sceneParticipantIds.map(id=>state.characters?.[id]).filter(Boolean);
  const hugScene=companions.length===1&&!entry?.movementKind&&!entry?.contactRejected&&(entry?.meetingKind==='hug'||/포옹|안아|hug|抱き|抱擁/i.test(entry?.title||''));
  const kissScene=companions.length===1&&!entry?.contactRejected&&!entry?.movementKind&&/키스|입맞춤|kiss|キス|口づけ/i.test(`${entry?.title||''}`),kissTone=/분노|화남|angry|怒/i.test(entry?.mood||'')?'intense':/화해|reconciliation|仲直り/i.test(entry?.title||'')?'reconcile':/조심|cautious|慎重/i.test(entry?.title||'')?'cautious':'warm';
  const lineupHtml=companions.length?`<span class="native-scene-lineup ${entry?.meetingKind==='affection'&&entry?.groupInteraction&&!entry?.contactRejected?'is-affection':''} ${hugScene?'is-hugging':''} ${kissScene?`is-kissing kiss-${kissTone}`:""} ${coResidentConversation?"is-conversation":""} ${sceneParticipants.length===2?"is-pair":""}" style="--scene-count:${sceneParticipants.length}" aria-label="${esc(sceneParticipants.map(person=>person.name).join(", "))}">${sceneParticipants.map((person,index)=>{
...
function dailyLogItems(entries,c){
  ...
  return sortLogEntries(uniqueDisplayedMoments(entries)).map(x=>{
    if(x.dateGroup){
...
function dailyLog(c){
  ...
  return `<section class="panel life-log shared-life-log"><div class="title"><h2>오늘의 생활 로그</h2><small>${esc(c.name)} · 관찰과 집에서 같은 기록을 보여줘요</small></div>${logOrderControl(state.uiLanguage)}<ol data-log-entries>${dailyLogItems(entries,c)}</ol></section>`;
}
...
function townActionFace(character,entry,place=null){const action=townActionPresentation(entry,place);return `<span class="place-person-face town-action-${action.kind}" role="button" tabindex="0" data-person="${character.id}" title="${esc(`${character.name} · ${action.label}`)}">${activityProgressMarkup(state,character,entry)}${avatar(character)}${action.icon?`<i class="town-action-symbol" aria-hidden="true">${action.icon}</i>`:""}</span>`}
...
function meetingWalker(character,scene,segment,stationary=false,surface="town"){
  ...
  return `<button type="button" class="meeting-walker ${stationary&&surface==="home"&&/씻|샤워|목욕|washing|shower|bath|洗|入浴/i.test(scene.title||"")?"is-washing":""} ${scene.privacyStartled?'is-startled':''} meeting-surface-${surface} ${stationary?"is-stationary":""} ${stationary&&scene.groupInteraction&&/키스|입맞춤|kiss|キス/i.test(scene.title||"")?"is-kissing":""}" data-person="${esc(character.id)}" data-journey-end="${segment.end}" style="--meeting-x0:${segment.from.x}%;--meeting-y0:${segment.from.y}%;--meeting-x1:${segment.to.x}%;--meeting-y1:${segment.to.y}%;--meeting-duration:${seconds}s;--meeting-delay:-${elapsed}s" aria-label="${esc(scene.title)}">${stationary?activityProgressMarkup(state,character,scene):''}${scene.privacyStartled?'<i class="privacy-exclamation" aria-hidden="true">!</i>':''}${avatar(character)}${surface==="home"?`<span class="meeting-caption"><b>${esc(character.name)}</b><small>${esc(scene.title)}</small></span>`:""}</button>`;
}
...
export function buildingDetailDialogs(selectedKey=""){
  ...
  const placeDialogs=state.world.places.filter(place=>place.id===selectedKey).map(place=>{
    ...
    const stockList=stock.length?`<ul class="building-stock-list">${stock.map(item=>`<li>${item.image?`<img src="${imageEsc(item.image)}" alt="">`:`<span>${esc(({food:"🍽️",ingredient:"🥕",fashion:"👕",music:"🎵",game:"🎮",media:"🎬",book:"📚"})[item.kind]||"✨")}</span>`}<b>${esc(item.name)}</b></li>`).join("")}</ul>`:`<p class="building-detail-empty">등록된 판매 상품이나 이용 항목이 없어요.</p>`;
    const interior=place.interiorImage?`<img src="${imageEsc(place.interiorImage)}" alt="${esc(place.name)} 내부">`:`<div class="building-interior-placeholder"><span>${esc(place.emoji||"🏢")}</span><b>내부 이미지가 아직 없어요</b><small>마을 편집에서 내부 사진을 등록할 수 있어요.</small></div>`;
  ...
  const homeDialogs=townHomes().filter(home=>`home:${home.id}`===selectedKey).map(home=>{
    ...
    const interior=interiorImage?`<img class="home-interior-photo" src="${imageEsc(interiorImage)}" alt="${esc(home.name)} 대표 실내">`:`<div class="building-interior-placeholder"><span>🏠</span><b>대표 실내 이미지가 아직 없어요</b><small>집 화면에서 집이나 방 사진을 등록할 수 있어요.</small></div>`;
...
function discoveryStatistics(characters){
 const rows=Object.entries(DISCOVERY_AXES).map(([field,axis])=>{
  const members=characters.filter(c=>axis.values.includes(c[field])||Number.isFinite(c.discovery?.scores?.[field])||(axis.numeric&&Number.isFinite(c[axis.numeric])));
  ...
  else{const mean=metrics.reduce((n,m)=>n+m.value,0)/metrics.length;value=mean.toFixed(1)+' / 100';const stage=axis.values[Math.round(mean/100*(axis.values.length-1))];summary=t(stage,stage);}
  return `<tr><th>${esc(t(axis.label,axis.label))}</th><td>${value}</td><td>${esc(summary)}</td></tr>`;
 }).join('');
...
function statisticsDashboard(){
  ...
  const personality=distribution(characters.flatMap(character=>character.personalityTypes||[]).filter(value=>value&&!/^(미설정|설정하지 않음|정하지 않음|Unset|Not set)$/.test(value)),10),personalityTotal=Math.max(1,personality.reduce((sum,[,count])=>sum+count,0)),palette=["#d6ad2f","#7d5d9b","#5c8fb5","#bb715d","#6e9b76","#c189b0","#8a755d","#789393","#b5a166","#888"];
...
function observe(shared=false){
  ...
  if(!localId){
    if(nativeHome){
      ...
      return `<section class="game-observe-hud game-observe-empty" data-native-hud-version="4" data-home-ui-theme="${theme.id}" style="${homeUiThemeStyle()}" aria-label="${esc(t("observe","관찰"))}">${tabletObserveMap()}<div class="native-observe-backdrop" style="background-image:url(&quot;${esc(state.world?.bg||TOWN_BACKGROUND)}&quot;)"></div><div class="native-observe-shade"></div><div class="game-hud-top game-hud-empty-top"><div><b>${esc(state.world?.name||t("town","마을"))}</b><small>${t("emptyTownTitle","이 마을에 사는 캐릭터가 없어요")}</small></div><time>${new Date().toLocaleTimeString(uiLocale(),{hour:"2-digit",minute:"2-digit"})}</time>${homeTownPicker()}</div>${gameHudSideMenu("left")}${gameHudSideMenu("right")}<div class="game-observe-empty-copy"><span>🏙️</span><h1>${t("emptyTownTitle","이 마을에 사는 캐릭터가 없어요")}</h1><p>${t("emptyTownHelp","캐릭터 화면에서 생활하는 마을을 지정해 주세요.")}</p><button class="primary" data-tab="character">${t("openCharacterSettings","캐릭터 설정 열기")}</button></div>${gameHudDock()}</section>${buildingDetailDialogs()}`;
    }
    return `<div class="standard-observe-view">${roster()}${townSwitcher}<div class="observe desktop-observe-map-only"><section><div class="viewport"><div class="world town-environment" style="--town-brightness:${townTimeAppearance().brightness};--town-dusk:${townTimeAppearance().dusk}" data-town-language="${state.uiLanguage||"ko"}">${townBackgroundMarkup(state.world?.bg)}${state.world.places.map(placeCard).join("")}${townHomes().map(homeMapCard).join("")}${townDecorationsMarkup()}</div></div></section></div>${buildingDetailDialogs()}</div>`;
  }
  ...
  const location=`<span class="game-hud-location">${locationExterior?`<img src="${imageEsc(locationExterior)}" alt="${esc(t("locationExterior","현재 건물 외관"))}">`:`<i class="${e.transit?"is-moving":""}" aria-hidden="true">${sceneMovementIcon(e)}</i>`}<b>${esc(locationText)}</b></span>`;
  ...
  const hudRoster=`<details class="game-hud-profile"><summary class="game-hud-profile-toggle" data-open-game-hud-roster aria-label="${esc(t("characterPicker","관찰 캐릭터 바꾸기"))}"><span class="game-hud-profile-frame">${profileAvatar(c,"game-hud-current-profile")}<img class="game-hud-profile-ring" src="${imageEsc(homeUiAsset(c,"profile-ring.png"))}" alt=""></span><span class="game-hud-profile-copy"><b>${esc(c.name)}</b><small><em>${esc(careerHudLabel(state,c)||t("생활 중","생활 중"))}</em></small></span></summary><section class="game-hud-roster-drawer"><small class="game-hud-roster-kicker">${t("선택한 그룹의 캐릭터","선택한 그룹의 캐릭터")}</small><div class="game-hud-roster-options" aria-label="${esc(t("characterPicker","관찰 캐릭터 선택"))}">${localIds.map(id=>{const person=state.characters[id];return `<button type="button" data-home-character="${id}" class="game-hud-button character-picker-button ${id===c.id?"on":""}" style="--picker-theme:${esc(person.theme?.primary||"#176b60")}" title="${esc(person.name)}" aria-label="${esc(person.name)}">${profileAvatar(person)}<small>${esc(person.name)}</small></button>`}).join("")}</div>${groupJump()}${otherTowns.length?`<div class="game-hud-town-jump"><b>${t("moveToAnotherTown","다른 마을로 이동")}</b><div>${otherTowns.map(town=>`<button type="button" data-observe-town="${town.id}">${esc(town.name)}</button>`).join("")}</div></div>`:""}</section></details>`;
  ...
  const gameHud=`<section class="game-observe-hud scene-tone-${presentation.tone} scene-action-${presentation.actionKind} ${movementClass}" data-native-hud-version="4" data-observed-character="${esc(c.id)}" data-life-sound="${lifeSound(e)}" data-life-sound-event="${esc(lifeSoundEpisode(e))}" data-home-ui-theme="${theme.id}" style="${homeUiThemeStyle(c)};--native-own:${esc(c.theme?.primary||"#176b60")};--native-own-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")};--log-theme:${logTheme}" aria-label="${esc(c.name)}의 관찰 화면">${tabletObserveMap()}<div class="native-observe-backdrop" style="background-image:url(&quot;${esc(nativeBackground)}&quot;)"></div><div class="native-observe-shade"></div><div class="native-scene-atmosphere atmosphere-${presentation.atmosphere}" aria-hidden="true"></div>${presentation.effects}<div class="game-hud-top">${hudRoster}<time>${gameTime}</time><small class="game-hud-date">${esc(gameDate)}</small>${homeTownPicker()}</div>${gameHudSideMenu("left",c)}${gameHudSideMenu("right",c)}<div class="game-hud-stage visual-mode-${visualMode}"><div class="native-character-stage ${stageClasses}" style="--home-visual-scale:${visualScale};${sceneLayoutVars(c,visualMode,e)}" aria-label="${esc(c.name)} 현재 장면">${sceneActors}</div></div>${statusCard}${gameHudDock(c)}</section>`;
...
const isLayeredBed=p=>!p?.image&&['침대','1인 침대','커플 침대'].includes(p?.item);
const furniturePlacementStyle=(placement,footprint)=>{if(isLayeredBed(placement))footprint=placement.item==="커플 침대"?{columns:3,rows:4}:{columns:1.5,rows:2};const sprite=furnitureSprite(placement);return `--sprite-width:${sprite?sprite.width/527*2/footprint.columns*sprite.scale:1};--sprite-ratio:${sprite?sprite.width/sprite.height:1};--furniture-x:${placement.x}%;--furniture-y:${placement.y}%;--furniture-scale:${placement.scale};--furniture-rotation:${isLayeredBed(placement)?bedPerspective(placement).artRotation:sprite&&sprite.kind!=="custom"?0:placement.rotation}deg;--furniture-layer:${placement.layer};--furniture-flip:${sprite?.flip??(isLayeredBed(placement)?bedPerspective(placement).artFlip:placement.flipped?-1:1)};--furniture-grid-width:${footprint.columns};--furniture-grid-height:${footprint.rows}`};
const coupleBedImage=(layer,extra="",placement={})=>placement.item!=="커플 침대"?`<img class="couple-bed-layer couple-bed-${layer} ${extra}" src="assets/furniture/wood/single-bed-${bedPerspective(placement).side?"side":"front"}-${layer}.png" alt="" aria-hidden="true">`:`<img class="couple-bed-layer couple-bed-${layer} ${extra}" src="${COUPLE_BED_ASSET_ROOT}/couple-bed-${bedPerspective(placement).side?"side-":""}${layer}.${bedPerspective(placement).side?"svg":"png"}" alt="" aria-hidden="true">`;
export function roomFurnitureMarkup(homeId,roomKey,room,edit,bedStates=new Map()){
  const placements=normalizeFurniturePlacements(room.furniturePlacements),hideArt=room.hideFurniture===true&&!edit;
  ...
  return `<div class="room-furniture-layer" aria-label="${esc(room.name||roomKey)} 가구">${placements.map(placement=>{
    const label=furnitureLabel(placement.item,state.uiLanguage),props=placement.props||[],footprint=furnitureFootprint(placement.item),coupleBed=isLayeredBed(placement),bedState=edit?"default":bedStates.get(placement.id)||"default";
    const sprite=furnitureSprite(placement);


    const art=sprite&&!coupleBed?`<span class="room-furniture-art">${sprite.kind==="counter"?counterArt(sprite,placement.counterSpan):`<img class="furniture-sprite" src="${esc(sprite.src)}" alt="">`}${["counter","nightstand","table"].includes(sprite.kind)?`<span class="furniture-surface-grid" data-surface-kind="${sprite.kind}" data-surface-side="${["left","right"].includes(sprite.direction)}" style="--surface-columns:${sprite.kind==="nightstand"?1:Math.round(placement.counterSpan*4)}"></span>`:""}</span>`:coupleBed?`<span class="room-furniture-art room-couple-bed-art" data-bed-layer-state="${bedState}" data-bed-side="${bedPerspective(placement).side}" aria-hidden="true">${coupleBedImage("base","",placement)}${bedState!=="under-cover"?coupleBedImage("quilt","",placement):""}${bedState==="default"?coupleBedImage("footboard","",placement):""}</span>`:`<span class="room-furniture-art" aria-hidden="true">${furnitureIcon(placement.item)}</span>`;
    return `<button type="button" class="room-furniture-item ${coupleBed?"is-couple-bed":""}" data-surface-id="${esc(placement.surfaceId||'')}" data-surface-u="${placement.surfaceU}" data-surface-v="${placement.surfaceV}" data-surface-rotation="${placement.rotation}" data-counter-span="${placement.counterSpan}" data-placement-scale="${Number(placement.scale)||1}" data-furniture-kind="${sprite?.kind||""}" data-seat-side="${esc(placement.seatSide||"")}" data-table-id="${esc(placement.tableId||"")}" data-seat-direction="${sprite?.direction||""}" data-bed-single="${coupleBed&&placement.item!=="커플 침대"}" data-bed-direction="${bedPerspective(placement).direction}" data-bed-side="${coupleBed&&bedPerspective(placement).side}" data-furniture-placement="${esc(placement.id)}" data-home-id="${esc(homeId)}" data-room-key="${esc(roomKey)}" data-furniture-name="${esc(label)}" data-furniture-supports-props="${supportsFurnitureProps(placement.item)}" data-furniture-columns="${footprint.columns}" data-furniture-rows="${footprint.rows}" style="${furniturePlacementStyle(placement,footprint)}" aria-label="${esc(label)}${edit?" · 끌어서 이동":""}" ${edit?"":"tabindex=\"-1\""}>${hideArt?"":art}${!hideArt&&props.length?`<span class="room-furniture-props" aria-hidden="true">${props.map((prop,index)=>`<i style="--prop-slot:${index}">${furniturePropIcon(prop.item)}</i>`).join("")}</span>`:""}${edit?`<small>${esc(label)} · ${footprint.columns}×${footprint.rows}</small>`:""}</button>`;
  }).join("")}${placements.filter(p=>!hideArt&&furnitureSprite(p)?.frame).map(p=>`<span class="chair-frame-overlay" data-chair-frame="${esc(p.id)}" style="${furniturePlacementStyle(p,furnitureFootprint(p.item))}"><span class="room-furniture-art"><img class="furniture-sprite" src="${furnitureSprite(p).frame}" alt=""></span></span>`).join("")}</div>`;
}
function roomFurnitureOverlayMarkup(room,bedStates=new Map()){
  if(room.hideFurniture===true)return "";
  const overlays=normalizeFurniturePlacements(room.furniturePlacements).filter(placement=>isLayeredBed(placement)&&["on-bed","under-cover"].includes(bedStates.get(placement.id))).map(placement=>{
    const footprint=furnitureFootprint(placement.item),bedState=bedStates.get(placement.id);
    return `<span class="room-couple-bed-overlay" data-bed-overlay="${esc(placement.id)}" data-bed-layer-state="${bedState}" style="${furniturePlacementStyle(placement,footprint)}">${bedState==="under-cover"?coupleBedImage("quilt","",placement):""}${coupleBedImage("footboard","",placement)}</span>`;
  });
...
function homeLifePersonMarkup(character,event,agent,room,roomKey,index,bedSlot=-1,options={}){
  ...
  const bedRotation=bedPerspective(options.bedPlacement).characterRotation,bedRadians=bedRotation*Math.PI/180,bedPlacementScale=Math.max(.55,Math.min(1.8,Number(options.bedPlacement?.scale)||1)),bedVisualScale=bedPlacementScale*COUPLE_BED_ART_SCALE;
  ...
  const quietBedSleep=bedSlot>=0&&sleeping,hideStatus=quietBedSleep||Boolean(options.hideStatus)||scene.meetingKind==='affection'&&scene.groupInteraction&&!scene.contactRejected;
  const sleepMotion=homeSleepAnimation(character.sleepHabit);
  return `<div class="home-person ${scene.remote?'is-remote-contact':''} ${!walking&&/씻|샤워|목욕|washing|shower|bath|洗|入浴/i.test(title)?"is-washing":""} ${lifeClass} ${scene.contactRejected?'is-contact-declined':scene.meetingKind==='affection'&&scene.groupInteraction?'is-affection':(scene.meetingKind==='hug'||/포옹|안아|hug|抱き|抱擁/i.test(scene.title||''))?'is-hugging':''} scene-action-${esc(actionKind)} ${sleeping?"is-sleeping":""}" data-sleep-style="${sleeping?sleepMotion.style:""}" style="--sleep-duration:${sleepMotion.duration}s;--sleep-delay:-${(now/1000+index*.71)%sleepMotion.duration}s;${lifeStyle}--home-float-delay:${-(index%4)*.37}s" role="button" tabindex="0" aria-label="${esc(`${character.name} · ${title}`)}" data-using-furniture="${agent?.phase==="using"?esc(agent.furnitureId||""): ""}" data-seat-order="${Number(options.seatRank)||0}" data-seat-id="${agent?.phase==="using"&&["의자","소파"].includes(agent?.item)?esc(agent.furnitureId):""}" data-couple-bed-id="${esc(options.bedPlacement?.id||"")}" data-bed-slot="${bedSlot}" data-home-person="${esc(character.id)}" data-journey-end="${walking?agent?.arrivesAt||0:0}" data-home-occupant="character" data-character-id="${esc(character.id)}" data-occupant-name="${esc(character.name)}" data-occupant-title="${esc(title)}" data-occupant-desc="${esc(scene?.desc||"")}" data-occupant-room="${esc(room.name||roomKey)}"><span class="home-person-visual">${activityProgressMarkup(state,character,scene)}${avatar(character)}${actionProp}${scene.remote?'<i class="remote-contact-prop" aria-hidden="true">📱</i>':''}${!walking&&['compliment','request_space'].includes(scene.meetingKind)?`<i class="home-person-chat-bubble" aria-hidden="true">${scene.meetingKind==='compliment'?'💛':'💧'}</i>`:hideStatus||agent?.phase==='using'&&agent?.furnitureId?'':scene.gossipReaction?`<i class="home-person-chat-bubble" aria-hidden="true">${scene.gossipReaction==='angry'?'💢':scene.gossipReaction==='surprised'?'!':'…'}</i>`:speech}${sleeping&&!quietBedSleep?'<i class="room-sleep-mark" aria-hidden="true">ZZ</i>':""}</span>${(quietBedSleep||Boolean(options.hideStatus)||(hideStatus&&!(agent?.phase==="using"&&agent?.furnitureId)))?"":`<span class="home-person-status" data-shared-furniture="${esc(scene.sharedFurnitureKey||'')}" data-interaction-id="${esc(scene.interactionId||'')}" data-person-name="${esc(character.name)}"><b>${esc(character.name)}</b><small>${esc(activity)}</small></span>`}</div>`;
}
function homeInteractionSummary(scene){
  if(scene?.contactRejected)return {kind:"declined",label:scene.title||"",symbol:"…"};
  if(scene?.meetingKind==="affection")return {kind:"affection",label:scene.title||"",symbol:"♡"};
  const directed={taunt:['taunt','…'],insult:['insult','!'],argue:['argue','💢'],fight:['fight','💢'],cook_together:['cook','🍳'],tea:['meal','☕'],drinks:['meal','🥂'],dine:['meal','🍽️']}[scene?.meetingKind];
  if(directed)return {kind:directed[0],label:scene.title||'',symbol:directed[1]};
  ...
  return {kind:"together",label:String(scene?.title||scene?.sharedActionText||t("함께 시간을 보내는 중","함께 시간을 보내는 중")),symbol:"✦"};
}
function homeOrderedCharacters(characters,scenes=[]){
  const relation=Object.values(state.relationships||{}).find(r=>characters.some(c=>c.id===r.a)&&characters.some(c=>c.id===r.b));
  const configured=relation?relationshipAnimationOrder(relation,relation.displayOrder,'home'):scenes.flatMap(scene=>Array.isArray(scene?.participantOrder)?scene.participantOrder:[]),rank=id=>{
  ...
  return orderAnimationCharacters([...characters].sort((a,b)=>rank(a.id)-rank(b.id)).map(c=>c.id),state.characters,state.relationships,scenes[0]?.interactionId||'home',true).map(id=>state.characters[id]).filter(Boolean);
}
function homeBedForegroundStatusMarkup(characters,scenes,room,roomKey,bedPlacement,index,{shared=false}={}){
  if(scenes.some(scene=>scene?.meetingKind==="affection"&&!scene.contactRejected))return "";
  ...
  return `<div class="home-bed-foreground-status ${shared?"is-shared":""}" style="--bed-status-x:${x}%;--bed-status-y:${y}%;--home-float-delay:${-(index%4)*.37}s" role="group" aria-label="${esc(title)}" data-bed-status-for="${esc(bedPlacement?.id||"")}"><span class="home-interaction-status" data-interaction-id="${esc(scenes[0]?.interactionId||'')}"><b>${esc(names)}</b><small>${esc(activity)}</small></span></div>`;
}
...
function homeLifeInteractionMarkup(characters,scenes,agents,room,roomKey,index){
  ...
  return `<div class="home-interaction-card home-life-interaction home-interaction-${summary.kind} ${summary.kind==="affection"?"is-affection":""}" style="${lifeStyle}--home-float-delay:${-(index%4)*.37}s" role="group" aria-label="${esc(title)}" data-home-interaction="${esc(scene.interactionId||"")}"><span class="home-interaction-visual">${ordered.slice(0,2).map((character,avatarIndex)=>{const characterScene=sceneByCharacterId.get(character.id)||scene;return `<button type="button" class="home-interaction-avatar home-interaction-avatar-${avatarIndex+1}" aria-label="${esc(`${character.name} · ${summary.label}`)}" data-home-occupant="character" data-character-id="${esc(character.id)}" data-occupant-name="${esc(character.name)}" data-occupant-title="${esc(characterScene?.title||summary.label)}" data-occupant-desc="${esc(characterScene?.desc||scene?.desc||"")}" data-occupant-room="${esc(room.name||roomKey)}">${avatar(character)}</button>`}).join("")}<em aria-hidden="true">${summary.symbol}</em>${speech}</span>${summary.kind==="affection"?"":`<span class="home-interaction-status" data-interaction-id="${esc(scene.interactionId||'')}"><b>${esc(names)}</b><small>${esc(summary.label)}</small></span>`}</div>`;
}
...
function home(sharedPass=false){
  const shared=window.DrawerVillageGroups?.getSnapshot?.();if(!sharedPass&&shared?.activeGroupId&&shared.group)return withSharedWorld(shared,()=>home(true));
...
export function homeCard(id,chars){
  ...
  const mobileRooms=scaleDefaultRooms(mobileRoomLayout(visibleRoomKeys,h.rooms||{}),h);
  ...
  const roomHtml=visibleRoomKeys.map(key=>{
    ...
    coupleBedUsers.forEach((users,bedId)=>{
      ...
      users.splice(0,users.length,...homeOrderedCharacters(users,users.map(sceneFor)));
    ...
    shownPeople.forEach((character,index)=>{
      ...
      if(bedPair.length>1){
        ...
        const pairInteraction=sceneFor(pair[0])?.interactionId||lifeAgents[pair[0].id]?.interactionId;
        const sharedInteraction=Boolean(pairInteraction&&pair.every(person=>(sceneFor(person)?.interactionId||lifeAgents[person.id]?.interactionId)===pairInteraction));
        pair.forEach((person,slot)=>{
          renderedPeople.add(person.id);
          peopleMarkup.push(homeLifePersonMarkup(person,sceneFor(person),lifeAgents[person.id]||{},room,key,index+slot,coupleBedSlots.get(person.id)??slot,{conversing:sharedInteraction,slot:slot+1,bedPlacement:sharedBed,bedState:"under-cover",bedConversation:sharedInteraction,hideStatus:true}));
        });
      ...
      if(agent?.phase==='using'&&['의자','소파'].includes(agent.item)&&agent.furnitureId){
        ...
        const seatPeers=homeOrderedCharacters(shownPeople.filter(p=>lifeAgents[p.id]?.furnitureId===agent.furnitureId));
        peopleMarkup.push(homeLifePersonMarkup(character,sceneFor(character),agent,room,key,index,-1,{seatRank:seatPeers.findIndex(p=>p.id===character.id)}));
        return;
      }
      ...
      const partners=interactionId?shownPeople.filter(other=>!renderedPeople.has(other.id)&&sceneFor(other)?.interactionId===interactionId):[];
      if(partners.length>1){
        partners.forEach(other=>renderedPeople.add(other.id));
        const summary=homeInteractionSummary(scene);
        if(summary.kind==="talk"||partners.length>2){
          const pair=homeOrderedCharacters(partners,partners.map(sceneFor));
          ...
          pair.forEach((person,slot)=>{
            ...
            peopleMarkup.push(homeLifePersonMarkup(person,sceneFor(person),paired,room,key,index+slot,coupleBedSlots.get(person.id)??-1,{conversing:summary.kind==="talk",slot:slot+1,bedPlacement:personBed,bedState:sharedBed?"under-cover":personBed?bedStates.get(personBed.id):"",bedConversation:Boolean(sharedBed),hideStatus:Boolean(sharedBed)}));
          });
      ...
      peopleMarkup.push(homeLifePersonMarkup(character,scene,characterAgent,room,key,index,coupleBedSlots.get(character.id)??-1,{bedPlacement:characterBed,bedState:characterBed?bedStates.get(characterBed.id):"",hideStatus:Boolean(activeBed)}));
      if(activeBed)foregroundMarkup.push(homeBedForegroundStatusMarkup([character],[scene],room,key,activeBed,index));
    });
    ...
    return `<div class="room room-${esc(room.type||key)} ${customFloor?"room-custom-floor":""} ${customTile?"room-custom-tile":""} ${hasOccupants?"room-has-occupants":""} ${edit?"room-edit-target":""}" ${roomStyle(h,key,packedRooms.items[key],mobileRooms[key])} ${editAttributes}>
      ...
      ${roomFurnitureMarkup(id,key,room,edit,bedStates)}${edit?'':roomFoodMarkup(state,id,key)}
  ...
  const meetingWalkers=inside.filter(c=>sceneFor(c)?.meetingLocation?.home&&!usesAnchoredFurniture(c)).map(c=>{
    ...
    const partner=!scene.meetingJourney&&scene.groupInteraction?inside.find(p=>p.id!==c.id&&(scene.withIds||[scene.withId]).includes(p.id)&&!sceneFor(p)?.meetingJourney&&sceneFor(p)?.room===scene.room):null;
    if(partner){const pairKey=[c.id,partner.id].sort().join('|');if(drawnPairs.has(pairKey))return '';drawnPairs.add(pairKey);const center=convert(segment.toRoom,segment.to),bounds=visualRoomLayout(segment.toRoom);return `<div class="meeting-pair ${scene.contactRejected?'is-contact-declined':(scene.meetingKind==='hug'||/포옹|안아|hug|抱き|抱擁/i.test(scene.title||''))?'is-hugging':scene.meetingKind==='affection'?'is-affection':/키스|입맞춤|kiss|キス/i.test(scene.title||'')?'is-kissing':/gift|선물/.test(scene.meetingKind||scene.title||'')?'is-gifting':['taunt','insult','argue','fight','cook_together'].includes(scene.meetingKind)?'is-social-'+scene.meetingKind:['talk','gossip','comfort','compliment','debate'].includes(scene.meetingKind)?'is-talking':''}" style="--pair-x:${center.x}%;--pair-room-left:${Number(bounds.x)||0}%;--pair-room-width:${Number(bounds.w)||100}%;top:${center.y}%;--gift-direction:${homeOrderedCharacters([c,partner],[scene,sceneFor(partner)])[0]?.id===scene.meetingActorId?1:-1}"><div class="meeting-pair-faces">${homeOrderedCharacters([c,partner],[scene,sceneFor(partner)]).map(p=>`<button type="button" class="${["affection","kiss","kiss_cautious","kiss_reconcile"].includes(scene.meetingKind)||/키스|입맞춤|kiss|キス/i.test(scene.title||"")?characterMood(p,sceneFor(p),state).tone==="excited"?"is-blushing":"":""}" data-person="${esc(p.id)}" aria-label="${esc(p.name)}">${activityProgressMarkup(state,p,sceneFor(p))}${avatar(p)}</button>`).join('')}</div>${scene.meetingKind==="affection"&&!scene.contactRejected?"":`<span class="meeting-caption"><b>${esc(homeOrderedCharacters([c,partner],[scene,sceneFor(partner)]).map(p=>p.name).join(" · "))}</b><small>${esc(scene.title)}</small></span>`}</div>`}
  ...
  const petCards=pets.map(p=>`<article class="pet-card" data-pet-editor-id="${p.id}">
    <div class="pet-avatar">${p.icon?`<img class="pet-icon-art" src="${imageEsc(p.icon)}" alt="">`:p.photo?`<img class="pet-photo-art" src="${imageEsc(p.photo)}" alt="">`:`<span>${petEmoji[p.species]||"🐾"}</span>`}</div>
  ...
  const cars=(h.cars||[]).map(car=>`<button type="button" class="car-card" data-open-car-editor="${car.id}" data-home-id="${id}">${car.image?`<img class="car-photo" src="${imageEsc(car.image)}" alt="">`:`<span class="car-icon">🚙</span>`}<span><b>${esc(car.name)}</b><small>${esc(car.type)}${car.color?` · ${esc(car.color)}`:""} · ${car.seats||5}인승</small><em>눌러서 편집</em></span></button>`).join("");
  const humanResidentScenes=chars.map(c=>{
    ...
    return `<article class="resident-scene-card" style="--resident-theme:${esc(c.theme?.primary||"#176b60")}">
      <div class="resident-profile"><span class="resident-profile-picture">${profileAvatar(c,"resident-character-image")}</span><span><h3>${esc(c.name)}</h3><small>${esc(careerCaption(state,c))}</small></span></div>
      <div class="resident-current"><small>CURRENT SCENE</small><h3>${esc(e.title)}</h3><p>${esc(e.desc)}</p><b>${location}</b>${image?`<img src="${imageEsc(image)}" alt="">`:""}</div>
    </article>`;
  }).join("");
  const petResidentScenes=pets.map(p=>{
    ...
    const visual=p.icon
      ?`<img class="resident-pet-icon" src="${imageEsc(p.icon)}" alt="">`
      :p.photo
        ?`<img class="resident-pet-photo" src="${imageEsc(p.photo)}" alt="">`
  ...
  return `<article class="home panel ${edit?"is-editing":""}" data-home-card="${id}">
    ...
    <div class="home-canvas-viewport" data-home-pan="${id}:${activeFloor}"><div class="rooms ${visibleRoomKeys.length>6?"has-extra":""}" data-room-canvas data-home-id="${id}" data-room-floor="${activeFloor}" data-room-grid-cols="${homeGrid(h).columns}" data-room-grid-rows="${homeGrid(h).rows}" style="--home-canvas-width:calc(100cqw * ${homeGrid(h).columns/12});--home-canvas-height:calc(100cqh * ${homeGrid(h).rows/16});--room-count:${visibleRoomKeys.length};--room-cols:4;--room-rows:${packedRooms.rows}">${roomHtml||`<button type="button" class="empty-floor-room" data-add-room>+ ${activeFloor}층에 방 추가</button>`}${canvasWalkers||meetingWalkers?`<div class="home-life-roaming-layer" aria-label="방 사이를 이동하는 캐릭터">${canvasWalkers}${meetingWalkers}</div>`:""}</div></div>${edit?homeFurnitureDrawer(h,state.uiLanguage):""}${furnitureToolbar}
...
const EYE_FEATURE_OPTIONS=["역안","눈매가 날카로움","눈매가 부드러움","눈꼬리가 올라감","눈꼬리가 내려감","큰 눈","작은 눈","쌍꺼풀 있음","속쌍꺼풀","무쌍","삼백안","사백안","졸린 눈","처진 눈","짝눈","오드아이","눈 밑 점","다크서클","속눈썹이 김","눈썹이 진함"];
...
const TATTOO_TYPE_OPTIONS=["설정하지 않음","이레즈미","문자·문구","기하학 무늬","꽃·식물","동물","상징·문장","인물·초상","추상 무늬","전통 문양","작은 포인트","넓은 면적의 문신","기타 문신"];
...
const WARDROBE_TAG_GROUPS={
  ...
  "분위기":["섹시","화려","성숙","청순","큐티","우아","시크","중성적","단정","캐주얼","클래식","고딕","펑크","스트리트","빈티지","로맨틱","스포티","미니멀","럭셔리","판타지","전통의상"],
...
function characterHomeLayoutEditor(c){
  ...
  const sdArt=previewSd
    ?`<img class="${usesProfilePhoto(c,previewEntry)?"profile-photo-fallback":""}" src="${imageEsc(previewSd)}" alt="${esc(c.name)} SD 미리보기">`
      :`<span class="home-layout-fallback">${esc((c.name||"새").slice(0,1))}</span>`;
  const ldArt=hasLdArt(c,previewEntry)
    ?`<img class="scene-ld-art" src="${imageEsc(ldArtSource(c,previewEntry))}" alt="${esc(c.name)} LD 미리보기">`
  ...
  const previewSide=side=>`<nav class="home-layout-preview-side ${side}">${GAME_HUD_SIDE_TABS[side].map(({key,labelKey,label,asset})=>`<span><img src="${imageEsc(homeUiAsset(c,asset))}" alt=""><small>${gameHudLabel(labelKey,label)}</small></span>`).join("")}</nav>`;
  const dockItems=[["home","집","home.png"],["mailbox","우편함","mailbox.png"],["todayLog","기록물","ink.png"],["shop","상점","shop.png"],["town","마을","town.png"]];
  const previewDock=`<nav class="home-layout-preview-dock">${dockItems.map(([key,label,asset])=>`<span><img src="${imageEsc(homeUiAsset(c,asset))}" alt=""><small>${gameHudLabel(key,label)}</small></span>`).join("")}</nav>`;
...
function characterFullOverview(c){
  ...
  const currentIcon=c.icon
    ?`<img src="${imageEsc(c.icon)}" alt="${esc(c.name)} 아이콘 미리보기">`
    :emptyPhotoSlot(t("아이콘","아이콘"));
  const placementArt=hasLdArt(c)
    ?`<img class="scene-ld-art" src="${imageEsc(ldArtSource(c))}" alt="${esc(c.name)} LD 배치 미리보기">`
    :c.icon?`<img src="${imageEsc(c.icon)}" alt="${esc(c.name)} SD 배치 미리보기">`:profileAvatar(c);
  return `<section class="character-full-svg-overview" aria-label="${esc(c.name)} ${t("이미지","이미지")}"><div class="character-full-content-group">
    ...
    <button type="button" class="character-full-image-slot profile ${c.photo?"has-image":"is-empty"}" data-image="photo">${c.photo?`<img src="${imageEsc(c.photo)}" alt="${esc(c.name)} 프로필 사진">`:emptyPhotoSlot(t("프로필 사진","프로필 사진"))}</button>
    ...
    <button type="button" class="character-full-image-slot ld ${c.ldImage?"has-image":"is-empty"}" data-image="ldImage">${c.ldImage?`<img src="${imageEsc(c.ldImage)}" alt="${esc(c.name)} LD 일러스트">`:emptyPhotoSlot("LD")}</button>
...
Object.assign(UI_TEXT.en,{"비밀 설정":"Secrets","비밀 설정하기":"Set up secrets"});
Object.assign(UI_TEXT.ja,{"비밀 설정":"秘密の設定","비밀 설정하기":"秘密を設定"});
function nativeCharacterHub(c){
  ...
  const reorderRows=characterRenderOptions.reorder?state.order.map((id,index)=>{const x=state.characters[id];return `<div class="mobile-character-reorder-row">${avatar(x)}<b>${esc(x.name)}</b><span><button type="button" data-sort="${id}" data-direction="-1" ${index===0?"disabled":""} aria-label="위로 이동">↑</button><button type="button" data-sort="${id}" data-direction="1" ${index===state.order.length-1?"disabled":""} aria-label="아래로 이동">↓</button></span></div>`}).join(""):"";
  const selectedIcon=c.icon?`<img class="sprite" src="${imageEsc(c.icon)}" alt="${esc(c.name)}">`:c.photo?`<img class="avatar profile-photo-fallback" src="${imageEsc(c.photo)}" alt="${esc(c.name)}">`:`<img class="sprite character-wallet-default-profile" src="./assets/home-ui/profile-placeholder.png" alt="${esc(t("기본 실루엣","기본 실루엣"))}">`;
  const profilePhoto=c.photo?`<img src="${imageEsc(c.photo)}" alt="${esc(c.name)}">`:`<span>${t("이곳에 사진을 넣어주세요","이곳에 사진을 넣어주세요")}</span>`;
  ...
  const draftRows=[{key:"name",label:"이름",value:c.name},{key:"birthday",label:"생일",value:birthdayLabel},{key:"job",label:"직업 종류",value:isCourtWorld()?courtJobName(c.courtJob,state.uiLanguage):careerCaption(state,c)||unknown},{key:"age",label:"나이",value:c.ageGroup||unknown},{key:"gender",label:"성별",value:c.gender||unknown}];
  ...
  const favoriteSlots=Array.from({length:3},(_,index)=>{const item=favoriteItems[index];return `<span class="character-favorite-object ${item?.image?"has-image":"is-empty"}">${item?.image?`<img src="${imageEsc(item.image)}" alt="${esc(item.name)}">`:`<i>${index===0?"♡":"·"}</i>`}</span>`}).join("");
  const orientationOptions=["설정하지 않음 · 누구에게도 끌리지 않음","여성에게 끌림","남성에게 끌림","여성과 남성에게 끌림","성별과 무관하게 끌림","그외 성별에게 끌림"];
  const quickSettings=characterRenderOptions.quick?`<dialog class="mobile-character-editor-dialog character-quick-settings-dialog" style="--character-accent:${esc(c.theme?.primary||"#176b60")};--character-accent-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")}" data-mobile-character-editor-dialog="quick" data-character-dialog-origin="hub"><div class="mobile-character-editor-shell character-quick-settings-shell"><header class="character-quick-header"><button type="button" class="character-quick-back" data-close-mobile-character-editor aria-label="${esc(t("캐릭터 화면으로 돌아가기","캐릭터 화면으로 돌아가기"))}"><span aria-hidden="true">←</span><b>${t("뒤로가기","뒤로가기")}</b></button><h1>${t("캐릭터 프로필","캐릭터 프로필")}</h1><button type="button" class="character-quick-save" data-save-mobile-character-editor>${t("저장하기","저장하기")}</button></header><div class="character-quick-fields"><div class="character-quick-media"><button type="button" data-image="photo">${t("프로필 사진 추가","Add profile photo","プロフィール写真を追加")}</button><button type="button" data-image="ldImage">${t("LD 사진 추가","Add LD image","LD画像を追加")}</button><button type="button" data-open-character-layout>${t("배치 조정하기","Adjust placement","配置を調整")}</button></div><button type="button" class="character-quick-icon" data-image="icon" aria-label="${esc(t("투명 SD 아이콘","투명 SD 아이콘"))}">${selectedIcon}<small>${t("아이콘","아이콘")}</small></button><label>${t("이름","이름")}<input data-field="name" value="${esc(c.name)}" maxlength="40" autocomplete="off" autocorrect="off" spellcheck="false"></label><div class="character-quick-pair"><label>${t("성별","성별")}<select data-field="gender">${["설정하지 않음","남성","여성","그외"].map(value=>`<option ${value===(c.gender||"설정하지 않음")?"selected":""}>${value}</option>`).join("")}</select></label><label>${t("성지향","성지향")}<select data-field="attractionTarget">${orientationOptions.map(value=>`<option ${value===(c.attractionTarget||orientationOptions[0])?"selected":""}>${value}</option>`).join("")}</select></label></div>${characterLanguageFields(c,state.uiLanguage)}${quickEconomyMarkup(state,c)}<fieldset><legend>${t("성격 키워드 · 최대 4개","Personality keywords · up to 4","性格キーワード・最大4つ")}</legend><div class="character-quick-personality">${PERSONALITY_TYPES.map(value=>`<button type="button" data-personality-type="${value}" class="${(c.personalityTypes||[]).includes(value)?"on":""}" aria-pressed="${(c.personalityTypes||[]).includes(value)}"><i aria-hidden="true">✓</i><span>${value}</span></button>`).join("")}</div></fieldset><label>${t("말투","말투")}<select data-field="speechStyle">${SPEECH_STYLE_OPTIONS.map(value=>`<option ${value===(c.speechStyle||SPEECH_STYLE_OPTIONS[0])?"selected":""}>${value}</option>`).join("")}</select></label><button type="button" data-open-secret-settings>${secretText(state.uiLanguage,"비밀 설정하기","Set up secrets","秘密を設定")}</button></div></div></dialog><dialog class="character-layout-dialog" data-character-layout-dialog><form method="dialog" class="character-layout-dialog-shell"><header><b>${t("배치 조정하기","Adjust placement","配置を調整")}</b><button value="close">×</button></header>${characterHomeLayoutEditor(c)}</form></dialog>`:"";
  const mobileProfile=`<section class="mobile-character-profile-draft"><button type="button" class="character-draft-back" data-tab="observe" aria-label="${esc(t("돌아가기","돌아가기"))}"></button><button type="button" class="character-wallet-selected" data-toggle-character-roster aria-label="${esc(t("선택된 캐릭터 바꾸기","선택된 캐릭터 바꾸기"))}"><span>${selectedIcon}</span><b><em>${t("선택됨","선택됨")}</em></b><small class="character-slot-count" aria-label="${esc(slotText(state,window.ParallelCityAuth?.getInfo?.()).text)}">${slotText(state,window.ParallelCityAuth?.getInfo?.()).used}/${slotText(state,window.ParallelCityAuth?.getInfo?.()).total}</small></button>${characterGroupSelector()}<div class="character-wallet-art" aria-hidden="true"></div><img class="character-setting-clip" src="./assets/character-ui/clip.png" alt=""><section class="character-registration-card"><button type="button" class="character-registration-photo" data-image="photo" aria-label="${esc(t(c.photo?"프로필 사진 변경":"프로필 사진 추가",c.photo?"프로필 사진 변경":"프로필 사진 추가"))}">${profilePhoto}</button><img class="character-registration-paper" src="./assets/character-ui/registration-card.png" alt="" aria-hidden="true"><div class="character-registration-fields"><h2>${t("서랍마을 주민등록증","서랍마을 주민등록증")}</h2><dl>${draftRows.map(({key,label,value})=>`<div class="character-registration-field-${key}"><dt>${t(label,label)}</dt><dd>${esc(value)}</dd></div>`).join("")}</dl></div></section><div class="character-wallet-roster" data-character-roster data-roster-count="${rosterItemCount}" style="--roster-visible:${rosterVisibleRows}" hidden><div class="mobile-character-strip">${mobileStrip}</div><span class="character-roster-actions"><button type="button" class="character-roster-reorder" data-open-character-reorder aria-label="${esc(t("위치 바꾸기","위치 바꾸기"))}">↕</button><button type="button" class="character-roster-close" data-toggle-character-roster aria-label="${esc(t("닫기","닫기"))}">×</button></span></div></section>`;
  const hubActions=`<section class="character-setting-choices" aria-label="${esc(t("캐릭터 설정 방식","캐릭터 설정 방식"))}"><span class="character-setting-cloth" aria-hidden="true"><img src="./assets/character-ui/character-cloth-white.png" alt=""></span><img class="character-setting-book" src="./assets/character-ui/book.png" alt=""><img class="character-setting-tape" src="./assets/character-ui/tape.png" alt=""><img class="character-setting-key" src="./assets/character-ui/key.png" alt=""><span class="character-favorite-preview" aria-label="${esc(t("선호 물품 미리보기","선호 물품 미리보기"))}">${favoriteSlots}</span><button type="button" class="character-setting-choice character-quick-choice" data-open-quick-character-settings><span><b>${t("캐릭터 프로필","캐릭터 프로필")}</b><small>${t("처음이라면 여기부터! 기본만 정하고 바로 시작해요.","Start here! Set the basics and begin playing.","まずはこちら！基本だけ決めて遊び始めましょう。")}</small></span></button><button type="button" class="character-setting-choice character-full-choice" data-open-full-character-settings><span><b>${t("전체설정","전체설정")}</b><small>${t("더 자세히 정하고 싶을 때만 열어 주세요.","Optional: open this for detailed customization.","細かく設定したいときだけ開いてください。")}</small></span></button></section>`;
  ...
  const tabletSummary=`<section class="tablet-character-summary" aria-label="${esc(c.name)} ${esc(t("캐릭터 정보","캐릭터 정보"))}"><div class="tablet-character-summary-paper"><header><span class="tablet-character-summary-avatar">${selectedIcon}</span><span><small>${t("선택한 캐릭터","선택한 캐릭터")}</small><h2>${esc(c.name)}</h2><p>${esc(careerCaption(state,c)||unknown)}</p></span><i class="tablet-character-theme" style="--primary:${esc(c.theme?.primary||"#176b60")};--secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")}" aria-hidden="true"></i></header><dl>${draftRows.map(({label,value})=>`<div><dt>${t(label,label)}</dt><dd>${esc(value)}</dd></div>`).join("")}</dl><div class="tablet-character-setting-actions"><button type="button" data-open-quick-character-settings><b>${t("캐릭터 프로필","캐릭터 프로필")}</b><small>${t("처음이라면 여기부터! 기본만 정하고 바로 시작해요.","Start here! Set the basics and begin playing.","まずはこちら！基本だけ決めて遊び始めましょう。")}</small></button><button type="button" data-open-full-character-settings><b>${t("전체설정","전체설정")}</b><small>${t("더 자세히 정하고 싶을 때만 열어 주세요.","Optional: open this for detailed customization.","細かく設定したいときだけ開いてください。")}</small></button></div><nav><button type="button" data-export-profile>${t("프로필 내보내기","프로필 내보내기")}</button><button type="button" data-save>${t("캐릭터 저장","캐릭터 저장")}</button><button type="button" class="danger" data-delete-character="${c.id}">${t("캐릭터 삭제","캐릭터 삭제")}</button></nav></div></section>`;
  return `<div class="editor character-editor character-editor-hub-only" style="--character-own:${esc(c.theme?.primary||"#176b60")};--character-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")};--character-accent:${esc(c.theme?.primary||"#176b60")};--character-accent-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")}"><aside class="panel desktop-character-list"><div class="title"><h2>${t("캐릭터 목록","캐릭터 목록")}</h2><button data-new ${state.order.length>=limit?"disabled":""}>${slotLabel}</button></div>${list}</aside><section class="panel form character-hub-shell">${tabletSummary}<section class="mobile-character-dashboard" data-character-ui-version="8" style="--character-own:${esc(c.theme?.primary||"#176b60")};--character-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")};--character-accent:${esc(c.theme?.primary||"#176b60")};--character-accent-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")}"><img class="character-wood-background" src="./assets/character-ui/character-wood-background.png" alt="" aria-hidden="true">${mobileProfile}${hubActions}${draftActions}</section>${quickSettings}${characterRenderOptions.reorder?`<dialog class="mobile-character-reorder-dialog" data-mobile-character-reorder-dialog><form method="dialog"><div class="mobile-editor-head"><span><small>CHARACTER ORDER</small><b>${t("캐릭터 위치 바꾸기","캐릭터 위치 바꾸기")}</b></span><button value="close">×</button></div><p>${t("화살표를 눌러 홈과 캐릭터 목록의 순서를 바꿔요.","화살표를 눌러 홈과 캐릭터 목록의 순서를 바꿔요.")}</p><div>${reorderRows}</div><button class="primary" value="close">${t("완료","완료")}</button></form></dialog>`:""}</section></div>`;
}
function character(){

  const c=active();
  // Only a landscape tablet uses the iPad spread. Portrait tablets deliberately
  // keep the exact phone hub and single-page book instead of entering desktop UI.
  const nativeApp=document.documentElement.classList.contains("native-app");
  const nativeTabletLandscape=nativeApp&&Boolean(window.matchMedia?.("(min-width:721px) and (orientation:landscape)")?.matches);
  const nativeTabletMode=nativeTabletLandscape;
  ...
  const personalityDetails=fixedBookMode?"": [
    ...
    personalityChoice(c,"도덕성","morality",DISCOVERY_AXES.morality.values,"낮을수록 자기 이익을 위해 선을 넘고, 높을수록 손해를 보더라도 양심과 원칙을 지켜요."),
    ...
    personalityChoice(c,"통제욕","interference",["방관자","요청할 때만 도움","적당히 관여","챙기고 확인함","강하게 간섭함","컨트롤프릭"],"방관자는 웬만한 일에 끼어들지 않고, 컨트롤프릭은 상대의 일정과 행동까지 통제하려 해 갈등 가능성이 커져요."),
  ...
  const overviewBasic=`<section class="character-overview-basic" aria-label="${esc(t("개요 기본 설정","개요 기본 설정"))}">
    ...
    <div class="overview-field overview-pronouns">${characterLanguageFields(c,state.uiLanguage)}</div>
    ${careerSettingsLink(c,true)}
    ...
    <div class="overview-field overview-wealth"><b>${t('재산 유형','Wealth','資産の種類')}</b><button type="button" data-character-money-settings="${esc(c.id)}">${esc(moneyProfileLabel(c.wealth||'평범한 형편'))}</button></div>
    <div class="overview-field overview-spending"><b>${t('소비 유형','Spending','消費傾向')}</b><button type="button" data-character-money-settings="${esc(c.id)}">${esc(moneyProfileLabel(c.income||'필요한 만큼 소비'))}</button></div>
  ...
  const overviewLife=`<section class="character-overview-basic character-overview-life" aria-label="${esc(t("개요 생활 설정","개요 생활 설정"))}">
    ...
    <label class="overview-field overview-sleep-habit"><b>${t("취침 습관","취침 습관")}</b>${overviewSelect("sleepHabit",["이불을 단정히 덮고 잠","이불을 걷어차며 잠","옆으로 웅크려 잠","팔다리를 뻗고 잠","베개를 끌어안고 잠","잠꼬대를 자주 함","뒤척임이 많음","아주 얌전히 잠","새벽에 자주 깸","코를 골며 깊이 잠","잠을 전혀 이루지 못함"],c.sleepHabit||"이불을 단정히 덮고 잠")}</label>
  ...
  const overviewPortrait=c.icon?`<img src="${imageEsc(c.icon)}" alt="${esc(c.name)} 아이콘 미리보기">`:profileAvatar(c);
  ...
  const profileOverviewBasicPane=()=>{const html=profileOverviewMarkup("basic");return state.characterProfileBook?html.replace(/<button[^>]*data-character-overview-pane="(?:career|life|court)"[^>]*>[\s\S]*?<\/button>/g,'') : html;};
  const profileOverviewLifePane=()=>profileOverviewMarkup("life");
  const profileOverviewPane=()=>overviewPane==="career"?profileOverviewCareerPane():overviewPane==="court"?courtCharacterPage(c):overviewPane==="basic"?profileOverviewBasicPane():profileOverviewLifePane();
  ...
  const bodyFigureImage=bodyFigureSource?`<img src="${imageEsc(bodyFigureSource)}" alt="${esc(c.name)} ${esc(t("신체 일러스트","신체 일러스트"))}">`:`<span>${t("사진 추가하기","사진 추가하기")}</span>`;
  ...
  const bodyFigurePane=()=>`<section class="character-body-svg-page character-body-figure-page" aria-label="${esc(c.name)} ${esc(t("신체 설정","신체 설정"))}">
    ...
    <nav class="character-book-page-controls body-controls" aria-label="${esc(t("신체 페이지 이동","신체 페이지 이동"))}"><button type="button" data-character-pane="profile" data-character-overview-target="life" aria-label="${esc(t("이전 페이지","이전 페이지"))}">◀</button><b>${5+(isCourtWorld()?1:0)}</b><button type="button" data-character-body-pane="appearance" aria-label="${esc(t("다음 페이지","다음 페이지"))}">▶</button></nav>
  ...
  const bodyAppearancePane=()=>`<section class="character-body-svg-page character-body-appearance-page" aria-label="${esc(c.name)} ${esc(t("신체 설정","신체 설정"))}">
    ...
    <nav class="character-book-page-controls body-controls" aria-label="${esc(t("신체 페이지 이동","신체 페이지 이동"))}"><button type="button" data-character-body-pane="figure" aria-label="${esc(t("이전 페이지","이전 페이지"))}">◀</button><b>${6+(isCourtWorld()?1:0)}</b><button type="button" data-character-body-pane="accessibility" aria-label="${esc(t("다음 페이지","다음 페이지"))}">▶</button></nav>
    ${bodyChoiceDialog}
  </section>`;
  const healthRecordCollection=()=>{
    const records=c.bodyProfile?.healthRecords||[];
    return `<div class="body-repeat-list">${records.map((r,i)=>`<button type="button" data-health-record="${i}">${esc(r.name||t('건강 기록','건강 기록'))}</button>`).join('')||`<span>${t('없음','없음')}</span>`}</div><div class="body-repeat-actions"><button type="button" data-health-record-add ${records.length>=30?"disabled":""}>＋ ${t('추가','추가')}</button></div>`;
  };
  const bodyAccessibilityPane=()=>`<section class="character-body-svg-page character-body-accessibility-page" aria-label="${esc(c.name)} ${esc(t("건강·접근성 설정","건강·접근성 설정"))}">
    <div class="body-accessibility-scroll">
    ...
    <section class="body-accessibility-choice body-health-conditions"><b>${t("만성질환·건강 관리","만성질환·건강 관리")}</b>${bodyInlineChoice("healthConditions",HEALTH_CONDITIONS,c.bodyProfile?.healthConditions||[])}<div class="health-record-collection"><b>${t("병명·건강 기록","병명·건강 기록")}</b>${healthRecordCollection()}</div></section>
    ...
    ${bodyChoiceDialog}
    <nav class="character-book-page-controls body-controls" aria-label="${esc(t("신체 페이지 이동","신체 페이지 이동"))}"><button type="button" data-character-body-pane="appearance" aria-label="${esc(t("이전 페이지","이전 페이지"))}">◀</button><b>${7+(isCourtWorld()?1:0)}</b><button type="button" data-character-pane="secrets" aria-label="${esc(t("다음 페이지","다음 페이지"))}">▶</button></nav>
  </section>`;
  ...
  const bookPageControls=(page,previous,next)=>`<nav class="character-book-page-controls book-section-controls" aria-label="${esc(t("페이지 이동","페이지 이동"))}"><button type="button" ${previous} aria-label="${esc(t("이전 페이지","이전 페이지"))}">◀</button><b>${page+1+(isCourtWorld()?1:0)}</b><button type="button" ${next} aria-label="${esc(t("다음 페이지","다음 페이지"))}">▶</button></nav>`;
  ...
  const wardrobePane=()=>`<section class="character-book-form-page wardrobe-book-page">
    <div class="book-form-grid">
      ...
      ${bookListButton("액세서리 착용","accessories",c.accessories||[])}
      ...
      ${bookField("옷을 고르는 기준","clothingPriority",["가격","편안함","실용성","상황에 맞춤","디자인","브랜드","유행","단벌신사"],c.clothingPriority||"가격")}
    ...
    </div>${bookPageControls(7,'data-character-pane="secrets"','data-character-pane="personality" data-character-personality-pane="core"')}
  </section>`;
  ...
  const personalityEmotionPane=()=>`<section class="character-book-form-page personality-emotion-book-page"><div class="personality-emotion-content"><div class="personality-emotion-heading"><small>EMOTIONAL TEMPERAMENT</small><h2>${t("기분과 정서 성향","기분과 정서 성향")}</h2><p>${t("현재 기분을 고정하는 설정이 아니라, 같은 일을 겪어도 이 캐릭터답게 받아들이고 회복하도록 만드는 기준이에요.","현재 기분을 고정하는 설정이 아니라, 같은 일을 겪어도 이 캐릭터답게 받아들이고 회복하도록 만드는 기준이에요.")}</p></div><div class="book-form-grid personality-emotion-grid">${personalityEmotionFields.map(([label,field,options])=>bookField(label,field,options,c[field]||(field==="morality"?"상황에 따라 판단함":options[0]))).join("")}${bookField("감정 표현의 크기","emotionalExpression",["표정 변화가 거의 없음","감정을 잘 드러내지 않음","상황에 따라 표현함","표현이 풍부함","감정이 바로 드러남"],c.emotionalExpression||"상황에 따라 표현함")}${bookField("신체 접촉 반응","touchReaction",["상황에 따라 자연스럽게 받아들임", "촉각에 둔감한 편", "간지럼을 잘 탐", "갑작스러운 접촉에 쉽게 놀람", "친한 사람 외의 기척에는 경계함", "몸에 손이 닿는 것을 극도로 꺼림", "몸에 손이 닿는 것을 싫어함", "허락 없는 접촉은 불편함", "가까운 사람에게만 허용함", "신체 접촉을 좋아함", "먼저 다가가는 편"],c.touchReaction||"상황에 따라 자연스럽게 받아들임")}${bookField("충동을 참는 정도","impulseControl",["매우 잘 참음","대체로 참음","가끔 욱하지만 멈춤","쉽게 욱함","거의 참지 않음"],c.impulseControl||"가끔 욱하지만 멈춤")}</div>${cognitiveAccessDialog(c)}</div>${bookPageControls(9,'data-character-personality-pane="core"','data-character-personality-pane="abilities"')}</section>`;
  // 이전 버전에서 저장된 details 상태도 합쳐진 9페이지로 자연스럽게 연다.
  const abilityText=(ko,en,ja)=>({ko,en,ja}[state.uiLanguage]||ko);
  const abilityFields=[['deceptionSkill','기만','Deception','欺瞞'],['intuitionSkill','직감','Intuition','直感'],['stealthSkill','은신','Stealth','隠密'],['intelligenceSkill','지능','Reasoning','知力'],['observationSkill','관찰','Observation','観察'],['composureSkill','침착','Composure','冷静さ']];
  const personalityAbilitiesPane=()=>`<section class="character-book-form-page personality-emotion-book-page"><div class="personality-emotion-content"><div class="personality-emotion-heading"><small>GAME ABILITIES</small><h2>${abilityText('놀이 능력','Game abilities','遊びの能力')}</h2><p>${abilityText('마피아 놀이의 관찰과 직감에 반영돼요. 높은 능력도 정답을 보장하지 않아요.','Used for observations and impressions in Mafia. High abilities never guarantee a correct answer.','マフィアの観察や直感に反映されます。高い能力でも正解は保証されません。')}</p></div><div class="book-form-grid">${abilityFields.map(([key,...labels])=>`<label class="book-form-field"><b>${abilityText(...labels)}</b><input type="number" min="0" max="100" step="5" data-field="${key}" value="${Number(c[key]??50)}"></label>`).join('')}</div></div>${bookPageControls(10,'data-character-personality-pane="emotion"','data-character-pane="taste" data-character-taste-pane="categories"')}</section>`;
  const personalityBookPane=()=>state.characterPersonalityPane==="abilities"?personalityAbilitiesPane():["details","emotion"].includes(state.characterPersonalityPane)?personalityEmotionPane():personalityCorePane();
  ...
  const tasteBookPane=()=>`<section class="character-book-form-page taste-book-page taste-menu-page"><div class="taste-menu-grid">${tasteMenuButton("관심사 선택",'data-open-book-list="interests"',bookListSummary(c.interests||[],"관심사"))}${tasteMenuButton("취미 선택",'data-open-book-list="hobbies"',bookListSummary(c.hobbies||[],"취미"))}${tasteMenuButton("기술 숙련 선택",'data-open-book-list="skills"',bookListSummary(c.skills||[],"기술 숙련"))}${tasteMenuButton("좋아하는 것 선택",'data-open-taste-group="favorites"')}${tasteMenuButton("싫어하는 것 선택",'data-open-taste-group="dislikes"')}${tasteMenuButton("소지품 선택",'data-open-taste-group="inventory"')}${tasteMenuButton(({ko:"욕구·관계·금지행동",en:"Needs, relationships and restrictions",ja:"欲求・関係・禁止行動"})[state.uiLanguage]||"금지행동 설정",`data-activity-settings="${esc(c.id)}" data-group-id="${esc(state.sharedContext?.groupId||'')}"`)}</div>${bookPageControls(11,'data-character-personality-pane="abilities" data-character-pane="personality"','data-character-pane="closet"')}</section>`;
  const closetOwned=new Set(c.inventory?.fashion||[]),closetItems=(state.catalog?.fashion||[]).filter(item=>closetOwned.has(item.id));
  const closetBookPane=()=>`<section class="character-book-form-page closet-book-page"><div class="closet-book-toolbar"><label><span class="sr-only">${t("검색","검색")}</span><input type="search" data-closet-search placeholder="${t("사진 검색","사진 검색")}"></label><nav><button type="button" class="on" data-closet-filter="all">${t("전체","전체")}</button><button type="button" data-closet-filter="uniform">${t("유니폼","유니폼")}</button><button type="button" data-closet-filter="formal">${t("격식","격식")}</button></nav></div><div class="closet-book-frame"><div class="closet-book-paper"><div class="closet-book-count"><b>${t("총","총")} ${closetItems.length} / 30</b><small>${t("상황과 드레스코드에 맞춰 자동 선택","상황과 드레스코드에 맞춰 자동 선택")}</small></div><div class="closet-book-grid">${closetItems.map(item=>`<button type="button" data-edit-clothing="${item.id}" data-closet-card data-closet-search-text="${esc(`${item.name} ${(item.occasionTags||[]).join(" ")} ${(item.flairs||[]).join(" ")}`.toLowerCase())}" data-uniform="${item.requiredUniform?"true":"false"}" data-formal="${/격식|정장/.test(item.formality||"")?"true":"false"}"><span class="closet-book-art">${item.iconImage||item.image?`<img src="${imageEsc(item.iconImage||item.image)}" alt="">`:`<i>👕</i>`}</span><b>${esc(item.name)}</b><small>${esc([item.warmth,item.formality,item.comfort].filter(Boolean).join(" · "))}</small></button>`).join("")}<button type="button" class="closet-book-add" ${closetItems.length>=30?"disabled":""} title="${t("추가 사진은 캐릭터마다 30개까지 등록할 수 있어요.","추가 사진은 캐릭터마다 30개까지 등록할 수 있어요.")}" data-new-clothing><span>＋</span><b>${t("사진 추가","사진 추가")}</b></button><p class="closet-book-no-results" hidden>${t("검색 결과가 없어요.","검색 결과가 없어요.")}</p></div></div></div>${bookPageControls(12,'data-character-pane="taste"','disabled')}</section>`;
  ...
  const tasteDialogs=`${tasteCategoryDialog("favorites","좋아하는 것 선택",favoriteTasteButtons)}${tasteCategoryDialog("dislikes","싫어하는 것 선택",dislikedTasteButtons)}${tasteCategoryDialog("inventory","소지품 선택",coffeeInventoryMarkup(c,state.uiLanguage)+catalogTasteKinds.map(([label,kind])=>catalogSelectionButton(label.replace("좋아하는","보유한"),kind,"inventory")).join(""))}`;
  const bookListSources={
    ...
    accessories:{groups:{"액세서리 착용":["스터드 귀고리", "링 귀고리", "드롭 귀고리", "이어커프", "귓바퀴 피어싱", "코 피어싱", "입술 피어싱", "목걸이", "펜던트", "초커", "반지", "여러 손가락 반지", "팔찌", "뱅글", "발찌", "손목시계", "브로치", "배지", "머리핀", "머리띠", "비녀", "리본", "안경", "선글라스", "체인 장식"]}},
    favoriteFashionStyles:{groups:WARDROBE_TAG_GROUPS},
    interests:{groups:{"관심사":INTERESTS}},hobbies:{groups:{"취미":HOBBIES}},favoriteStoryGenres:{groups:{"장르":storyGenres}},skills:{groups:{"기술":["요리","악기","그림","글쓰기","운동",...MARTIAL_ARTS,"춤","연기","공예","프로그래밍","외국어","정비","의료","교육","심리학","정신건강 지식"]}},foodPreferences:{groups:{"음식":FOOD_PREFERENCES}},drinks:{groups:{"음료":DRINKS}},musicGenres:{groups:{"음악":MUSIC}},favoriteVideoGenres:{groups:{"영상":videoFormats}},favoriteGameGenres:{groups:{"게임":gameGenres}},favoriteScentNotes:{groups:{"향":PERFUME_NOTES}},favoriteAnimals:{groups:{"동물":["개","고양이","새","토끼","파충류","어류","말","야생동물"]}},favoriteElectronics:{groups:{"전자기기":["스마트폰","컴퓨터","게임기","카메라","오디오","스마트홈","웨어러블"]}},favoriteWeapons:{groups:{"무기":["검","활","총기","창","도끼","둔기","마법 도구"]}},favoriteBooks:{groups:{"책":storyGenres}},
  ...
  const fullActivePane=["visual","profile","body","wardrobe","personality","taste","secrets","closet"].includes(state.characterPane)?state.characterPane:"visual";
  const fullPane=fullActivePane==="visual"?characterFullOverview(c):fullActivePane==="profile"?profileOverviewPane():fullActivePane==="body"?bodyPane():fullActivePane==="wardrobe"?wardrobePane():fullActivePane==="personality"?personalityBookPane():fullActivePane==="secrets"?secretsPane(state,c):fullActivePane==="closet"?closetBookPane():tasteBookPane();
  ...
  const mobileProfileDraft=`<section class="mobile-character-profile-draft">
    ...
    ${characterGroupSelector()}<div class="character-wallet-art" aria-hidden="true"></div>
  ...
  const fullPaneMeta=(profileBook?[["visual","사진·색상·배치"],["profile","개요"]]:[["visual","사진·색상·배치"],["profile","개요"],["body","신체"],["secrets",secretText(state.uiLanguage,"비밀 설정","Secrets","秘密の設定")],["wardrobe","의상 취향"],["personality","성격"],["taste","취향·소지품"],["closet","추가 LD 사진"]]);
  ...
  const currentFullPageKey=fullActivePane==="profile"?`overview-${overviewPane}`:fullActivePane==="body"?`body-${state.characterBodyPane}`:fullActivePane==="personality"?`personality-${state.characterPersonalityPane==="abilities"?"abilities":["details","emotion"].includes(state.characterPersonalityPane)?"details":"core"}`:fullActivePane;
  const fullPageEntries=[
    ...
    {key:"body-appearance",pane:"body",get html(){return bodyAppearancePane()}},
    {key:"body-accessibility",pane:"body",get html(){return bodyAccessibilityPane()}},
    {key:"secrets",pane:"secrets",get html(){return secretsPane(state,c)}},
    {key:"wardrobe",pane:"wardrobe",get html(){return wardrobePane()}},
    {key:"personality-core",pane:"personality",get html(){return personalityCorePane()}},
    {key:"personality-details",pane:"personality",get html(){return personalityEmotionPane()}},
    {key:"personality-abilities",pane:"personality",get html(){return personalityAbilitiesPane()}},
  ...
  const fullSpreadContent=nativeTabletLandscape?fullSpreadPages.map((page,index)=>`<section class="character-book-spread-leaf" data-spread-side="${index===0?"left":"right"}" data-spread-page="${page.key}"><main class="character-book-v8-page" data-book-page="${page.pane}">${page.html}</main></section>`).join(""):"";
  const fullSpreadDialogs=nativeTabletLandscape?`${fullSpreadPages.some(page=>page.key==="visual")?layoutDialogMarkup:""}${bookListDialog()}${bookCatalogDialog}${tasteDialogs}`:"";
  ...
  return `<div class="editor character-editor" style="--character-own:${esc(c.theme?.primary||"#176b60")};--character-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")};--character-accent:${esc(c.theme?.primary||"#176b60")};--character-accent-secondary:${esc(c.theme?.secondary||c.theme?.primary||"#176b60")}">
    ...
    <section class="panel form">
      ...
      ${tabletFullBook}
      ${quickSettings}
      <nav class="character-tablet-settings-entry" aria-label="${esc(t("캐릭터 설정 방식","캐릭터 설정 방식"))}"><button type="button" data-open-quick-character-settings>${t("캐릭터 프로필","캐릭터 프로필")}</button><button type="button" data-open-full-character-settings>${t("전체설정","전체설정")}</button></nav>
...
function mailbox(){return renderMailbox()}
function mailboxLegacy(){
...
function wardrobe(){
  ...
  const itemCard=item=>`<article class="closet-item-card" data-edit-clothing="${item.id}">${item.image?`<img src="${imageEsc(item.image)}" alt="">`:`<span>👕</span>`}<div><b>${esc(item.name)}</b><small>${esc([item.category,item.ordinary,...(item.occasionTags||[]),...(item.colors||[])].filter(Boolean).join(" · "))}</small></div><button data-edit-clothing="${item.id}">편집</button></article>`;
  const outfitCard=outfit=>`<article class="saved-outfit-card"><div class="outfit-collage ${esc(outfit.layout||"cluster-1")}">${outfit.itemIds.map(id=>items.find(item=>item.id===id)).filter(Boolean).map(item=>item.image?`<img src="${imageEsc(item.image)}" alt="">`:`<span>👕</span>`).join("")}</div><div><b>${esc(outfit.name)}</b><small>${esc((outfit.tags||[]).join(" · ")||"일상 코디")}</small></div><button data-edit-outfit="${outfit.id}">코디 편집</button></article>`;
...
export function catalogCardMarkup(kind,item,{editor=false}={}){
  ...
  if(!editor)return `<button type="button" class="catalog-tile" data-open-catalog="${esc(item.id)}" data-kind="${esc(kind)}" aria-label="${esc(item.name)}"><span class="catalog-tile-art">${item.image?`<img loading="lazy" decoding="async" src="${imageEsc(item.image)}" alt="">`:`<span>${CATALOG_ICONS[kind]||"📦"}</span>`}</span><b>${esc(item.name)}</b></button>`;
...
const characterViewEditor=()=>{
  ...
  const field=(sourceId,targetId,key,label,help)=>{
    ...
    if(!fieldViews.has(pairKey))fieldViews.set(pairKey,characterViewFor(sourceId,targetId));
    const effective=fieldViews.get(pairKey);
  ...
  return `<section class="character-view-editor relationship-stage relationship-motion-${relationshipMotion}" style="--relationship-own:${esc(sourceColor)};--relationship-own-secondary:${esc(source.theme?.secondary||sourceColor)};--relationship-target:${esc(targetColor)};--relationship-left:${esc(heroLeft.theme?.primary||sourceColor)};--relationship-right:${esc(heroRight.theme?.primary||targetColor)}">
    ...
    <dialog class="character-view-dialog relationship-fullscreen-dialog relationship-viewpoint-dialog" data-view-dialog="${sourceId}:${targetId}"><form method="dialog"><header class="relationship-editor-head"><button value="close" class="relationship-back-button" aria-label="${copy.back}"><img src="./assets/home-ui/back.png" alt=""></button><div class="relationship-editor-pair"><figure>${avatar(source)}<b>${esc(source.name)}</b></figure><i aria-hidden="true"></i><figure>${avatar(target)}<b>${esc(target.name)}</b></figure></div></header><div class="character-view-fields relationship-all-fields"><div class="view-copy-actions"><button type="button" data-copy-view>${({ko:"시선 복사",en:"Copy viewpoint",ja:"視線をコピー"}[state.uiLanguage]||"시선 복사")}</button><button type="button" data-paste-view>${({ko:"시선 붙여넣기",en:"Paste viewpoint",ja:"視線を貼り付け"}[state.uiLanguage]||"시선 붙여넣기")}</button></div>${viewFields}</div></form></dialog>
  </section>`;
};
...
const relationshipAnimationOrder=(relation,requestedIds=[],seed="")=>{
  ...
  return orderAnimationCharacters(official,state.characters,state.relationships,`${relation.id}:${seed}`,Array.isArray(relation.displayOrder)&&relation.displayOrder.length>0);
};
...
function relationship(sharedPass=false){
  const shared=window.DrawerVillageGroups?.getSnapshot?.();if(!sharedPass&&shared?.activeGroupId&&shared.group)return withSharedWorld(shared,()=>relationship(true));
...
function routine(sharedPass=false){
 const shared=window.DrawerVillageGroups?.getSnapshot?.();if(!sharedPass&&shared?.activeGroupId&&shared.group)return withSharedWorld(shared,()=>routine(true));
...
const ownerNameSettings=()=>`<section class="setting-card owner-name-card"><h2>${t('사용자 닉네임','사용자 닉네임')}</h2><p>${esc(state.ownerName||'')}</p>${state.ownerPhoto?`<img src="${imageEsc(state.ownerPhoto)}" alt="" style="width:64px;height:64px;object-fit:cover;border-radius:50%">`:''}<button type="button" data-user-profile>${({ko:'유저 이름·프로필 사진 설정',en:'Edit name and profile photo',ja:'ユーザー名・プロフィール写真'}[state.uiLanguage]||'유저 이름·프로필 사진 설정')}</button></section>`;
...
Object.assign(UI_TEXT.en,{
  ...
  "도움말":"Help","페이지 안내, 초기화":"Page guides and reset",
...
function settingsContent(){
  const soundState=audioSettings(state);
  const colorMode="";
  const musicText=(ko,en,ja)=>({ko,en,ja}[state.uiLanguage]||ko);
  const sound=`<section class="setting-card sound-setting-card"><h2>${musicText('배경음악','Background music','背景音楽')}</h2><p>${musicText('서랍마을 메인 테마','Drawer Village Main Theme','引き出し村 メインテーマ')}</p><label class="notification-update-option"><input type="checkbox" data-music-muted ${soundState.backgroundMusicMuted?'checked':''}>${musicText('배경음악 끄기','Mute background music','背景音楽をオフ')}</label><label class="sound-volume-control">${musicText('음악 크기','Music volume','音楽の音量')} <output>${soundState.backgroundMusicVolume??35}%</output><input data-music-volume type="range" min="0" max="100" step="1" value="${soundState.backgroundMusicVolume??35}" ${soundState.backgroundMusicMuted?'disabled':''}></label></section><section class="setting-card sound-setting-card"><h2>${t("이동과 생활 효과음","이동과 생활 효과음")}</h2><p>${t("캐릭터가 걷거나 뛰는 동안 들리는 효과음을 조절해요.","캐릭터가 걷거나 뛰는 동안 들리는 효과음을 조절해요.")}</p><label class="notification-update-option"><input type="checkbox" data-sound-muted ${soundState.soundMuted?"checked":""}><span><b>${t("모든 효과음 음소거","모든 효과음 음소거")}</b><small>${t("앱의 이동·생활 효과음을 한 번에 끕니다.","앱의 이동·생활 효과음을 한 번에 끕니다.")}</small></span></label><label class="sound-volume-control">${t("효과음 크기","효과음 크기")} <output>${Math.round(Number(soundState.soundEffectsVolume)||0)}%</output><input type="range" min="0" max="100" step="5" value="${Number(soundState.soundEffectsVolume)||0}" data-sound-volume ${soundState.soundMuted?"disabled":""}></label><div class="sound-preview-actions"><button type="button" data-sound-preview="walk" ${soundState.soundMuted?"disabled":""}>${t("걷기 구두소리 듣기","걷기 구두소리 듣기")}</button><button type="button" data-sound-preview="run" ${soundState.soundMuted?"disabled":""}>${t("달리기 구두소리 듣기","달리기 구두소리 듣기")}</button></div>${soundState.soundMuted?`<small>${t("음소거를 해제하면 미리 들을 수 있어요.","음소거를 해제하면 미리 들을 수 있어요.")}</small>`:""}</section>`;
  ...
  const motionDisplay=`<section class="setting-card"><h2>${motionCopy[0]}</h2><select data-setting="animationIntensity" aria-label="${motionCopy[0]}">${["normal","reduced","off"].map((v,i)=>`<option value="${v}" ${(state.animationIntensity||"normal")===v?"selected":""}>${motionCopy[i+1]}</option>`).join("")}</select><p>${motionCopy[4]}</p></section>`;
  ...
  const sync=`<section class="sync-panel setting-card"><h2>저장과 동기화</h2>${ownerNameSettings()}<p data-account-identity>${esc(accountIdentity(window.ParallelCityAuth?.getInfo?.().user,state.uiLanguage))}</p><p id="account-status">${esc(accountText)}</p><div class="sync-actions"><button class="primary" data-auth>${window.ParallelCityAuth?.getInfo?.().user?({ko:"로그아웃",en:"Sign out",ja:"ログアウト"}[state.uiLanguage]):(iosAppleAvailable()?({ko:"로그인",en:"Sign in",ja:"ログイン"}[state.uiLanguage]):({ko:"Google 로그인",en:"Sign in with Google",ja:"Googleでログイン"}[state.uiLanguage]))}</button>${iosAppleAvailable()&&window.ParallelCityAuth?.getInfo?.().user?`<button type="button" data-link-apple>${appleCopy().link}</button>`:""}<button data-sync-upload>동기화</button><button data-sync-download>불러오기</button></div><small>${({ko:"캐릭터 정보와 사진을 로그인한 계정에 동기화합니다.",en:"Sync character data and photos with your signed-in account.",ja:"キャラクター情報と写真をログイン中のアカウントに同期します。"}[state.uiLanguage])}</small><small>LD는 자르지 않고 원본 비율을 유지하며, 큰 사진은 저장용 사본만 비율대로 축소해요.</small></section>`;
  const danger=`<section class="setting-card"><div class="account-deletion-links"><h3>${settingsLabels(state.uiLanguage).danger}</h3><div class="account-reset-control"><button type="button" class="danger" data-reset>${({ko:"계정 초기화",en:"Reset account data",ja:"アカウントデータを初期化"})[state.uiLanguage]}</button><small>${({ko:"현재 계정의 이 기기 저장 데이터를 초기화합니다. 로그인 계정 자체를 삭제하지 않습니다.",en:"Reset this account’s data on this device. This does not delete the login account.",ja:"このアカウントのこの端末の保存データを初期化します。ログインアカウント自体は削除しません。"})[state.uiLanguage]}</small></div><button type="button" data-delete-account-images>${({ko:"내 이미지 전체 삭제",en:"Delete all my images",ja:"自分の画像をすべて削除"})[state.uiLanguage]||"내 이미지 전체 삭제"}</button><button type="button" data-delete-own-account>${({ko:"계정 삭제",en:"Delete account",ja:"アカウント削除"})[state.uiLanguage]||"계정 삭제"}</button><a href="./privacy.html#account-deletion" target="_blank" rel="noopener">계정·클라우드 데이터 삭제 안내</a><a href="mailto:kkyaareuk@gmail.com?subject=%EC%84%9C%EB%9E%8D%EB%A7%88%EC%9D%84%20%EA%B3%84%EC%A0%95%20%EB%B0%8F%20%EB%8D%B0%EC%9D%B4%ED%84%B0%20%EC%82%AD%EC%A0%9C%20%EC%9A%94%EC%B2%AD">삭제 요청 메일 보내기</a></div></section>`;
  const map=`<section class="setting-card map-display-card"><h2>마을 지도 표시</h2><label>${t("내 마을 묶음 이름","내 마을 묶음 이름")}<input data-setting="personalTownLabel" maxlength="24" value="${esc(state.personalTownLabel||"내 마을")}"><small>${t("멀티에 들어가지 않았을 때 상단에 표시되는 이름이에요.","멀티에 들어가지 않았을 때 상단에 표시되는 이름이에요.")}</small></label><label>건물 표기 방식<select data-setting="buildingLabelMode"><option value="full" ${state.buildingLabelMode==="full"?"selected":""}>이름과 건물 유형 표시</option><option value="name" ${state.buildingLabelMode==="name"?"selected":""}>이름만 표시</option><option value="none" ${state.buildingLabelMode==="none"?"selected":""}>아무 글자도 표시하지 않기</option></select></label><label>지도 위 캐릭터 표기<select data-setting="mapCharacterLabelMode"><option value="none" ${state.mapCharacterLabelMode==="none"?"selected":""}>캐릭터 아이콘만 표시</option><option value="name" ${state.mapCharacterLabelMode==="name"?"selected":""}>아이콘 아래 이름 표시</option></select></label><small>같은 건물에 있는 캐릭터는 지도에서 한 묶음으로 표시됩니다.</small></section>`;
  ...
  const backup=`<section class="setting-card backup-file-card"><h2>${({ko:'전체 설정 백업',en:'Full settings backup',ja:'全設定のバックアップ'}[state.uiLanguage]||'전체 설정 백업')}</h2><p>${({ko:'모든 개인 마을·캐릭터 설정·관계·사전을 한 파일에 담아요. 기기에만 저장된 이미지 파일과 구매한 슬롯 권리는 포함하지 않아요. 불러올 때 기존 데이터와 병합해요.',en:'One file contains all personal towns, character settings, relationships and catalog entries. Device-only images and purchased slot rights are excluded. Import merges with existing data.',ja:'個人の村・キャラクター設定・関係・事典を1つのファイルにまとめます。端末内のみの画像と購入枠の権利は含みません。読み込み時は既存データと統合します。'}[state.uiLanguage]||'')}</p><div class="sync-actions"><button data-settings-transfer="character-code-import">${t("캐릭터 코드로 불러오기","캐릭터 코드로 불러오기")}</button><button data-export-file>백업 파일 내보내기</button><button data-import-file>백업 파일 불러오기</button></div></section>`;
  const feedback=`<section class="setting-card feedback-card"></section>`;
  const guide=`<section class="setting-card page-guide-card"><h2>페이지 안내</h2><p>각 페이지를 처음 열었을 때 나오는 안내를 다시 볼 수 있어요.</p><button type="button" data-intro-tour>${state.uiLanguage==="en"?"Start the walkthrough":state.uiLanguage==="ja"?"操作ガイドを始める":"따라하며 배우기"}</button><button data-guide-reset>모든 페이지 안내 다시 보기</button></section>`;
  const noticeSettings=state.characterNotificationSettings||{},noticeAvailable=notificationCharacters(state,window.DrawerVillageGroups?.getSnapshot?.(),window.ParallelCityAuth?.getInfo?.()?.user?.uid),noticeIds=selectedNotificationIds(noticeSettings,noticeAvailable);
  const noticeCharacters=(settingsPane==="notifications"?notificationCharacters(state,window.DrawerVillageGroups?.getSnapshot?.(),window.ParallelCityAuth?.getInfo?.()?.user?.uid):[]).map(character=>{const id=character.id,image=character?.icon||character?.photo||"";return `<label class="notification-character-option ${noticeIds.has(id)?"on":""}"><input type="checkbox" data-character-notification-character="${esc(id)}" ${noticeIds.has(id)?"checked":""}>${image?`<img src="${imageEsc(image)}" alt="">`:`<span>${esc(character?.name?.slice(0,1)||"새")}</span>`}<div><b>${esc(character?.name||"캐릭터")}</b><small>${esc(character.notificationAffiliation||"")}</small></div></label>`}).join("")||"<p>알림을 받을 캐릭터를 먼저 만들어 주세요.</p>";
  ...
  const achievementList=settingsPane==="achievements"?achievementRows(state,state.uiLanguage):[],achievementUnlocked=achievementList.filter(item=>item.unlocked).length;
  ...
  const menu=settingsMenu(state.uiLanguage);
  const relationCopy=({ko:['관계 변화','점수·관계 모두 고정','점수만 변화 · 관계 고정','점수 변화 · 관계는 우편으로 결정','캐릭터들의 자동 관계 변화를 함께 설정해요. 직접 편집은 가능해요.'],en:['Relationship changes','Freeze scores and relationships','Change scores · freeze relationships','Change scores · decide relationships by letter','Control automatic relationship changes together. Manual editing remains available.'],ja:['関係の変化','スコア・関係をすべて固定','スコアのみ変化・関係は固定','スコアは変化・関係は手紙で決定','自動的な関係の変化をまとめて設定します。直接編集は可能です。']}[state.uiLanguage]||[]);
  const relationShared=window.DrawerVillageGroups?.getSnapshot?.(),relationGroup=relationShared?.activeGroupId?relationShared.group:null,relationUid=window.ParallelCityAuth?.getInfo?.()?.user?.uid,relationCanEdit=!relationGroup||relationGroup.ownerUid===relationUid||['owner','manager'].includes(relationShared.members?.find(m=>m.uid===relationUid)?.role);
  const relationMode=(relationGroup?relationGroup.rules?.relationshipChangeMode||'dynamic':state.relationshipChangeMode)||(['fixed','score'].find(mode=>Object.values(state.characters).some(c=>c.relationshipChangeMode===mode))||'dynamic');
  const relationshipSettings=`<section class="setting-card"><h2>${relationCopy[0]}</h2><label>${relationCopy[0]}<select data-setting="relationshipChangeMode" ${relationCanEdit?"":"disabled"}>${['fixed','score','dynamic'].map((mode,i)=>`<option value="${mode}" ${relationMode===mode?'selected':''}>${relationCopy[i+1]}</option>`).join('')}</select></label><p>${relationCopy[4]}</p></section>`;
  const paneContent={
    gameplay:`${relationshipSettings}${homeCharacterDisplay}${map}${movement}`,
    sound,
    notifications,
    display:`${language}${measurement}${motionDisplay}${colorMode}${settingsPane==="display"?visualThemeSettings():""}${settingsPane==="display"?screenScaleSettings():""}`,
    achievements,
    account:`${sync}<section class="setting-card"><button type="button" data-safety-manage>${musicText("차단한 계정 관리","Blocked accounts","ブロックしたアカウント")}</button><button type="button" data-supporter-menu="editor" ${Number(window.ParallelCityAuth?.getInfo?.()?.entitlements?.teaSupportCount)>0?"":"hidden"}>${musicText("후원자 이름 편집","Edit supporter name","応援者名を編集")}</button></section>${danger}`,
    data:`<section class="setting-card" data-settings-storage></section>${backup}`,
    support:`${guide}<section class="setting-card"><button type="button" data-tab="settings" data-settings-pane="achievements">${achievementCopy.heading}</button><button type="button" data-supporter-menu="credits">${musicText("크레딧 · 후원자 명단","Credits · Supporters","クレジット・応援者一覧")}</button></section>${feedback}${appInfo}`
  };
  const paneName=settingsPane==='achievements'?achievementCopy.heading:settingsPaneName(settingsPane,state.uiLanguage);
  const body=settingsPane==="home"?`${settingsAccount(state,window.ParallelCityAuth?.getInfo?.(),esc,imageEsc)}${menu}`:`<div class="settings-pane-heading"><button type="button" data-tab="settings" data-settings-pane="${settingsPane==='achievements'?'support':'home'}" aria-label="${settingsLabels(state.uiLanguage).back}">‹</button><span><h2>${paneName}</h2></span></div>${paneContent[settingsPane]||menu}`;
...
Object.assign(UI_TEXT.ja,{
  ...
  ,"원하는 상품과 수량을 장바구니에 담아 한 번에 결제할 수 있어요.":"ほしい商品と数量をカートに入れて、まとめて決済できます。","캐릭터 슬롯":"キャラクタースロット","캐릭터 1명 추가":"キャラクター枠を1人追加","캐릭터 슬롯 1개가 결제 즉시 계정에 영구 적용되며, 서랍마을 서비스 운영 기간 동안 유지됩니다.":"キャラクター枠1人分が決済直後にアカウントへ永続的に適用され、ひきだし村のサービス運営期間中は維持されます。","마을 슬롯":"村スロット","마을 1개 추가":"村スロットを1つ追加","마을 슬롯 1개가 결제 즉시 계정에 영구 적용되며, 서랍마을 서비스 운영 기간 동안 유지됩니다.":"村スロット1つが決済直後にアカウントへ永続的に適用され、ひきだし村のサービス運営期間中は維持されます。","사진 저장 공간":"画像ストレージ","사진 저장 공간 50MB 추가":"画像ストレージを50MB追加","구매하면 계정의 사진 저장 공간이 50MB로 늘어납니다.":"購入するとアカウントの画像保存容量が50MBになります。","일회성 구매":"買い切り","개발 응원":"開発を応援","개발자에게 녹차 사주기 🍵":"開発者に緑茶をおごる 🍵","잘 먹겠습니다 🥹":"ありがとうございます 🥹","같은 상품도 여러 개 담을 수 있어요.":"同じ商品を複数入れることもできます。","한 번 결제 금액은 5만원 미만이어야 해요.":"1回の決済金額は5万ウォン未満にしてください。","한도에 닿는 상품은 더 담을 수 없어요.":"上限に達する商品は追加できません。","이 상품을 더 담으면 5만원 이상이에요":"この商品を追加すると5万ウォン以上になります","수량을 줄여 주세요":"数量を減らしてください","아직 장바구니가 비어 있어요.":"カートは空です。","총 결제금액":"合計金額","장바구니 결제하기":"カートを決済"
});
Object.assign(UI_TEXT.en,{
  "캐릭터 슬롯 1개가 결제 즉시 계정에 영구 적용됩니다. 결제일부터 최소 6개월간 이용을 보장하며, 이후에도 서비스 운영 기간 동안 유지됩니다.":"One character slot is permanently applied to the account immediately after payment. Access is guaranteed for at least six months from the payment date and continues while the service remains in operation.",
...
Object.assign(UI_TEXT.ja,{
  "캐릭터 슬롯 1개가 결제 즉시 계정에 영구 적용됩니다. 결제일부터 최소 6개월간 이용을 보장하며, 이후에도 서비스 운영 기간 동안 유지됩니다.":"キャラクター枠1人分が決済直後にアカウントへ永続的に適用されます。決済日から最低6か月間の利用を保証し、その後もサービス運営期間中は維持されます。",
...
Object.assign(UI_TEXT.en,{
  ...
  "초성 쓰는 반말":"Casual texting slang","캐릭터 말투":"Character speech style","자동 · 성격에 맞춤":"Auto · Match personality","반말":"Casual speech","존댓말 · 해요체":"Polite speech · Haeyo style","격식 있는 존댓말 · 하십시오체":"Formal polite speech · Hasipsio style","극존칭":"Highly honorific speech","무뚝뚝한 단답":"Curt, brief replies","다정하고 부드러운 말투":"Warm and gentle speech","고풍스러운 말투":"Archaic or classical speech",
...
Object.assign(UI_TEXT.ja,{
  "했다체 · 건조한 서술":"乾いた宣言調の叙述","기계적인 말투":"機械的な話し方","사무적인 말투 · 직장 메일체":"事務的・社内メール調","판교어 · 스타트업 업무체":"パンギョ系スタートアップ業務用語","상냥하고 배려하는 말투":"優しく思いやりのある話し方","소심하고 머뭇거리는 말투":"気弱でためらいがちな話し方","열정적인 말투":"情熱的な話し方","능글맞고 여유로운 말투":"飄々として余裕のある話し方","냉소적인 말투":"皮肉っぽい話し方","걸걸한 아저씨 말투":"しゃがれた親父風の話し方","거칠고 상스러운 말투":"荒っぽく口の悪い話し方","중2병 말투":"中二病風の話し方","귀여니체 · 2000년대 인터넷소설체":"2000年代ネット小説風","하드보일드 누아르체":"ハードボイルド・ノワール調","사극 선비 말투":"時代劇の士人風","군인식 말투":"軍人風の話し方","마왕의 말투":"魔王の話し方","군주의 말투":"君主の話し方","신탁을 내리는 신의 말투":"神託を告げる神の話し方","옛날 번역기체":"昔の機械翻訳調","귀엽고 애교 있는 말투":"可愛く甘える話し方","수다스럽고 말이 많은 말투":"おしゃべりで話の長い口調",
...
Object.assign(UI_TEXT.en,{
  ...
  "통제욕":"Desire for control","논리 최우선":"Logic above all","눈앞의 현실 중시":"Focuses on present reality","부산스럽게 여러 일을 오감":"Juggles several tasks restlessly","불안 관련 특성":"Anxiety-related trait","사고·감정 정도":"Thinking · feeling","사람이 버거움":"Finds social contact overwhelming","사이코패스 성향 설정 · 비임상":"Psychopathic traits · non-clinical setting","사회적 신호를 해석하는 데 시간이 필요함":"Needs time to interpret social cues","상상의 세계":"Imaginative world","상황에 따라 다름":"Depends on the situation","상황에 따라 표현함":"Expresses it depending on the situation","생각나면 바로 움직임":"Acts as soon as an idea occurs","생각이 떠오르면 바로 시작함":"Starts immediately when an idea occurs","생활 에너지":"Daily energy",
...
Object.assign(UI_TEXT.ja,{
  ...
  "통제욕":"支配欲","논리 최우선":"論理を最優先","눈앞의 현실 중시":"目の前の現実を重視","부산스럽게 여러 일을 오감":"落ち着かず複数のことを行き来する","불안 관련 특성":"不安関連の特性","사고·감정 정도":"思考・感情の度合い","사람이 버거움":"人との関わりが負担","사이코패스 성향 설정 · 비임상":"サイコパシー傾向・非臨床設定","사회적 신호를 해석하는 데 시간이 필요함":"社会的な合図の解釈に時間が必要","상상의 세계":"想像の世界","상황에 따라 다름":"状況によって異なる","상황에 따라 표현함":"状況に応じて表現する","생각나면 바로 움직임":"思いついたらすぐ動く","생각이 떠오르면 바로 시작함":"考えが浮かぶとすぐ始める","생활 에너지":"生活エネルギー",
...
function townBuildingBrowser(character){
  ...
  return `<aside class="town-building-screen town-building-browser-screen" aria-label="${esc(t("건물 정보","건물 정보"))}"><div class="town-building-screen-head"><button type="button" class="town-building-screen-back" data-mobile-town-close aria-label="${esc(t("마을로 돌아가기","마을로 돌아가기"))}"><img src="${imageEsc(homeUiAsset(character||active(),"back.png"))}" alt=""></button><label class="town-building-search"><span class="sr-only">${t("건물 검색","건물 검색")}</span><input type="search" data-building-search placeholder="${esc(t("검색","검색"))}"></label></div>${pinnedFilter("town",townFilters)}${pinnedFilter("type",typeFilters)}<div class="town-building-paper"><div class="town-building-catalog-head"><span>${t("총","총")} <b data-building-count>${entries.length}</b></span><button type="button" data-building-recovery>${t("건물 복구","건물 복구")}</button></div><div class="town-building-card-grid" aria-live="polite">${entries.map(entry=>`<button type="button" class="town-building-browser-card" data-building-browser-card data-building-browser-open="${entry.kind==="home"?`home:${entry.id}`:entry.id}" data-building-browser-town="${entry.townId}" data-building-browser-type="${esc(entry.type)}" data-building-browser-name="${esc(entry.name)}"><span><img src="${imageEsc(entry.art)}" alt=""></span><b>${esc(entry.name)}</b><small>${esc(entry.townName)} · ${esc(t(entry.type,entry.type))}</small></button>`).join("")}<button type="button" class="town-building-add" data-add-place aria-label="${esc(t("건물 추가","건물 추가"))}">＋<small>${t("건물 추가","건물 추가")}</small></button><button type="button" class="town-building-add town-home-add" data-add-town-home aria-label="${esc(t("집 추가","집 추가"))}">＋<small>${t("집 추가","집 추가")}</small></button></div></div></aside>`;
}
function townBuildingDetailScreen(character,place,home,items,audiences){
  ...
  return `<aside class="town-building-screen town-building-detail-screen" aria-label="${esc(name)}"><div class="town-building-detail-head"><button type="button" class="town-building-screen-back" data-building-browser-back aria-label="${esc(t("건물 목록으로 돌아가기","건물 목록으로 돌아가기"))}"><img src="${imageEsc(homeUiAsset(character||active(),"back.png"))}" alt=""></button><b>${esc(name)}</b></div><div class="town-building-detail-scroll"><section class="town-building-hero"><button type="button" class="town-building-photo-upload" ${home?`data-home-interior-image="${esc(home.id)}"`:`data-place-interior-image="${esc(place.id)}"`} aria-label="${esc(t(home?"집 사진 등록":"내부 사진 업로드",home?"집 사진 등록":"내부 사진 업로드"))}">${interior?`<img class="town-building-interior" src="${imageEsc(interior)}" alt="${esc(name)} ${esc(t(home?"집 사진":"내부 사진",home?"집 사진":"내부 사진"))}">`:`<div class="town-building-interior town-building-interior-empty"><span>${esc(place?.emoji||"🏠")}</span><small>${t(home?"집 일러스트나 가족사진을 넣어 주세요.":"내부 사진을 등록해 주세요",home?"집 일러스트나 가족사진을 넣어 주세요.":"내부 사진을 등록해 주세요")}</small></div>`}</button><img class="town-building-exterior" src="${imageEsc(exterior)}" alt="${esc(name)} ${esc(t("건물 모양","건물 모양"))}"></section><div class="place-editor town-building-detail-form">${editor}<div class="editor-save-actions"><button type="button" class="primary" data-editor-save>${t("저장","저장")}</button></div></div></div></aside>`;
}
...
function townInformationScreen(character){
  ...
  return `<aside class="town-information-screen" data-town-information-id="${esc(state.world.id||state.activeTownId)}" aria-label="${esc(t("마을 정보 편집","마을 정보 편집"))}"><img class="town-information-backdrop" src="${imageEsc(state.world.bg)}" alt="" aria-hidden="true">
    <header class="town-information-head"><button type="button" class="town-information-back" data-mobile-town-close aria-label="${esc(t("마을로 돌아가기","마을로 돌아가기"))}"><img src="${imageEsc(homeUiAsset(character||active(),"back.png"))}" alt=""></button><b data-town-information-title>${esc(state.world.name)}</b></header>
    <div class="town-information-content">
      <section class="town-information-hero"><img src="${imageEsc(state.world.bg)}" alt="${esc(t("숲과 연못 마을","숲과 연못 마을"))}"></section>
      <div class="town-information-form">
        <label class="town-information-browser"><img src="${imageEsc(homeUiAsset(character||active(),"town.png"))}" alt=""><span class="sr-only">${t("마을 선택","마을 선택")}</span><select data-town-browse aria-label="${esc(t("마을 선택","마을 선택"))}">${state.towns.map(town=>`<option value="${esc(town.id)}" ${town.id===state.activeTownId?"selected":""}>${esc(town.name)}</option>`).join("")}</select></label>
...
function townMobile(sharedPass=false){
  const shared=window.DrawerVillageGroups?.getSnapshot?.();if(!sharedPass&&shared?.activeGroupId&&shared.group)return withSharedWorld(shared,()=>townMobile(true));
  const items=catalogItems(),audiences=["아재 입맛","어린이 입맛","가족","연인·데이트","학생","고소득","오타쿠"];
  const multiplayerSnapshot=window.DrawerVillageGroups?.getSnapshot?.()||{},multiplayer=multiplayerSnapshot.group||null;
  const multiplayerTown=multiplayer?.towns?.find(town=>town?.id===multiplayerSnapshot.selectedTownId)||multiplayer?.towns?.[0]||null;
  ...
  if(mobileTownPanel&&!["world","info","decorations","buildings","residents"].includes(mobileTownPanel)&&!selectedPlace&&!selectedHome)mobileTownPanel="";
  const panelType=mobileTownPanel==="residents"?"residents":mobileTownPanel==="world"?"world":mobileTownPanel==="info"?"info":mobileTownPanel==="decorations"?"decorations":mobileTownPanel==="buildings"?"buildings":selectedPlace?"place":selectedHome?"home":"";
  const personalResidentIds=state.order.filter(id=>{
  ...
  const residentIds=multiplayer?(multiplayerSnapshot.residents||[]).filter(item=>!multiplayerTown?.id||item.townId===multiplayerTown.id).map(item=>item.id):personalResidentIds;
  const localIds=multiplayer?[...residentIds]:state.preventInterTownMovement?[...residentIds]:state.order.filter(id=>visibleTownId(state.characters[id])===state.activeTownId);
  ...
  const menuButtons=`<button type="button" class="home-native-pill" data-mobile-town-layout-mode><span>${t("마을 정보","마을 정보")}</span></button><button type="button" class="home-native-pill" data-mobile-building-edit-mode><span>${t("건물 정보","건물 정보")}</span></button><button type="button" class="home-native-pill" data-mobile-town-decoration-mode ${canEditShared?'':'disabled'}><span>${mobileTownMode==="decorations"?t("편집완료","편집완료"):t("편집모드","편집모드")}</span></button>`;
  const townHeader=`<header class="town-native-header"><button type="button" class="home-native-back town-native-back" data-tab="observe" aria-label="${esc(t("메인 화면으로 돌아가기","메인 화면으로 돌아가기"))}"><img src="${imageEsc(homeUiAsset(character||active(),"back.png"))}" alt=""></button><div class="town-native-context"><button type="button" class="town-native-community" data-open-multiplayer-switcher aria-label="${esc(t("멀티 변경","멀티 변경"))}"><span>${esc(multiplayerName)} <i aria-hidden="true" style="font-style:normal">▾</i></span></button><button type="button" class="town-native-town-pill" data-open-town-switcher aria-label="${esc(t("마을 이동","마을 이동"))}"><span>${esc(headerTownName)} <i aria-hidden="true" style="font-style:normal">▾</i></span></button></div><span class="town-native-status">${t("현재 {current}명 · 거주 {resident}명","현재 {current}명 · 거주 {resident}명").replace("{current}",localIds.length).replace("{resident}",residentIds.length)}</span><div class="town-native-menu" role="navigation" aria-label="${esc(t("마을 메뉴","마을 메뉴"))}">${menuButtons}</div></header>`;
  const townSwitchOptions=multiplayer?(multiplayer.towns||[]).map(town=>`<button type="button" data-multiplayer-town-open="${esc(town.id||"")}" class="${town.id===multiplayerSnapshot.selectedTownId||town===multiplayerTown?"on":""}"><i aria-hidden="true" style="background-image:url(&quot;${imageEsc(town.bg||TOWN_BACKGROUND)}&quot;)"></i><span><b>${esc(town.name||multiplayer.hostTownName||t("멀티 마을 없음","No multiplayer town"))}</b><small>${({ko:"현재 멀티 마을",en:"Current multiplayer town",ja:"現在のマルチタウン"}[state.uiLanguage]||"현재 멀티 마을")}</small></span></button>`).join(""):state.towns.map(town=>`<button type="button" data-town-select="${town.id}" class="${town.id===state.activeTownId?"on":""}"><i aria-hidden="true" style="background-image:url(&quot;${imageEsc(town.bg||TOWN_BACKGROUND)}&quot;)"></i><span><b>${esc(town.name)}</b><small>${town.id===state.activeTownId?t("현재 마을","현재 마을"):t("이 마을로 이동","이 마을로 이동")}</small></span></button>`).join("")+`<button type="button" class="town-switch-add" data-add-town data-add-town-switcher><i aria-hidden="true">＋</i><span><b>${t("새 마을 만들기","새 마을 만들기")}</b><small>${t("새로운 마을 슬롯을 추가해요","새로운 마을 슬롯을 추가해요")}</small></span></button>`;
  const townSwitcher=`<dialog class="town-switch-dialog" data-town-switch-dialog><form method="dialog"><header><span><small>TOWN SELECT</small><b>${t("마을 이동","마을 이동")}</b></span><button value="cancel" aria-label="${esc(t("닫기","닫기"))}">×</button></header><div>${townSwitchOptions}</div></form></dialog>`;
  const multiplayerSwitcher=`<dialog class="town-switch-dialog multiplayer-switch-dialog" data-multiplayer-switch-dialog><form method="dialog"><header><span><small>MULTIPLAYER SELECT</small><b>${t("멀티 변경","멀티 변경")}</b></span><button value="cancel" aria-label="${esc(t("닫기","닫기"))}">×</button></header><div><button type="button" data-multiplayer-select="" class="${multiplayer?"":"on"}"><i aria-hidden="true" style="background-image:url(&quot;${imageEsc(state.world.bg||TOWN_BACKGROUND)}&quot;)"></i><span><b>${esc(state.personalTownLabel||t("내 마을","내 마을"))}</b><small>${t("내가 보유한 마을","내가 보유한 마을")}</small></span></button>${(multiplayerSnapshot.groups||[]).map(group=>{const town=group.towns?.find(item=>item?.sourceTownId||item?.ownerUid)||group.towns?.[0];return `<button type="button" data-multiplayer-select="${esc(group.id)}" class="${group.id===multiplayer?.id?"on":""}"><i aria-hidden="true" style="background-image:url(&quot;${imageEsc(group.photoURL||town?.bg||TOWN_BACKGROUND)}&quot;)"></i><span><b>${esc(group.name)}</b><small>${esc(town?.name||group.hostTownName||t("연결된 마을 없음","연결된 마을 없음"))}</small></span></button>`}).join("")}<button type="button" class="town-switch-add" data-tab="groups"><i aria-hidden="true">＋</i><span><b>${t("새 멀티 만들기","새 멀티 만들기")}</b><small>${t("내 마을로 친구를 초대해요","내 마을로 친구를 초대해요")}</small></span></button></div></form></dialog>`;
  ...
  const townInfo=`<div class="town-info-card"><img class="town-info-art" src="${imageEsc(state.world.bg)}" alt=""><dl><div><dt>${t("마을 유형","마을 유형")}</dt><dd>${t(state.world.townSubtype||state.world.townType,state.world.townSubtype||state.world.townType)}</dd></div><div><dt>${t("도시화 정도","도시화 정도")}</dt><dd>${t(state.world.urbanization||"소도시",state.world.urbanization||"소도시")}</dd></div><div><dt>${t("마을 평판","마을 평판")}</dt><dd>${t(state.world.reputation||"평판 정보 없음",state.world.reputation||"평판 정보 없음")}</dd></div><div><dt>${t("마을 인지도","마을 인지도")}</dt><dd>${t(state.world.fameLevel||"거의 알려지지 않음",state.world.fameLevel||"거의 알려지지 않음")}</dd></div><div><dt>${t("지형","지형")}</dt><dd>${t(state.world.terrain||"평야",state.world.terrain||"평야")}</dd></div><div><dt>${t("교통편","교통편")}</dt><dd>${(state.world.transportModes||[]).map(value=>t(value,value)).join(" · ")||t("없음","없음")}</dd></div><div><dt>${t("마을 시대","마을 시대")}</dt><dd>${settingLabel(TOWN_ERAS,state.world.era,state.uiLanguage)} · ${settingLabel(TOWN_CULTURES,state.world.culture,state.uiLanguage)}</dd></div></dl>${state.world.description?`<p>${esc(state.world.description)}</p>`:""}</div>`;
  ...
  const editorPanel=panelType==="residents"?sharedResidentsScreen(multiplayerSnapshot):panelType==="world"?townInformationScreen(character):panelType==="buildings"?townBuildingBrowser(character):["place","home"].includes(panelType)?townBuildingDetailScreen(character,selectedPlace,selectedHome,items,audiences):panelType?`<aside class="panel form town-editor-panel"><div class="mobile-town-sheet-head"><span><small>${panelType==="info"?"TOWN INFO":"TOWN DECORATIONS"}</small><b>${panelType==="info"?esc(state.world.name):t("마을 장식","마을 장식")}</b></span><button type="button" data-mobile-town-close aria-label="${esc(t("편집 창 닫기","편집 창 닫기"))}">×</button></div>${generalEditor}</aside>`:"";
  const worldMarkup=`${townBackgroundMarkup(state.world.bg)}${state.world.places.map(placeCard).join("")}${townHomes().map(homeMapCard).join("")}${townDecorationsMarkup()}${state.world.places.map(peopleAtPlaceCard).join("")}${townHomes().map(peopleAtHomeCard).join("")}${townTravelersMarkup()}`;
  return `<section class="mobile-town-shell ${multiplayer?"multiplayer-town":""} ${mobileTownMode?`${mobileTownMode}-editing`:""} ${panelType?`sheet-open ${panelType}-panel`:""}" data-town-mode="${mobileTownMode}" data-town-id="${esc(multiplayerTown?.id||state.activeTownId)}" style="${homeUiThemeStyle(character||active())}">${desktopTabs}${townHeader}${multiplayer?`<button type="button" data-multiplayer-select="">${({ko:"내 마을로 돌아가기",en:"Return to my village",ja:"自分の村へ戻る"})[state.uiLanguage]||"내 마을로 돌아가기"}</button>`:""}${townSwitcher}${multiplayerSwitcher}<div class="town-edit"><div class="town-map-scroll"><div class="world town-environment" style="--town-brightness:${townTimeAppearance().brightness};--town-dusk:${townTimeAppearance().dusk}" data-town-language="${state.uiLanguage||"ko"}">${worldMarkup}</div></div></div>${editorPanel}${townPlacementToolbar()}${buildingDetailDialogs()}</section>`;
}
Object.assign(UI_TEXT.en,{
...
Object.assign(UI_TEXT.en,{"멀티 상세에서 보기":"View multiplayer details"});
Object.assign(UI_TEXT.ja,{
...
Object.assign(UI_TEXT.ja,{"멀티 상세에서 보기":"マルチ詳細で見る"});
...
Object.assign(UI_TEXT.en,{groups:"Multi",group:"Group","개인 마을":"My towns","내가 보유한 마을":"Towns I own","멀티 변경":"Switch multiplayer","연결된 마을 없음":"No linked town","새 멀티 만들기":"Create multiplayer","내 마을로 친구를 초대해요":"Invite friends to my town"});
Object.assign(UI_TEXT.ja,{groups:"マルチ",group:"グループ","개인 마을":"自分の村","내가 보유한 마을":"所有している村","멀티 변경":"マルチを変更","연결된 마을 없음":"接続された村なし","새 멀티 만들기":"新しいマルチを作成","내 마을로 친구를 초대해요":"自分の村に友だちを招待"});
Object.assign(UI_TEXT.en,{"내 마을":"My towns","그룹·멀티":"Groups · Multiplayer","내 마을 묶음 이름":"My town collection name","멀티에 들어가지 않았을 때 상단에 표시되는 이름이에요.":"Shown at the top when you are not in multiplayer."});
Object.assign(UI_TEXT.ja,{"내 마을":"自分の村","그룹·멀티":"グループ・マルチ","내 마을 묶음 이름":"自分の村グループ名","멀티에 들어가지 않았을 때 상단에 표시되는 이름이에요.":"マルチに入っていない時、上部に表示される名前です。"});
function welcome(){
  const authInfo=window.ParallelCityAuth?.getInfo?.();
  const multiCopy=({ko:'멀티 마을 이어하기',en:'Continue in multiplayer',ja:'マルチの村を続ける'}[state.uiLanguage]||'멀티 마을 이어하기');
  ...
  return `<section class="village-welcome" aria-labelledby="welcome-title">
    ...
    <div class="welcome-note"><p class="welcome-eyebrow">${esc(t("캐릭터의 하루가 모이는 곳","캐릭터의 하루가 모이는 곳"))}</p>
      ...
      <div class="welcome-actions">${authInfo?.user?`<button type="button" data-tab="groups" ${busy?"disabled":""}>${multiCopy}</button>`:""}<button type="button" data-welcome-create ${busy?"disabled":""}>${esc(t("첫 캐릭터 만들기","첫 캐릭터 만들기"))}<span aria-hidden="true"> →</span></button><button type="button" data-welcome-restore ${busy?"disabled":""}>${esc(t("내 마을 불러오기","내 마을 불러오기"))}</button></div>
...
function desktopWebShell(content){
  ...
  const tabs=[["observe","観察","profile-placeholder.png"],["character","キャラクター","profile-placeholder.png"],["home","家","home.png"],["town","村","town.png"],["groups",copy.multi,"town.png"],["relationship","関係","relationship.png"],["catalog","辞典","catalog.png"],["mailbox","郵便箱","mailbox.png"],["routine","予定","routine.png"],["statistics","統計","statistics.png"],["shop","ショップ","shop.png"],["settings","設定","settings.png"]];
  const labels=GAME_HUD_LABELS[state.uiLanguage]||GAME_HUD_LABELS.ko;
  const nav=tabs.map(([key,fallback,asset],i)=>`${i===0?`<small class="web-nav-caption">${esc(copy.play)}</small>`:i===7?`<small class="web-nav-caption">${esc(copy.records)}</small>`:""}<button type="button" data-tab="${key}" ${state.activeTab===key?'aria-current="page"':''}><img src="./assets/home-ui/${asset}" alt=""><span>${esc(key==="groups"?copy.multi:labels[key]||copy[key]||t(key,fallback))}</span></button>`).join("");
  return `<div class="web-drawer-shell"><aside class="web-drawer-sidebar"><button type="button" class="web-drawer-brand" data-tab="observe"><img src="./icons/drawer-village-logo.png" alt=""><b>${esc(t("brandName","서랍마을"))}</b><small>${esc(copy.hint)}</small></button><nav aria-label="${esc(copy.home)}">${nav}</nav><button type="button" class="web-account-card" data-tab="settings" data-settings-pane="home">${state.ownerPhoto?`<img src="${imageEsc(state.ownerPhoto)}" alt="">`:'<span aria-hidden="true">✦</span>'}<span><b>${esc(state.ownerName||copy.guest)}</b><small>${esc(copy.account)}</small></span></button></aside><div class="web-drawer-workspace"><div class="web-drawer-topbar"><span>${esc(copy.today)}<b>${esc(labels[state.activeTab]||copy[state.activeTab]||copy.home)}</b></span><div><button type="button" data-web-mute aria-pressed="${webMuted()}" title="${({ko:'이 브라우저에서만 적용돼요',en:'Only affects this browser',ja:'このブラウザーだけに適用'})[state.uiLanguage]}">${webMuted()?({ko:'소리 켜기',en:'Unmute',ja:'音をオン'})[state.uiLanguage]:({ko:'음소거',en:'Mute',ja:'ミュート'})[state.uiLanguage]}</button>${state.activeTab==='shop'?'':`<span class="web-account-status">${esc(accountText)}</span>`}<button type="button" data-tab="settings" data-settings-pane="home">${esc(copy.profile)}</button></div></div><main class="web-drawer-content">${content}</main><footer class="web-drawer-footer"><span id="save-state">${esc(copy.saved)}</span><a href="./privacy.html">${esc(t("개인정보처리방침","개인정보처리방침"))}</a></footer></div></div>`;
}
...
function renderAppContents(next){
  ...
  const showingWelcome=content.includes('class="village-welcome"');
  ...
  bindBannerAds();
  commitInputBoundary();
  bindSceneDepth(appRoot);
  scheduleTownLighting(document);watchMovement(document);
  appRoot.querySelectorAll("img").forEach((image,index)=>{
    image.decoding="async";
    const currentArt=image.matches(".sprite,.avatar,.welcome-landscape,.native-observe-background,.native-main-character,.native-scene-lineup-person img,.app-loading-logo")||image.closest(".game-hud-profile,.character-registration-photo,.native-current-scene,.home-current-scene,.app-loading-card");
...
export function setAccountLabel(text=accountText){
  accountText=text;const el=document.querySelector("#account-status");if(el)el.textContent=translatedUiText(text);document.querySelectorAll(".web-account-status").forEach(node=>node.textContent=translatedUiText(text));
...
export function setAccountEntitlements(value){accountEntitlements={backgroundPacks:Array.isArray(value?.backgroundPacks)?value.backgroundPacks:[],iconPacks:Array.isArray(value?.iconPacks)?value.iconPacks:[],dlcPacks:Array.isArray(value?.dlcPacks)?value.dlcPacks:[],purchases:Array.isArray(value?.purchases)?value.purchases:[],characterSlotPacks:Math.max(0,Number(value?.characterSlotPacks)||0),characterSingleSlots:Math.max(0,Number(value?.characterSingleSlots)||0),townSlotPacks:Math.max(0,Number(value?.townSlotPacks)||0),adFree:value?.adFree===true,storage50:Boolean(value?.storage50),teaSupportMonth:String(value?.teaSupportMonth||"")};setDiscoveryEntitlement(accountEntitlements.adFree)}

const CART_KEY="drawer-village-cart";
const SHOP_PRODUCTS={
  character_slots_5:{label:"캐릭터 슬롯",title:"캐릭터 5명 추가",description:"캐릭터 슬롯 5개가 결제 즉시 계정에 적용됩니다. 기존 구매 슬롯은 그대로 유지됩니다.",price:4800},
  character_slot_1:{label:"캐릭터 슬롯",title:"캐릭터 1명 추가",description:"캐릭터 슬롯 1개가 결제 즉시 계정에 영구 적용됩니다. 결제일부터 최소 6개월간 이용을 보장하며, 이후에도 서비스 운영 기간 동안 유지됩니다.",price:1000},
  town_slots_5:{label:"마을 슬롯",title:"마을 5개 추가",description:"마을 슬롯 5개가 계정에 추가됩니다. 기존 구매 슬롯은 그대로 유지됩니다.",price:9300},
  ...
  ad_free:{label:"광고 제거",title:"광고 없이 편안하게",description:"일회성 구매로 배너·전면 광고를 영구 제거하고 자아만들기 대기시간을 계정 공통 1분으로 줄입니다. 보상형 광고는 원하는 경우 이용할 수 있어요.",price:9900},
...
const shopProductBadge=(id,item)=>id==="character_slot_1"
...
function nativePlayShop({browseOnly=false,webBilling=false}={}){
  const language=state.uiLanguage||"ko";
  const copies={
    ko:{back:"메인으로 돌아가기",title:"서랍 상점",sellerHello:"어서 와요. 천천히 둘러보고 필요한 게 있으면 편하게 불러 주세요.",bundle:"번들",base:"기본상점",skin:"스킨상점",expansion:"확장팩상점",checking:"상품 확인 중",restore:"구매 내역 복원",bundleSoon:"신규 번들은 아직 판매하지 않아요",skinSoon:"스킨 상점 준비 중",expansionSoon:"확장팩 상점 준비 중",soonDetail:"상품 구성이 확정되면 이곳에서 알려드릴게요.",buy:"구매하기"},
    en:{back:"Back to home",title:"Drawer Shop",sellerHello:"Welcome. Take your time, and let me know if you need anything.",bundle:"Bundles",base:"Base shop",skin:"Skins",expansion:"Expansions",checking:"Checking product",restore:"Restore purchases",bundleSoon:"New bundles are not on sale yet",skinSoon:"Skin shop coming soon",expansionSoon:"Expansion shop coming soon",soonDetail:"We will announce products here after the lineup is finalized.",buy:"Buy"},
    ja:{back:"メインへ戻る",title:"ひきだし商店",sellerHello:"いらっしゃいませ。ゆっくり見て、必要なものがあれば気軽に声をかけてくださいね。",bundle:"バンドル",base:"基本ショップ",skin:"スキン",expansion:"拡張パック",checking:"商品を確認中",restore:"購入を復元",bundleSoon:"新しいバンドルはまだ販売していません",skinSoon:"スキンショップ準備中",expansionSoon:"拡張パック準備中",soonDetail:"商品内容が確定したら、こちらでお知らせします。",buy:"購入"}
  };
  ...
  const icons={character_slots_5:`👤`,character_slot_1:`👤`,town_slot_1:"🏘️",town_slots_5:"🏘️",ad_free:"🌿",storage_50mb:"🗄️",green_tea:"🎁"};
  const productCard=id=>{const item=id==='ad_free'?{...SHOP_PRODUCTS[id],...({en:{label:'Remove ads',title:'A quieter village',description:'One-time purchase: permanently removes banners and interstitial ads, and reduces account-wide self-discovery cooldown to 1 minute. Rewarded ads remain optional.'},ja:{label:'広告を削除',title:'広告なしでゆったり',description:'買い切りでバナー・全画面広告を永久に削除し、自分づくりの待ち時間をアカウント共通で1分に短縮。リワード広告は任意で利用できます。'}}[language]||{})}:SHOP_PRODUCTS[id],fallback=`₩${item.price.toLocaleString("ko-KR")}`,owned=id==="ad_free"?accountEntitlements.adFree:id==="storage_50mb"&&accountEntitlements.storage50;return `<article class="drawer-shop-product"><span class="drawer-shop-product-icon ${id}">${icons[id]}</span><div><small>${esc(t(item.label,item.label))}</small><b>${esc(t(item.title,item.title))}</b><p>${esc(t(item.description,item.description))}</p></div>${webBilling?`<button type="button" class="drawer-shop-purchase" data-cart-add="${id}" ${owned?"disabled":""}><span>${fallback}</span><small>${esc(t(owned?"이미 적용 중":"장바구니에 담기",owned?"이미 적용 중":"장바구니에 담기"))}</small></button>`:browseOnly?`<button type="button" class="drawer-shop-purchase" disabled>${esc(previewCopy.pending)}</button>`:`<button type="button" class="drawer-shop-purchase" data-play-purchase="${id}" data-play-owned="${owned}" data-purchase-state="${owned?"unavailable":"checking"}" disabled aria-label="${esc(item.title)} · ${esc(owned?"이미 구매함":copy.checking)}"><span data-play-price="${id}">${owned?esc(t("이미 적용 중","이미 적용 중")):fallback}</span><small class="drawer-shop-sr" data-play-label>${esc(owned?"이미 적용 중":copy.checking)}</small></button>`}</article>`};
  const comingTitle=section==="bundle"?copy.bundleSoon:section==="skin"?copy.skinSoon:copy.expansionSoon;
  const content=section==="base"?`<div class="drawer-shop-products">${((window.PARALLEL_CITY_CONFIG?.iosApp||window.PARALLEL_CITY_CONFIG?.iosPreview)?["character_slot_1","town_slot_1","green_tea",...(webBilling?[]:["ad_free"])]:["character_slot_1","town_slot_1","green_tea",...(webBilling?[]:["ad_free"]),"storage_50mb"]).map(productCard).join("")}</div>`:`<div class="drawer-shop-coming" role="status"><span aria-hidden="true">✦</span><b>${esc(comingTitle)}</b><small>${esc(copy.soonDetail)}</small></div>`;
  const sellerLabel=language==="en"?"Shopkeeper":language==="ja"?"店主":"상점주인";
  return `<section class="drawer-shop-shell" aria-label="${esc(t("shop","상점"))}"><div class="drawer-shop-stage" data-shop-section="${section}"><div class="drawer-shop-hero"><img class="drawer-shop-wood" src="./assets/shop/drawer-shop-wood.jpg" alt=""><img class="drawer-shop-seller" src="./assets/shop/drawer-shop-nerine.png?v=20260909dev305" alt=""><p class="drawer-shop-greeting"><b>${sellerLabel}</b><span>${esc(copy.sellerHello)}</span></p><button type="button" class="drawer-shop-back" data-tab="observe" aria-label="${esc(copy.back)}"><img src="./assets/home-ui/back.png" alt=""></button></div><nav class="drawer-shop-tabs" aria-label="${esc(t("상점 메뉴","상점 메뉴"))}">${tab("bundle",copy.bundle)}${tab("base",copy.base)}${tab("skin",copy.skin)}${tab("expansion",copy.expansion)}</nav><div class="drawer-shop-content"><p class="drawer-shop-preview-notice" data-apple-billing-progress role="status" hidden></p><button type="button" data-apple-billing-diagnostics hidden>${language==="en"?"Check payment status":language==="ja"?"決済状況を確認":"결제 상태 확인"}</button>${browseOnly?`<p class="drawer-shop-preview-notice" role="status">${esc(previewCopy.notice)}</p>`:""}${content}</div>${webBilling?webShopCart():browseOnly?"":`<button type="button" class="drawer-shop-restore" data-play-restore>${esc(copy.restore)}</button>`}</div></section>`;
}
function webShopCart(){
  const entries=Object.entries(readCart()).filter(([id,n])=>SHOP_PRODUCTS[id]&&saleAllows(id)&&Number(n)>0);
  const total=entries.reduce((sum,[id,n])=>sum+SHOP_PRODUCTS[id].price*Number(n),0);
  const labels=({ko:{title:"장바구니",empty:"아직 장바구니가 비어 있어요.",pay:"장바구니 결제하기",limit:"한 번 결제 금액은 5만원 미만이어야 해요."},en:{title:"Cart",empty:"Your cart is empty.",pay:"Checkout",limit:"Each payment must be below KRW 50,000."},ja:{title:"カート",empty:"カートは空です。",pay:"購入手続きへ",limit:"1回の決済は5万ウォン未満です。"}})[state.uiLanguage];
  return `<section class="web-shop-cart"><h2>${labels.title}</h2>${entries.map(([id,n])=>`<div class="web-shop-cart-line"><span>${esc(t(SHOP_PRODUCTS[id].title,SHOP_PRODUCTS[id].title))}</span><button type="button" data-cart-minus="${id}" aria-label="−">−</button><b>${Number(n)}</b><button type="button" data-cart-plus="${id}" aria-label="＋">＋</button></div>`).join("")||`<p>${labels.empty}</p>`}<b>₩${total.toLocaleString("ko-KR")}</b><a class="primary" ${entries.length&&Number.isSafeInteger(total)?`href="./payment.html?cart=1&lang=${state.uiLanguage}"`:'aria-disabled="true"'}>${labels.pay}</a></section>`;
}
function shop(){
  if(window.PARALLEL_CITY_CONFIG?.iosPreview){
    return nativePlayShop({browseOnly:!window.PARALLEL_CITY_CONFIG?.appleBilling?.enabled});
  }
  if(window.PARALLEL_CITY_CONFIG?.nativeApp||new URLSearchParams(location.search).has("native-preview"))return nativePlayShop();

  return nativePlayShop({webBilling:true});
}

Object.assign(UI_TEXT.en,{"웨딩드레스 부티크":"Wedding dress boutique","원형 경기장":"Circular stadium","도심 오피스":"City office","학사모 학교":"Graduation school","여행가방 호텔":"Suitcase hotel","시계탑 학교":"Clock school","책더미 도서관":"Book-stack library","회사 건물":"Rooftop garden building","직접 그린 건물":"Hand-drawn building","시간별 조명":"Scheduled lighting","원화의 흰색 유지":"Original white artwork preserved"});
Object.assign(UI_TEXT.ja,{"웨딩드레스 부티크":"ウェディングドレス・ブティック","원형 경기장":"円形スタジアム","도심 오フィス":"都心オフィス","학사모 학교":"卒業帽の学校","여행가방 호텔":"スーツケースホテル","시계탑 학교":"時計台の学校","책더미 도서관":"本の山の図書館","회사 건물":"屋上庭園の建物","직접 그린 건물":"手描きの建物","시간별 조명":"時間指定の照明","원화의 흰색 유지":"原画の白色を保持"});
Object.assign(UI_TEXT.en,{"· 끌어서 이동":"· Drag to move","깨우고 할 일 정하기":"Wake up and choose an activity","‘집만 생성’을 눌러 캐릭터와 별개로 집부터 만들 수 있어요.":"Choose ‘Create home only’ to create a home before adding characters.","캐릭터 정보":"Character information","선택한 캐릭터":"Selected character","이 캐릭터의 중심을 정하기":"Define who this character is","설정책에서 자세히 수정":"Edit every detail in the settings book","펼친 책 페이지 이동":"Turn open-book spreads","이전 두 페이지":"Previous two pages","다음 두 페이지":"Next two pages"});
Object.assign(UI_TEXT.ja,{"· 끌어서 이동":"· ドラッグで移動","깨우고 할 일 정하기":"起こして行動を決める","‘집만 생성’을 눌러 캐릭터와 별개로 집부터 만들 수 있어요.":"「家だけ作成」を押すと、キャラクターとは別に家を先に作れます。","캐릭터 정보":"人物情報","선택한 캐릭터":"選択中の人物","이 캐릭터의 중심을 정하기":"基本情報をすばやく編集","설정책에서 자세히 수정":"設定帳ですべての詳細を編集","펼친 책 페이지 이동":"見開きページを移動","이전 두 페이지":"前の2ページ","다음 두 페이지":"次の2ページ"});

Object.assign(I18N.en,{'다른 그룹으로 이동':'Switch group','선택한 그룹의 캐릭터':'Characters in the selected group','구성원 정보':'Member information','생활 기록 불러오는 중':'Loading life records'});
Object.assign(I18N.ja,{'다른 그룹으로 이동':'グループを移動','선택한 그룹의 캐릭터':'選択中のグループのキャラクター','구성원 정보':'メンバー情報','생활 기록 불러오는 중':'生活記録を読み込み中'});

for(const [ko,en,ja] of creativeCopy){I18N.en[ko]=en;I18N.ja[ko]=ja;}
Object.assign(I18N.en,{"전통 의상":"Traditional attire"});Object.assign(I18N.ja,{"전통 의상":"伝統衣装"});

Object.assign(I18N.en,{"첫 번째 인물의 역할":"First character's role","두 번째 인물의 역할":"Second character's role","보호자·백부·이모 등":"Guardian, uncle, aunt, etc.","피보호자·조카 등":"Ward, nephew, niece, etc."});
Object.assign(I18N.ja,{"첫 번째 인물의 역할":"一人目の役割","두 번째 인물의 역할":"二人目の役割","보호자·백부·이모 등":"保護者・伯父・叔母など","피보호자·조카 등":"被保護者・甥・姪など"});

Object.assign(I18N.en,{"옷장은 캐릭터마다 30개까지 등록할 수 있어요.": "Each character can register up to 30 wardrobe items.", "사진 포함 공유 코드 만들기": "Create a sharing code with photos", "캐릭터 코드로 불러오기": "Import by character code", "신체 접촉 반응": "Response to physical contact", "촉각에 둔감한 편": "Less sensitive to touch", "간지럼을 잘 탐": "Ticklish", "갑작스러운 접촉에 쉽게 놀람": "Startled by unexpected touch", "친한 사람 외의 기척에는 경계함": "Wary of anyone approaching except close friends", "사제 관계": "Teacher and student", "배움을 시작한 사이": "Beginning their training", "서로 신뢰하는 사제": "Teacher and student who trust each other", "가르침을 이어가는 사이": "Continuing their learning together"});
Object.assign(UI_TEXT.en,{"옷장은 캐릭터마다 30개까지 등록할 수 있어요.": "Each character can register up to 30 wardrobe items.", "사진 포함 공유 코드 만들기": "Create a sharing code with photos", "캐릭터 코드로 불러오기": "Import by character code", "신체 접촉 반응": "Response to physical contact", "촉각에 둔감한 편": "Less sensitive to touch", "간지럼을 잘 탐": "Ticklish", "갑작스러운 접촉에 쉽게 놀람": "Startled by unexpected touch", "친한 사람 외의 기척에는 경계함": "Wary of anyone approaching except close friends", "사제 관계": "Teacher and student", "배움을 시작한 사이": "Beginning their training", "서로 신뢰하는 사제": "Teacher and student who trust each other", "가르침을 이어가는 사이": "Continuing their learning together"});

Object.assign(I18N.ja,{"옷장은 캐릭터마다 30개까지 등록할 수 있어요.": "クローゼットは一人につき30着まで登録できます。", "사진 포함 공유 코드 만들기": "写真付き共有コードを作成", "캐릭터 코드로 불러오기": "キャラクターコードで読み込む", "신체 접촉 반응": "身体接触への反応", "촉각에 둔감한 편": "触覚が鈍い方", "간지럼을 잘 탐": "くすぐったがり", "갑작스러운 접촉에 쉽게 놀람": "突然触れられると驚きやすい", "친한 사람 외의 기척에는 경계함": "親しい人以外の気配には警戒する", "사제 관계": "師弟関係", "배움을 시작한 사이": "学び始めた関係", "서로 신뢰하는 사제": "信頼し合う師弟", "가르침을 이어가는 사이": "教えを受け継ぐ関係"});
Object.assign(UI_TEXT.ja,{"옷장은 캐릭터마다 30개까지 등록할 수 있어요.": "クローゼットは一人につき30着まで登録できます。", "사진 포함 공유 코드 만들기": "写真付き共有コードを作成", "캐릭터 코드로 불러오기": "キャラクターコードで読み込む", "신체 접촉 반응": "身体接触への反応", "촉각에 둔감한 편": "触覚が鈍い方", "간지럼을 잘 탐": "くすぐったがり", "갑작스러운 접촉에 쉽게 놀람": "突然触れられると驚きやすい", "친한 사람 외의 기척에는 경계함": "親しい人以外の気配には警戒する", "사제 관계": "師弟関係", "배움을 시작한 사이": "学び始めた関係", "서로 신뢰하는 사제": "信頼し合う師弟", "가르침을 이어가는 사이": "教えを受け継ぐ関係"});

Object.assign(I18N.en,{"상황에 따라 자연스럽게 받아들임": "Accepts touch naturally depending on the situation", "몸에 손이 닿는 것을 극도로 꺼림": "Strongly avoids being touched", "몸에 손이 닿는 것을 싫어함": "Dislikes being touched", "허락 없는 접촉은 불편함": "Uncomfortable with touch without permission", "가까운 사람에게만 허용함": "Allows touch only from close people", "신체 접촉을 좋아함": "Enjoys physical contact", "먼저 다가가는 편": "Often approaches first"});

Object.assign(I18N.ja,{"상황에 따라 자연스럽게 받아들임": "状況に応じて自然に受け入れる", "몸에 손이 닿는 것을 극도로 꺼림": "体に触れられるのを非常に避ける", "몸에 손이 닿는 것을 싫어함": "体に触れられるのが苦手", "허락 없는 접촉은 불편함": "許可なく触れられると不快", "가까운 사람에게만 허용함": "親しい人にだけ触れることを許す", "신체 접촉을 좋아함": "身体接触が好き", "먼저 다가가는 편": "自分から近づく方"});

Object.assign(I18N.en,{"스터드 귀고리": "Stud earrings", "링 귀고리": "Hoop earrings", "드롭 귀고리": "Drop earrings", "이어커프": "Ear cuffs", "귓바퀴 피어싱": "Cartilage piercings", "코 피어싱": "Nose piercings", "입술 피어싱": "Lip piercings", "목걸이": "Necklaces", "펜던트": "Pendants", "초커": "Chokers", "반지": "Rings", "여러 손가락 반지": "Rings on multiple fingers", "팔찌": "Bracelets", "뱅글": "Bangles", "발찌": "Anklets", "손목시계": "Wristwatches", "브로치": "Brooches", "배지": "Badges", "머리핀": "Hair clips", "머리띠": "Headbands", "비녀": "Hair sticks", "리본": "Ribbons", "안경": "Glasses", "선글라스": "Sunglasses", "체인 장식": "Decorative chains", "전통의상": "Traditional attire"});
Object.assign(UI_TEXT.en,{"스터드 귀고리": "Stud earrings", "링 귀고리": "Hoop earrings", "드롭 귀고리": "Drop earrings", "이어커프": "Ear cuffs", "귓바퀴 피어싱": "Cartilage piercings", "코 피어싱": "Nose piercings", "입술 피어싱": "Lip piercings", "목걸이": "Necklaces", "펜던트": "Pendants", "초커": "Chokers", "반지": "Rings", "여러 손가락 반지": "Rings on multiple fingers", "팔찌": "Bracelets", "뱅글": "Bangles", "발찌": "Anklets", "손목시계": "Wristwatches", "브로치": "Brooches", "배지": "Badges", "머리핀": "Hair clips", "머리띠": "Headbands", "비녀": "Hair sticks", "리본": "Ribbons", "안경": "Glasses", "선글라스": "Sunglasses", "체인 장식": "Decorative chains", "전통의상": "Traditional attire"});

Object.assign(I18N.ja,{"스터드 귀고리": "スタッドピアス", "링 귀고리": "フープピアス", "드롭 귀고리": "ドロップピアス", "이어커프": "イヤーカフ", "귓바퀴 피어싱": "軟骨ピアス", "코 피어싱": "鼻ピアス", "입술 피어싱": "リップピアス", "목걸이": "ネックレス", "펜던트": "ペンダント", "초커": "チョーカー", "반지": "指輪", "여러 손가락 반지": "複数の指の指輪", "팔찌": "ブレスレット", "뱅글": "バングル", "발찌": "アンクレット", "손목시계": "腕時計", "브로치": "ブローチ", "배지": "バッジ", "머리핀": "ヘアピン", "머리띠": "カチューシャ", "비녀": "かんざし", "리본": "リボン", "안경": "眼鏡", "선글라스": "サングラス", "체인 장식": "チェーン飾り", "전통의상": "伝統衣装"});
Object.assign(UI_TEXT.ja,{"스터드 귀고리": "スタッドピアス", "링 귀고리": "フープピアス", "드롭 귀고리": "ドロップピアス", "이어커프": "イヤーカフ", "귓바퀴 피어싱": "軟骨ピアス", "코 피어싱": "鼻ピアス", "입술 피어싱": "リップピアス", "목걸이": "ネックレス", "펜던트": "ペンダント", "초커": "チョーカー", "반지": "指輪", "여러 손가락 반지": "複数の指の指輪", "팔찌": "ブレスレット", "뱅글": "バングル", "발찌": "アンクレット", "손목시계": "腕時計", "브로치": "ブローチ", "배지": "バッジ", "머리핀": "ヘアピン", "머리띠": "カチューシャ", "비녀": "かんざし", "리본": "リボン", "안경": "眼鏡", "선글라스": "サングラス", "체인 장식": "チェーン飾り", "전통의상": "伝統衣装"});

Object.assign(UI_TEXT.en,{"스승":"Teacher","제자":"Student"});Object.assign(UI_TEXT.ja,{"스승":"師匠","제자":"弟子"});

Object.assign(UI_TEXT.en,{"캐릭터 5명 추가":"Add 5 character slots","캐릭터 슬롯 5개가 결제 즉시 계정에 적용됩니다. 기존 구매 슬롯은 그대로 유지됩니다.":"Five character slots are applied to your account after payment. Previously purchased slots are preserved."});
Object.assign(UI_TEXT.ja,{"캐릭터 5명 추가":"キャラクター枠を5人追加","캐릭터 슬롯 5개가 결제 즉시 계정에 적용됩니다. 기존 구매 슬롯은 그대로 유지됩니다.":"決済後にキャラクター枠5人分がアカウントに適用されます。購入済みの枠は維持されます。"});

for(const [ko,en,ja] of [
['혼자가 편함','Comfortable alone','ひとりが気楽'],['낯을 가림','Shy with strangers','人見知りする'],['조용히 어울림','Socializes quietly','静かに人と接する'],['먼저 다가감','Approaches others first','自分から近づく'],['무리의 중심','At the center of the group','集団の中心になる'],
['눈앞의 현실 중시','Focuses on present reality','目の前の現実を重視'],['구체적인 편','Prefers concrete details','具体的に考える'],['균형형','Balanced','バランス型'],['가능성 중시','Focuses on possibilities','可能性を重視'],['직관과 상상 중시','Values intuition and imagination','直感と想像を重視'],
['논리 우선','Prioritizes logic','論理を優先'],['이성적인 편','Tends to reason things out','理性的に考える'],['마음을 살핌','Considers feelings','気持ちをくみ取る'],['공감 우선','Prioritizes empathy','共感を優先'],
['즉흥적','Spontaneous','即興的'],['유연한 편','Flexible','柔軟な方'],['상황에 따라','Adapts to the situation','状況に応じる'],['미리 정리함','Organizes in advance','前もって整理する'],['계획적','Plans ahead','計画的'],
['한 가지씩 차분히','Calmly handles one thing at a time','一つずつ落ち着いて取り組む'],['잠깐 쉬고 다음 일','Pauses before the next task','一息ついて次へ進む'],['생각나면 바로 움직임','Acts as soon as an idea occurs','思いついたらすぐ動く'],['여러 일을 오감','Switches between tasks','複数の作業を行き来する'],
['방관자','Stays uninvolved','傍観する'],['요청할 때만 도움','Helps when asked','頼まれた時だけ助ける'],['적당히 관여','Gets moderately involved','適度に関わる'],['챙기고 확인함','Looks after others and checks in','気にかけて確認する'],['강하게 간섭함','Intervenes strongly','強く干渉する'],['통제광','Insists on control','強く支配したがる'],
['어질러도 편함','Comfortable with mess','散らかっても気にならない'],['조금 느슨함','Somewhat relaxed about tidiness','整理には少し大らか'],['정돈을 좋아함','Likes tidiness','整理整頓を好む'],['흐트러짐을 못 참음','Cannot stand disorder','乱れを我慢できない'],
['매우 느긋함','Very laid-back','とてものんびり'],['필요할 때만 움직임','Acts only when necessary','必要な時だけ動く'],['부지런함','Diligent','勤勉'],['쉴 새 없이 움직임','Constantly active','休みなく動く'],
['피하는 편','Tends to avoid conflict','避けることが多い'],['시간을 두고 말함','Talks after taking time','時間を置いて話す'],['대화로 해결','Resolves through discussion','話し合いで解決する'],['바로 따짐','Challenges it immediately','すぐに問いただす'],['끝까지 결론을 냄','Pursues a clear conclusion','最後まで結論を出す'],
['표현이 서툼','Struggles to express affection','愛情表現が苦手'],['조용히 곁에 있음','Stays quietly nearby','静かにそばにいる'],['말로 표현','Expresses affection in words','言葉で伝える'],['행동으로 표현','Expresses affection through actions','行動で伝える'],['적극적으로 챙김','Actively cares for others','積極的に世話を焼く'],
['집에서 충전','Recharges at home','家で充電する'],['느긋한 편','Usually laid-back','のんびりした方'],['활동적인 편','Usually active','活動的な方'],['가만히 못 있음','Finds it hard to stay still','じっとしていられない'],
['장난을 거의 하지 않음','Rarely jokes','ほとんど冗談を言わない'],['건조한 농담만 함','Prefers dry humor','淡々とした冗談を言う'],['가끔 장난을 즐김','Sometimes enjoys joking','時々冗談を楽しむ'],['장난을 즐김','Enjoys joking','冗談を楽しむ'],['유머로 분위기를 이끎','Sets the mood with humor','ユーモアで場を盛り上げる'],
['분노할 때 보이는 반응','Response to anger','怒った時の反応'],['유혹·호감 신호를 받을 때','Response to flirtation','好意を向けられた時'],
['차분히 이유를 확인함','Calmly asks why','落ち着いて理由を確かめる'],['말수가 차갑게 줄어듦','Becomes cold and quiet','口数が減り冷たくなる'],['즉시 잘못을 따짐','Immediately points out the wrong','すぐに非を問いただす'],['목소리가 커짐','Raises their voice','声が大きくなる'],['자리를 피해 식힘','Steps away to cool down','その場を離れて落ち着く'],['울컥하지만 말을 고름','Chooses words despite a surge of anger','こみ上げても言葉を選ぶ'],['해결책을 분명히 요구함','Clearly asks for a solution','解決策をはっきり求める'],
['눈치채지 못함','Does not notice','気づかない'],['알아도 모른 척함','Pretends not to notice','気づかないふりをする'],['당황해 거리를 둠','Gets flustered and steps back','戸惑って距離を置く'],['은근히 받아줌','Subtly reciprocates','さりげなく応じる'],['장난스럽게 맞받음','Responds playfully','冗談めかして返す'],['직접 호응함','Responds openly','はっきり応じる'],['상대를 경계함','Stays wary of the person','相手を警戒する'],
['상황에 따라 표현함','Expresses feelings as the situation allows','状況に応じて表現する'],['가끔 욱하지만 멈춤','Sometimes flares up but stops','時々かっとなるが踏みとどまる'],['매우 잘 참음','Very restrained','とてもよく抑える'],['대체로 참음','Usually restrained','おおむね抑える'],['쉽게 욱함','Easily loses composure','かっとなりやすい'],['거의 참지 않음','Rarely holds back','ほとんど抑えない']
]){I18N.en[ko]=en;I18N.ja[ko]=ja;}

Object.assign(UI_TEXT.en,{"도덕성":"Morality","이익을 위해 선을 넘음":"Crosses lines for personal gain","내 이익을 먼저 따짐":"Puts personal gain first","상황에 따라 판단함":"Judges by the situation","양심을 지키려 함":"Tries to follow their conscience","손해를 봐도 원칙을 지킴":"Keeps principles at a personal cost"});
Object.assign(UI_TEXT.ja,{"도덕성":"道徳性","이익을 위해 선을 넘음":"利益のために一線を越える","내 이익을 먼저 따짐":"自分の利益を優先する","상황에 따라 판단함":"状況に応じて判断する","양심을 지키려 함":"良心に従おうとする","손해를 봐도 원칙을 지킴":"損をしても原則を守る"});

Object.assign(UI_TEXT.en,{"낮을수록 자기 이익을 위해 선을 넘고, 높을수록 손해를 보더라도 양심과 원칙을 지켜요.":"Lower values favor personal gain across ethical boundaries; higher values uphold conscience and principles even at a cost."});
Object.assign(UI_TEXT.ja,{"낮을수록 자기 이익을 위해 선을 넘고, 높을수록 손해를 보더라도 양심과 원칙을 지켜요.":"低いほど利益のために一線を越え、高いほど損をしても良心と原則を守ります。"});

function imageEsc(value){return esc(displayImageSource(value));}

Object.assign(I18N.en,{"잠을 전혀 이루지 못함":"Unable to fall asleep","거의 먹지 않음":"Rarely eats","먹지 않음":"Does not eat","병명·건강 상태 직접 입력":"Enter a condition or health details"});
Object.assign(I18N.ja,{"잠을 전혀 이루지 못함":"まったく眠れない","거의 먹지 않음":"ほとんど食べない","먹지 않음":"食べない","병명·건강 상태 직접 입력":"病名・健康状態を直接入力"});

Object.assign(UI_TEXT.en,{"마을 5개 추가":"Add 5 town slots","마을 슬롯 5개가 계정에 추가됩니다. 기존 구매 슬롯은 그대로 유지됩니다.":"Five town slots are added to your account. Existing purchased slots are preserved."});
Object.assign(UI_TEXT.ja,{"마을 5개 추가":"村枠を5個追加","마을 슬롯 5개가 계정에 추가됩니다. 기존 구매 슬롯은 그대로 유지됩니다.":"村枠5個をアカウントに追加します。購入済みの枠は維持されます。"});

Object.assign(UI_TEXT.en,{"가구 숨기기":"Hide furniture","가구 그림만 숨겨요. 사용과 배치는 유지되며 편집할 때 다시 보여요.":"Only furniture artwork is hidden. Furniture stays usable and reappears while editing."});
Object.assign(UI_TEXT.ja,{"가구 숨기기":"家具を隠す","가구 그림만 숨겨요. 사용과 배치는 유지되며 편집할 때 다시 보여요.":"家具の絵だけを隠します。配置と使用は維持され、編集時には再び表示されます。"});

Object.assign(UI_TEXT.en,{"계정 카드에서 프로필과 로그인을 관리해요. 아래 6개 메뉴에서 게임, 화면, 소리, 알림, 백업과 도움말을 설정할 수 있어요.":"Manage your profile and login through the account card. The six menus below cover gameplay, display, sound, notifications, backup and help.","계정 삭제":"Delete account","계정 초기화":"Reset account data"});
Object.assign(UI_TEXT.ja,{"계정 카드에서 프로필과 로그인을 관리해요. 아래 6개 메뉴에서 게임, 화면, 소리, 알림, 백업과 도움말을 설정할 수 있어요.":"アカウントカードでプロフィールとログインを管理します。下の6つのメニューからゲーム、表示、サウンド、通知、バックアップ、ヘルプを設定できます。","계정 삭제":"アカウント削除","계정 초기화":"アカウントデータを初期化"});
