...
function renderAppContents(next){
  ...
  appRoot.innerHTML=`${showingWelcome||showingAccountLoading||showingMultiplayerDirectory||state.activeTab==="mailbox"?"":header()}<main>${content}</main>`;
...
function shop(){
  ...
  const cart=readCart();
  const lines=Object.entries(cart).filter(([id,qty])=>SHOP_PRODUCTS[id]&&saleAllows(id)&&!SHOP_PRODUCTS[id].disabled&&Number(qty)>0);
  ...
  return `<section class="panel form dlc-store shop-store"><div class="title"><div><h1>상점</h1><p>원하는 상품과 수량을 장바구니에 담아 한 번에 결제할 수 있어요.</p></div></div>${previewMode()?`<section class="preview-notice"><b>${esc(previewConfig().label||"사전 체험")} 기간이에요</b><p>${esc(previewConfig().message||"현재 기능을 점검하고 있어 실제 결제는 진행되지 않아요.")}</p></section>`:""}${window.PARALLEL_CITY_CONFIG?.paymentEnvironment==='test'?`<p class="preview-notice">${({ko:'웹 결제는 카드사 심사 중입니다. 현재 웹에서는 실제 구매가 진행되지 않아요.',en:'Web payments are awaiting payment-provider review. Purchases are not available on the web yet.',ja:'ウェブ決済はカード会社の審査中です。現在ウェブでは購入できません。'}[state.uiLanguage]||'웹 결제는 카드사 심사 중입니다.')}</p>`:''}<div class="shop-product-grid">${product(characterSlotProduct(),SHOP_PRODUCTS[characterSlotProduct()],Number(characterSlotProduct()==="character_slots_5"?accountEntitlements.characterSlotPacks:accountEntitlements.characterSingleSlots)||0)}${product("town_slot_1",SHOP_PRODUCTS.town_slot_1,Number(accountEntitlements.townSlotPacks)||0)}${product("storage_50mb",SHOP_PRODUCTS.storage_50mb,accountEntitlements.storage50?1:0)}${product("green_tea",SHOP_PRODUCTS.green_tea,0)}</div><div class="shop-expansion-heading"><small>COMING NEXT</small><h2>확장팩</h2></div>${jobExpansionCard()}<section class="shop-cart"><div class="title"><div><h2>장바구니</h2><p>${previewMode()?"사전 체험이 끝난 뒤 이용할 수 있어요.":"같은 상품도 여러 개 담을 수 있어요."}</p></div><b>${count}개</b></div><div class="premium-current"><b>한 번 결제 금액은 5만원 미만이어야 해요.</b><small>${limitDetail}</small></div><div class="cart-lines">${cartHtml}</div><div class="cart-total"><span>총 결제금액</span><b>${total.toLocaleString("ko-KR")}원</b></div>${previewMode()?`<span class="premium-buy disabled" aria-disabled="true">사전 체험 중에는 결제하지 않아요</span>`:`<a class="primary premium-buy ${lines.length&&!isOverLimit?"":"disabled"}" ${lines.length&&!isOverLimit?'href="./payment.html?cart=1" aria-disabled="false"':'aria-disabled="true"'}>${isOverLimit?"수량을 줄여 주세요":"장바구니 결제하기"}</a>`}</section><div class="dlc-hidden" hidden>${dlc()}</div></section>`;
}
