...
(()=>{
 ...
 function isHome(c){const e=eventOf(c),homeEvent=Boolean(e&&(e.home||e.kind==='home'||/집에서|귀가|취침|기상|하루 정리/.test(e.title||'')));if(e&&!homeEvent)return false;return homeEvent||sleepState(c).sleeping}
 ...
 function action(c,room,people,clean){
  ...
  if((c.hobbies||[]).some(h=>h==='청소'||h==='정리정돈')&&new Date().getHours()%4===hash(c.id)%4)return`${room.name}의 물건을 정리하고 먼지를 닦는 중`;
 ...
 function cleanliness(owner,people){
  ...
  const awake=people.filter(c=>isHome(c)&&!sleepState(c).sleeping),cleaner=awake.sort((a,b)=>(hash(a.id)%5)-(hash(b.id)%5))[0],likesCleaning=cleaner?.hobbies?.some(h=>h==='청소'||h==='정리정돈'),interval=cleaner?(likesCleaning?1:1+(hash(`${cleaner.id}-cleaning`)%4)):4;
  if(cleaner&&owner.homeAutoCleanDate!==today&&owner.homeCleanliness<(likesCleaning?92:78)&&hash(`${today}-${cleaner.id}`)%interval===0){owner.homeCleanliness=Math.min(100,owner.homeCleanliness+(likesCleaning?26:18));owner.homeAutoCleanDate=today;cleaner._autoCleaningUntil=Date.now()+45*60000;state.pendingCloudSave=true;queueMicrotask(()=>{save?.();window.queueParallelCityCloudSync?.()})}
