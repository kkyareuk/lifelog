...
export function bindFurnitureDrag(root,{getHome,select=()=>{},move,language='ko'}){
 ...
 const toggle=document.createElement('button');toggle.type='button';toggle.dataset.chairMagnet='';const label=()=>{toggle.textContent=text('의자 자동 붙이기','Snap chairs to tables','椅子をテーブルに吸着');toggle.setAttribute('aria-pressed',String(magnet))};label();toggle.onclick=()=>{magnet=!magnet;label();try{localStorage.setItem('drawer-chair-magnet',magnet?'on':'off')}catch{}};bar?.prepend(toggle);
 ...
 const paint=()=>{frame=0;if(!drag?.point)return;const d=drag,p=d.point;const target=d.rooms.find(r=>p.x>=r.box.left&&p.x<=r.box.right&&p.y>=r.box.top&&p.y<=r.box.bottom);if(!target)return;
  ...
  if(magnet&&d.item.item==='의자'){
   ...
   for(const table of target.tables){for(const [side,sx,sy,rotation] of [['north',.5,.10,180],['south',.5,.95,0],['west',.02,.5,90],['east',.98,.5,270]]){
  ...
  d.latest={...pos,roomKey:target.key};if(!d.ghost){d.ghost=d.element.cloneNode(true);d.ghost.removeAttribute('data-furniture-placement');d.ghost.classList.add('furniture-drag-preview');d.ghost.inert=true;preserveFurnitureDragSize(d.element,d.ghost);d.element.classList.add('furniture-drag-source')}
  if(d.ghost.parentElement!==target.layer)target.layer.append(d.ghost);d.ghost.style.setProperty('--furniture-x',pos.x+'%');d.ghost.style.setProperty('--furniture-y',pos.y+'%');
 };
 ...
 const end=e=>{if(!drag||drag.pointer!==e.pointerId)return;stop(e);if(frame){cancelAnimationFrame(frame);paint()}const d=drag;drag=null;d.ghost?.remove();d.element.classList.remove('furniture-drag-source');if(root.hasPointerCapture(e.pointerId))root.releasePointerCapture(e.pointerId);if(d.latest){last=null;move(d.element,d.latest)}};
 ...
 cleanupActive=()=>{controller.abort();if(frame)cancelAnimationFrame(frame);drag?.ghost?.remove();drag?.element.classList.remove('furniture-drag-source');toggle.remove()};
}
