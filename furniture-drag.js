...
export function bindFurnitureDrag(root,{getHome,select=()=>{},move,resize=()=>{},language='ko'}){
 cleanupActive?.();cleanupActive=null;if(!root)return;const controller=new AbortController(),signal=controller.signal;let drag=null,last=null,frame=0,consumedTap=null;
 ...
 const end=e=>{if(!drag||drag.pointer!==e.pointerId)return;stop(e);if(frame){cancelAnimationFrame(frame);paint()}const d=drag;drag=null;consumedTap=e.type==='pointerup'?{pointer:e.pointerId,x:e.clientX,y:e.clientY,at:performance.now()}:null;clearPreview(d);if(root.hasPointerCapture(e.pointerId))root.releasePointerCapture(e.pointerId);if(d.latest){last=null;if(e.type==='pointerup')move(d.element,d.latest)}else if(e.type==='pointerup'){const {hits,...tap}=d.tap;last=tap;const selected=hits[tap.index];select(selected);selectedCounter=selected;stretch.hidden=selected.dataset.furnitureKind!=='counter';length.value=selected.dataset.counterSpan||1}};
 for(const event of ['pointerup','pointercancel','lostpointercapture'])root.addEventListener(event,end,{capture:true,signal});
 // Pointer capture can retarget the synthetic click to the room or root.
 root.addEventListener('click',e=>{const tap=consumedTap;consumedTap=null;const consumed=tap&&e.detail!==0&&performance.now()-tap.at<1500&&(e.pointerId===tap.pointer||Math.hypot(e.clientX-tap.x,e.clientY-tap.y)<18);if(consumed||e.target.closest('[data-furniture-placement]')&&(root.querySelector('.home.is-editing')||root.matches('.home.is-editing')))stop(e)},{capture:true,signal});
