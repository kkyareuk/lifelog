...
(()=>{
 ...
 function render(){
  ...
  root.innerHTML=groups().map(people=>{
   ...
   const cards=list.map(r=>`<section class="photo-room ${r.photo?'has-photo':''}" ${r.photo?`style="background-image:linear-gradient(#fff3,#fff3),url('${r.photo}')"`:''}><div class="photo-room-title">${esc(r.name)}</div>${r.photo?'':'<div class="photo-room-empty">방 사진을 추가해 주세요</div>'}<div class="photo-room-people">${(placed.get(r.id)||[]).map(c=>person(c,action(c,r,people))).join('')}</div>${r.type==='living'?pet(owner):''}</section>`).join('');
   return`<article class="home-card"><header><div><h3>${esc(owner.homeLayout||'우리 집')} · ${esc(owner.home||'생활권 미설정')}</h3><p>${people.map(p=>esc(p.name)).join(' · ')} 함께 거주 중</p></div><b>${inside.length}명 귀가</b></header><div class="photo-room-grid">${cards}</div></article>`;
